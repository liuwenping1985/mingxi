...
function doSearch() {
	...
	if(seatCountObj != null){
		if(seatCountObj.value == ""){
			alert(v3x.getMessage("meetingLang.meeting_contain_count_alert"));
			return ;
		}else if(!patrn.exec(seatCountObj.value)){
			alert(v3x.getMessage("meetingLang.meeting_int_alert"));
			return ;
		}
	}
 	...
 	<td class="webfx-menu-bar">
 		<form name="searchForm" action="meetingroomList.do?method=listAdd" method="post" onsubmit="">
 			<div class="div-float-right">
 				<div class="div-float">
 					<select id="selectCondition" name="selectCondition" class="condition" onChange="showCondition(this)" inputName="<fmt:message key='mr.label.querycondition'/>" validate="notNull">
...
<form name="listForm" id="listForm" method="post" style="margin: 0px" target="hiddenIframe">
   <v3x:table htmlId="listTable" data="list" var="bean" pageSize="${pageSize }" size="${size }" showHeader="true" showPager="true" isChangeTRColor="true">
	...
	<v3x:column width="36%" type="String" onDblClick="showModify('${bean.roomId }')" onClick="showDetail('${bean.roomId }')" label="mr.label.meetingroomname" className="cursor-hand sort mxtgrid_black" alt="${bean.roomName }">
		<c:out value="${bean.roomName }" />
		<c:if test="${bean.roomNeedApp=='0' }">※</c:if>
	</v3x:column>
	
	<v3x:column width="30%" type="String" onDblClick="showModify('${bean.roomId }')" onClick="showDetail('${bean.roomId }')" label="mr.label.place" className="cursor-hand sort" alt="${bean.roomPlace }">
		<c:out value="${bean.roomPlace }" />
	</v3x:column>
	
	<v3x:column width="15%" type="Number" onDblClick="showModify('${bean.roomId }')" onClick="showDetail('${bean.roomId }')" label="mr.label.seatCount" className="cursor-hand sort" alt="${bean.roomSeatCount }">
		<c:out value="${bean.roomSeatCount }" />
	</v3x:column>
	
	<v3x:column width="15%" type="String" onDblClick="showModify('${bean.roomId }')" onClick="showDetail('${bean.roomId }')" label="mr.label.status" className="cursor-hand sort" alt="">
