...
<script text="text/javascript">
	...
	$(function() {
		...
		var docPlugin = ${ctp:hasPlugin('doc')};
		var toolbar = [{
			id : "listPending",
		...
		if (app != '4') {
	        ...
	        condition.push({
				...
				name : 'title',
				type : 'input',
				text : '${ctp:i18n("supervise.subject.label")}',//标题
				value : 'title'
			});
	        condition.push({
				id : 'importantLevel',
	        ...
	        condition.push({
				...
				name : 'sender',
				type : 'input',
				text : '${ctp:i18n("supervise.sender.label")}',//发起人
				value : 'sender'
			});
	        condition.push({
				id : 'sendDate',
			...
			searchobj = $.searchCondition({
				...
				conditions : [{
					...
					value : 'secretLevel',
					items : [{
						text : '${ctp:i18n("edoc.secretlevel.normal")}',//普通
						value : '1'
					},{
						text : '${ctp:i18n("edoc.secretlevel.secret")}',//秘密
						value : '2'
					},{
						text : '${ctp:i18n("edoc.secretlevel.highlysecret")}',//机密
						value : '3'
					},{
						text : '${ctp:i18n("edoc.secretlevel.topsecret")}',//绝密
						value : '4'
					}]
				},{
					id : 'urgentLevel',
		...
		var _managerMethod = "getSuperviseList4App";
				...
				.ajaxgrid(
						{
							...
							isHaveIframe : true,
							slideToggleBtn : true,
							customId : "_supervise_grid_list_app_" + app,
							customize : true,
							onSuccess : function() {
								xyms = false;
								$('#summary')
		...
		if (xyms) {
			$('#summary')
					.attr(
							"src",
							_ctxPath
									+ "/collaboration/listDesc.do?method=listDesc&type=listSupervise&size=0");
		}
		//定义回调函数
		function rend(txt, data, r, c, col) {
			if (app != '4') {
				...
				if ("${ctp:escapeJavascript(param.status)}" == '0' || "${ctp:escapeJavascript(param.status)}" == '') {//增加一列当前待办人
					if (c === 4) {
						return "<a class='noClick' href='javascript:void(0)' onclick='showFlowChart(\""
								+ data.caseId
								+ "\",\""
								+ data.processId
								+ "\",\""
						...
						if (data.status == 1) {
							...
							return "<a class='noClick' href='javascript:void(0)' onclick='showSuperviseLog(\""
									...
									+ "\",true,\""
									+ data.id
									+ "\",null,\"collaboration\",refreshWindow,\"\",\"\",\"${ctp:i18n('supervise.hasten.label')}\")'></a>";
						}
					}
					//已办结
					if ("${ctp:escapeJavascript(param.status)}" == '1') {
						...
						if (c == 8) {
							...
							return "<a href='javascript:void(0)' class='ico16 process_16 noClick' onclick='superviousWFCDiagram(getCtpTop(),\""
									...
									+ "\",\""
									+ data.defaultNodeName
									+ "\",\""
									+ data.defaultNodeLable
					...
					if (col.name == "count") {
						if (data.status == 1) {
							...
							return "<div style='text-align:right;width:50%;font-family:Arial,Helvetica,sans-serif'><a class='noClick' href='javascript:void(0)' onclick='showSuperviseLog(\""
									...
									+ "\",true,\""
									+ data.id
									+ "\",null,\"edoc\",refreshWindow,\"\",\"\",\"${ctp:i18n('supervise.hasten.label')}\")'></a></div>";
						}
					}
						...
						if (col.name == "count") {
							...
							if (data.status == 1) {
								...
								return "<a class='noClick' href='javascript:void(0)' onclick='showSuperviseLog(\""
										...
										+ "\",true,\""
										+ data.id
										+ "\",null,\"edoc\",refreshWindow,\"\",\"\",\"${ctp:i18n('supervise.hasten.label')}\")'></a>";
							}
						}
						//未办结
						if (col.name == "processDescBy") {
	...
	function changeAwake(e, id) {
		...
		var maintop = parseInt($(getCtpTop().document).find(".layout_content").css("top"));
		var left = (getCtpTop().document.documentElement.clientWidth - 295) /2;
		var top = (getCtpTop().document.documentElement.clientHeight - 260) /2 - maintop;
		$.calendar({
			displayArea : "ssss" + id,
			position : [ left, top ],
			returnValue : true,
					...
					.push({
						...
						handler : function() {
							...
							if (returnValue != null) {
								...
								setTimeout(function(){dialogs.close()}, 10);
							}
						}
					});
			but.push({
	...
	function editEdocWorkflow(finished, caseId, processId, isTemplate, appName,
			flowPermAccountId,defaultNodeName,defaultNodeLable) {
		if (finished == 'true') {
			$.alert("${ctp:i18n('collaboration.process.finished')}");
			return;
		}
		superviousWFCDiagram(getCtpTop(), caseId, processId, isTemplate,
				window, appName, flowPermAccountId, defaultNodeName,
				defaultNodeLable,
				"${ctp:i18n('supervise.edoc.label')}");
	}
	//二维码传参chenxd
...
<body class="body-pading" leftmargin="0" topmargin="" marginwidth="0"
	...
	<div id='layout'>
		<div class="layout_north bg_color" id="north" style="overflow:hidden;">
