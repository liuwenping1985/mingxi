   ...
   <body class="h100b">
    	...
    	<div class="adapt_w font_size12 form_area people_msg  h100b" style="background:#fff;width: 370px;">
    		<table cellpadding="0" cellspacing="0" width="100%" class="padding_5">
    			...
    			<tr>
    				<th nowrap="nowrap">${ctp:i18n('member.location')}:</th><td><div class="text_overflow" style="width:100px;"><label id="worklocal"/></div></td>
    				<c:if test ="${ffpeoplecardform.isInternal == true || ffpeoplecardform.isInternal == 'true'}">
	    				<th nowrap="nowrap">${ctp:i18n('member.report2')}:</th><td><div class="text_overflow" style="width:100px;"><label id="reportTo"/></div></td>
    				</c:if>
    			</tr>
    		</table>
    		<table cellpadding="0" cellspacing="0" width="100%" class="padding_5">
    			...
    			<c:forEach items="${bean}" var="bean" varStatus="status">
			           <tr>
	                       <th nowrap="nowrap">${v3x:toHTML(bean.label)}:</th>
	                       <td colspan="3" width="100%"><div class="text_overflow" style="width:300px;"><label id="${bean.id}" /></div></td>                        
                      </tr>
		  		 </c:forEach>
 		  		 ...
 		  		 <c:forEach items="${beanValue}" var="beanValue">
		  		        ...
		  		        $("#"+key).text('${v3x:escapeJavascript(beanValue.value)}');
		  		        $("#"+key).attr("title","${v3x:escapeJavascript(beanValue.value)}");
                     </script>
		  		 </c:forEach> 
