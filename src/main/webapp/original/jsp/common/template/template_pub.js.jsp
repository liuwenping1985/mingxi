...
function deleteSomething(currentSelected) {
    if (currentSelected == null) {
        // 请选择模板分类!
        $.alert($.i18n('collaboration.template.toolbar.selectCategory'));
        return;
    }
...
function categoryManager(act, categoryType) {
    ...
    if (act == 'delete') {
        ...
        if (!businessId || businessId == 0 || businessId == 1 || businessId == 2 || businessId == 3 || businessId == 4
                || businessId == 5) {
            $.alert("collaborationLang.templete_alertSelectTreeNode");
            return;
        }
         ...
         * location.href = templeteURL + "?method=deleteCategory&id=" +
         * selectId; } }
         */
        var dialog = new MxtDialog({
            html : "${ctp:i18n('collaboration.template.pub.deleteTemplateClass')}", // 确认要删除该模板分类?
                                                                                    // 该操作不能恢复!
            ...
            buttons : [
                    {
                        ...
                        handler : function() {
                            templeteTreeFrame.hiddenIframe.location.href = templeteURL + "?method=deleteCategory&id="
                                    + selectId;
                            dialog.close();
                        }
                    }, {
        ...
        // if(window.confirm(_("collaborationLang.templete_confirmDeleteTreeNode"))){
        // location.href =
        // "/seeyon/template/template.do?method=deleteCategory&id=" + selectId;
        // }
    }
}
function deleteFormCategory(act, categoryType) {
    ...
    var dataUrl = v3x.baseURL
            + encodeURI(encodeURI("/formappMgrController.do?method=judgeuerscond&categoryid=" + selectId));
...
function showTempleteList(categoryId, categorType, canManage) {
    ...
    if (categoryId != null && parent.templeteListFrame) {
        parent.templeteListFrame.location.href = templeteURL + "?method=systemList&categoryId=" + categoryId
                + "&categoryType=" + categorType + "&from=" + getParameter("from");
    }
}
...
function newTemplete(categoryType, category) {
    ...
    url += "?method=systemNewTemplete&categoryId=" + selectId + "&categoryType=" + curCategorType + "&from="
            + getParameter("from");
...
function editTemplete(templeteId, categoryType, from) {
    ...
    parent.location.href = url + "?method=systemNewTemplete&templeteId=" + templeteId + "&categoryType=" + categoryType
            + "&from=" + from;
}
...
function saveTemplete() {
    ...
    if (checkForm(theForm) && checkRepeatTempleteSubject(theForm, true)
            && checkTemplateCategory(categoryId.value, trim(categoryId.text))) {
        if (_type != 'text' && !checkSelectWF1()) { // 协同正文，不用流程
            return;
        }
        /*
         * if(_type == 'text'){ var bodyType =
         * document.getElementById("bodyType").value; if (bodyType != "HTML") {
            ...
            for (var i = 0, j = keys.length; i < j; i++) {
                ...
                if (branch != null) {
                    var str = "<input type=\"hidden\" name=\"branchs\" value=\"" + keys[i] + "↗" + branch.id + "↗"
                            + branch.conditionType + "↗" + branch.formCondition + "↗" + branch.conditionTitle + "↗"
                            + branch.isForce + "↗" + (branch.conditionDesc ? branch.conditionDesc.escapeQuot() : "")
        ...
        /*
         * if(nodes != null && nodes.length>0){ var hidden; for(var i=0;i<nodes.length;i++){
         * hidden = document.createElement('<INPUT TYPE="hidden" name="policys"
         * value="' + policys[nodes[i]].name + '" />');
         * theForm.appendChild(hidden); hidden = document.createElement('<INPUT
         * TYPE="hidden" name="itemNames" value="' + policys[nodes[i]].value + '"
...
function refTemplete() {
    ...
    _parent.location.href = collaborationURL + "?method=newColl&templeteId=" + id + '&from=' + from + "&projectId="
            + projectId;
    top.window.close();
    /*
     * }else { if (parent.templeteDetailFrame.document.readyState == 'complete') {
     * if (intervalObj) { window.clearInterval("intervalObj"); }
     * 
     * parent.templeteDetailFrame.refTempleteContent(type, id);
     * top.window.close(); } else { intervalObj =
     * window.setInterval("refTemplete('" + id + "', '" + type + "')"); } }
     */
}
function refTempleteIpad() {
    ...
    /*
     * }else { if (parent.templeteDetailFrame.document.readyState == 'complete') {
     * if (intervalObj) { window.clearInterval("intervalObj"); } return
     * parent.templeteDetailFrame.refTempleteContentIpad(type, id);
     *  } else { intervalObj = window.setInterval("refTempleteIpad('" + id + "', '" +
     * type + "')"); } }
     */
}
function refTempleteContent(type, id) {
    ...
    if (type != 'workflow') {
        ...
        } else if (bodyType == "OfficeWord" || bodyType == "OfficeExcel" || bodyType == "WpsWord"
                || bodyType == "WpsExcel") {
            rv[5] = {
                bodyType : bodyType,
                fileType : fileType,
...
function deleteTemplete() {
    /*
     * var id_checkbox = document.getElementsByName('id'); if (!id_checkbox) {
     * return 0; }
     * 
     * var hasSelected = false; var len = id_checkbox.length; for (var i = 0; i <
     * len; i++) { if (id_checkbox[i].checked) { hasSelected = true; break; } }
     * 
     * if(!hasSelected){
     * $.alert(_("collaborationLang.templete_alertSelectTemplete")); return; }
     */
...
function authTemplete() {
    $.alert("${ctp:i18n('collaboration.template.pub.authorize')}"); // 授权
    var num = validateCheckbox("id");
    if (num == 0) {
        $.alert(_("collaborationLang.templete_alertSelectTemplete"));
        return;
...
function moveTemplete() {
    var template_tree_dialog = $.dialog({
        title : "${ctp:i18n('collaboration.template.pub.moveTo')}", // 移动至
    ...
    // 'collaboration/templete/systemMoveTemplete&categoryType=' +
    // getParameter('categoryType') + "&from=" +
    // getParameter('from')+"&move=true",
    // width : 400,
    // height : 500
    // });
    //
    ...
    // var listForm =
    // parent.templeteListFrame.document.getElementsByName("listForm")[0];
    // if(listForm){
    // listForm.action = templeteURL + "?method=doMoveTemplete&newCategoryId=" +
    // newCategoryId;
    // listForm.method = "post";
    // listForm.target = "_self";
    // listForm.submit();
...
function openTempleteConfigDlg() {
    ...
    for (var i = 0; i < templeteIds.length; i++) {
        var element_id = document.createElement("<INPUT TYPE='HIDDEN' NAME='templeteIds' value='" + templeteIds[i]
                + "' />");
        var element_type = document.createElement("<INPUT TYPE='HIDDEN' NAME='templeteTypes' value='"
                + templeteTypes[i] + "' />");
...
function openTempleteType(fromPage) {
    var resultList = v3x.openWindow({
        url : genericControllerURL + "collaboration/templete/templeteConfigIndex&categoryType=0,1,2,3,4,5&from="
                + fromPage,
        height : "600",
        width : "800"
    });
...
function cancelPush() {
    ...
    for (var i = 0; i < len; i++) {
        if (!id_checkbox[i].checked)
            ...
            // var element = document.createElement("<INPUT TYPE=HIDDEN
            // NAME=configIds value='" + selectId + "' />");
...
function sortSetting() {
    ...
    for (var i = 0; i < idsList.length; i++) {
        // var element = document.createElement("<INPUT TYPE='HIDDEN'
        // NAME='sortConfigIds' value='" + idsList[i] + "' />");
        var element = document.createElement("input");
        element.setAttribute('type', 'hidden');
            ...
            if (oSelect.options[selIndex].selected) {
                if (selIndex > 0) {
                    if (!oSelect.options[selIndex - 1].selected) {
                        ...
                        // oSelect.options[selIndex].swapNode(oSelect.options[selIndex
                        // - 1]);
                    }
                }
            }
...
function alertChangeType(obj) {
    ...
    // alert(_("collaborationLang.templete_alertChangeType_" + opt.value,
    // opt.text))
    if (opt.value == 'template') {
        // 协同模板：将协同所有的信息作为模版保存，调用者不允许更改流程!
        $.alert("${ctp:i18n('collaboration.template.pub.label1')}");
    } else if (opt.value == 'text') {
        // 格式模板：将把协同的正文模板保存，调用者只引用正文!
        $.alert("${ctp:i18n('collaboration.template.pub.label2')}");
    } else if (opt.value = "workflow") {
        // 流程模板：将把协同流程作为模板保存，调用者只能引用流程，不允许修改流程!
        $.alert("${ctp:i18n('collaboration.template.pub.label3')}");
    }
    var colPigeonhole = document.getElementById("colPigeonhole");// 预归档
    var project = document.getElementById("projectId");// 关联项目
    var implrtLevel = document.getElementById("importantLevel");// 重要程度
    var tn = document.getElementById("templeteNumber");// 模板编号
    var referenceTime = document.getElementById("referenceTime");// 基准时长
    var deadline = document.getElementById("deadline");// 流程期限
    var advanceRemind = document.getElementById("advanceRemind");// 提醒时间
    ...
    var canAnyMerge = document.getElementById("canAnyMerge");
    ...
    var canTrackWorkFlow = document.getElementById("canTrackWorkFlow");
    var copyFlow = document.getElementById("copyFlow");
    canTrackWorkFlow.selectedIndex = 0;
    if (opt.value == "template") {
	    if(colPigeonhole!=null){
        ...
        tn.disabled = false;
        referenceTime.disabled = false;
        deadline.disabled = false;
        advanceRemind.disabled = false;
        implrtLevel.disabled = false;
        ...
        canMergeDeal.checked = false;
        canMergeDeal.disabled = false;        
        canAnyMerge.checked = false;
        canAnyMerge.disabled = false;
        ...
        copyFlow.disabled = false;
        canTrackWorkFlow.disabled = false;

    } else if (opt.value == "text") {
        // 格式模板:1、保存协同标题、正文、正文类型、模板分类、模板类型
        // 2、不保存流程、附言，附件、流程期限、提醒时间、基准时长 预归档和关联项目、允许操作、重要程度、模板编号、督办设置
        copyFlow.disabled = true;
        tn.disabled = true; // 模板编号
        setParaWhenIsWorkflowOrText();
        canForward.checked = false;// 设置5个高级属性
        canForward.disabled = true;
        canModify.checked = false;
        canModify.disabled = true;
        canMergeDeal.checked = false;
        canMergeDeal.disabled = true;
        canAnyMerge.checked = false;
        canAnyMerge.disabled = true;
        ...
        canAutoStopFlow.checked = false;
        canAutoStopFlow.disabled = true;
        canTrackWorkFlow.disabled = true;
    } else if (opt.value = "workflow") {
        // 流程模板:1、保存协同的标题、流程、模板类型、分类、允许操作、模板编号
        // 2、不保存协同正文、发起者附言,附件，流程期限和提前提醒时间、、基准时长 ，重要程度、正文类型、预归档和关联项目、督办设置、
        copyFlow.disabled = false;
        setParaWhenIsWorkflowOrText();
        tn.disabled = false;
...
// 格式模版或者流程模板的时候设置共有的一些不保存的信息
function setParaWhenIsWorkflowOrText() {
    var colPigeonhole = document.getElementById("colPigeonhole");// 预归档
    var project = document.getElementById("projectId");// 关联项目
    var implrtLevel = document.getElementById("importantLevel");// 重要程度
    var referenceTime = document.getElementById("referenceTime");// 基准时长
    ...
    var advanceRemind = document.getElementById("advanceRemind");// 提醒时间
	if(colPigeonhole != null){
	    colPigeonhole.options[0].selected = "selected";// 预归档
    	colPigeonhole.disabled = true;
	}
    if(project){
	    project.options[0].selected = "selected";// 关联项目
   	 	project.disabled = true;
    }
    implrtLevel.options[0].selected = "selected";// 重要程度
    implrtLevel.disabled = true;
    referenceTime.options[0].selected = "selected";// 基准时长
    referenceTime.disabled = true;
    deadline.options[0].selected = "selected";// 流程期限
    deadline.disabled = true;
    advanceRemind.options[0].selected = "selected";// 提醒时间
