                ...
                <tr>
                    <c:forEach items="${templeteListSend }" var="templete" varStatus="ordinal">
                        <td class="text-indent-1em sorts" width="25%"> 
                          ...
                          <a title="${ctp:toHTMLWithoutSpace(templeteName)}&#13${templeteCreatorAlt[templete.id] }" class='defaultlinkcss' 
                ...
                <tr>
                    <c:forEach items="${templeteListRec }" var="templete" varStatus="ordinal">
                        <td class="text-indent-1em sorts"  width="25%"> 
                            ...
                            <a title="${ctp:toHTMLWithoutSpace(templeteName)}&#13${templeteCreatorAlt[templete.id] }" class='defaultlinkcss' 
                            href="${path}/edocController.do?method=entryManager&entry=recManager&toFrom=newEdoc&listType=newEdoc&templeteId=${templete.id}">
                             <span class="ico16 ${templeteIcon[templete.id]} }"></span>&nbsp;${ctp:toHTMLWithoutSpace(templeteShowName)}</a>
                        </td>
                        ${(ordinal.index + 1) % 4 == 0 && !ordinal.last ? "</tr><tr>" : ""}
                        <c:set value="${(ordinal.index + 1) % 4}" var="i" />
                ...
                <tr> 
                    <c:forEach items="${templetes }" var="templete" varStatus="ordinal">
                        <td class="text-indent-1em sorts"  width="25%">
                            ...
                            <c:set var="createTitle" value="${templeteName }"/>
                            <a title="${ctp:toHTMLWithoutSpace(createTitle)}&#13${templeteCreatorAlt[templete.id] }" class='defaultlinkcss' href="${path }/collaboration/collaboration.do?method=newColl&templateId=${templete.id}"><span class="ico16 ${templeteIcon[templete.id]} }"></span>
                            &nbsp;${ctp:toHTMLWithoutSpace(templeteShowName)}</a> 
                        </td>
                        ${(ordinal.index + 1) % 4 == 0 && !ordinal.last ? "</tr><tr>" : ""}
                        <c:set value="${(ordinal.index + 1) % 4}" var="i" />
                    </c:forEach>
