...
var wfProcessId = '${contentContext.wfProcessId==null ? "" : contentContext.wfProcessId}'
</script>
</head>
                ...
                <tr>
                    ...
                    <td width="28%" colspan="2">
                        <div class="common_form_wrap">
                            <!-- 标题 -->
                            <input name="subject" type="text" id="subject" style="color: #000; line-height:24px;" maxlength="60" inputName="${ctp:i18n('common.subject.label')}" 
                            validate="name:'${ctp:i18n('collaboration.newcoll.subject') }',type:'string',notNull:true,maxLength:60,character:'<>|\'&quot;'" value="${ctp:toHTMLWithoutSpace(template.subject)}" defaultValue="${ctp:i18n('common.default.subject.value')}">
                        </div>
                    </td>
                ...
                <tr>
                    ...
                    <td width="20%">
                        <div class="common_form_wrap" style="width: 100%;">
                            <input id="process_info" name="process_info" style="font-size:12px; line-height:24px;" value="${ctp:toHTML(contentContext.workflowNodesInfo)}" 
                    ...
                    <td width="8%" style="text-align: right;" class="padding_l_10">
                        <div class="common_form_wrap" >
                            ...
                            id="copyFlow" class="common_button common_button_icon comp" style="height: 25px;"/>
                        </div>
                    </td>
                    ...
                    <c:if test="${ctp:hasPlugin('doc')}">
	                    ...
	                    <td width="9%">
	                        <div class="common_form_wrap">
	                            ...
	                            <input type="hidden" id="archiveCollPathName" value="${archiveName}">
	                            <input type="hidden" id="archiveCollPathId" value="${summary.archiveId}">
	                            <input type="hidden" id="attachmentArchiveName" value="${attachmentArchiveName}">
	                            <input type="hidden" id="attachmentArchiveId"  value="${summary.attachmentArchiveId}">
	                        </div>
	                    </td>
                    </c:if>
    ...
    <div id="advanceHTML" class="hidden">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="font_size12 margin_t_5">
            ...
            <tr>
                ...
                <td width="70%">
                    <select id="canTrackWorkFlow" name="canTrackWorkFlow" onchange="" value="" <c:if test='${template.type == "text" || template.type == "workflow"}'>disabled</c:if> >
            ...
            <tr>
                <td colspan="2" style="padding-left: 116px"  class="padding_t_5">
                    ...
                    <div style="margin-bottom:12px;">
                     ...
                     <div style="margin-bottom:12px;">
                        <label for="canMergeDeal">
                            <%--合并处理 --%>
                            <input type="checkbox" class="padding_r_5" name="canMergeDeal" id="canMergeDeal" value="1"  <c:if test="${template.type == 'text' or summary.canAnyMerge}">disabled</c:if> <c:if test="${summary.canMergeDeal or summary.canAnyMerge}">checked</c:if> />
                            ${ctp:i18n('collaboration.allow.canmergedeal.label')}
                        </label>
                    </div>
                     <div style="margin-bottom:12px;">
                        <label for="canAnyMerge">
                            <%-- 处理人与前面任一处理人相同时，合并处理 --%> 
                            <input type="checkbox" class="padding_r_5" name="canAnyMerge" onclick="canAnyMergeClk(this);" id="canAnyMerge" value="1"  <c:if test="${template.type == 'text'}">disabled</c:if> <c:if test="${summary.canAnyMerge}">checked</c:if> />
                            ${ctp:i18n('collaboration.allow.before.same.merge.label')}
                        </label>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td width="70%"><input type="text" id="templeteNumber" name="templeteNumber" class="" value="${template.templeteNumber}" maxlength="20" <c:if test='${template.type == "text"}'>disabled</c:if>>
