...
function checkRepeatTempleteSubject(form1){
    ...
    var temManager = new templateManager();
 	var returnVal = temManager.getCtpTemplates(categoryId,subject,null,null);
...
function notSpecialChar(element){
	...
	if(/^[^\|\\"'<>]*$/.test(value)){
		...
		$.messageBox({
	        ...
	        'msg' : inputName+"${ctp:escapeJavascript(ctp:i18n('collaboration.newColl.tszf'))}", //不能包含特殊字符（|\"'\<>），请重新输入！
	        'imgType':2,
	        ok_fn : function() {
	    		$(element).select();
	        }
...
function checkTemplateIsDelete(templateId){
	  ...
	  var result = tm.isTemplateDelete(templateId);
	  if(result.isDel =='1'){
		  return true;
	  }else{
		  return false;
