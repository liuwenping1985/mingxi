        ...
        function changeImgUrl(option) {
            ...
            switch (obj.attr("alt")) {
                ...
                case "关闭按钮":
                    ...
                    break;
                case "进入主题":
                    if("${ctp:hasPlugin('news') || ctp:hasPlugin('bulletin') || ctp:hasPlugin('bbs') || ctp:hasPlugin('inquiry') || ctp:hasPlugin('show')}" == "true"){
                        $.ajax({
                          url : "${path}/portal/portalController.do?method=showCorporationSpace",
                          async : true,
                          success : function(data){
                            ...
                            if(content&&content.result == "true"){
                                ...
                                //显示单位空间
                                var corporationSpaceId = content.CorporationSpace;
                                if(corporationSpaceId){
                                    productTop.refreshNavigation(corporationSpaceId);
                                }
                                productTop.productView_Obj.close();
                                productTop.productView_Obj = null;
                            }else{
...
<body class="over_hidden">
    <img id="map" usemap="#map" src="${path}/skin/${CurrentUser.skin}/images/productView/whjs.jpg">
