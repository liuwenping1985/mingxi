...
$(document).ready(function() {
  $('#grid_detail').resize(function(){
    $('#linkSystem_div').height($(this).height()-50);
	    ...
	    subMenu: [{
	      ...
	      click: function(){
	    	...
	    	$("#sort_td").append("<div class='common_txtbox_wrap'>"+
            		"<input type='text' id='sort' class='validate'  validate=\"name:'${ctp:i18n('link.jsp.add.order')}',notNull:true,isInteger:true,min:0,max:999999999,notNullWithoutTrim:true\" /></div>");
            ...
            $("#image").attr("src","${path}/decorations/css/images/portal/defaultLinkSystemImage.png?V=V6_1_2017-04-13");
...
//               $("#linkTab_head li:eq(1)").show();
  ...
  function treeClk(e, treeId, node) {
    ...
    $("#rpInputChange").val("20");//OA-120915.....点树的时候把每页显示条数回复到20...
	mytable.p.rp="20";
    $("#mytable").ajaxgridLoad(params);
    $("#grid_detail").hide();
    $("#manual").show();
  ...
  function showDetail(linkSystem){
    ...
    */
    ...
    $("#linkSystemAcl_txt").html(aclTxts);
    ...
    for(var i = 0; i < linkSystem.linkSpaces.length; i++){
      var spaceTableClone = customClone($("table[spaceTable][sample='true']"), "spaceTable");
      if(linkSystem.linkSpaces[i]==null||linkSystem.linkSpaces[i]==""||linkSystem.linkSpaces[i]==undefined){
    	  continue;
      }
      $(spaceTableClone).attr("sample", "false");
  ...
  function mytableSuccess() {
	  $(".noClick").bind("click",function(event){
		   if(!isSystemAdmin){
			  if(currentCategoryId==0||currentCategoryId==5){
				linktoolbar.disabledAll();
	          }else{
				linktoolbar.enabled("newLink");
				linktoolbar.enabled("modifyLink");
    ...
    if (!$._isInValid(formobj)) {
      ...
      if(linkSystem.allowedAsSpace == 1){
        ...
        for(var i = 0; i < $(spaceTables).size(); i++){
          ...
          linkSpace.sort = linkSystem.orderNum;
...
function addOptionClickHandler(option){
  ...
  html += "<td width='135'><div class='common_txtbox_wrap'><input paramName='' id='paramName" + optionId + "' type='text' value='" + escapeStringToHTML(paramName) + "' class='validate paramName' validate=\"name:'参数名称',notNull:true,maxLength:128\" /></div></td>";
  html += "<td width='135'><div class='common_txtbox_wrap'><input  paramSign='' id='paramSign" + optionId + "' type='text' value='" + escapeStringToHTML(paramSign) + "' class='validate paramSign' validate=\"name:'参数标记',notNull:true,maxLength:128\" /></div></td>";
  html += "<td><div class='common_txtbox_wrap'><input  paramValue='' id='paramValue" + optionId + "' type='"+type+"' value='" + escapeStringToHTML(paramValue) + "' class='validate paramValue' validate=\"name:'参数默认值',maxLength:1000\" /></div></td>";
				...
				<div id='linkTab_body' class="common_tabs_body">
					<div id="linkSystem_div" style="overflow-y:scroll">
						<div class="form_area" id="form_area" style="width:565px; margin:0 auto;">
			                ...
			                <div class="align_center hidden padding_10">
