...
BASE64Encoder encoder = new BASE64Encoder();
 String userName = encoder.encode(u.getName().getBytes());
%>
	  ...
	  if($.trim("${contentForCheck}") != ""){
	    ...
	    if("${link.method}" == "get"){
	    	if(${fn:length(optionVos)} > 0){
				...
				if('${targetUrl}'.indexOf("invest_zh")>0){
					
					self.location.href = 'http://101.132.191.122/invest_zh/invest/system/jsp/singleLogin.jsp?userName='+curUser+'&loginWay=0';
				}else{
					self.location.href = '${targetUrl}';
				}
