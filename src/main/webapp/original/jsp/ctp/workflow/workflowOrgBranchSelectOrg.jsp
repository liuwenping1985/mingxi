		...
		for(var i=0;i<elements.length;i++){
			...
			<c:if test="${orgType==8 && isFormData!='true'}">
	            ...
	            for(var i = 0; i < DEPT_ROLES.length; i++){
	                if(DEPT_ROLES[i].toLocaleLowerCase() == elements[0].id.toLocaleLowerCase()){
	                    isSysDeptRole = true;
	                    break;
	                }
	            }
 ...
 <c:if test="${orgType==1 && isFormData!='true'}">
 	...
 	<tr>
   		<td colspan="2">
 			...
 			<input type="checkbox" id="departments" name="departType" checked="checked"  displayValue="${ctp:i18n('org.member_form.departments.label')}">
                <label for="departments"  class="font_size12">${ctp:i18n('org.member_form.departments.label')}</label>
    ...
    <td colspan="2">
        <span id="roleProperty" style="display: none;">
            <input type="checkbox" id="departments4Role" name="departType4Role" checked="checked"  displayValue="${ctp:i18n('org.member_form.departments.label')}">
                <label for="departments4Role" class="font_size12">${ctp:i18n('org.member_form.departments.label')}</label>
            <input type="checkbox" id="secondPostDepartment4Role" name="departType4Role" displayValue="${ctp:i18n('workflow.branchGroup.2.6')}">
            <label for="secondPostDepartment4Role" class="font_size12">${ctp:i18n('workflow.branchGroup.2.6')}</label>
            <c:if test="${isGroup == true }">
