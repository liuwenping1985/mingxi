...
$(function(){
    ...
    $("#processMode li").bind("click",function(){
        $(this).addClass("current").find("input").prop("checked",true);
        $(this).siblings('li').removeClass('current').find("input").prop("checked",false);
...
function OK(){
    ...
    var backToMeObj= $("#backToMe");
    if(backToMeObj && backToMeObj[0] && backToMeObj[0].checked==true){
    	workflowAddNodeMessage.backToMe = "1";
    }else{
    	workflowAddNodeMessage.backToMe = "0";
    }
...
function policyExplain(){
    var dialog = $.dialog({
        url : '<c:url value="/workflow/designer.do?method=policyExplain&type=2"/>',
        transParams : window,
        width : 364,
        height : 275,
