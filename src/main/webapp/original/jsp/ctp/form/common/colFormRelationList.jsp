...
<div id='layout'>
    ...
    <div class="layout_north bg_color" id="north">
        ...
        <div id="toolbars" class="margin_l_5 margin_t_5 font_size12">
            <label for="mySent"><input id="mySent" class="radio_com" type="radio" name="colType" value = "1">${ctp:i18n("form.relation.search.mysent")}</label>
            <label for="delegateToMe"><input id="delegateToMe" class="radio_com" type="radio" name="colType" value = "2">${ctp:i18n("form.relation.search.other")}</label>
        </div>  
    </div>
    <!-- 中间 -->
...
function OK(){
    ...
    $(selectedBox.toArray()).each(function(masterIndex){
        ...
        for(var i=0;i<toFormBean.tableList.length;i++){
            ...
            var tableName = tempTable.split("|")[0];
            var tableType = tempTable.split("|")[1];
            if(tableType.toLowerCase()==="slave"){
                var tempSubData = new Object();
                tempSubData.tableName = tableName;
                if(jqThis.val()==moduleId){
                    var subDom = $("#"+tableName,viewFrame);
                    var tempSubArray = new Array();
                    if(subDom){
                        var allCheckedBox = $(":checkbox[checked][tableName='"+tableName+"']",$(viewFrame[0].contentWindow.document));
                        if(allCheckedBox.length==0){
                            allCheckedBox = $(":checkbox[tableName='"+tableName+"']:eq(0)",$(viewFrame[0].contentWindow.document));
                        }
...
$(document).ready(function () {
    ...
    var slideToggleBtnTag = (showView =="false" ? false:true);
    //表格加载
    var grid = $('#listSent').ajaxgrid({
        colModel: [{
            ...
            display: "${ctp:i18n('form.forminputchoose.rename')}",
            name: 'subject',
            sortable : true,
            width: '45%'
        },{
            display: "${ctp:i18n('form.system.start.member.field.label')}",
            name: 'startMemberName',
            sortable : true,
            width: '20%'
        }, {
            display: "${ctp:i18n('form.system.start.time.field.label')}",
            name: 'startDate',
            sortable : true,
            width: '30%'
        }],
        click: clickRow,
        render : rend,
        showTableToggleBtn: slideToggleBtnTag,
        isToggleHideShow:slideToggleBtnTag,
        parentId: $('.layout_center').eq(0).attr('id'),
        vChange: slideToggleBtnTag,
        vChangeParam: {
            overflow: "hidden",
            autoResize:slideToggleBtnTag
        },
        isHaveIframe:slideToggleBtnTag,
        height:200,
        slideToggleBtn:slideToggleBtnTag,
        managerName : "formRelationManager",
    ...
    function clickRow(data,rowIndex, colIndex) {
        ...
        var viewFrame = $("#viewFrame");
        getCtpTop().processBar =  getCtpTop().$.progressBar({text: ""});
        viewFrame.attr("src",getUnflowFormViewUrl("true",data.summaryId,${enu.ModuleType.collaboration},data.operationId,2));
        viewFrame.unbind("load").bind("load",function(){
            setTimeout(function(){
                if(viewFrame[0].contentWindow.initRelationSubTable){
                    viewFrame[0].contentWindow.initRelationSubTable({"type":"relationForm"});
                }
