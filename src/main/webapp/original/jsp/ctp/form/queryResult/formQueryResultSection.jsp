    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function clickRow(id) {
                <c:if test="${hasRight}">
                showFormData4Statistical(${formBean.formType},id,"${rightStr}","${queryBean.name}",null,"${queryBean.formBean.id}","${queryBean.id}");
                </c:if>
            }
        </script>
    </head>
                      ...
                      <c:forEach items="${result.data }" var="resultData">
                          <c:choose>
                              ...
                              <c:when test="${resultData.isImage and not empty resultData.value}">
                                  <td class="erow" onclick="clickRow('${result.id }')" style="${hasRight ? 'cursor:pointer' : ''}" id="${result.id }" nowrap="nowrap" ><img class="showImg" src="${path}/fileUpload.do?method=showRTE&fileId=${resultData.value }&expand=0&type=image" height=25 />&nbsp;</td>
                              </c:when>
                          <c:otherwise>
                              <td class="erow" onclick="clickRow('${result.id }')" id="${result.id }" nowrap="nowrap" title="${resultData.value }" style="max-width: 120px;${hasRight ? 'cursor:pointer' : ''}"><div class="ellipsis">${ctp:toHTMLAlt(resultData.value)}&nbsp;</div></td>
                          </c:otherwise>
                          </c:choose>
                  </c:forEach>
		        </tr>
