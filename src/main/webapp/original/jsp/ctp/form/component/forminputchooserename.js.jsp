...
<script type="text/javascript">
	...
	var departType = "${inputType}";
	var digitNum = "${digitNum}";
		...
		if (queryDataArea.selectedIndex < 0){
			...
			if($.trim(renameJSON) != ""){
				...
				if(_inputType == 'checkbox'){
					...
					//--------------
					$("#leftDataTimeValue").val($.toJSON(jsonObj.leftdateTimeYzxxValue));
					$("#leftDataTimeName").val(jsonObj.leftdateTimeYzxxName);
					//缺省选中第一项
	...
	function OK() {
		...
		if (titleObj.value.Trim() == ""){
			return "${ctp:i18n('form.forminputchoose.renametitlecantnull')}";
		}

		//判断修改后标题长度是否超过255
		if (len(titleObj.value.Trim()) > 255) {
			return "${ctp:i18n('form.forminputchoose.lengtherror')}" + "不能超过255个字符!";
		}
		...
		if(_inputType == 'checkbox'){
			...
			var leftDataTimeYzxxValue = $.trim($("#leftDataTimeValue").val());
			if(dateTimeYzxxValue == ""){
				isSettingDefaultVal = false;
				if(leftDataTimeYzxxValue == ""){
					leftDataTimeYzxxValue = "{leftArray:[],rightArray:[]}";
				}
				resultObj.leftdateTimeYzxxValue = $.parseJSON(leftDataTimeYzxxValue);
				resultObj.leftdateTimeYzxxName = $("#leftDataTimeName").val();
			}else{
			...
			if ("text" == checkedDefaultType) {
				...
				if("DECIMAL" == _fieldType){
					if(defaultVal.length>0){
						if(isNumber(defaultVal) == false ||defaultVal.indexOf(" ")!=-1){
							defaultVal = defaultVal.trim();
							return "${ctp:i18n('form.query.defaultvalue.label')}:" + "${ctp:i18n('plan.alert.plansummary.enternumber')}";
						}else{
							if(defaultVal.indexOf(".") != -1){
								var digitStr = defaultVal.substring(defaultVal.indexOf(".") + 1, defaultVal.length);
								if(defaultVal.length - 1 > fieldLength){
									return "数字位数不正确!";
								}
								if(digitStr.length > digitNum){
									return "小数位数不正确!";
								}
							}else{
								if(defaultVal.length > fieldLength){
									return "数字位数不正确!";
								}
							}
						}
	...
	function dateTimeYzxxFocus(obj){
		...
		var leftJSON = $("#leftDataTimeValue").val();
		var options = "";
		if($.trim(optionsJSON) != ""){
			options = $.parseJSON(optionsJSON);
		}
		if($.trim(optionsJSON) == "" && $.trim(leftJSON) != ""){
			options = $.parseJSON(leftJSON);
		}
		...
		var dialog = $.dialog({
			...
			buttons:[{
				...
				handler : function(){
					...
					if(result.OK == "error"){
						$("#dateTimeYzxxName").val("");
						$("#dateTimeYzxxValue").val("");
						...
						if(rightArray.length == 0){
							var leftArray = result.leftArray;
							var leftShowName = "";

							for(var i=0;i < leftArray.length;i++){
								var rObj = leftArray[i];
								if(i < leftArray.length - 1){
									leftShowName += rObj.name + "、";
								}else{
									leftShowName += rObj.name;
								}
							}

							$("#leftDataTimeName").val(leftShowName);
							$("#leftDataTimeValue").val($.toJSON(result));
							$("#dateTimeYzxxName").val("");
							$("#dateTimeYzxxValue").val("");
