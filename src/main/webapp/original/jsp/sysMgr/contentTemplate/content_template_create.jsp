...
<body scroll='no' class="border-top">
<script type="text/javascript">
		...
		function saveContent() {
			var templateFormat = document.getElementById("bodyType").value;
			if($('dataForm').onsubmit()){
				if (templateFormat == "OfficeWord" || templateFormat == "OfficeExcel" || templateFormat == "WpsWord" || templateFormat == "WpsExcel") {
					document.getElementById("content").value = getOfficeOcxRecordID();
					saveOffice();
				}
				isFormSumit = true;
				disableButtons();
	...
	//	bodyTypeSelector.add(new WebFXMenuItem("menu_bodytype_WpsExcel", "<fmt:message key='common.body.type.wpsexcel.label' bundle='${v3xCommonI18N}' />", "chanageBodyTypeExt('<%=Constants.EDITOR_TYPE_WPS_EXCEL%>')", "<c:url value='/common/images/toolbar/bodyType_wpsexcel.gif'/>"));
    	...
    	myBar.add(new WebFXMenuButton("submit", "<fmt:message key='common.toolbar.save.label' bundle='${v3xCommonI18N}' />", "saveContent()", [1,5], "", null));   	
		...
		var onlyLoginAccount_spAccountSpace = true;
		var hiddenOtherMemberOfTeam_spAccount = true;
    	
    	function selectIssueArea(type){
			if(type=="group"){
				selectPeopleFun_spGroupSpace();
			}
			if(type=="account"){
				selectPeopleFun_spAccountSpace();
			}
		}
    	...
    	function setPeopleFields(elements){
			...
			var a = document.getElementById("useRange");
			a.value = getNamesString(elements);
			hasUseRange = true;
		}
</script>
<div class="newDiv">
<form id="dataForm" name="dataForm" action="${contentTemplateURL}" method="post" onsubmit="return checkForm(this)">
<input type="hidden" name="method" value="save" />
<input type="hidden" name="id" value="${bean.id}" />
<input type="hidden" name="templateFormat" id="templateFormat" value="${bean.templateFormat}" />
<input type="hidden" id="issueArea" name="issueArea" value="${issueArea}">
...
<input type="hidden" id="valiname" value="${valiName}" >
	<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" align="center" class="CollTable">
		...
		<tr class="bg-summary lest-shadow">
			...
			<input type="text" style="width: 400px;height:18px;" name="templateName"
				maxlength="20"
				value="<c:out value="${bean.templateName}" default="${_myLabelDefault}" escapeXml="true" />" 
				title="${v3x:toHTML(bean.templateName)}"
		...
		-->	
			    ...
			    <TD width="25%"><select name="type" style="font-size:12px;width:300;font:'瀹嬩綋'; height:20">
			    ...
			    <c:if test="${v3x:hasPlugin('meeting')}">
			     ...
			     <!-- 使用授权 -->
			     <c:set value="${v3x:parseElementsOfTypeAndId(issueArea)}" var="org"/>
				 <c:set var="issueAreaName" value="${v3x:showOrgEntitiesOfTypeAndId(issueArea, pageContext)}" />
					
				 <v3x:selectPeople panels="Department,Team,Post,Level,Outworker" originalElements="${org}" departmentId="${v3x:currentUser().departmentId}" jsFunction="setPeopleFields(elements)" selectType="Account,Department,Team,Post,Level,Member" id="spAccountSpace"/>
			     <v3x:selectPeople panels="Account,Department,Team,Post,Level" originalElements="${org}"  departmentId="${v3x:currentUser().departmentId}" jsFunction="setPeopleFields(elements)" selectType="Account,Department,Team,Post,Level,Member" id="spGroupSpace"/>
			     <td width="15%" class="bg-gray"><fmt:message key="mt.mtContentTemplate.Authorization"/><fmt:message key="label.colon" /></td>
			     <td width="20%" class="bg-gray" style="height:18px; text-align:left;">
			     	<input type="text" name="useRange" value="${issueAreaName}"  onclick="javascript:selectIssueArea('${userType}');" id="useRange" readonly style="width:200px;"></input>
			     </td>
		...
		<tr>
			<td>
				<div id="contentDiv">
					<v3x:editor htmlId="content" barType="BasicAdmin" content="${bean.content}" type="${bean.templateFormat}" createDate="${bean.createDate}" category="<%=ApplicationCategoryEnum.meeting.getKey()%>" />
				</div>
			</td>
		</tr>
	</table>
...
<jsp:include page="deal_exception.jsp" />

</body>
<script type="text/javascript">
initIe10AutoScroll("contentDiv", 38);
</script>
</html> 