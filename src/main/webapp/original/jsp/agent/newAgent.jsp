...
var startDate = "";
var lastSelectIndex;
var isGroupFlag = '${isGroupFlag}';
var isAdmin = '${isAdmin}';
//只显示当前单位
if(isAdmin=='true'){
    var onlyLoginAccount_surrogateTo = true;
}
if(isGroupFlag=='true'){
	var accountId_surrogateTo = '${accountId}';
	var accountId_surrogate = '${accountId}';
}
window.onload = function(){
  ...
  var freecoll = document.getElementById("freecoll");
    ...
    if ("${operationType}" == "modify") {
      ...
      if (document.getElementById("surrogateTo") != null) {
        document.getElementById("surrogateTo").value = "${agentToName}";
        document.getElementById("surrogateValueTo").value = "${agent.agentToId}";
      }
      ...
      var isEnableEdoc = "${v3x:hasPlugin('edoc')}";
  ...
  function dataToSurrogate(elements) {
    ...
    var accountShortname = person.accountShortname;
    if (accountShortname != null) {
      accountShortname = "(" + accountShortname + ")";
    } else {
      accountShortname = "";
    }
    var surrogateName = person.name + accountShortname;
  ...
  function dataToSurrogateTo(elements) {
    if (!elements) {
      return;
    }
    var person = elements[0] || [];
    ...
    var surrogateNameTo = person.name + accountShortname;
    var surrogateIdTo = person.id;
    document.getElementById("surrogateTo").value = surrogateNameTo;
    document.getElementById("surrogateValueTo").value = surrogateIdTo;
  }
  ...
  function checkoutDate() {
    ...
    var d1 = new Date(parseInt(temp[0], 10), parseInt(temp[1], 10) - 1,
        parseInt(temp[2], 10), parseInt(temp2[0], 10), parseInt(temp2[1], 10));
    days = beginDate.substring(0, beginDate.indexOf(" "));
    hours = beginDate.substring(beginDate.indexOf(" "));
    temp = days.split("-");
    temp2 = hours.split(":");
    var d2 = new Date(parseInt(temp[0], 10), parseInt(temp[1], 10) - 1,
        parseInt(temp[2], 10), parseInt(temp2[0], 10), parseInt(temp2[1], 10));
  ...
  function ok(from) {
    ...
    var agentId = "";
    var agentToId = "";
    agentId = document.getElementById("surrogateValue").value;
    var beginDate = document.getElementById("beginDate").value;
    var endDate = document.getElementById("endDate").value;
    ...
    for (var i = 0; i < agentOptionObj.length; i++) {
      if (agentOptionObj[i].getAttribute('type') == 'checkbox'
          && agentOptionObj[i].checked) {
        selectedValues.push(agentOptionObj[i].getAttribute("extvalue"));
      }
    }
    if (template.checked == true && templateSelect
    ...
    if (template.checked == false
        && templateIds == '<fmt:message key="templatecoll.select" bundle="${agentI18N}"/>') {
      document.getElementById("templateIds").value = "";
    }
    if (isGroupFlag == 'true' || isAdmin == 'true') {
      agentToId = document.getElementById("surrogateValueTo").value;
      if (agentToId == "") {
        alert(_("agentLang.agentto_not_permit_null_alert"));
        return;
      } else if (agentToId == agentId) {
        alert(_("agentLang.agent_not_permit_equal_alert"));
        return;
      }
    }
    if (agentId == "") {
    ...
    var requestCaller = new XMLHttpRequestCaller(this,
        "ajaxAgentIntercalateManager", "checkDataValidity", false);
    ...
    requestCaller.addParameter(7, "String", agentToId);
    var rv = requestCaller.serviceRequest();
    if (rv != null && rv != "") {
      alert(rv);
      return;
...
<script type="text/javascript">
	...
	if ("${operationType}" == "new") {
    	if (document.getElementById("surrogateTo")) {
    		document.getElementById("surrogateTo").focus();
    	}
	}
</script>
<script type="text/javascript">
