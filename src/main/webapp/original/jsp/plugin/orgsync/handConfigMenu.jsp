...
<script type="text/javascript">
	...
	function modify() {
		...
		if (ids.size() == 2) {
			...
			if (idStrs[2] == "NULL"){
				alert("<fmt:message key='orgsync.org.delgroupbind' />");
					return ;
			}
			...
			alert('<fmt:message key='orgsync.org.select.onlyone' />');
			return false;
		} else {
			alert('<fmt:message key='orgsync.org.select.one' />');
			return false;
		}
	}
	...
	var ids = parent.listAndDetailFrame.listFrame.document.getElementsByName('id');
		...
		if (!confirm('<fmt:message key='orgsync.org.deletebind' />')){
			return false;
		}
