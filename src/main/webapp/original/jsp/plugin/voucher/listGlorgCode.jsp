...
$().ready(function(){
    ...
    $("#postingdate").val(billdate);
    $("#fiscalYear").val(date.print("%Y"));
    $("#fiscalPeriod").val(date.print("%m"));
    ...
    $("#billdate").change(function(){
    	if($(this).val()==''){
    		...
    		var date2 = $(this).val();
        	$("#fiscalYear").val(date2.substring(0,4));
            $("#fiscalPeriod").val(date2.substring(5,7));
    	}
    });
	...
	function OK() {
		...
		var mergeflag = "";
		if("${flag}"=="merge"){
			mergeflag = $("#mergeRule").val();
		}
		return {"accountId":$("#accountId").val(),"bookCode":$("#bookCode").val(),"billdate":$("#billdate").val(),"groupCode":$("#groupCode").val(),
		"postingdate":$("#postingdate").val(),"fiscalYear":$("#fiscalYear").val(),"fiscalPeriod":$("#fiscalPeriod").val(),"mergeflag":mergeflag};	    
	}
</script>
</head>
<body>
		...
		<div class="one_row" style="width:70%;">
			...
			<table border="0" cellspacing="0" cellpadding="0" style="margin-top: 40px;">
				...
				<tbody>					
					...
					<tr id="postTr" class="hidden">
						<th nowrap="nowrap"><label class="margin_r_10" for="text"><font color="red">*</font>${ctp:i18n('voucher.plugin.generate.postdate')}</</label></th>
						<td>
							<div class="common_txtbox_wrap">
								<input id="postingdate" type="text" class="comp validate" readonly="readonly" comp="type:'calendar',ifFormat:'%Y-%m-%d'" validate="notNull:true,minLength:1,maxLength:255,name:'${ctp:i18n('voucher.plugin.generate.postdate')}'">
							</div>
						</td>
					</tr>
					<tr>
						<th nowrap="nowrap"><label class="margin_r_10" for="text"><font color="red">*</font>${ctp:i18n('voucher.plugin.generate.accountYear')}</label></th>
						<td>
							<div class="common_txtbox_wrap">
								<input type="text" id="fiscalYear"  class="validate word_break_all" name="${ctp:i18n('voucher.plugin.generate.accountYear')}"
									validate="notNull:true,minLength:4,maxLength:4,type:'number',regExp:'([12][0-9][0-9][0-9])'">
							</div>
						</td>
					</tr>
					<tr>
						<th nowrap="nowrap"><label class="margin_r_10" for="text"><font color="red">*</font>${ctp:i18n('voucher.plugin.generate.accountPeriod')}</label></th>
						<td>
							<div class="common_txtbox_wrap">
								<input type="text" id="fiscalPeriod"  name="${ctp:i18n('voucher.plugin.generate.accountPeriod')}" class="validate word_break_all"
									validate="notNull:true,minLength:2,maxLength:2,regExp:'([01][0-9])'">
							</div>
						</td>
					</tr>					
					<tr id="merge" class="hidden">
						<th nowrap="nowrap"><label class="margin_r_10" for="text">${ctp:i18n('voucher.plugin.generate.mergingRules')}</label></th>
						<td>
							<div class="common_selectbox_wrap">
								<select  id="mergeRule" name="${ctp:i18n('voucher.plugin.generate.mergingRules')}">
									<option value="sum" selected>${ctp:i18n('voucher.plugin.generate.entrySummary')}</option>
									<option value="merge">${ctp:i18n('voucher.plugin.generate.entryMerge')}</option>
								</select>								
							</div>
						</td>
					</tr>
