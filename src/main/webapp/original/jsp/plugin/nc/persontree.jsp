...
function personInit(){
	...
	if(!EmptyProperties.containsKey("${person.pkPsndoc}"))
	{
		memberDataBody.add(createOption("${person.pkPsndoc}","${person.psnname}"));
	}
	</c:forEach>
}
function personFilderInit(){
	...
	<c:forEach items="${filderlist}" var="person">
		memberDataBody.add(createOption("${person.pkPsndoc}","${person.psnname}"));
		EmptyProperties.put("${person.pkPsndoc}","${person.pkPsndoc}");
	</c:forEach>
}
function doSelect(){
	var memberDataBody = document.getElementById("memberDataBody");
...
{
var  displayName="<fmt:message key='ntp.user.filder.person.question'/>";
return displayName;
}
...
function  savePerson()
{
	...
	if(items==null||items.length==0)
	{
	   alert("<fmt:message key='ntp.user.filder.title.person'/>");
	   return;
	}
	...
	document.form1.action="${ncFilderSynchron}?method=saveFilderPerson";
...
window.onload = function(){
		...
		function deleteFilderPerson(ncorgCorp,pk_psndoc)
	{
          try
          {
          	var requestCaller = new XMLHttpRequestCaller(this, "ajaxNCOrgManager", "deleteFilderPerson",false);
			...
			{
			//alert("<fmt:message key='ntp.user.filder.person.success'/>");
			}
          }catch(ex1)
          {
          alert("Exception : " + ex1);
		...
		<table width="100%" height="100%" border="0"
			...
			<tr align="center">
				...
				<td align="left" class="PopupTitle">
					<fmt:message key='ntp.user.filder.title.person'>
					</fmt:message>
				</td>
			</tr>
			<tr>
								...
								<tr>
									...
									<form name="form1" action="" method="post">
										...
										<input type="hidden" value="${param.ncOrgDeptId}"
											...
											id="listSelect1" name="listSelect1"/>
