...
function checkPeopleBeSet(){
	...
	if(!isDepartmentChecked && isPeopleChecked){
		alert("<fmt:message key="ntp.org.alert.noPeopleChecked"/>");
...
<legend><fmt:message key="ntp.org.synchron.hand.org"/></legend>
<table  width="90%" height="100%" border="0" cellspacing="0" cellpadding="0"  align="center">
	<tr>
		 <td class="bg-gray" width="30%" nowrap="NOWRAP"><label for="name"><font color="red">*</font><fmt:message key="ntp.org.list.name${v3x:getSysFlagByName('NCSuffix')}"/>:</label></td>
		 <td class="new-column" width="70%" nowrap="nowrap">
			<input type="text" id="orgName" name="orgName" deaultValue=""  readonly="readonly"
				...
				inputName="<fmt:message key="ntp.org.list.name"/>" validate="notNull" class="cursor-hand input-80per"
	...
	<tr>
		<td class="bg-gray"  nowrap="nowrap"><font color="red">*</font><label for="code"><fmt:message key="ntp.org.list.nc.org" />:</label></td>
		<td class="new-column" >
		    <select id="ncId" name="ncId" class="input-80per" validate="notNull" inputName="<fmt:message key="ntp.org.list.nc.org" />" ${v3x:outConditionExpression(edit == "1"||edit == "2", 'disabled', '')}>
	...
	<tr>
		<td class="bg-gray"  nowrap="nowrap"><font color="red">*</font><label for="dept.sortId"><fmt:message key="ntp.org.hand.content" />:</label></td>
		...
		<label for="department">
        	<input name="department" id="department" type="checkbox" onclick="checkPeopleBeSet()"
        	             ...
        	             <c:if test="${department}">checked</c:if>><fmt:message key="ntp.org.hand.dept" />
        	</label>
        	<label for="level">
        	<input name="level" id="level" type="checkbox" 
        	             value="level" ${v3x:outConditionExpression(edit == "1", 'disabled', '')} 
        	             <c:if test="${level}">checked</c:if>><fmt:message key="ntp.org.hand.level" />
        	</label>
        	<label for="post">         
        	<input name="post" id="post" type="checkbox" value="post" ${v3x:outConditionExpression(edit == "1", 'disabled', '')} 
        	            <c:if test="${post}">checked</c:if>><fmt:message key="ntp.org.hand.post" />
        	</label>
        	<label for="people">             
        	<input name="people" id="people" type="checkbox" onclick="setDepart()" value="people" ${v3x:outConditionExpression(edit == "1", 'disabled', '')} 
        	            <c:if test="${people}">checked</c:if>><fmt:message key="ntp.org.hand.member" />  
        	</label>                 			    			      
		</td>
	</tr>	
	<tr>
		<td class="bg-gray"  nowrap="nowrap"><label for="person.sortId"><fmt:message key="ntp.org.user.communication" />:</label></td>
		<td class="new-column" nowrap="nowrap">
		<label for="mobile">   
        	<input onclick="checkCommunication()" name="mobile" id="mobile" type="checkbox" 
        	             ...
        	             <c:if test="${mobile!=null}">checked</c:if>><fmt:message key="ntp.org.user.communication.mobile" />
        	</label>
        	<label for="office">               
        	<input onclick="checkCommunication()" name="office" id="office" type="checkbox" 
        	            ...
        	            <c:if test="${office!=null}">checked</c:if>><fmt:message key="ntp.org.user.communication.office" />
        	 </label>   
        	 <label for="email">         
        	<input onclick="checkCommunication()" name="email" id="email" type="checkbox" value="<%=com.seeyon.apps.nc.constants.NCConstants.CommunicationType.eMail.name()%>" ${v3x:outConditionExpression(edit != "1" && people, '', 'disabled')} 
        	            <c:if test="${email!=null}">checked</c:if>><fmt:message key="ntp.org.user.communication.email" />
        	 </label>                  		
		</td>
	</tr>	
	<tr>
		 ...
		 <label for="isNewPerson"> 
        	<input name="isNewPerson" id="isNewPerson" type="checkbox"  onclick="checkCommunication()"
        	             value="<%=com.seeyon.apps.nc.constants.NCConstants.CommunicationType.isNewPerson.name()%>" 
                           ${v3x:outConditionExpression(edit != "1" && people, '', 'disabled')} 
        	             <c:if test="${isNewPerson!=null}">checked</c:if>><fmt:message key="ntp.org.user.communication.newoperation" />
        	 </label>            
...
<td valign="top" align="left">
 <label for="allMap"> 
<input name="allMap" id="allMap" type="checkbox" value="allMap" ${v3x:outConditionExpression(edit == "1", 'disabled', '')}><fmt:message key="ntp.org.user.all" />
</label>
</td>
</tr>
</table>
