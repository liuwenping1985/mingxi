...
<script>
 function showAccountInfo(accountId){
 	...
 	parent.listAndDetailFrame.detailFrame.location.href = "<c:url value='/common/detail.jsp?direction=Down'/>";
 }
...
var root = new WebFXTree("root", "<fmt:message key='menu.group.organ.setting.label' bundle='${v3xMainI18N}'/>", "");
	...
	<c:forEach items="${accountlist}" var="account">
		...
		<c:otherwise>
		    <c:choose>
		    <c:when test="${accoutMap[account.v3xOrgAccount.id].unitshortname!=null&&accoutMap[account.v3xOrgAccount.id].unitshortname!=''}">
			var account${fn:replace(account.v3xOrgAccount.id,'-','_')} = new WebFXTreeItem('${account.v3xOrgAccount.id}','${v3x:escapeJavascript(account.v3xOrgAccount.name)}(${v3x:escapeJavascript(accoutMap[account.v3xOrgAccount.id].unitshortname)})',"javascript:showAccountInfo('${account.v3xOrgAccount.id}');", "return false");
