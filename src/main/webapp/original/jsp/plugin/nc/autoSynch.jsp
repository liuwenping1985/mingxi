...
<script type="text/javascript">
	...
	function autoSynchron(form){
		...
		if(handRadioObj.checked){
			...
			if(count <= 0 && ${v3x:getSysFlagByName('sys_isEnterpriseVer')!=true}){
				alert("<fmt:message key='ntp.org.synchron.hand.selectOrg'/>");
				return false;
			}
		       var requestCaller= new XMLHttpRequestCaller(this, "ajaxNCOrgManager", "isSyningHand",false);
				 var ds= requestCaller.serviceRequest();
				 if(ds=='true'){
					 alert("<fmt:message key='ntp.org.hand.syning'/>");
					 return false;
				 }
		       var requestCaller1= new XMLHttpRequestCaller(this, "ajaxNCOrgManager", "isSyningAuto",false);
				var ds1= requestCaller.serviceRequest();
				if(ds1=='true'){
						 alert("<fmt:message key='ntp.org.auto.syning'/>");
						 return false;
					 }
			...
			if(document.getElementById("isStart1").checked && document.getElementById("intervalTimeRadio").checked){
				if(document.all.intervalDay.value=='0' && document.all.intervalHour.value=='0' && document.all.intervalMin.value=='0'){
					alert("<fmt:message key='ntp.org.synchron.timeSetting.timeMustGT0'/>");
					return false;
				}
			}
	...
	function init(){
		checkSynchState("${ctp:escapeJavascript(synchType)}");
		switchSynchType("${ctp:escapeJavascript(synchType)}");
		if( '${isStart}'=='true'){
			if('${synchTimeType}'=='0'){
				clickSetTimeRadio();
			}else{
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" align="center">
	<tr>
		...
		<table width="90%" height="100%" border="0" cellspacing="0" cellpadding="0" align="center">
			<tr>
				<td height="40" nowrap>
					<b><fmt:message key="ntp.org.synchron.type"/>:</b>&nbsp;&nbsp;
					<label for="handRadio">
						<input id="handRadio" type="radio" name="synchType" value="hand" ${synchType=='auto'? '':'checked'} onclick="switchSynchType('hand')"><fmt:message key="ntp.org.synchron.type.hand"/>
					</label>&nbsp;&nbsp;
					<label for="autoRadio">
						<input id="autoRadio" type="radio" name="synchType" value="auto" ${synchType=='auto'? 'checked':''} onclick="switchSynchType('auto')"><fmt:message key="ntp.org.synchron.type.auto"/>
					</label>
				</td>
			</tr>
			<tr>
				<td height="20" nowrap>
					<font color="red"><p id="synchState" style="display: none;"><fmt:message key="ntp.org.synching"/></p></font>
				</td>
			</tr>
			...
			<tr>
				<td height="100%" valign="top">
					<fieldset style="vertical-align: top">
						<legend style="height: 20px; padding-top: 5px;"><b><fmt:message key="ntp.org.synchron.option"/></b></legend>
						<table width="100%" height="300" id="handSynchOption">
							...
							<tr>
			   				...
			   				<td height="30" style="padding-left: 30px;">
								<font color="red">*</font><fmt:message key="ntp.org.hand.starttime" />:
						...
						<table width="100%" id="autoSynchOption" style="display: none;">
							<tr>
								<td width="60%" class="padding-L">
									<table width="100%">
										<tr>
								    		<td width="100" height="50" nowrap="nowrap" align="right"><font color="red">*</font><fmt:message key="ntp.org.synchron.auto.enable" />：</td>
								    		<td colspan="2">
								    			<label for="isStart1">
			        			  		...
			        			  		<tr>
			        			  			<td rowspan="2" nowrap="nowrap" align="right">
			        			  				<font color="red">*</font><fmt:message key="ntp.org.synchron.timeSetting"/>：
			        			  			</td>
											<td width="80" nowrap="nowrap"><label for="setTimeRadio"><input id="setTimeRadio" type="radio" name="synchTimeType" ${synchTimeType==0?'checked':''} ${isStart?'':'disabled'} value="setTime" onclick="clickSetTimeRadio()"><fmt:message key="ntp.org.synchron.timeSetting.0"/>：</label></td>
											<td>
												<select id="typeSel" name="type" ${isStart?'':'disabled'}>
						        			      <option value="0" ${type == '0'?'selected':''}><fmt:message key="ntp.org.auto.everyday" /></option>
						        			      <option value="1" ${type == '1'?'selected':''}><fmt:message key="ntp.org.auto.sunday" /></option>
						        			      <option value="7" ${type == '7'?'selected':''}><fmt:message key="ntp.org.auto.saturday" /></option>
						        			      <option value="6" ${type == '6'?'selected':''}><fmt:message key="ntp.org.auto.friday" /></option>
						        			      <option value="5" ${type == '5'?'selected':''}><fmt:message key="ntp.org.auto.thursday" /></option>
						        			      <option value="4" ${type == '4'?'selected':''}><fmt:message key="ntp.org.auto.wednesday" /></option>
						        			      <option value="3" ${type == '3'?'selected':''}><fmt:message key="ntp.org.auto.tuesday" /></option>
						        			      <option value="2" ${type == '2'?'selected':''}><fmt:message key="ntp.org.auto.monday" /></option>
						        			    </select>
												...
												<fmt:message key="ntp.org.auto.hour" />
						        			    ...
						        			    <fmt:message key="ntp.org.auto.min" />
									        </td>
										</tr>
										<tr>
											<td nowrap="nowrap"><label for="intervalTimeRadio"><input ${isStart?'':'disabled'} id="intervalTimeRadio" type="radio" name="synchTimeType" ${synchTimeType!=0?'checked':''} value="intervalTime" onclick="clickIntervalTimeRadio()"><fmt:message key="ntp.org.synchron.timeSetting.1"/>：</label></td>
											<td>
												...
												<fmt:message key="ntp.org.synchron.timeSetting.day"/>
												...
												<fmt:message key="ntp.org.synchron.timeSetting.hour"/>
						        			    ...
						        			    <fmt:message key="ntp.org.synchron.timeSetting.min"/>
									        </td>
										</tr>
										</table>
										</td>
