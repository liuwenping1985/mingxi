...
<script type="text/javascript">
	...
	function submitForm(){
        ...
        var array=newNcUserCodes.split(",");
        var flag=isRepeat(array);
        if(flag==true){
        	alert('<fmt:message key="ntp.user.mapper.depulyname"/>');
            return;
        }
        var ncBusinessCodes = "";
        var newBusinessCodes = "";
        if("${ncbusinessEnable}"=='true'){
        	ncBusinessCodes = encodeURI(document.getElementById("ncBusinessUserCodes").value);
        	newBusinessCodes = (document.getElementById("ncBusinessUserCodes").value).replace(/\s+/g,"");
        }
        var ncBindUserCodes;
		var ncBusinessFlag = "${ncbusinessEnable}"=='true'?"${ncBusinessStateNames}"==''&&newBusinessCodes=='':true;
        if(oldNcUserCodes=='' && newNcUserCodes=='' && ncBusinessFlag){
			alert("<fmt:message key='ntp.user.mapper.empty'/>");
			document.getElementById("ncUserCodes").value = "";
			document.getElementById("ncUserCodes").focus();
			return false;
		}else if(oldNcUserCodes=='' && newNcUserCodes==''){
			ncBindUserCodes="";
		}else{
			ncBindUserCodes="&ncUserCodes="+ncUserCodes;
		}
		var ncBusinessUserCodes;
		if("${ncbusinessEnable}"=='true' && "${ncBusinessStateNames}"=='' && newBusinessCodes==''){
			ncBusinessUserCodes="";
		}else if("${ncbusinessEnable}"=='true'){
			ncBusinessUserCodes = "&ncBusinessUserCodes="+ncBusinessCodes;
		}else{
			ncBusinessUserCodes="";
		}
	    var form = document.getElementById("userMapperForm");
	    if(checkForm(form)){
	        parent.detailFrame.location.href = "${urlNCUserMapper}?method=updateUserMapper&id=${member.v3xOrgMember.id}&valideLogin="+valideLogin+ncBindUserCodes+"&oper=update&ncid="+document.getElementById("ncid").value+ncBusinessUserCodes;
	    }
	}
</script>
...
<form id="userMapperForm" method="post" target="editMemberFrame" action="${urlNCUserMapper}?method=updateUserMapper&id=${member.v3xOrgMember.id}" onsubmit="return (submitForm(this))">
	...
	<input type="hidden" id="ncid" name="ncid" value="${v3x:toHTML(ncid)}">
  ...
  <tr valign="top">
    ...
    <fieldset style="width:95%;border:0px;" align="center">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
			...
			<tr>
			  	<td class="bg-gray" width="25%" nowrap="nowrap"><fmt:message key="ntp.user.account"/>:</td>
				<td class="new-column" width="75%" nowrap="nowrap">
				<input class="input-100per" type="text" name="ncUserCodes" maxSize="40" maxLength="40" <c:if test="${oper == 'edit'}">disabled readonly="readonly"</c:if>
					...
					inputName="<fmt:message key="ntp.user.account" />" /></td>
			</tr>
			<c:if test="${ncbusinessEnable == 'true'}">
			<tr>
			  	<td class="bg-gray" width="25%" nowrap="nowrap"><fmt:message key="ntp.user.mapper.shareBinding"/>:</td>
				<td class="new-column" width="75%" nowrap="nowrap">
				<input class="input-100per" type="text" name="ncBusinessUserCodes" maxSize="40" maxLength="40" <c:if test="${oper == 'edit'}">disabled readonly="readonly"</c:if>
					id="ncBusinessUserCodes" value="${ncBusinessStateNames}" validate="isWord" character="，"
					inputName="" /></td>
			</tr>
			</c:if>
			<tr>
			  	<td class="bg-gray" width="25%" nowrap="nowrap"></td>
				<td class="new-column" width="75%" nowrap="nowrap"><font color="#008000"><fmt:message key="ntp.user.account.label"/>
				<c:if test="${ncbusinessEnable == 'true' }"><fmt:message key="ntp.user.account.business.label"/></c:if>
				</font></td>
			</tr>
			<fmt:setBundle basename="com.seeyon.v3x.organization.resources.i18n.OrganizationResources"/>
		</table>
