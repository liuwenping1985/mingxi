...
<script type="text/javascript"
	src="${path}/ajax.do?managerName=deeDataSourceManager"></script>
<script type="text/javascript">
	/**
	 * 是否包含字符 <>\"'|&
	 * @param params 参数
	...
	function testCon() {
		...
		if(hasSpecialChar(map["url"]) || hasSpecialChar(map["user"]) || hasSpecialChar(map["password"])){
			$.error("<fmt:message key='dee.dataSource.hasSpecialChar.msg'/>");
			return;
		}
		map["jndi"] = $("#jndi").val();
				...
				var options = {
					...
					ok_fn : function() {
						window.location.href = _ctxPath
								+ "/deeDataSourceController.do?method=showDataSourceDetail&id="
								+ $("#resource_id").val();
					}
				};
				$.infor(options);
			} else if (retMap.ret_code == "2001") {
				$.error("<fmt:message key='dee.dataSource.error.label'/>"
						+ retMap.ret_desc);
			} else if (retMap.ret_code == "2002") {
				$.error("<fmt:message key='dee.dataSource.error.label'/>"
						+ "<fmt:message key='dee.update.dataSource.tips'/>");
			}
		}
	}
	...
	function changeTemplate(type) {
		if (type == "MySQL") {
			...
			$("#url")
					.val(
							"jdbc:mysql://[host]:[port|3306]/[DB_Name]?autoReconnection=true");
		} else if (type == "Oracle") {
	...
	<div class="form_area">
		<div class="one_row">
			<table border="0" cellspacing="0" cellpadding="0">
				<tbody>
					<tr>
						<th nowrap="nowrap"><label class="margin_r_10" for="dis_name"><font
								color="red">*</font>
							<fmt:message key="dee.dataSource.dis_name.label" />:</label></th>
						<td width="100%">
							<div class="common_txtbox_wrap">
								<input readonly="readonly" type="text" id="dis_name"
									name="dis_name" value="${deeResource.dis_name}">
							</div>
						</td>
					</tr>
					<tr>
						<th nowrap="nowrap"><label class="margin_r_10"><font
								color="red">*</font>
							<fmt:message key="dee.dataSource.template_name.label" />:</label></th>
						<td>
							<div class="common_selectbox_wrap">
								<select name="resource_template_id" id="resource_template_id"
									onchange="changeSourceType(this);">
									<c:forEach var="sourceType" items="${sourceTypes}">
										<option value="${sourceType.value}"
											<c:if test="${dsType == sourceType.value}">selected</c:if>>${sourceType.name}</option>
									</c:forEach>
								</select>
							</div>
						</td>
					...
					<tr class="jdbc-category">
						<th nowrap="nowrap"><label class="margin_r_10"><font
								color="red">*</font>Driver:</label></th>
						<td>
							<div class="common_selectbox_wrap">
								<select id="db" name="db" onchange="changeTemplate(this.value)"
									style="width: 30%">
									<option value="MySQL"
										<c:if test="${dsType=='5'}"> <c:if
								...
								</select> <input type="text" readonly="readonly" id="driver"
									name="driver" size="35" class="cursor-hand input-50per"
									style="width: 68%;"
					...
					<tr class="jdbc-category">
						<th nowrap="nowrap"><label class="margin_r_10" for="url"><font
								color="red">*</font>&nbsp;URL:</label></th>
						<td width="100%">
							<div class="common_txtbox_wrap">
								<input type="text" id="url" name="url"
									<c:if test="${dsType=='5'}"> value="${deeResourceSubBean.url}" </c:if> />
							</div>
						</td>
					</tr>
					<tr class="jdbc-category">
						<th nowrap="nowrap"><label class="margin_r_10" for="user"><font
								color="red">*</font>&nbsp;Username:</label></th>
						<td width="100%">
							<div class="common_txtbox_wrap">
								<input type="text" id="user" name="user"
									<c:if test="${dsType=='5'}"> value="${deeResourceSubBean.user}" </c:if> />
							</div>
						</td>
					</tr>
					<tr class="jdbc-category">
						<th nowrap="nowrap"><label class="margin_r_10" for="password"><font
								color="red">*</font>&nbsp;Password:</label></th>
						<td width="100%">
							<div class="common_txtbox_wrap">
								<input type="password" id="password" name="password"
									<c:if test="${dsType=='5'}"> value="${deeResourceSubBean.password}" </c:if> />
							</div>
						</td>
					</tr>
					<tr class="jndi-category">
						<th nowrap="nowrap"><label class="margin_r_10" for="jndi"><font
								color="red">*</font>&nbsp;<fmt:message
									key="dee.dataSource.JNDIContent.label" />:</label></th>
						<td width="100%">
							<div class="common_txtbox_wrap">
								<input type="text" id="jndi" name="jndi"
									<c:if test="${dsType=='10'}"> value="${deeResourceSubBean.jndi}" </c:if> />
							</div>
						</td>
					</tr>
					<tr>
						<th nowrap="nowrap"><label class="margin_r_10" for="isA8Meta">&nbsp;<fmt:message
									key="dee.dataSource.is_a8meta.label" />:
						</label></th>
						<td width="100%">
							<div class="common_selectbox_wrap">
								<input type="checkbox" id="isA8Meta" name="isA8Meta"
									<c:if test="${metaFlag == 'true'}">checked</c:if> />
							</div>
						</td>
					</tr>
				</tbody>
		...
		<div class="align_center">
			<input type="hidden" id="resource_id" name="resource_id" size="70"
				value="${deeResource.resource_id}" /> <input type="hidden"
				id="resource_desc" name="resource_desc" size="70"
				value="${deeResource.resource_desc}" /> <a href="javascript:"
				onclick="javascript:testCon();"
				class="common_button common_button_gray"><fmt:message
					key="dee.dataSource.TestConnection.label" /></a> <a href="javascript:"
				onclick="javascript:saveDS();"
				class="common_button common_button_gray"><fmt:message
					key='dee.dataSource.save.label' /></a>
		</div>
	</div>
</form>
</html>
