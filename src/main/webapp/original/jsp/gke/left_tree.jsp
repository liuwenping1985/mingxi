...
<script type="text/javascript">
 ...
 function showChildDeptInfo(accountId){
 	parent.detailFrame.location.href = gkeURL + "?method=showChildDeptInfo&id="+accountId;
 }
  function showAccountInfo(){
 	parent.detailFrame.location.href = gkeURL + "?method=showAccountState";
 }
	<%--改变显示模式--%>
	function change(){
	}
...
var root = new WebFXTree("root", "<fmt:message key='menu.group.organ.setting.label' bundle='${v3xMainI18N}'/>", "javascript:showAccountInfo();");
    ...
    <c:if test="${account.v3xOrgAccount.status == 3}"><c:set var="status" value="(${orgDeleted})"/></c:if>
		<c:choose>
		<c:when test="${account.v3xOrgAccount.group==true}">
