...
function submit_name(type, id) {
	...
	if (isCheck == true) {
		...
		if (type && type == 'meeting') {
			...
			var requestCaller = new XMLHttpRequestCaller(this, xmlManager,
					xmlFunction, false);
		...
		} else if (type && type == 'col' || type == 'edoc'
				|| type == 'info') {
			//branches_a8_v350_r_gov GOV-2814 于荒津增加公文个人模版分类查询end
			var requestCaller = new XMLHttpRequestCaller(this, "templateManager",
					"hasRepeatedPersonTemplate", false);
			
			requestCaller.addParameter(1, "Long", "${CurrentUser.id}");
			requestCaller.addParameter(2, "String", newName.trim());
			requestCaller.addParameter(3, "String", "${templateType}" );
			requestCaller.addParameter(4, "Long", id);
			var ds = requestCaller.serviceRequest();
			if (ds == 'true') {
				alert(v3x.getMessage("MainLang.mytemplate_has_exist"));
				return;
