...
<head>
	...
	<title>${ctp:i18n('personalInfo.track.bindphonenum')}</title>
...
function sendVerificationCodeToBindNum(){
	var telNumber='${param.telNumber}';
	$(".gray_button").show();
	$(".get_code").hide();
	resend();
	...
	$.ajax({
		sync : true,
		type: "POST",
		url: "/seeyon/personalBind.do?method=sendVerificationCodeToBindNum",
		data: {"telNumber": telNumber,"type":"bind"},
		dataType : 'text',
...
function bindsuccess(){
	 $.ajax({
			...
			data: {"type":"num","code":'${param.telNumber}'},
			dataType : 'text',
		      success : function(data) {
		    	  var content = jQuery.parseJSON(data);
	...
	<body  onselectstart="return false">
		<div class="container">
			...
			<div class="found_pwd_top">
				<span class="found_pwd_title">${ctp:i18n('personalInfo.track.bindphonenum')}</span>
				<p class="found_pwd_msg">${ctp:i18n('personalInfo.bind.numstepts')}</p>
			</div>
			<div class="found_pwd_body">
				<div class="phone_bind_1 phone" >
					<div class="step_3_header">
						<span class="font18">${ctp:i18n('personalInfo.bind.msg1')}“<span class="step_3_type">${ctp:i18n('personalInfo.track.bindphonenum')}</span>”${ctp:i18n('personalInfo.bind.msg2')}</span>
					</div>
					<div class="type_code">
						<p class="padding_l_10 margin_b_15">${ctp:i18n('personalInfo.bind.verifynumber')} <span class="check_type">${param.telNumber}</span></p>
						...
						<a href="javascript:sendVerificationCodeToBindNum();" class="button get_code"  onclick="hiddenerr();">${ctp:i18n('personalInfo.bind.getverificationcode')}</a>
						<!-- 重新发送 -->
				...
				<div class="phone_bind_2" style="display:none;">
					...
					<span class="left phone_bind_success">
						<span class="success_info">${ctp:i18n('personalInfo.bind.msg4')}”${ctp:i18n('personalInfo.bind.phonenumtitle')}”${ctp:i18n('personalInfo.bind.success')}</span>
					</span>
				</div>
			</div>
		</div>
