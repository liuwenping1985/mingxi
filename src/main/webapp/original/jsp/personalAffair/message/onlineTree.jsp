...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<td height="25">
			<form name="userForm" action="${messageURL}" method="get" onsubmit="return false;">
				<table width="100%" height="25" border="0" cellspacing="0" cellpadding="0" class="message-title-color">
					<tr>
					  	<td valign="top" style="padding:2px 2px;">
					  		<c:if test="${isShowAccountSwitch && currentUser.internal}">
								<div id="select_input_div" class="select_input_div" onclick="showMenu(); return false;">
				                	<input name="currentAccountId" id="currentAccountId" type="text" class="select_input" readonly="readonly" value="${v3x:getAccount(currentAccountId).name}" />
								</div>
					  		</c:if>
						</td>
					</tr>
	...
	<tr>
		<td>
			<div id="scrollListDiv" class="scrollList" style="background: #ffffff;">
				<c:choose>
					<c:when test="${isRoot}">
						<script type="text/javascript">
					...
					<c:otherwise>
						...
						<script type="text/javascript">
							var root = new WebFXTree("${account.id}", "${account.name}", "javascript:showAccountUser('${account.id}')");
							root.setBehavior('classic');
							root.icon = "<c:url value='/common/images/templete.gif'/>";
							root.openIcon = "<c:url value='/common/images/templete.gif'/>";	
							...
							<c:set value="${v3x:toString(t.v3xOrgDepartment.id)}" var="dId"/>
								<c:choose>
									...
									<c:otherwise>
										<c:set var="onlick" value="javascript:showSelDep('${t.v3xOrgDepartment.id}')" />
									</c:otherwise>
								</c:choose>
