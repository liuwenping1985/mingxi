...
<script type="text/javascript">
	...
	function moveMemberToOffline(obj,loginName,currentAccountId){
		var url = "/seeyon/online.do?method=moveMemberToOffline&loginName="+encodeURI(loginName)+"&currentAccountId="+currentAccountId;
							...
							<c:forEach items="${onlineUserList}" var="user">
								...
								<tr height="25" class="sort cursor-hand" onclick="${onClick}" ondblclick="${onDblclick}" onmouseover="${onmouseover}" onmouseout="${onmouseout}">
									...
									<c:if test="${canMoveToOffline}">
										<td align='center' class="sort">
										<c:choose>
											<c:when test="${currentUser.systemAdmin || currentUser.groupAdmin || (currentUser.administrator && v3x:containInCollection(user.loginAccountId, currentUser.loginAccount))}">
												<a class='link-blue' onmouseover="moveMemberToOffline(this,'${user.loginName}','${currentAccountId}');"><fmt:message key="message.moveToOffline.label" /></a>
											</c:when>
											<c:otherwise>
												&nbsp;
											</c:otherwise>
