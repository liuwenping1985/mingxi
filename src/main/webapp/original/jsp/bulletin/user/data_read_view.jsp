...
<style>
 ...
 .body-detail-su{background:#fafafa;color:#666;font-weight:normal;}
 #bDivlistTable{overflow-x:hidden;}
 #remind{
 	min-width:0;
 	height:25px;
 	line-height:25px;
 ...
 .mxt-window .mxt-window-head .mxt-window-head-title{
 	font-size: 16px;
 	color:#999;
 	font-weight: normal;
 }
 .body-bgcolor{background:#fafafa;}
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="body-detail" align="center">
				<tr valign="top" >
					<td class="body-detail-border" width="100%">
						<div class="body-detail-su" style="padding:6px 12px 0px 12px;height:25px;" >
						...
						<div style="width:100px;position:absolute;height: 20px;right:30px;top:8px;border:1px solid #333;border-radius:5px;cursor:pointer;" onclick="exportBulReadInfo('${param.id}')" >
						...
						<div class="body-detail-su" style="padding:6px 20px 0px 20px;" id="scrollList1">
							<form action="" name="mainForm" id="mainForm" method="post">
								<v3x:table htmlId="listTable" data="bulreadcount" var="beanread" showPager="true" pageSize="20" className="sort body-detail-table">
									<c:if test="${beanread.deptId!=null}">
										...
										<v3x:column type="String" label="bul.type.spaceType.1" className="sort font-12px" width="40%">
											<a title="${v3x:toHTML(v3x:showDepartmentFullPath(beanread.deptId))}" class="font-12px" onclick="articleDetail('${beanread.deptId}','${beanread.endReadCount}','${beanread.notReadCount}');">${v3x:toHTML(v3x:showOrgEntitiesOfIds(beanread.deptId, 'Department', pageContext))} </a>
										</v3x:column>
									</c:if>
									<c:if test="${beanread.accountId!=null}">
										...
										<v3x:column type="String" label="bul.type.spaceType.2" className="sort font-12px" value="${v3x:toHTML(v3x:showOrgEntitiesOfIds(beanread.accountId, 'Account', pageContext))}" width="40%"/>
									</c:if>
									<v3x:column type="String" label="bul.dataview.alreadyCall" className="sort font-12px" value="${beanread.endReadCount}" width="30%"/>
									<v3x:column type="String" label="bul.dataview.notCall" className="sort font-12px" value="${beanread.notReadCount}" width="30%"/>
								</v3x:table>
							</form>
						</div>
					</td>
...
<script type="text/javascript">
 	if(parent.window.document.getElementById("showReadDiv") != null ){
	...
	if(userAgent.indexOf('MSIE 8.0')>-1){
		bindOnresize('scrollList1',10,37);
	}else{
		bindOnresize('scrollList1',40,37);
	}

	var div = document.getElementById('bDivlistTable');
	setTimeout(function(){
