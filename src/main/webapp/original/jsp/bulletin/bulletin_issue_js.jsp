 ...
 * @param affairId 事项ID
 * @param bodyType 正文类型
 * @param formContent 是启有表单正文
 * @param callback 回调函数（由调用方处理）
 * @returns 
 */
 function bulletinIssue(summaryId,affairId,bodyType,formContent,callback,rightId,failed_callback){
   ...
   var dialog = $.dialog({
     ...
     width:400,
     targetWindow:top,
     url:_ctxPath+'/bulIssue.do?method=preIssue&summaryId='+summaryId+"&affairId="+affairId+"&bodyType="+bodyType+"&formContent="+formContent+"&entry=0&outworkerPanel=1",
     closeParam : {
         'show':true,
         'autoClose':false,
         handler:function(){
     ...
     buttons : [{
         ...
         handler : function() {
           ...
           if( rv && rv.length==7 ){
               ...
               if( rv[3]=="1" ){
                   pdfFileId = getUUID();
                   var transFlag = transformWordToPdf(pdfFileId);
                   if(!transFlag){
                    return ;
                   }
               }
               ...
               issueBullBack_Properties.put("rv3", rv[6]);
               issueBullBack_Properties.put("rv4", rv[0]);
               issueBullBack_Properties.put("rv5", rv[1]);
               issueBullBack_Properties.put("rv6", rv[2]);
               issueBullBack_Properties.put("rv7", pdfFileId);
               issueBullBack_Properties.put("rv8", rightId);
               issueBullBack_Properties.put("rv9", rv[4]);
               issueBullBack_Properties.put("rv10", rv[5]);
...
function issueBullBack() {
    ...
    requestCaller.addParameter(11, "String", issueBullBack_Properties.get("rv10"));
...
function bulletinIssueAuthority(){
  return callBackendMethod("bulIssueManager","hasAuthority");
}
//-->
</script>