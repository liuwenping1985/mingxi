...
<script type="text/javascript">
    ...
    function validate(){
    	var count=0;
    	var boxes = document.getElementById('showColumnFields').getElementsByTagName("input");
    	...
    	if(count>20){
    		getA8Top().$.alert("<fmt:message key='addressbook.fieldset.maxfields' />");
    		return false;
    	}
    	return true;
    }
...
<body class="h100b over_hidden">
  ...
  <div class="stadic_layout">
    ...
    <div class="stadic_layout_body stadic_body_top_bottom">
      <form id="form" name="form" method="post" action="addressbook.do?method=updateAddSet" onsubmit="return validate();">
        ...
        <div class="margin_t_10" id="showColumnFields">
          ...
          <div class="margin_t_10 margin_l_20">
            ...
            <label for="workLocal" class="hand margin_l_20">
              <input id="workLocal" name="displayColumn" type="checkbox" value="workLocal" ${bean.workLocal ? 'checked' :''} /><span class="valign_m margin_l_5"><fmt:message key='addressbook.company.worklocal.label' /></span>
            </label>
          ...
          <!--集团管理员定义的通讯录字段  -->
          <div class="margin_t_10 margin_l_20">
	      	 <c:forEach items="${customerBean}" var="customer" varStatus="status">
				 <c:choose>
					<c:when test="${status.index == 0}">
					    <label for="${customer.id}" class="hand">
	                       <input id="${customer.id}" name="displayColumn" type="checkbox" value="${customer.id}"/><span id="${customer.id}_span" class="valign_m margin_l_5"></span>
	                   </label>
					</c:when>
					<c:otherwise>
						<label for="${customer.id}" class="hand margin_l_20">
	                       <input id="${customer.id}" name="displayColumn" type="checkbox" value="${customer.id}"/><span id="${customer.id}_span" class="valign_m margin_l_5"></span>
	                   </label>
					</c:otherwise>
				 </c:choose>
                     <script type="text/javascript">
                        //列头名
                        var spanid='${customer.id}_span';
                        var spanlable=document.getElementById(spanid);
                        spanlable.innerHTML="${v3x:toHTML(customer.label)}";
                        	//列头勾选
                     	var alldisplayColumn="${bean.displayColumn}";
                     	var customerid="${customer.id}";
                         if(alldisplayColumn.indexOf(customerid)>0){
                        	 var ischeckedlable=document.getElementById(customerid);
                        	 ischeckedlable.checked=true;
                         }
                     </script>
		   </c:forEach>
          </div>
            <!--  占位，保证上面的字段能够全部显示出来-->
            <div class="margin_t_10 margin_l_20"><label  class="hand margin_l_20"></label></div>
