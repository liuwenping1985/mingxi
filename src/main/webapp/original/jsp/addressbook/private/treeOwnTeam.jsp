...
function clk(e, treeId, node) {
	...
	if(node.data.parentId!='0' && node.data.parentId!=0 ){
		if(parent.listFrame.document && parent.listFrame.document.readyState === "complete") {
			$("#listFrame",parent.document).attr("src","/seeyon/addressbook.do?method=listOwnTeamMembers&addressbookType=2&showType="+showType+"&tId="+tid);
			$("#frameUrl",parent.document).val("/seeyon/addressbook.do?method=listOwnTeamMembers&addressbookType=2&showType="+showType+"&tId="+tid);
		}
		$("#tId").val(tid);
	}else{
		$("#tId").val("-1");
...
<body scroll="no" onclick="parent.hideMenu();">
