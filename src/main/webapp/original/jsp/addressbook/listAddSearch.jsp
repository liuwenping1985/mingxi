...
function showSpecialExpressionInput(etObj) {
   ...
   // var searchDiv=document.getElementsByClassName("div-float hidden");  
   var searchDiv=getClassNames('div-float hidden','div');
 	...
 	var value=etObj.value;
	var indexnum=value.indexOf('customerSearch'); 
	if(indexnum==0){//自定义通讯录字段的搜索
		//type=value.substring(len-1);
		var index1=value.indexOf(':'); 
	    var index2=value.indexOf(','); 
		id=value.substring(index1+1,index2);
		var eiToShow=document.getElementById(id);
		if (eiToShow) {
			eiToShow.value = "";
...
function getClassNames(classStr,tagName){ 
	if (document.getElementsByClassName) { 
		...
		for(i = 0; i < nodes.length; i++) { 
			var arr=nodes[i].className; 
			if(arr==classStr){ ret.push(nodes[i]); } 
		} 
		return ret; 
	} 
} 
...
function doSearch(formId) {
	...
	if (et === "") {
		...
		var indexnum=et.indexOf('customerSearch'); 
		if(indexnum==0){
			var index1=et.indexOf(':'); 
		    var index2=et.indexOf(','); 
			id=et.substring(index1+1,index2);
			var searchEv=document.getElementById(id+"_id");
			ev=searchEv.value;
			ev=encodeURI(ev);
		}else{
...
<form id="searchForm" method="post" onsubmit="return false" style="margin: 0px">
	<div class="div-float">
		<select id="<%=expressionTypeParamName%>" name="expressionType" onChange="showSpecialExpressionInput(this)" class="condition" style="width: 120px;">
		 ...
		 <c:forEach items="${customerSearch}" var="customer" varStatus="status">
		 		<option value="customerSearch:${customer.id},${customer.type}">${v3x:toHTML(customer.label)}</option>
		 </c:forEach>
  ...
  <div id="<%=inputTypeTemp%>Div" class="div-float hidden" style="display:<%=s%>">
  	...
  	<c:forEach items="${customerSearch}" var="bean" varStatus="status">
	     <!--文本类型  -->
	 	<c:if test="${bean.type == 0}">
			   <div id="${bean.id}" class="div-float hidden" style="display:none" name="customerTextDiv">
					<input type="text" id="${bean.id}_id" name="${bean.id}" maxLength="20" class="textfield-search"  onkeydown="javascript:if(event.keyCode==13)doSearch('searchForm');">
			  	</div>
	 	</c:if>
	 	
        <!--数字类型  -->
		 	<c:if test="${bean.type == 1}">
			 	<div id="${bean.id}" class="div-float hidden" style="display:none" name="customerNumtDiv">
		 		 	<input type="text" id="${bean.id}_id" name="${bean.id}"  class="textfield-search"  maxlength="10"  onkeydown="javascript:if(event.keyCode==13)doSearch('searchForm');"/>
		 		</div>
	 		</c:if>
	 	
	 	<!--日期类型  -->
	 	<c:if test="${bean.type == 2}">
	 	<div id="${bean.id}" class="div-float hidden" style="display:none" name="customerDateDiv">
	        <input id="${bean.id}_id" name="${bean.id}_id" type="text"  class="textfield-search" onClick="whenstart('${pageContext.request.contextPath}', this, 175, 140);" onkeydown="javascript:if(event.keyCode==13)doSearch('searchForm');"
				               readonly="readonly"  />
			  </div>
	 	</c:if> 
  </c:forEach>
	...
	<script type="text/javascript">
	 ...
	 if(searchId!=""){
		var searchlabel=document.getElementById(searchId);
		searchlabel.style.display = "block";
		
		var searchlabelvalue=document.getElementById(searchId+"_id");
		searchlabelvalue.value=searchValue;
	} 
	</script>
