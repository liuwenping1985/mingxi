        ...
        <div class="clearfix margin_t_5 margin_l_5">
            ...
            <div>
                <fieldset style="width: 505px;padding:0px;border:1px solid #ccc;">
                    <legend>${ctp:i18n("weixin.app.label.image")}</legend>
                    <div style="width: 465px; margin: 10px;">
                        ...
                        <div id="bannerImageDiv" class="clearfix margin_t_5" style="line-height: 20px;">
                            <div class="left like_a hand" style="width: 100px;text-align: right;padding-top:4px;" onclick="addParamImage(this)">${ctp:i18n("weixin.app.label.bgImg")}</div>
                            <div class="left common_txtbox_wrap" style="width: 310px;padding: 0px;">
                                <div style="height: 60px;overflow: auto;" onclick="addParamImage(this)">
                                    <input id="bannerImageId" name="bannerImageId" value="${ctp:i18n("weixin.app.label.clickToUpload")}" style="padding-left:5px;color:#999999;width:auto;" />
                                    <img id="bannerImage" src="" class="hidden" />
                                </div>
                            </div>
                        </div>
                        <div id="bannerImageTitleDiv" class="clearfix margin_t_5" style="line-height: 20px;">
                            <div class="left" style="width: 100px;text-align: right;">${ctp:i18n("weixin.app.label.imageTitle")}：</div>
                            <div class="left common_txtbox_wrap" style="width: 300px;">
                                <input type="text" id="bannerImageTitle" name="bannerImageTitle" value="" maxLength="30" class="validate" validate="type:'string',maxLength:30,name:'${ctp:i18n("weixin.app.label.imageTitle")}',avoidChar:'!@#$%^&\|\\\/<>\'*'" />
                            </div>
                        </div>
                        <div id="bannerImageUrlDiv" class="clearfix margin_t_5" style="line-height: 20px;">
                            <div class="left" style="width: 100px;text-align: right;">${ctp:i18n("weixin.app.label.imageUrl")}：</div>
                            <div class="left common_txtbox_wrap" style="width: 300px;">
                                <input type="text" id="bannerImageUrl" name="bannerImageUrl" value="" maxLength="200" class="validate" validate="type:'string',maxLength:200,name:'${ctp:i18n("weixin.app.label.imageUrl")}',avoidChar:'<>\''" />
                            </div>
                        </div>
                        <%--数据记录id--%>
...
<script type="text/javascript">
    ...
    var count = 0;
    var txt = $.i18n("weixin.app.label.clickToUpload");
    $().ready(function() {
        ...
        if (banners != null && banners.length > 0) {
            for (var i = 0; i < banners.length; i++) {
                ...
                currentBannerSet.find("#recordId").val(banner.id);
    ...
    function addBannerSet(obj) {
        ...
        if (length == 5) {
            $.alert($.i18n("weixin.app.message.imageAtMost"));
            return;
        }
        count = count + 1;
    ...
    function delBannerSet() {
        if ($(".bannerSetting", "#bannerContainer").length > 1) {
            ...
            $.alert($.i18n("weixin.app.message.imageAtLeast"));
//            currentBannerSet.remove();
//            addBannerSet();
        }
    }
        ...
        if (isValidate) {
            ...
            $(".bannerSetting", "#bannerContainer").each(function(i){
                ...
                if (bannerImageId == "") {
                    ...
                    if (count > 0) {
                        dataStr += ","+ $.toJSON({"recordId": recordId,"bannerImageId":bannerImageId, "bannerImageTitle":bannerImageTitle, "bannerImageUrl":bannerImageUrl});
                    }else{
                        dataStr += $.toJSON({"recordId": recordId,"bannerImageId":bannerImageId, "bannerImageTitle":bannerImageTitle, "bannerImageUrl":bannerImageUrl});
                    }
                    count ++;
