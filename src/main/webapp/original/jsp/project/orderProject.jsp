...
function OK(){
	...
	for(i=0;i<sel.options.length;i++) {
		orderArray.push(sel.options[i].value);
		newOrder+=sel.options[i].value;
		if(i!=sel.options.length-1){newOrder+=";";}
	}
	var oldOrder = document.getElementById('oldProjects').value;
	if(newOrder != oldOrder && orderArray.length > 0){
		var ajax = new projectConfigManager();
		ajax.updateProjectOrder(orderArray);
	}
}
function init(){
	var projectsObj=document.getElementById("projectsObj");
	<%--OA-89056项目管理：项目排序--\显示成了\/
	var projectList=new Array(jQuery.parseJSON(${projectJsonList}));
	var test=new Array(${projectJsonList});
	--%>
	var test = JSON.parse($("#projectJsonList").val());
	var projectList=test;
...
<input id="projectJsonList" type="hidden" value="${ctp:toHTMLWithoutSpace(projectJsonList)}" />
