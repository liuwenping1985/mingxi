...
<script type="text/javascript">
	...
	function queryMoreProjectCol() {	
		...
		document.getElementById("searchForm").action ="./project.do?method=queryMoreProjectColByCondition";	
	...
	function init(){
		...
		//设置列表内容的高度使其可以铺满一页
		scrollbDivcolls.style.height = document.body.clientHeight - 35-32-30 + "px";
	}
		            ...
		            <td>
					   ...
					   <tr>
							...
							<td class="webfx-menu-bar">
								<form action="" id="searchForm" name="searchForm" method="post" onkeydown="doMySearchEnter()">
									<input type="hidden" name="method" value="queryMoreProjectColByCondition" />
							    	...
							    	<div class="div-float-right" style="vertical-align:middle;">
							    		<div class="div-float">
											<select name="condition" id="condition" style="height:22px;" onChange="showNextSpecialCondition(this)" class="">
										    	<option value="choice" <c:if test="${condition == 'choice' }">selected</c:if>><fmt:message key="project.info.select.label" bundle="${collI18N}"/></option>
			...
			hasAttachments="${col.attsFlag}"  flowState="${col.colSummaryState}" 
				...
				<c:choose>
					<c:when test="${col.state==2}">
						<a class="title-more" href="javascript:myOpenDetail('${colURL}?method=summary&openFrom=listSent&affairId=${col.id }')">${v3x:toHTML(col.addition)}</a>
					</c:when>
					<c:when test="${col.state==3}">
						<a class="title-more" href="javascript:myOpenDetail('${colURL}?method=summary&openFrom=listPending&affairId=${col.id }')">${v3x:toHTML(col.addition)}</a>
					</c:when>
					<c:otherwise>
						<a class="title-more" href="javascript:myOpenDetail('${colURL}?method=detail&openFrom=listDone&affairId=${col.id }')">${v3x:toHTML(col.addition)}</a>
					</c:otherwise>
				</c:choose>
			</v3x:column>
