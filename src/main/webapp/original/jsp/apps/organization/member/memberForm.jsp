...
function stateOnChange(){
	var en=document.getElementById("enabled").checked;
	if(!en){
		memberDisableAndLeave();
	}
}

/** 离职办理 */
function memberDisableAndLeave() {
  ...
  $.confirm({
    ...
    ok_fn: function() {
      var dialog = $.dialog({
        ...
        url: "<c:url value='/organization/memberLeave.do' />?method=showLeavePage&memberId=" + memberId+"&noupdateState=1",
        title: $.i18n("member.leave.label") + " : " +name,
        width: 700,
        height: 540,
...
<%--                             <option value="zh_CN" selected>${ctp:i18n('org.member_form.primaryLanguange.zh_CN')}</option>
            ...
            <tr>
                ...
                <td>
                    <div class="common_radio_box clearfix">
                        <label class="margin_r_10 hand">
                            <input type="radio" value="true" id="enabled" name="enabled" onchange="stateOnChange()"
                            class="radio_com m_enable">${ctp:i18n('common.state.normal.label')}
                        </label>
                        <label class="margin_r_10 hand">
                            <input type="radio" value="false" id="enabled" name="enabled" onchange="stateOnChange()"
    ...
    <script type="text/javascript" charset="UTF-8" src="<c:url value="/common/js/distpicker/distpicker.data.js${ctp:resSuffix()}" />"></script>
    <script type="text/javascript" charset="UTF-8" src="<c:url value="/common/js/distpicker/distpicker.js${ctp:resSuffix()}" />"></script>
</form>