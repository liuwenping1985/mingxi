...
$().ready(function() {
  ...
  $("#addForm").find("table").css({"margin-left":"0"});
  $("#addForm").find("table").attr("width","60%");
    if (node.level == 0) {
      $("#depform").hide();
      $("#welcome").show();
      $("#button").hide();
  ...
  $("#addForm").find("table").attr("width","60%");
    ...
    if (nodes.length < 1 ) {
        ...
        var deptdetil = dManager.viewDept(nodes[0].id);
        $("#depform").clearform();
        var rolelist = dManager.getDepRoles(accountId)["deproles"];
        for(i=0;i<rolelist.length;i++){
      	  $("#deptrole"+i).addClass("w100b");
  ...
  function addform() {
  $("#addForm").find("table").css({"margin-left":"0"});
    $("#addForm").find("table").attr("width","71.5%");
    $("#depform").clearform({
      clearHidden: true
    });
  ...
  $("#toolbar").toolbar({
    ...
    },
    {
      ...
      click: function() {
        var nodes = $("#deptTree").treeObj().getSelectedNodes();
        var deptId = $("#addForm").formobj().id;
        if (nodes.length < 1 || nodes[0].level == 0 || deptId==='') {
    ...
    },
    {
      ...
      click: function() {
        location.href = "${path}/organization/department.do?method=showDepartmentFrame&style=list&accountId=" + accountId;
      }
    }]
  });
  ...
  $("#btnok").click(function() {
    if(!($("#addForm").validate())){
      return;
       ...
       var rolelist = dManager.getDepRoles(accountId)["deproles"];
       for(i=0;i<rolelist.length;i++){
          if('DepartmentSpecialTrainAdmin'==rolelist[i]['code']){
	          var check = rm.checkOnlyOneRole(rolelist[i]['id'],$("#deptrole"+i).val(),$("#id").val());
