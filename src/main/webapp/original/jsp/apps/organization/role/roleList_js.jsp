      ...
      },
      {
        ...
        click: function() {
          ...
          if (v.length != 1) {
            ...
            if(v[0].bond == "0") {
                $.error("${ctp:i18n('role.group.defult.cannot')}");
                return;
              }
            var rManager = new roleManager();
            if (isgroup && isallowaccpriv == true && nowtab != 0) {
...
function clk(data, r, c) {
  $("#selectResouce").attr("src", "../privilege/menu.do?method=getTree&roleId=" + data.id);
}

// 查看资源
function showResource(id) {
  var dialog = $.dialog({
    url: _ctxPath + '/privilege/menu.do?method=getTree&appResCategory=1&roleId=' + id,
    width: 300,
    height: 400,
    targetWindow: getCtpTop(),
...
function selectResource(id) {
	var rolecheck = oManager.checkRolePrefabricated(id);
	if(rolecheck||rolecheck=="true"){
		$.alert("${ctp:i18n('role.prefabricate.allowaccount.no')}");
		return;
	} 
  var dialog = $.dialog({
    id: 'treedialog',
    url: _ctxPath + '/privilege/menu.do?method=getTree&cmd=selectAll&showAll=1&roleId=' + id + '&appResCategory=1&isAllocated=true',
    width: 450,
    height: 400,
    targetWindow: getCtpTop(),
    isDrag:false,
...
function role2people(id) {
  ...
  var dialog = $.dialog({
    id: 'role2people',
    url: _ctxPath + '/organization/role.do?method=showEntity4Role&roleId=' + id + "&accountId=" + accountId,
    width: 500,
    targetWindow: getCtpTop(),
    height: 180,
...
function newRole() {
  var dialog = $.dialog({
    url: _ctxPath + '/organization/role.do?method=createRole&accountId=' + accountId,
    width: 400,
    height: 340,
    isDrag:true,
...
function updateRole() {
  ...
  var id = boxs[0].id;
  var dialog = $.dialog({
    url: _ctxPath + '/organization/role.do?method=edit&id=' + id + "&accountId=" + accountId,
    width: 400,
    height: 340,
    isDrag: false,
    targetWindow: getCtpTop(),
...
function deleteRole() {
  ...
  if (v.length === 0) {
    ...
    $(v).each(function(index, domEle) {
      if (domEle.type === 3 && domEle.isBenchmark === false) {
        roles.push(domEle.id);
    	  ...
    	  if(i==0){
    		notDeleteRole = domEle.showName;
    	  }else{
    		notDeleteRole = notDeleteRole+"、"+domEle.showName;
    	  }
    	  i++;
        checkResult = false;
      }
    ...
    if (!checkResult){
        $.alert(notDeleteRole + "${ctp:i18n('role.system.preset.not.delete')}");
    	return;
    }
    $.confirm({
