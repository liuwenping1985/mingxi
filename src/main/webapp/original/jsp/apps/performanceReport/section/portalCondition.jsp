...
//========================================================栏目相关js begin=======================================================
	...
	function fillPortalChange(){
		var pt=new Object();
		var pv = decodeURIComponent(getA8Top().paramValue);
		var st=pv.split(",");
	    	...
	    	$.each(content,function(i,n){
	    		...
	    		if(item.attr("type")=='hidden'){
	    			...
	    			if(item.attr("id")=="managerRangeIds"||item.attr("id")=="departmentIds"){
	    				...
	    				array[j]=""+item.attr('id')+"="+encodeURIComponent(managerRangeIds.replaceAll(',','、',false));
	    			}else if(item.attr("id")=="templeteId"){
	    				//替换模板名称和模板Id的","为"|"
	    				var value=item.val().replaceAll(",","|",false);
	    				array[j]=""+item.attr("id")+"="+value;
	    			}else{
		    			array[j]=""+item.attr("id")+"="+encodeURIComponent(item.val());
	    			}
	    			j++;
	    		}
	    		...
	    		if(item.attr("type")=='text'){
	    			if(item.attr("id")=="department"){
	    				array[j]=""+item.attr("id")+"="+encodeURIComponent(item.val().replaceAll(',','、',false));
	    				j++;
