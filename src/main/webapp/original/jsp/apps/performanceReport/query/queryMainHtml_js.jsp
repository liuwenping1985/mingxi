...
<script type="text/javascript">
    	 ...
    	 "<table border=0 cellpadding=0 cellspacing=0 class='common_center'>"+"<tbody>"+
         	"<tr>"+
         		...
         		"<td width='50%' vAlign='top' class='padding_lr_10'>"+
         			"<table border=0 cellpadding=0 cellspacing=0>" +"<tbody>" ;
				         if(personGroupTab==2||personGroupTab==1){
				             hh+="<tr><th nowrap='nowrap'><label class='margin_r_10' for='text'>&nbsp;</label></th><td style='width:50%'>&nbsp;</td></tr>";
				         }
    ...
    function workTotal_() {
    	 ...
    	 "<table width='100%' id='Worktab' border=0 cellpadding=0 cellspacing=0 style='margin-left:10px;' class='common_center'>"+"<tbody>"+
         	"<tr>"+
    ...
    //时间分析,事项统计,会议参与角色,回忆参加情况,知识贡献榜,知识活跃度公共条件区
    function commonHtml(){
    var hh = "<div class='form_area set_search'><table border=0 cellpadding=0 cellspacing=0 width='100%'><tr>";
    	 if(personGroupTab==2||personGroupTab==1){
    	    hh+="<td><div class='one_row'>" +
    	    "<table border=0 cellpadding=0 cellspacing=0 style='margin-left:120px'>" +
    	    "<tbody>" ;
    	    hh+="<tr>"+
    	    getTh("${ctp:i18n('performanceReport.queryMainHtml.selectePerson')}") + managerRange(true)+
    	    "</tr>";
    ...
    function storageSpaceStatistics(){
   	 var hh = "<div class='form_area set_search one_row' style='text-align:text'>" +
     "<table border=0 cellpadding=0 cellspacing=0>" +
     "<tbody>" ;
     hh+="<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>"+
     getTh("${ctp:i18n('performanceReport.queryMainHtml.selectePerson')}") + managerRange(true)+
    	...
    	"<table border=0 cellpadding=0 cellspacing=0 class='common_center'>" +"<tbody>"+
     		"<tr>"+
				"<td width='50%' vAlign='top' class='padding_lr_10'>"+
			        ...
			        "</table>" +
			     "</td>" +
			     "<td width='55%' vAlign='top' class='padding_lr_10'>"+
		        	"<table border=0 cellpadding=0 cellspacing=0>" +
			        "<tbody>" +
			        "<tr id='flowContentReplace'>"  +"<th nowrap='nowrap'><label class='margin_r_10' for='text'>&nbsp;</label></th>" +flowContentType() +
			        "</tr>" +
			        "<tr>" +getTh("${ctp:i18n('performanceReport.queryMainHtml.getTime')}")+"<td style='width:100%'><table><tbody><tr>"+getTimeInterval("%Y-%m",true)+
    ...
    function getType_() {
		...
		if(false){
         ...
         hh +=
            "<div class='common_selectbox_wrap'>" +
            "<select id='appType'>" ;
            hh+="<option value='5' selected>${ctp:i18n('performanceReport.queryMainHtml.sortsAllDone')}</option>"+
            ...
            <c:if test="${ctp:hasPlugin('collaboration')}">
            	hh+="<option value='0' >${ctp:i18n('performanceReport.queryMainHtml.flowType.col')}</option>" ;
            </c:if>
            <c:if test="${ctp:hasPlugin('edoc')}">
            	hh+=  "<option value='1'>${ctp:i18n('performanceReport.queryMainHtml.flowType.edoc')}</option>" ;
            </c:if>
            <c:if test="${ctp:hasPlugin('plan')}"> 
           hh+= "<option value='2'>${ctp:i18n('performanceReport.queryMainHtml.flowType.plan')}</option>" ;
           </c:if>
           <c:if test="${ctp:hasPlugin('meeting')}">
           	hh+=   "<option value='3'>${ctp:i18n('performanceReport.queryMainHtml.flowType.meeting')}</option>" ;
           	</c:if>
           	<c:if test="${ctp:hasPlugin('taskmanage')}">
	           if(!checkSystemProperty()){
	            hh += "<option value='4'>${ctp:i18n('performanceReport.queryMainHtml.taskInfo')}</option>" ;
	           }
           </c:if>
          hh+="</select>" +
            "</div>" +
...
//报表状态
   ...
   function getcolGovType(){
    ...
    function getTime() {
        return "<div class='common_selectbox_wrap' id='timeDiv'>" +
            ...
            "<option value='0'>${ctp:i18n('performanceReport.common.dateSelect.today')}</option>" )+ 
            "<option value='1' selected>${ctp:i18n('performanceReport.common.dateSelect.week')}</option>" +
            "<option value='2'>${ctp:i18n('performanceReport.common.dateSelect.month')}</option>" +((Constants_report_id==MEETINGJOINSTATISTICS||Constants_report_id==MEETINGJOINROLESTATISTICS||Constants_report_id==ONLINETIMESTATISTICS||Constants_report_id==EVENTSTATISTICS)?"":
            ...
            "<option value='5'>${ctp:i18n('performanceReport.common.dateSelect.year')}</option>" :"")+(
            "<option value='4'>${ctp:i18n('performanceReport.common.dateSelect.anytime')}</option>") +
            "</select>" +
            "</div>";
    }
    function getTime2() {//知识活跃度 默认为本月
        return "<div class='common_selectbox_wrap' id='timeDiv'>" +
            "<select id='time' onchange='timeChange()'>" +((Constants_report_id==MEETINGJOINROLESTATISTICS||Constants_report_id==EVENTSTATISTICS||Constants_report_id==KNOWLEDGESCORESTATISTICS||Constants_report_id==KNOWLEDGEACTIVITYSTATISTICS)?"":
            "<option value='0'>${ctp:i18n('performanceReport.common.dateSelect.today')}</option>" )+
            "<option value='1'>${ctp:i18n('performanceReport.common.dateSelect.week')}</option>" +
            "<option value='2' selected>${ctp:i18n('performanceReport.common.dateSelect.month')}</option>" +((Constants_report_id==MEETINGJOINSTATISTICS||Constants_report_id==MEETINGJOINROLESTATISTICS||Constants_report_id==ONLINETIMESTATISTICS||Constants_report_id==EVENTSTATISTICS)?"":
  ...
  //时间选择：dateFormat(时间格式)displayIntervalFlag(是否显示时间 间隔)
    function getTimeInterval(dateFormat,displayIntervalFlag) {
	  var hh="";
	   	 if(hiddenChartGridTab=='true'){
	   		 hh="<td width='60' ><input id='start_time' name='${ctp:i18n('performanceReport.queryMain.textbox.startTime.name')}' type='text' class='comp mycal validate' validate='notNull:true' readOnly='true' comp='cache:false,type:\"calendar\",ifFormat:\""+dateFormat+"\"'/></td>";
	   	     if(displayIntervalFlag){
	   	    	 hh+="<td id='mid'>-</td>"+"<td width='60'><input  id='end_time' name='${ctp:i18n('performanceReport.queryMain.textbox.endTime.name')}' type='text'readOnly='true' class='comp mycal validate' validate='notNull:true'  comp='cache:false,type:\"calendar\",ifFormat:\""+dateFormat+"\"'/></td>";
	   	     }
	   	 }else{
	   		 hh="<td style='width:50%' ><div class='common_txtbox_wrap'><input id='start_time' name='${ctp:i18n('performanceReport.queryMain.textbox.startTime.name')}' type='text' class='comp mycal validate' validate='notNull:true' readOnly='true' comp='cache:false,type:\"calendar\",ifFormat:\""+dateFormat+"\"'/></div></td>";
	   	     if(displayIntervalFlag){
	   	    	 hh+="<td id='mid'>-</td>"+"<td style='width:50%' ><div class='common_txtbox_wrap'><input  id='end_time' name='${ctp:i18n('performanceReport.queryMain.textbox.endTime.name')}' type='text'readOnly='true' class='comp mycal validate' validate='notNull:true'  comp='cache:false,type:\"calendar\",ifFormat:\""+dateFormat+"\"'/></div></td>";
	   	     }
	   	 }
...
//本人角色  (和项目数据库里面的定义保持一致)
  ...
  function flowType_() {
      var hh = "<td style='width:100%'>" +
          ...
          if($.ctx.plugins.contains('edoc')){
          	hh +="<option value='2'>${ctp:i18n('performanceReport.queryMainHtml.flowType.edoc')}</option>" ;
          }
          hh += "</select>" +
          "</div>" +
          "</td>";
...
//流程状态
  ...
  function effeiciencyAnalysis(){
  		var hh = "<div class='form_area set_search'>" +
    	 "<table width='78%' border=0 cellpadding=0 cellspacing=0 class='common_center'>"+"<tbody>"+
         	"<tr>"+
 				"<td width='50%' vAlign='top' id='type_td' class='padding_lr_10'>";
			    	...
			    	hh+="<tr>"+
			        	getTh("${ctp:i18n('performanceReport.query.getTemplate')}")+getTemplate()+
         				...
         				hh+="</tbody>" +"</table>" +
         			"</td>" +
         			"<td width='65%' vAlign='top' class='padding_lr_10'>"+
         			"<table border=0 cellpadding=0 cellspacing=0>" +"<tbody>" ;
				         hh+="<tr id='timeAll'>" +getTh("${ctp:i18n('performanceReport.queryMainHtml.start.getTime')}") + getDate()+
  ...
  var hh = "<div class='form_area set_search'>" +
    	 "<table width='80%' border=0 cellpadding=0 cellspacing=0 class='common_center'>"+"<tbody>"+
         	"<tr>"+
 				"<td width='50%' vAlign='top' id='type_td' class='padding_lr_10'>";
	            	hh+="<table border=0 style='padding-left:70px;' cellpadding=0 cellspacing=0>" +"<tbody>";
			    	hh+="<tr>"+
  ...
  function getProcessStatus(){
	  var hh="";
	  hh+="<td width=''><input name='flowstate' id='flowstate0' type='checkbox' checked='checked' value='${finish}'/>${ctp:i18n('formquery_finished.label')}</td>"+
	  "<td><input name='flowstate' class='padding_l_10' id='flowstate1' type='checkbox' checked='checked' value='${run}'/>${ctp:i18n('formquery_finishedno.label')}</td>"+
	  "<td><input name='flowstate' class='padding_l_10' id='flowstate3' type='checkbox' checked='checked' value='${terminate}'/>${ctp:i18n('col.state.10.stepstop')}</td>";
	  return hh;
  }
  function overTimeAnalysis(){
  				var hh = "<div class='form_area set_search'>" +
    	 		"<table width='85%' border=0 cellpadding=0 cellspacing=0 class='common_center'>"+"<tbody>"+
         		"<tr>"+
 				"<td width='50%' vAlign='top' id='type_td' class='padding_lr_10'>";
			    	...
			    	hh+="<tr>"+
			        	getTh("${ctp:i18n('performanceReport.query.getTemplate')}")+getTemplate()+
			         	"</tr>";
			         	hh+="<tr id='timeAll'>"+
			         	 getTh("${ctp:i18n('performanceReport.queryMainHtml.start.getTime')}") + getDate()+
				         "</tr>" ;
         				hh+="</tbody>" +"</table>" +
         			"</td>" +
         			"<td width='60%' vAlign='top' class='padding_lr_10'>"+
  ...
  function improvementAnalysis(){
  		 var hh = "<div class='form_area set_search'>" +
    	 		"<table width='87%' border=0 cellpadding=0 cellspacing=0 class='common_center'>"+"<tbody>"+
         		"<tr>"+
 				"<td width='50%' vAlign='top' id='type_td' class='padding_lr_10'>";
	            	hh+="<table border=0 style='padding-left:70px;' cellpadding=0 cellspacing=0>" +"<tbody>";
			    	hh+="<tr>"+
	    ...
	    if(Constants_report_id==6991064496354839525){
	    	if(isGroupAdmin=='false'){
	    		...
	    		if(isAdministrator=='true'){
	    			hh+="${ctp:i18n('performanceReport.queryMain.reports.self.build')}";
	    		}else{
	    		...
	    		if(isAdministrator=='true'){
	    			hh+="${ctp:i18n('performanceReport.queryMain.getTemplate')}</td>";
	    		}else{
			...
			hh+="<td nowrap='nowrap'><div id='specTempleteDiv' class='common_txtbox_wrap' style='height: 25px;width:100px;float: left;display:none'>"+
  ...
  function appType(){
	   var hh='';
		   //流程统计
		   if(Constants_report_id==6991064496354839525){
			   if(isGroupAdmin=='false'&&isGroupAdmin!=''){
				   hh+="<td>"+
				   ...
				   hh+="<td>"+
				   "<div class='common_selectbox_wrap' ><select id='appType' name='appType' style='width:88%'>"
				   <c:forEach items='${appEnumKeyList}' var='appEnumKey' >
						hh+="<c:set value='${ctp:getApplicationCategoryName(appEnumKey, pageContext)}' var='objName' />"
						hh+="<option value='${appEnumKey}' ${appEnumKey==1 ? 'selected' : '' }>${ctp:i18n(objName)}</option>"
					</c:forEach>
					hh+="</select>"+
					"<input type='hidden' name='statScope' id='statScope' value='group'></div></td>"
			   }
		   }else{
			   hh+="<td width='50%'>" +
		       "<div class='common_selectbox_wrap' >" +
		       ...
		       <c:if test="${ctp:hasPlugin('edoc')}">
		       	"<option value='4'>${ctp:i18n('performanceReport.queryMainHtml.flowType.edoc')}</option>" +
		       </c:if>
		       "</select>" +
		       "</div>" +
		       "</td>";
  ...
  function comprehensiveAnalysis(){
  				var hh = "<div class='form_area set_search'>" +
    	 		"<table width='75%' border=0 cellpadding=0 cellspacing=0 class='common_center'>"+"<tbody>"+
         		"<tr>"+
 				"<td width='50%' vAlign='top' id='type_td' class='padding_lr_10'>";
			    	...
			    	hh+="<tr>"+
			        	getTh("${ctp:i18n('performanceReport.queryMainHtml.flowType.name')}")+appType()+
			         	"</tr>";
			        	hh+="<tr>"+
			        	getTh("${ctp:i18n('performanceReport.queryMainHtml.end.getTime')}") + getTimeInterval("%Y-%m",true)+
			         	"</tr>";
         				hh+="</tbody>" +"</table>" +
         			"</td>" +
         			"<td width='50%' vAlign='top' class='padding_lr_10'>"+
  ...
  function getAccount(){
	  ...
	  hh+="<td nowrap='nowrap'><label for='toDep'><input type='radio' name='statWhat' id='toDep' value='${accountId==null ? 'Department' : 'Account' }' ${isGroupAdmin ? '' : 'checked' } onclick='switchIt(this)'>"+getName("${ctp:i18n('performanceReport.queryMain.reports.department')}")+"</label>"+
		...
		"<input type='text' name='department' id='department' style='width:80px;' value='${ctp:i18n('performanceReport.queryMain.reports.click')}' onclick='selectPeopleDA()'   readonly class='cursor-hand'/>";
		if(isGroupAdmin=="true"){
			hh+="<input type='hidden' name='departmentIds' id='departmentIds' value='${accountId }'/>";
		}else{
			hh+="<input type='hidden' name='departmentIds' id='departmentIds' value='-1' />";
		}
		hh+="</div></td>"+
		"<td width='60px' id='perContent' style='display:none' nowrap='nowrap'><div class='common_txtbox_wrap'>"+
		"<input type='text' name='person' id='person' style='width:80px' value='${ctp:i18n('performanceReport.queryMain.reports.click')}' readonly class='cursor-hand' />"+
	...
	function getName(name){
  	...
  	var hh = "<div class='form_area set_search'>" +
    	 		"<table width='92%' border=0 cellpadding=0 cellspacing=0 style='margin-left:60px;' class='common_center'>"+"<tbody>"+
         		"<tr>"+
 				"<td width='50%' vAlign='top' id='type_td' class='padding_lr_10'>";
	            	hh+="<table border=0  cellpadding=0 cellspacing=0>" +"<tbody>";
			    	hh+="<tr>"+
			        	getTh("${ctp:i18n('performanceReport.queryMainHtml.flowType.name')}")+appType()+
			         	...
			         	hh+="<tr id='timeAll'>";
			         	 if(isGroupAdmin=='true'||isAdministrator=='true'){
	    	 				hh+=getTh("${ctp:i18n('performanceReport.queryMain.textbox.sendTime.name')}");
	     				}else if(isGroupAdmin=='false'&&isAdministrator=='false'){
         			...
         			"<table border=0 cellpadding=0 cellspacing=0>" +"<tbody>" ;
	     				...
	     				hh+=templateRadio()+
				         "</tr>";
				         hh+="<tr id='statisticsRange'>"+getTh("${ctp:i18n('performanceReport.queryMain.textbox.Statistics')}")+getAccount()+
	     				"</tr>";
         			hh+="</tbody>" +"</table>" +
         			"</td>" +
         			"</tr>" +
