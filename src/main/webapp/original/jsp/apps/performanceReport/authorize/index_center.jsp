...
$(document).ready(function() {
  ...
  var report_grid = $("#report_grid").ajaxgrid({
    ...
    colModel : [ {
      ...
      name : 'personsName',
      <c:if test="${hasRanges}">width : '35%',</c:if>
      <c:if test="${!hasRanges}">width : '95%',</c:if>
      sortable : false,
      align : 'left',
      cutsize : 10
    }<c:if test="${hasRanges}">, {
      display : "${ctp:i18n('performanceReport.authorize.queryRangs')}",
      name : 'queryRangs',
      width : '40%',
      sortable : false,
      align : 'left',
      cutsize : 10
    } </c:if><c:if test="${hasType}">,{
      display : "${ctp:i18n('performanceReport.queryMainHtml.getType')}",
      name : 'authType',
      width : '20%',
      sortable : false,
      align : 'left',
    ...
    managerMethod : "findReportAuthList",
    usepager: true,
  ...
  function render(text, data, rowIndex, colIndex ,colum) {
    ...
    } else if(currentColumName=="authType"){
    	if(data.authType=="1"){
    		 return "<span class='grid_black'>系统授权</span>";
    	}else{
    		 return "<span class='grid_black'>用户授权</span>";
    	}
    }else {
  ...
  function edit_report() {
    ...
    if (checkedValue == undefined || checkedValue.length == 0) {
      ...
      return;
    } else if(checkedValue[0].authType=="1"){
      $.alert("${ctp:i18n('report.reportDesign.set.systemAuth.edit')}");
      return;
    }else {
  ...
  function delete_report() {
    ...
    for(var i=0;i<checkedValue.length;i++){
	    if(checkedValue[i].authType==='1'){
	    	 $.alert("${ctp:i18n('performanceReport.system.auth.delete')}");
	    	return;
	    }
    }
...
<div class="comp"comp="type:'breadcrumb',comptype:'location',code:'F08_workFlowAnalysis'"></div>
    ...
    <div class="layout_center page_color " layout="border:false" style="overflow:hidden">
        ...
        <div id="grid_detail"><%-- ${url_performanceReport_personRight}&reportId=${reportId}&parentId=${parentId} --%>
