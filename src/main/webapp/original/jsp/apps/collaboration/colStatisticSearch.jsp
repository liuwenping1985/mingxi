...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            //页面底部说明加载
            $('#summary').attr("src",_ctxPath + "/collaboration/listDesc.do?method=listDesc&type=listStatistic&size="+grid.p.total);
            ...
            function clickRow(data,rowIndex, colIndex) {
                ...
                $('#summary').attr("src",collaborationApi.getCollDetailUrl(data.affairId,'','',openFrom,''));
            }
            //双击事件
            function dbclickRow(data,rowIndex, colIndex){
                ...
                var url = collaborationApi.getCollDetailUrl(data.affairId,'','',openFrom,'');
                var title = data.subject;
                doubleClick(url,title);
            }
        });
        
        //点击待处理协同是可以进行处理的， 处理后的回调函数
        function afterCommitSummary(){
            $('#listStatistic').ajaxgridLoad();//刷新列表
