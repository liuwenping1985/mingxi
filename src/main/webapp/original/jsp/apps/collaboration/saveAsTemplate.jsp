...
function checkRepeatTempleteSubject(){
	...
	try {
	    ...
	    var temManager = new templateManager();
	 	...
	 	var idList = temManager.getPersonTemplateIds(subjectObj.value,type);
...
function setCanSelect(type,s){
	if("all" == type){
		var tt = document.getElementById("template");
		tt.checked = true;	
		tt.disabled = false;
		var ttext = document.getElementById("text");
		document.getElementById("workflow").disabled = false;
		if(!s){//自由协同，无流程的时候
			tt.disabled = true;
...
window.onload = function(){
	...
	var parentTembodyType = "${ctp:escapeJavascript(tembodyType)}";
	var parentFormtitle = '${ctp:escapeJavascript(formtitle)}';
	...
	if(parentTembodyType.value =="FORM"){
	    document.getElementById("subject").value = parentFormtitle;
	    document.getElementById("workflow").disabled = true;		
		document.getElementById("text").disabled = true;
	}
	setCanSelect('${canSelectType}',hasWorkflow);
}
//-->
</script>
</head>
		...
		<td class="bg-advance-middel" valign="top">
			<table width="100%" border="0" cellspacing="0" cellpadding="3">
				<tr>
					...
					<td width="100%"><input class="input-100per" inputName="${ctp:i18n('collaboration.newcoll.subject')}" id="subject"  value="" maxlength="300" /></td>
				</tr>
							...
							<label for="templete">
								<c:choose>
    								<c:when test="${param.edocType!=1}">
    								    <input type="radio" name="type" value="template" id="template"/>
    								</c:when>
    								<c:otherwise>
    								    <input type="hidden" name="type" value="template" id="template"/>
    								</c:otherwise>
								</c:choose>
                                ${ctp:i18n('collaboration.saveAsTemplate.collTemplate')}<!-- 协同模板 -->
							</label>
						</div>

						<c:if test="${param.edocType!=1}">
						<div class="font_size12 margin_t_10">
							<label for="text">
								<input type="radio" name="type" value="text" id="text" ${onlyTemplate ? 'disabled' : ''} />
                                ${ctp:i18n('collaboration.saveAsTemplate.formatTemplate')}<!-- 格式模板 -->
							</label>
						</div>
						<div class="font_size12 margin_t_10">
							<label for="workflow">
								<input type="radio" name="type" value="workflow" id="workflow"  ${onlyTemplate ? 'disabled' :''}/>
                                ${ctp:i18n('collaboration.saveAsTemplate.flowTemplate')}<!-- 流程模板 -->
							</label>
						</div>
						</c:if>
