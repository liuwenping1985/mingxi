	...
	var server2LocalTime = <%=System.currentTimeMillis()%> - new Date().getTime();
        $(document).ready(function () {
            ...
            function rend(txt, data, r, c, col) {
                ...
                if(col.name=='subject'){
                	...
                	if(data.hasResPerm == true){
	                    txt = "<a onclick='linkToWaitSend(\""+data.applicationCategoryKey+"\")'>"+txt+"</a>";
                	}
                }
                return txt;
           }
           ...
           function edit(affairId,summaryId,subState,app,templateId){
               ...
               //原 if(app == 19 && !$.ctx.resources.contains('F07_sendNewEdoc')){
               if(app == 19 && !$.ctx.resources.contains('F07_sendManager')){ //发文
               ...
               if(app == 19){ //发文
            	   ...
            	   window.location =_ctxPath + "/edocController.do?method=entryManager&entry=recManager&recListType=listDistribute&toFrom=newEdoc&listType=newEdoc&from=newEdoc&summaryId="+summaryId+"&edocType=1&affairId="+affairId+"&isSendBackBox=false&backBoxToEdit="+backBoxToEdit;
               }else if(app == 21){//签报
           ...
           function send(){
               ...
               if(app == 19 ||app == 20 ||app == 21){//公文的直接发送
					...
					callBackendMethod("edocManager","isQuickSend",objectId,{
                   ...
                   //查询processId
                   callBackendMethod("colManager","getSummaryById",summaryId,{
        ...
        function linkToWaitSend(app){
            ...
            if(app == 19 || app==20 || app==21 ){
              ...
              if(app == 19){
            	...
            	url = "/edocController.do?method=entryManager&entry=recManager&toFrom=listWaitSend&edocType=1&listType=listWaitSend";
              }else{
...
<body>
    <div id='layout' class="font_size12">
        <div class="layout_north f0f0f0" id="north">
            <div class="clearfix">
                <span class="left color_666 font_size14 padding_l_10 padding_t_5">${ctp:toHTML(columnsName)}${ctp:i18n_1('common.items.count.label',total)}</span><!-- 待发事项 -->
            </div>
            <div id="toolbars"> </div>  
        </div>
