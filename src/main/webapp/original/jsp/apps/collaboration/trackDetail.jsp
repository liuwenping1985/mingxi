...
<head>
          	 ...
          	 $.selectPeople({
        	        ...
        	        ,hiddenPostOfDepartment:true
          	        ...
          	        ,callback : function(res){
          	        	...
          	        	//trackName(res);
          	        	zdgzr = "";
          	        	var newzdgzr = res.value.split(",");
          	        	for(var i = 0 ; i < newzdgzr.length ; i++){
          	        		zdgzr+= "Member|"+newzdgzr[i] + ",";
	          	       	}
         				var partText = document.getElementById("zdgzryName");
         	        	partText.style.display="";
    ...
    function OK(){/**/
        if(isCset){
	        ...
	        if(newTrackType == "2"){
		        var zdgzryName = $("#zdgzryName").val();
	        	if(!zdgzryName){
	        		$.alert($.i18n("collaboration.track.menber.js"));
	        		return;
	        	}
	        }
	       ...
	       //构造传送对象  穿一个数组过去  affairID summaryId isTrack 
	        ...
	        callBackendMethod("colManager","getTrackInfo",tranObj);
	        return  newTrackType;
        }else{
			return  "cantset";
        }
...
<body scroll="no" style="overflow: hidden">
	<input type="hidden" id="zdgzry" name="zdgzry" />
         <div id="htmlID">
        <div class="padding_tb_10 padding_l_10 font_size12" style="padding: 0px 20px;color:#333;background:rgb(250,250,250);height:100px;">
            <!-- 跟踪 -->
            <span class="valign_m">${ctp:i18n('collaboration.forward.page.label4')}:</span>
            <select id="gz" class="valign_m" style="color:#333;width:100px;">
            ...
            <div id="gz_ren" class="common_radio_box clearfix margin_t_10">
                <label for="radio1" class="margin_r_10 hand margin_t_5" style="display:inline-block;float:left;">
                    <!-- 全部 -->
                    <input type="radio" value="0" id="radio1" name="option" class="radio_com">${ctp:i18n('collaboration.listDone.all')}</label>
                <label for="radio4" class="margin_r_10 hand margin_t_5" style="display:inline-block;float:left;">
                    <!-- 指定人 -->
                    <input type="radio" value="0" id="radio4" name="option" class="radio_com">${ctp:i18n('collaboration.listDone.designee')}</label>
                    <input type="text" id="zdgzryName" name="zdgzryName" size="10" readonly style="display: none" onclick="$('#radio4').click()"/>
            </div>
