...
window.onload = function() {
	var parentHeight = parent.$(".stadic_body_top_bottom").height();
	$(".scrollList").height(parentHeight-40);
   showCtpLocation("F02_projectPersonPage");
}

function newColl(templeteId,projectId){
	//OA-99268【项目】项目模板更多页面请求时候会，点击模板名称在后台会抛出异常（TemplateManager的ajax方法已经不存在）
	var requestCaller = new XMLHttpRequestCaller(this, "templateManager", "isTemplateEnabled", false); 
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" >
	  <%@ include file="/WEB-INF/jsp/project/moreProjectBanner.jsp"%> 
	<tr>
							   		...
							   		<tr>
								   		<c:forEach items="${templeteList}" var="templete" varStatus="ordinal">
										   	<td class="text-indent-1em sorts"  width="25%"> 
												...
												<a title="${templeteName}" class='defaultlinkcss' onClick="newColl('${templete.id}','${project.id}')"><span class="ico16  margin_r_5" style="background-position:${templeteIcon[templete.id]};"></span>
											    	&nbsp;${templeteShowName }
											    </a>
											</td>
