...
$().ready(function() {
  ...
  $("#confirmBtu").click(function(){
    ...
    window.parentDialogObj.treedialog.disabledBtn('submit');
    window.parentDialogObj.treedialog.disabledBtn('close');
    ...
    callerResponder.success = function() {
      //try{if(getCtpTop()&&getCtpTop().endProc){getCtpTop().endProc()}}catch(e){};
    	  alert("${ctp:i18n('label.operation.success')}");
  ...
  $("#showAllBtu").click(function(){
    if ($._autofill) {
      var $af = $._autofill, $afg = $af.filllists;
      window.location = "menu.do?method=getTree&showAll=1&appResCategory=1&isAllocated=true&roleId=" + $afg.roleId;
    }
  });
  ...
  $("#showSelectBtu").click(function(){
    if ($._autofill) {
      var $af = $._autofill, $afg = $af.filllists;
      window.location = "menu.do?method=getTree&appResCategory=1&roleId=" + $afg.roleId;
    }
  });
...
function checkTreeNodes(afg, treeName){
  ...
  // 设置节点的勾选子级取消勾选时不影响父级
  treeObj.setting.check.chkboxType.N = "ps";
  ...
  $(treeNodes).each(function (index, domEle) {
    ...
    if(cmd != "selectResource"&&node!=null){
