...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    var chartSetStr= "${ctp:escapeJavascript(chartSet)}";
    _chartset = JSON.parse(chartSetStr);
   
    var paramter = 
    {
    	dom: 'chart',
    	type: _chartset.type,
    	option: _chartset.option,
    	click: function(params)
    	{
    		if (params.seriesName === "${ctp:i18n('colCube.chart.myWork')}")
    		{
    			chartDrillDown(params);
    		}
    	},
    	/* func:function(p) {
    		var polar = p.option.polar[0].indicator;
    		$.each(polar,function(i,indi){
    			indi.min = parseFloat(indi.min);
    			indi.max = parseFloat(indi.max);
    		});
    		return p;
    	}, */
    	args:
    ...
    seeyonUiChartApi.render(paramter);
    /* var chart1 = new SeeyonUiChart2().render(
			"chart", 
			_chartset.type,  
			_chartset.option,
			{
				...
				event: [
                     ...
                     name: "click",
                     func: function(e){
        	            if(e.seriesName=="${ctp:i18n('colCube.chart.myWork')}"){
        		        //个人协同360
          		        chartDrillDown(e);
          	            }
                       }
    ...
    function chartDrillDown(e){
    	...
    	var user_Id="${userId}";
    	var event_type = e.special.toString();
    	getA8Top().up = window;
		var url_=_ctxPath +"/colCube/colCube.do?method=queryColl360DetailList&user_Id="+user_Id+"&dateType="+dateType+"&event_type="+event_type;
		queryDialog = $.dialog({
          id : 'url',
          url : url_,
    ...
    $("#dateTypeSelect").change(function(){
      window.location = url_colCube_colAllRound + "&userId=${userId}&dateTypeSelect="+$("#dateTypeSelect").val()+"&fromPortal=${ctp:toHTML(fromPortal)}";
    });
