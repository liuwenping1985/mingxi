...
$().ready(function() {
  ...
  var mbindService = new mClientBindService();
  $("#applyform").hide();
  $("#button").hide();
  //$("#welcome").hide();
  $("#welcome").show();
  ...
  var toolbar = $("#toolbar").toolbar({
    toolbar: [{
      id: "agree",
      name: "${ctp:i18n('m1.handleOperationElement.attitudeAgree')}",
    ...
    {
      id: "disagree",
      name: "${ctp:i18n('m1.handleOperationElement.attitudeDisAgree')}",
      ...
      click: griddisagree
        ...
        {
        id: "selectView",
        text: "${ctp:i18n('m1.bind.apply.list')}",
        type:'select',
        value:2,
        onchange : changeView,
        items:[{
        	text:"${ctp:i18n('m1.bind.binded.list')}",
        	value:1
        }]
        }]
  });
   ...
   grid = $("#mytable").ajaxgrid({
    ...
    {
      display: "${ctp:i18n('m1.bind.bind.memberNum')}",
      sortable: true,
      name: 'userName',
      width: '20%'
    },
    {
      display: "${ctp:i18n('m1.bind.clientName')}",
      sortable: true,
      name: 'clientName',
      width: '20%'
    },
    {
      display: "${ctp:i18n('m1.bind.clientType')}",
      sortable: true,
      name: 'clientType',
      width: '20%'
    },
    {
      display: "${ctp:i18n('m1.bind.binded.clientNum')}",
      name: 'clientNum',
      sortable: true,
      width: '20%'
    },
    {
        display: "${ctp:i18n('m1.bind.apply.applyDate')}",
    ...
    managerName: "mClientBindService",
    ...
    mbindService.agreeApply($("#addForm").formobj({
      ...
      success: function(result) {
		...
		if(result == -1){
			...
			alert("${ctp:i18n('m1.bind.clientBinded')}");
		} else if (result == -2){

			alert("${ctp:i18n('m1.bind.apply.outnumber')}");
		} else {
			
		alert("${ctp:i18n('m1.bind.apply.disagree.label')}");
		}
          if (getCtpTop() && getCtpTop().endProc) {
  ...
  var searchobj = $.searchCondition({
    ...
    conditions: [{
      ...
      name: 'search_name',
      type: 'input',
      text: "${ctp:i18n('m1.bind.apply.memberNum')}",
      value: 'memberName'
    },
    {
      id: 's_code',
      name: 's_code',
      type: 'input',
      text: "${ctp:i18n('m1.bind.binded.clientNum')}",
      value: 'clientNum'
    },
    {
        id: 's_cn',
        name: 's_code',
        type: 'input',
        text: "${ctp:i18n('m1.bind.clientName')}",
        value: 'clientName'
      },
	{
      id: 's_type',
      name: 's_type',
      type: 'select',
      text: "${ctp:i18n('m1.bind.clientType')}",
      value: 'clientType',
	  items:[{
        	text:"android",
        	value:"android"
  ...
  //切换列表
  function changeView(){
  	window.location = "<c:url value='/m1/mClientBindController.do'/>?method=toBindManage";
  ...
  function gridagree(){
   ...
   var boxs = $("#mytable input:checked");
      if (boxs.length === 0) {
          ...
          var confirm = $.confirm({
            'title': "${ctp:i18n('common.prompt')}",
            'msg': "${ctp:i18n('m1.bind.apply.disagree')}",
            ok_fn: function() {
              var boxs = $("#mytable input:checked");
              if (boxs.length === 0) {
                $.alert("${ctp:i18n('m1.bind.binded.noselect')}");
