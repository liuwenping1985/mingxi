...
var from='${param.from}';
  var fromFlag ="${param.fromFlag}";
	 ...
	 var exist = '${dataExist}';
		...
		if('false' == exist){	
			...
			if(fromFlag == "share") {
				...
				window.location.href=genericURL+"?method=listAllFavoritesArticle&resourceMethod=${resourceMethod}&t="+ getUUID();
	...
	function modifyShare(){
	 ...
	 function reloadPage(){
		 window.location.href = genericURL + "?method=showPost&articleId=${article.id}&familyId=${familyId}&resourceMethod=${resourceMethod}&where=${param.where}&t=" + getUUID() + "&v=${v}";
	 }
	 function closePage(){
		 if (typeof(parent.window.transParams) != 'undefined' && parent.window.transParams != null) {
             parent.window.transParams.parentWin.openBlogDetailCallBack('false');
