...
<div class="dataContain">
	...
	<div class="dataBase marLeftRight hidden" >
		<div class="titleSpan">
			<span>${ctp:i18n("wfanalysis.account.dataBase.t1") }</span>
			<span class="titleSpan_title">
				<em class="ico16 help_16 help_16_red"></em>
				<em class="em_title em_title_bg"></em>
				<em class="em_title em_title_content">${ctp:i18n("wfanalysis.account.dataBase.hovor1") }</em>
			</span>
			<c:if test="${empty isThrough || isThrough eq false}">
				<ul class="name_node_filter">
					<c:if test="${isA8Group eq 'true'}">
						<li style="border-radius: 25px 0 0 25px;" onclick="accountAnalysis.chooseAccountName();">
							<span class="ico16 search_16 search"></span>
							<c:set var="selectedData" value="${ctp:i18n('wfanalysis.process.selected.data') }" />
							<c:set var="accountName" value="${ctp:i18n('wfanalysis.department.dataBase.accountName') }" />
							<span class="text" title="${wfaParam.nameTextCondition}">${!empty wfaParam.nameTextCondition ? selectedData : accountName }</span>
						</li>
					</c:if>
		...
		<div>
			...
			<div class="left">
				<div class="titleSpan">
					<span>${ctp:i18n("wfanalysis.account.dataBase.t2") }</span>
					<span class="titleSpan_title">
						<em class="ico16 help_16 help_16_red"></em>
						<em class="em_title em_title_bg"></em>
						<em class="em_title em_title_content">${ctp:i18n("wfanalysis.account.dataBase.hovor2") }</em>
					</span>
				</div>
			</div>
			...
			<div class="right" style="position:relative;top:10px">
				...
				<span class="hand" onclick="accountAnalysis.toExcel()" title="${ctp:i18n("wfanalysis.common.exportexcel") }">
...
<script type="text/javascript" src="${path }/apps_res/wfanalysis/js/wfa-account-debug.js${ctp:resSuffix()}"></script>
...
<script type="text/javascript">
	$(function(){
		...
		/* 初始化 */
		accountAnalysis.init('${empty param.chart ? "" : param.chart }');
	});
</script>