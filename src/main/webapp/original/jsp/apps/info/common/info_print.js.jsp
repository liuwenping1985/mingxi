...
var GovPrinter = {
		...
		tempMethod : function(colMydocument) {
			if (colMydocument.indexOf("src=\'/common/images") >= 0) {
				colMydocument = colMydocument.replace("src=\'/common/images",
						this.path);
		...
		formartTextField : function(result) {
			...
			result = result.replace(arearReget,
					"<textarea$1style=$2$3;border:0;white-space:normal;$2$4>");
			var selectReget = /<select([\s\S]*?)style[ ]*?=(['"]{1})(.*?)['"]{1}([\s\S]*?)>/ig;
			result = result.replace(selectReget,
					"<select$1style=$2$3;border:0;$2$4>");
			return result;
		},
		...
		infoFormPrint : function() {
			...
			// 消息报送单内容打印
			try {
				var printEdocBody = "${ctp:i18n('infosend.label.print.subject')}";// 公文单
				var selectList = [];
                ...
                if(contentEl){
                    ...
                    // document.getElementById("my:content").scrollHeight;
              ...
              //打印组件，select没有选项时报错，新增一条空的选项，后续再移除
				...
				infoBody = infoBody.replace('id="govFormData"', 'id="govFormData" class="processing_view"');
				...
				if (notPrintDiv) {
					...
					var tempCheckedLabel = "";
					for (var i = 0; i < attitudes.length; i++) {
						if (attitudes[i].checked) {
							tempCheckedLabel = attitudes[i].parentElement.innerText;
							break;
						}
					}

					var tempAttitudeContentEl = document.getElementById("contentOP");
					
					var attiHtml = "";
					attiHtml += '<div style="color: black; clear: both; font-size: 12px; margin-bottom: 5px;">';
					
					//attiHtml += '【' + tempCheckedLabel + "】 ";

					var tempAttitudeContentValue = tempAttitudeContentEl.innerHTML;
					tempAttitudeContentValue = tempAttitudeContentValue.replace(
							/&/ig, "&amp;");
					tempAttitudeContentValue = tempAttitudeContentValue.replace(
							/</ig, "&lt;");
					tempAttitudeContentValue = tempAttitudeContentValue.replace(
					...
					attiHtml += tempAttitudeContentValue;
					
					/**********
							+ "&nbsp;&nbsp;&nbsp;&nbsp; ";
					attiHtml += "${departNameForPrint} ";
					attiHtml += ' <span class="link-blue">'
							+ $.ctx.CurrentUser.name + '</span> ';
					var nowDate = new Date();

					attiHtml += nowDate.getFullYear()
							+ '-'
							+ ((nowDate.getMonth() + 1) < 10 ? "0"
									+ (nowDate.getMonth() + 1) : (nowDate
									.getMonth() + 1))
							+ "-"
					...
					**********/
				    ...
				    var tempParentEl = notPrintDiv.parentElement;
				    var childsEles = tempParentEl.childNodes||tempParentEl.children;
				    var tempOpEl = null;
					for(var i = 0; i < childsEles.length; i++){
						if(childsEles[i].nodeType == 1 && childsEles[i].nodeName && childsEles[i].nodeName.toUpperCase() == "SPAN"){
							tempOpEl = childsEles[i];
							break;
						}
					}
					if(tempOpEl){
						var inStr = tempOpEl.innerHTML;
						var outStr = tempOpEl.outerHTML;
						outStr = outStr.replace(inStr, attiHtml + inStr).replace(/height[ ]*?:[ ]*?\d+?/, "");
						infoBody = infoBody.replace(tempOpEl.outerHTML, outStr);
					}
					infoBody = infoBody.replace(notPrintDiv.outerHTML, "");
				}
				...
				// textarea改成div
				result = result.replace(/<textarea([\s\S]*?)>/ig, "<div$1>")
						.replace(/<\/textarea>/ig, "</div>");
			...
			try {
				...
				if (attNumber != 0) {
					colAttachment = "<table class='margin_t_20' style='width:95%' align='center'><tr><td valign='top' nowrap='nowrap' style='width:100px;'><div class='div-float' style='color: #335186; font-weight: bolder; font-size: 12px;'>"
							+ "${ctp:i18n('infosend.label.summary.attachment')}"
							+ " : ("
							+ attNumber
			...
			// 关联附件
			try {
				var printColMydocument = "${ctp:i18n('common.toolbar.insert.mydocument.label')}"; // 关联文档
				var att2Number = this.getFileAttachmentNumber(2, 'Doc1');
				var colMydocument = "";
				if (att2Number != 0) {
					colMydocument = "<table class='margin_t_20' style='width:95%' align='center'><tr><td valign='top' nowrap='nowrap' style='width:100px;'><div class='div-float' style='color: #335186; font-weight: bolder; font-size: 12px;'>"
							+ printColMydocument
							+ " : ("
							+ att2Number
							+ ")</div></td><td valign='top'>"
							+ this.getFileAttachmentName(2)
							+ "</td></tr></table><br>";
					colMydocument = this.cleanSpecial(colMydocument);
					colMydocument = this.tempMethod(colMydocument);
				}
				var colAttachment2Frag = new PrintFragment(printColMydocument,
						colMydocument);
			...
			// 打印组件引入两个样式，保证文单打印效果
			cssList.add(_ctxPath + "/apps_res/info/css/info_print.css");
