...
function actionRdn(name,dnName,rootType)
{
	document.getElementById("selectLoginName").value="";	
	if((rootType == "user" || rootType == "")  && dnName!=null && dnName!="" && (dnName.indexOf("cn")==0 || dnName.indexOf("CN")==0)){
...
function searchUser(key){
	...
	if(key!=null && key!=""){
		...
		for(i=0;i<searchUserList.length;i++){
			...
			+"<a onclick=\"javascript:actionRdn('"+name+"','"+showDnName+"','"+rootType+"')\"" 
...
<body style="overflow:auto;" scroll="yes" onload="checkLoginName()">
			...
			<script type="text/javascript">
				try{
					...
					<c:forEach items="${userList}" var="root">
						...
						tree_${root.id}.src = encodeURI("<c:url value='/ldap/ldap.do?method=getSubNode&parentDn=${root.dnName}&parentId=${root.id}&type=${root.type}'/>");
					   	tree_${root.id}.action = "javascript:actionRdn('${v3x:toHTML(root.name)}','${v3x:toHTML(root.dnName)}','${root.type}')";
