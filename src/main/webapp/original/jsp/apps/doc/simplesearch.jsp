...
<form action="" name="searchForm" id="searchForm" method="post" onsubmit="return false" style="margin: 0px" target="${target}">
	...
	<div class="div-float-right condition-search-div" style="padding-top: 7px">
		<div class="div-float">
			<select name="condition" id="condition" onChange="setFlag(this.value);showNextCondition(this)" class="condition" style="height:26px;">
				...
				<c:forEach items="${searchConditions}" var="sc">
					<c:choose>
					    ...
					    <c:when test="${CurrentUser.externalType != 0 && (sc.id == 2 || sc.id == 9)}">
					        <option value="${sc.physicalName}|${sc.type}|${sc.isDefault}">${sc.showName}</option>
					    </c:when>
					</c:choose>
				</c:forEach>
		...
		<c:forEach items="${searchConditions}" var="sc">
			...
			<c:when test="${sc.type == 10}">
				...
				<select name="${sc.physicalName}" id="${sc.physicalName}Value" style="width:120px;height:26px;" >
			...
			<c:when test="${sc.type == 13}">
				...
				<select name="${sc.physicalName}" id="${sc.physicalName}" style="width:120px;height:26px;" >
			...
			<c:when test="${sc.type == 7}">
                <c:set var="valueEl" value="${sc.physicalName}" />
				<select name="${sc.physicalName}" id="${sc.physicalName}" style="width:80px;height:26px;" >
			...
			<c:otherwise>
				<c:choose>
					...
					<c:otherwise>
                        <c:set var="valueEl" value="${sc.physicalName}" />
						<input type="text" name="${sc.physicalName}" value="${v3x:toHTML(param[valueEl])}" id="${sc.physicalName}" onkeydown="docSimpleSearchEnter(${param.isQuote eq 'true'})" class="textfield" maxLength='85'/>
					</c:otherwise>
				</c:choose>
			</c:otherwise>
			</c:choose>
		...
		<c:if test="${CurrentUser.externalType == 0 && (param.docLibType ne '1' && docLibType ne '1' && param.isQuote ne 'true')}">
			<span class="advanceSearchDown" ${CurrentUser.externalType == 0 || (param.docLibType ne '1' && docLibType ne '1' && param.isQuote ne 'true')} title="<fmt:message key='doc.advancedquery.label'/>" onclick="showOrHideAdvancedSearch()" id="advancedSearchImg">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
		</c:if>
	</div>
</form>