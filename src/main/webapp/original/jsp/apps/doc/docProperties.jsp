...
function doProperty(){
    ...
    if(commentEnabled_changed || versionEnabled_changed || recommendEnabled_changed) {
    	...
    	var dHeight = (commentEnabled_changed && versionEnabled_changed && recommendEnabled_changed) ? 400 : 240;
    	var appAll = null;
	...
	if(appAll==null) { 
        ...
        propertyMainForm.action = "${detailURL}?method=docLabeldSave&docResId=${param.docResId}&isLib=${v3x:escapeJavascript(param.isLib)}&isFolder=${v3x:escapeJavascript(param.isFolder)}&docLibType=${param.docLibType}&fceChangeFlag="
            ...
            + "&ucfPersonal=" + ucfPersonalShare + "&ucfBorrow=" + ucfBorrow + "&_docLibId=" + _docLibId + "&_docLibType=" + _docLibType
               ...
               + "&_parentCommentEnabled=" + _parentCommentEnabled + "&_flag=" + encodeURI(_flag) +"&_isPersonalShare="+_isPersonalShare;
...
<c:if test="${isGroupLib == true}">
	<c:set value="Account,Department,Post,Level,Team,JoinOrganization,JoinAccountTag,JoinPost" var="publicPanels" />
	<c:set value="Account,Department,Post,Level,Team" var="publicPanels1" />
	<c:set value="Account,Department,Level,Post,Team,Member,JoinAccountTag" var="publicTypes" />
...
<c:if test="${isGroupLib != true}">
    ...
    <!-- 共享 -->
    <c:choose>
    	<c:when test = "${param._docLibType == 3}">
	        <c:set value="Department,Post,Level,Team" var="publicPanels" />
			<c:set value="Account,Department,Level,Post,Team,Member" var="publicTypes" />
    	</c:when>
		<c:when test = "${(param._docLibType == 0 || param._docLibType == 2) && isLibOwner}"><!-- 单位,自定义文档库 -->
	        <c:set value="Department,Post,Level,Team,Outworker,JoinOrganization,JoinAccountTag,JoinPost" var="publicPanels" />
			<c:set value="Account,Department,Level,Post,Team,Member,JoinAccountTag" var="publicTypes" />
	    </c:when>
		<c:otherwise>
    ...
    <c:choose>
    	<c:when test = "${param._docLibType == 3}">
	        <c:set value="Department,Post,Level,Team" var="publicPanels1" />
    	</c:when>
		<c:otherwise>
	        <c:set value="Department,Post,Level,Team,Outworker" var="publicPanels1" />
	    </c:otherwise>
    </c:choose>
</c:if>	

<v3x:selectPeople id="borrow" panels="${publicPanels1}"
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
    <tr id="normalTR" style="display:none"  valign="top">
		<td>
 <style type="text/css">
.newdivStyle tr td{padding: 5px 0;}	
</style>       
<div id="scroll" class="scrollList newdivStyle" style="height:400px;background:rgb(250,250,250);">
...
<c:if test="${isLib == false }">
	<c:if test="${param.isFolder == 'false' }">
		...
		<tr style="color: #333;">
			...
			<td>${v3x:_(pageContext, docPropVO.pathHtml)}</td>
		</tr>
	...
	<c:if test="${isPersonalLib == 'true' && !isDocLink}">
		...
		<tr style="color: #333;">
			...
			<td style="color: #333;">
				...
				<label for="icommentEnabled2" style="color: #333;">
		...
		<tr style="color: #333;">
			...
			<td>
				<label for="recommendEnabled1" style="color: #333;">
				...
				<label for="recommendEnabled2" style="color: #333;">
	...
	<tr id="versionCommentTr" class="hidden" style="color: #333;">
	...
	<c:if test="${docPropVO.isShortCut == false || docPropVO.isCollectDoc == true}">
		...
		<c:if test="${bool == true }">
			<td><input type="text" name="docKeywords" id="docKeywords" style="width: 278px;height:25px;line-height:25px;"  onchange="userChange('ucfProp')" value="${v3x:toHTML(docPropVO.keywords)}" size="52" maxSize="85" validate="maxLength" inputName="<fmt:message key='doc.metadata.def.keywords'/>"/></td>			
		</c:if>
		<c:if test="${bool == false }">
			<td><input type="text" name="docKeywords" id="docKeywords" style="width: 278px;height:25px;line-height:25px;" value="${v3x:toHTML(docPropVO.keywords)}" size="52"  disabled="disabled"/></td>
		</c:if>
	</tr>
    <!-- <tr style="color: #333;"><td>&nbsp;</td></tr> -->
	<tr style="color: #333;">
	...
	<tr style="color: #333;"><td colspan="2"  height="5"></td></tr>
	...
	<tr style="color: #333;">
		<td align="right" nowrap="nowrap"><fmt:message key='doc.jsp.properties.common.comment.enabled'/>:&nbsp;</td>
		<td>
			<label for="foldCommentEnabled1" style="color: #333;">
			...
			<label for="foldCommentEnabled2" style="color: #333;">
	...
	<tr style="color: #333;">
		<td align="right" nowrap="nowrap"><fmt:message key='doc.jsp.properties.common.recommend.enabled'/>:&nbsp;</td>
		<td>
			<label for="foldRecommendEnabled1" style="color: #333;">
			...
			<label for="foldRecommendEnabled2" style="color: #333;">
	...
	<tr style="color: #333;">
		<td align="right" nowrap="nowrap"><fmt:message key='doc.jsp.properties.common.version.enabled'/>:&nbsp;</td>
		<td>
		<label for="foldVersionEnabled1" style="color: #333;">
		...
		<label for="foldVersionEnabled2" style="color: #333;">
...
<c:if test="${isLib == true }">
	...
	<tr id="extendTR" style="display:none"  valign="top">
		<td>
			<div class="scrollList newdivStyle" style="height:370px;">${metadataHtml}</div>
	...
	<td>
		...
		<tr id="borrowTR" valign="top"  style="display:none;background:rgb(250,250,250);">
			<td>
				<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
				<tr height="100%" >
					<td>
					<div class="scrollList" id="jieyue" style="height:370px;">
					<div class="scrollList border_all" style="width:99%;height:98%;background:rgb(250,250,250);border:1px solid rgb(221,221,221);">  
					<c:set value="0" var="i" />
    ...
    $(function(){
      function fixHeight(){
          setTimeout(function(){
              ...
              $("#scroll").height(bodyHeight);
              if($("#jieyue")){
                $("#jieyue").height(bodyHeight - 35);
              }
          }, 10);
      }
      fixHeight();
