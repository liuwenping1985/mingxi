...
function reStyle(){
	$("#attachmentArea").removeAttr("style");
	$("#attachmentArea").attr("overflow", "auto");
	$("#attachmentArea").attr("overflow-x","hidden");
}

</script>
</head>
<body class="h100b over_hidden page_color" onload="reStyle();">
                        ...
                        <div class="stadic_layout_head stadic_head_height2" layout="height:30,sprit:false,border:false" id="divToolBarId">
                            ...
                            <div class="margin_t_5">
                                <span class="color_gray2" title="${ctp:i18n('doc.jsp.properties.common.accesscount')}：${prop.accessCount}">${ctp:i18n('doc.knowledge.read')}(${prop.accessCount})</span>
                                 <c:choose>
                                    <c:when test="${ctp:getBrowserFlagByRequest('HideMenu', pageContext.request) && (docPotent.all || docPotent.edit || docPotent.readOnly || (docPotent.create && doc.createUserId == CurrentUser.id)) && (param.openFrom != 'glwd')}">
                                ...
                                <c:choose>
                                	<c:when test="${doc.recommendEnable && param.openFrom != 'glwd' && !isHistory && CurrentUser.externalType == 0}">
                                ...
                                <div id="submitPublishCommentId" class="form_area margin_t_5">
                                    ...
                                    <div class="align_right margin_t_5">
                                        <a id="btnPublish" href="javascript:void(0)" ${isHistoryVersion ? "":clickEvent} 
                                            docId="${doc.id}" class="common_button common_button_emphasize ${isHistoryVersion ? "common_button_disable":""}">${ctp:i18n('doc.knowledge.publication')}</a>
                                    </div>
                                    <div class="hr_heng margin_t_5"></div>
