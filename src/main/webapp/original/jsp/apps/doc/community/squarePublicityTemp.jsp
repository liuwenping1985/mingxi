...
<c:set var="isOwner" value="${CurrentUser.id == docZoneHot.id }"/>
	...
	<div class="clearfix padding_10">
		<span class="ico32 fileType_${docZoneHot.mimeTypeId} margin_r_5 left"></span>
		<div class="over_auto_hiddenY left_ie6">
			...
			<c:if test="${!docZoneHot.pig}">
				<span class="margin_t_10 ${docZoneHot.avgScoreCss}" title="${docZoneHot.avgScore}"></span>
			</c:if>	
            <c:if test="${docZoneHot.pig}">
            ...
            <c:choose>
                <c:when test="${docZoneHot.createUserValid}">
                    <a class="left text_overflow" style="position:relative;bottom:2px;display: inline-block;max-width:43px;padding-left:0;padding-right:0;height:16px;" onclick='javascript:fnPersonCard(this);' title="${docZoneHot.createUserName}" userId="${docZoneHot.createUserId}">
                </c:when>
                <c:otherwise>
                    <a class='left text_overflow disabled_color' style="position:relative;bottom:2px;display: inline-block;max-width:43px;padding-left:0;padding-right:0;height:16px;" title="${ctp:i18n('doc.alert.user.inexistence')}" userId="${docZoneHot.createUserId}">
                </c:otherwise>
            </c:choose>
	...
	<div class="clearfix" style="border-top:1px solid #e9eaec;">
            <c:choose>
