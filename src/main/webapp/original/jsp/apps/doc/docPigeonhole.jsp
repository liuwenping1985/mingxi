...
<body scroll="no" style="overflow: hidden" onkeydown="listenerKeyESC()">
	<table class="popupTitleRight" class="popupTitleRight" width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td height="20" class="PopupTitle" align="right" style="background:#fafafa;">
			...
			<td height="20" class="PopupTitle" align="right" colspan="2" style="background:#fafafa;">
				<input type="text" name="<fmt:message key='doc.search.title.label'/>" id="frName" value="<fmt:message key='doc.jsp.search.label'/>"  onfocus="if(this.value==defaultValue) {this.value='';this.style.color='#000'}" onblur="if(this.value==''){value='<fmt:message key='doc.jsp.search.label'/>';this.style.color='#999'}" style="width: 100px;color:#999999" class="validate" onkeydown="on_return()" validate="type:'string',avoidChar:'&&quot;&lt;&gt;'"/>
			</td>
			<td height="20"  align="right" style="background:#fafafa;">
				<div id="sub" onclick="javascript:search()"  class="div-float condition-search-button"></div>
			</td>
		</tr>

		<tr style="background:#fafafa;">
			<td valign="top" class="bg-advance-middel" colspan="5" style="padding:0;">  
				<iframe src="${detailURL}?method=listRoots&isrightworkspace=pigeonhole&appName=${v3x:toHTML(param.appName)}&atts=${v3x:toHTML(param.atts)}&validAcl=${v3x:toHTML(param.validAcl)}&pigeonholeType=${v3x:toHTML(param.pigeonholeType)}&departPigeonhole=${param.departPigeonhole}" width="100%" height="100%" frameborder="0" name="treeMoveFrame" id="treeMoveFrame" marginheight="0" marginwidth="0" scrolling="yes">
				</iframe>
			</td>
		</tr>
		<tr>
