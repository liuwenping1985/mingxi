...
// 	}
	...
	function openDoc(docResId){
    	...
    	var ret = openDetailURL('${detailURL}?method=docOpenIframeOnlyId&docResId=' + docResId);
	...
	function init(){
		...
		//设置列表内容的高度使其可以铺满一页
		scrollbDivdocs.style.height = document.body.clientHeight - 53-32-30 + "px";
	}
	...
	function folderOpenFunById(id,frType){
		window.top.document.getElementById("main").src="${detailURL}?method=docHomepageIndex&docResId=" + id + "&frType=" + frType + "&t="+getUUID();
	}
				...
				<td class="webfx-menu-bar">
						<form action="" id="searchForm" name="searchForm" method="get" onkeydown="doMySearchEnter()">
					    	...
					    	<div class="div-float-right" style="vertical-align:middle;">
					    		<div class="div-float">
									<select name="condition" id="condition" style="border: 1px solid #ccc; padding: 0 5px; width: 140px; height: 26px; background: #fff;" onChange="showNextSpecialCondition(this)" class="">
							  	...
							  	<div id="nameDiv" class="div-float hidden">
									<input type="text" name="name" value="${name }" class="textfield-search" onkeydown="" style="border: 1px solid #ccc; padding: 0 5px; width: 140px; height: 24px; line-height: 24px; background: #fff;">
							  	</div>
                                <div id="lastUserIdDiv" class="div-float hidden">
                                  <input type="text" id="lastUserId" name="lastUserId" value="${lastUserId}" class="textfield-search" onkeydown=""  style="border: 1px solid #ccc; padding: 0 5px; width: 140px; height: 24px; line-height: 24px; background: #fff;">
                                </div>
							  	<div id="modifyDateDiv" class="div-float hidden">
									<input type="text" id="startdate" name="beginTime" value="${ beginTime}"  class="textfield-search" onclick="whenstart('${pageContext.request.contextPath}',this,575,240);" readonly  style="border: 1px solid #ccc; padding: 0 5px; width: 140px; height: 24px; line-height: 24px; background: #fff;">
									<input type="text" id="enddate" name="endTime" value = "${endTime}"  class="textfield-search" onclick="whenstart('${pageContext.request.contextPath}',this,575,240);" readonly  style="border: 1px solid #ccc; padding: 0 5px; width: 140px; height: 24px; line-height: 24px; background: #fff;">
								</div>	
