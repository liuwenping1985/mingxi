...
<script type="text/javascript">
	...
	function addCondtion4Search() {
		...
		<%-- 部门  --%>
			...
			obj1.setAttribute('style', 'width:154px');
		...
		<%-- 枚举(公文种类129、行文类型130、文件密级133、紧急程度134其实也应当是枚举类型，但之前是作为文本类型保存，兼容老数据处理) --%>
			...
			obj.innerHTML = "<select name='" + propName + "' id='" + propName + "' style='width:154px;'><option value=''><fmt:message key='doc.please.select' /></option>" + ret + "</select>";
			oTd2.appendChild(obj);
		}
		<%-- 布尔类型  --%>
		else if(propType == '7') {
			...
			obj.setAttribute('id', propName);
			obj.setAttribute('style', 'width:154px');
			obj.options.add(new Option("<fmt:message key='doc.please.select' />", ""));
...
<input type="hidden" id="adv_isNewView" value=""/>
						...
						<tr>
							...
							<tr>
								...
								<table width="100%">
									<tr>
										<td height="80px">
											<div class="scrollList" style="height: 80px">
												<table width="100%">
														...
														<c:forEach items="${searchConditions}" var="sc" varStatus="status">
															...
															<td width="22%" align="left">
																...
																<c:when test="${sc.type == 10}">
																	<select name="${sc.physicalName}" id="${sc.physicalName}" style="width:154px;height:22px;" >
																...
																<c:when test="${sc.type == 13}">
																	<select name="${sc.physicalName}" id="${sc.physicalName}" style="width:154px;" >
																...
																<c:otherwise>
																	<c:choose>
																		...
																		<c:otherwise>
																			<input type="text" name="${sc.physicalName}" id="${sc.physicalName}" onkeydown="docAdvancedSearchEnter()" class="textfield" style="width: 154px" maxLength='20' />
																		</c:otherwise>
																	</c:choose>
																</c:otherwise>
																</c:choose>
