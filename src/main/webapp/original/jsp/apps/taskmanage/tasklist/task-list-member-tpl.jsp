...
<%-- 左侧状态栏模板 --%>
<script type="text/html" id="task-list-member-left-tpl">
<li class="item_all current " memberId="-1" style="margin-top:0px;"><span class="text">${ctp:i18n('taskmanage.all')}</span><span class="number ">{{ d.total }}</span><em class="ico16"></em></li>
{{# for(var i =0; i < d.list.length; i++){ }}
{{#   var t = d.list[i];                     }}
      <li class="" memberId="{{t.memberId}}"><span class="text">{{=t.name}}</span><span class="number ">{{t.count}}</span><em class="ico16 top projectTask_setTop_16" style="margin-top:16px"></em></li>
{{#}                                        }}
</script>
<%-- 右侧任务模板 --%>
<script type="text/html" id="task-list-member-center-tpl">
<%-- 右侧任务模板 --%>
{{# for(var i = 0; i < d.list.length; i++){                                         }}
{{#    var t = d.list[i];                                                           }}
    <li task_id="{{t.id}}" is_overdue="{{t.overdue}}" class="list_item clearfix {{t.overdue ? 'li_equipment' : ''}} {{t.auth.canView ? "" : "li_noView" }} {{!t.auth.canUpdate && !t.auth.canDelete && !t.auth.canFeedback && !t.auth.canHasten ? 'li_noControl' : ''}}">
<%-- 任务名称 --%>
...
<!--[if IE 7]>
<style>
.ie7style{height: 22px; margin-top: 6px; display: inline-block;height: 22px; line-height: 22px;}
</style>
<![endif]-->