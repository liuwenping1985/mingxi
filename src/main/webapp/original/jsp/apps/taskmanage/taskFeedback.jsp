...
<head>
	...
	<script type="text/javascript" src="${path}/apps_res/taskmanage/js/taskFeedback.js${v3x:resSuffix()}"></script>
	<script type="text/javascript">
		$(document).ready(function() {
			initBtnEvent();
			initPageUI();
			$("#content").height("75");
			<c:if test='${param.isDialog != 1 and param.isPortal != 1}'>
				fnGetDialog(118);
			</c:if>
...
<body class="h100b" style="background:transparent;">
    ...
    <div class="form_area">
        <%--记录页面原始数据 --%>
        <input type="hidden" id="back_status" value="${ffdomain_task_feedback['status']}"/>
        <input type="hidden" id="back_finishrate_text" value="${ffdomain_task_feedback['finishrate_text']}"/>
        <input type="hidden" id="back_select_risk" value="${ffdomain_task_feedback['select_risk']}"/>
        <input type="hidden" id="back_elapsed_time_text" value="${ffdomain_task_feedback['elapsed_time_text']}"/>
        <%--表单 --%>
        <form id="task_feedback_form" action="${path }/taskmanage/taskfeedback.do?method=savaTaskFeedback">
		        ...
		        <tr>
		            <td width="220px" id="statusTd" class="padding_r_30 padding_t_5">
		                <div class="common_selectbox_wrap">
	                        	...
	                        	codecfg="codeType:'java',codeId:'com.seeyon.apps.taskmanage.enums.TaskStatus'"></select>
		                </div>
		            </td>
