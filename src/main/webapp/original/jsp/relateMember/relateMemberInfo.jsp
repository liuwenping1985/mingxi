...
<script type="text/javascript">
    ...
    function resetWidth()
	{
	    var cloumnWidth = parseInt((document.getElementById("right_div_portal_sub").clientWidth-30)/2)+"px";
	    var column0 = document.getElementById("column0");
	    var column1 = document.getElementById("column1");
	    column0.style.width = cloumnWidth;
	    column1.style.width = cloumnWidth;

	}
	window.onload=resetWidth;//不要括号
</script>
<style> 
...
<body scroll="no" style="overflow: hidden; background: transparent;">
	<c:set value="${v3x:hasPlugin('collaboration') && v3x:hasNewCollaboration()}" var="hasNewColl" />
	...
	<div class="main_div_center">
		<div class="right_div_center">
			<div id="right_div_portal_sub" class="center_div_center">
				<div class="banner-layout" style="margin:0 15px 10px 0;">
				...
				<div class="project-layout-TwoColumns">
					<div id="column0" class="portal-layout-column">
						<div class="portal-layout-cell column_part">
							<div class="portal-layout-cell_head">
								<div class="portal-layout-cell_head_l"></div>
								<div class="portal-layout-cell_head_r"></div>
							</div>
							<table border=0 cellSpacing=0 cellPadding=0 width="100%" class="portal-layout-cell-right main-bg">
								<tbody>
									<tr>
									...
									<tr>
										<td class="sectionBody sectionBodyBorder">
											<v3x:table htmlId="pending" dragable="false" leastSize="8" data="senderList" var="col" pageSize="8" showHeader="false" showPager="false" size="1" className="sort ellipsis">
												<v3x:column type="String" maxLength="20" className="cursor-hand1 sort padding_l_15" hasAttachments="${col.hasAttsFlag}" importantLevel="${col.importantLevel}" bodyType="${col.bodyType}" flowState="${col.finish ? 3 : 0}">
													<a class="title-more" href="javascript:openCollaboration('${col.subjectUrl}')" title="${v3x:toHTMLescapeRN(col.subject)}">${ctp:toHTMLWithoutSpace(col.subject)}</a>
												...
												<v3x:column width="50" type="String" align="right" className="cursor-hand1 sort padding_r_15">
									...
									<tr>
										<td>
											<div class="align_right" style="padding: 2px 15px 5px 15px;">
												...
												<c:if test="${v3x:hasPlugin('webmail') && v3x:hasNewMail()}">
						...
						<div class="portal-layout-cell column_part">
							...
							<c:if test="${ctp:hasResourceCode('F04_docIndex')}">
							<table border="0" cellSpacing="0" cellPadding="0" width="100%" class="portal-layout-cell-right main-bg">
								<tbody>
									<tr>
										<td class="sectionTitleLine sectionTitleLineBackground">
												...
												<tr class="border-bottom">
													...
													<td width="48%" class="sort">
														<c:if test="${docVO[index1]!=null}">
															...
															<a class="title-more" href="<html:link renderURL='/doc.do?method=docHomepageIndex&isShareAndBorrowRoot=true&docResId=${docVO[index1].docResourceId }'/>" title="${docVO[index1].showName}">${v3x:getLimitLengthString(docVO[index1].showName, 33,'...')}</a>
														</c:if>&nbsp;
													</td>
													<td width="47%" class="sort">
														<c:if test="${docVO[index2]!=null}">
															...
															<a class="title-more" href="<html:link renderURL='/doc.do?method=docHomepageIndex&isShareAndBorrowRoot=true&docResId=${docVO[index2].docResourceId }'/>" title="${docVO[index2].showName}">${v3x:getLimitLengthString(docVO[index2].showName, 33,'...')}</a>
														</c:if>&nbsp;
													</td>
												</tr>
                                                </c:forEach>
						...
						<div class="portal-layout-cell column_part">
							...
							<table border=0 cellSpacing=0 cellPadding=0 width="100%" class="portal-layout-cell-right main-bg">
								<tbody>
									...
									<tr>
										<td class="sectionBody sectionBodyBorder red">
											<v3x:table htmlId="sending" dragable="false" leastSize="8" data="memberList" var="col" pageSize="8" showHeader="false" showPager="false" size="1" className="sort ellipsis">
												<v3x:column type="String" maxLength="20" className="cursor-hand1 sort padding_l_15" hasAttachments="${col.hasAttsFlag}" importantLevel="${col.importantLevel}" bodyType="${col.bodyType}" flowState="${col.finish ? 3 : 0}">
													<a class="title-more" href="javascript:openCollaboration('${col.subjectUrl}')" title="${v3x:toHTML(col.subject)}">${ctp:toHTMLWithoutSpace(col.subject)}</a>
													<c:if test="${col.bodyType=='41'}">
														<span class="ico16 doc_16"></span>
													</c:if>
													<c:if test="${col.bodyType=='42'}">
