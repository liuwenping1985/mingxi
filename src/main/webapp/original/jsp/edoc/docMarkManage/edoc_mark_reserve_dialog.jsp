...
function init(){
	...
	var inputWidth = ((titleDivWidth-titleSpanWidth-addMarkReserveWidth-forMatCLabelWidth-80)/2) + "px";
...
<form id="markReserveForm" name="markReserveForm" action="edocMark.do?method=saveMarkReserve" mthod="post" target="reserveIframe">
	...
	<div id="dialog_main" class="bg_color_white">
		...
		<div id="titleDiv" class="margin_10 w100b">
			...
			<input id="startNo" name="startNo" value="${reserveVO.edocMarkCategory.currentNo }" onkeypress="var k=event.keyCode; return k>=48&&k<=57" onpaste="return !clipboardData.getData('text').match(/\D/)" ondragenter="return false" style="width:15px;font-size:12px;ime-mode:Disabled;" /> —— 
			<input id="endNo" name="endNo" size="2" onkeypress="var k=event.keyCode; return k>=48&&k<=57" onpaste="return !clipboardData.getData('text').match(/\D/)" ondragenter="return false" style="width:15px;font-size:12px;ime-mode:Disabled;" /> <label id="forMatCLabel" style="font-size:12px;font-family:SimSun;">${v3x:getLimitLengthString(reserveVO.formatC,11,'...')}</label>
			...
			<div class="margin_t_10 margin_b_10">
				<div class="margin_r_20 margin_b_10" style="height:20px;">
					...
					<span style="padding-left:240px;float:right;">
						<input id="queryNumber" name="queryNumber" style="width:80px" onkeypress="var k=event.keyCode; return k>=48&&k<=57"/><span id="queryButton" class="ico16 search_16"></span>
					</span>
				</div>
	...
	<div class="dialog_main_footer left padding_t_5 w100b" style="height: 44px; color: #fff; background-image: none; background-attachment: scroll; background-repeat: repeat; background-position-x: 0%; background-position-y: 0%; background-size: auto; background-origin: padding-box; background-clip: border-box; background-color: rgb(77, 77, 77);">
		<div class="align_right right" style="width:100%;">
			<a href="javascript:void(0)" id="submitBtn" onclick="doIt()" class="common_button common_button_emphasize margin_r_10" style="cursor: pointer;">${ctp:i18n('common.button.ok.label')}</a>
