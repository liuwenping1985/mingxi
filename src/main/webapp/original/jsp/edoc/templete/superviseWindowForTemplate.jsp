...
<script type="text/javascript">
	...
	function doIt(){
		...
		/*if(/[\|'"@#￥%]/.test(superviseTitle.trim())){
			alert(_("collaborationLang.col_supervise_isnotwellformated"));
			return;
		}*/
		if(!(/^[^\|"']*$/.test(superviseTitle.trim()))){
		...
		transParams.parentWin.openSuperviseWindowForTemplateCallback([mId.value,sDate.value,sNames.value,superviseTitle,role,roleNames]);
...
<input type="hidden" name="supervisorMemberId" id="supervisorMemberId" value="${v3x:toHTML(supervisorId)}"/>
<input type="hidden" name="remindMode" id="remindMode" />
<c:if test="${supervisorId!=null}">
<c:set value="${v3x:showOrgEntitiesOfIds(supervisorId, 'Member', pageContext)}" var="authStr" />
</c:if>
<c:if test="${supervisorId==null}">
<c:set value="" var="authStr" />
</c:if>
<c:set value="${v3x:parseElementsOfIds(supervisorId, 'Member')}" var="authIds" /> 
<v3x:selectPeople id="sv" panels="Department" selectType="Member" minSize="0" maxSize="10"
...
<table class="popupTitleRight" width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
			    ...
			    <tr>
			        ...
			        <td width="83%">
			        	<input type="text" id="supervisorNames" class="input-100per cursor-hand" name="supervisorNames" readonly="true" 
			           				...
			           				value="<c:out value='${authStr==""?"":authStr}' default='${spd}' escapeXml='true' />" >
			        </td>
			    </tr>
			    <tr>
			        ...
			        <td width="83%">
			        	...
			        	<label for="senderDepManager"> <input type="checkbox" id="senderDepManager" name="senderDepManager" <c:if test="${senderDepManager!='' && senderDepManager!=null}" > checked </c:if>><fmt:message key="collaboration.common.common.supervise.initiatorManager" bundle="${v3xSysI18N }"/></label>
			        </td>
			    </tr>
			    <tr>
			        ...
			        <select id="superviseDate" name="superviseDate">
			        	<option value="1" <c:if test="${awakeDate == '1'}">selected</c:if> ><fmt:message key="collaboration.deadline.one.day" /></option>
			        	<option value="2" <c:if test="${awakeDate == '2'}">selected</c:if> ><fmt:message key="collaboration.deadline.two.day" /></option>
			        	<option value="3" <c:if test="${awakeDate == '3'}">selected</c:if> ><fmt:message key="collaboration.deadline.three.day" /></option>
			        	<option value="4" <c:if test="${awakeDate == '4'}">selected</c:if> ><fmt:message key="collaboration.deadline.four.day" /></option>
			        	<option value="5" <c:if test="${awakeDate == '5'}">selected</c:if> ><fmt:message key="collaboration.deadline.five.day" /></option>
			        	<option value="6" <c:if test="${awakeDate == '6'}">selected</c:if> ><fmt:message key="collaboration.deadline.six.day" /></option>			        	
			        	<option value="7" <c:if test="${awakeDate == '7'}">selected</c:if> ><fmt:message key="collaboration.deadline.one.week" /></option>
			        	<option value="14" <c:if test="${awakeDate == '14'}">selected</c:if> ><fmt:message key="collaboration.deadline.two.week" /></option>
			        	<option value="21" <c:if test="${awakeDate == '21'}">selected</c:if> ><fmt:message key="collaboration.deadline.three.week" /></option>
			        	<option value="30" <c:if test="${awakeDate == '30'}">selected</c:if> ><fmt:message key="collaboration.deadline.one.month" /></option>
			        	<option value="60" <c:if test="${awakeDate == '60'}">selected</c:if> ><fmt:message key="collaboration.deadline.two.month" /></option>
			        	<option value="90" <c:if test="${awakeDate == '90'}">selected</c:if> ><fmt:message key="collaboration.deadline.three.month" /></option>			        				        	
			        </select>
			        </td>			         
			    </tr>
			    <tr>
			    	...
			    	<td>
						<textarea name="title" id="title" rows="7" cols="60" >${v3x:toHTML(superviseTitle)}</textarea>
			    	</td>
			    </tr>
			    <%--  tr> 
			    	<td colspan="2">
	...
	<tr>
		<td height="42" align="right" class="bg-advance-bottom" colspan="2">
		    ...
		    <input type="button" onclick="commonDialogClose('win123');" name="close" class="button-default-2" value="<fmt:message key='common.button.cancel.label' bundle='${v3xCommonI18N}'/>" />
		</td>
	</tr>
</table>
</form>
