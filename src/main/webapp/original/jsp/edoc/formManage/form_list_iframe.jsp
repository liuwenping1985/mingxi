...
function submitAuth(elements){
	if(elements){
		...
		if(listForm){
		    ...
		    newEle.setAttribute("name", "auth");
		    newEle.setAttribute("type", "hidden");
		    newEle.value = getIdsString(elements);
		    listForm.appendChild(newEle);
		    
			listForm.action ="${edocForm}?method=doAuthEdocForm";
			...
			<script type="text/javascript">
				...
				<div class="div-float-right">
						<div class="div-float">
					  	...
					  	<div id="nameDiv" class="div-float hidden"><input type="text" name="textfield" style="height:24px;" class="textfield"></div>
...
<v3x:table data="list" var="bean" htmlId="listTable" showHeader="true" showPager="true" className="sort ellipsis">
	...
	<%--不是企业版本才显示授权 --%>
	<c:if test ="${ctp:getSystemProperty('system.ProductId')!=0 &&ctp:getSystemProperty('system.ProductId')!=1 && ctp:getSystemProperty('system.ProductId') != 3 &&ctp:getSystemProperty('system.ProductId')!=7&&ctp:getSystemProperty('system.ProductId')!=12}">
