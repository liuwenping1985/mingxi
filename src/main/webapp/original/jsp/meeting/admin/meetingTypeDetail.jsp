...
function doSubmit(){
	...
	if(name.trim()==""){
		alert("${ctp:i18n('meeting.alert.categoryIsNotEmpty')}");//分类名称不能为空
              return false;
	}
}
...
function validateSubject(v){
	var patrn = /^[^#￥%&~<>|\"']*$/;
	if(!patrn.test(v)){
		alert("${ctp:i18n('meeting.alert.categoryNoAllowedCharacter')}");//会议分类名称不能包含特殊字符（# ￥ % & ~ < > | \ \" '），请重新录入！
		return true;
	}
	return false;
}
...
<form name="myForm" id="myForm" action="meetingType.do?method=execAdd" method="post" target="hiddenIframe" onsubmit="return doSubmit();">
		...
		<table width="700" border="0" cellspacing="0" cellpadding="0" align="center">
			<tr>
				...
				<td width="35%" nowrap="nowrap" class="new-column">
					<input type="text" name="name" id="name" inputName="<fmt:message key='admin.meetingtype.name.label' />" class="input-300px" maxSize="50" maxLength="50" value="${bean.name }" />
				</td>
			</tr>
			<tr>
