...
function doModify(){
	...
	if(id == ""){
		alert("${ctp:i18n('meeting.alert.chooseModifyCategory')}"); //请选择要修改的会议分类！
		return false;
	}
	if(id == "false"){
		alert("${ctp:i18n('meeting.alert.modifyOnlyOneCategory')}");//一次只能修改一条会议分类！
		return false;
	}
	if(id=='cantUpdate'){
		alert("${ctp:i18n('meeting.alert.formCannotModify')}");
		return false;
	}
	parent.detailFrame.location = "meetingType.do?method=createAdd&id="+id;
}
...
function doDelete() {
	...
	if(id == ""){
		alert("${ctp:i18n('meeting.alert.chooseDeleteCategory')}");//请选择要删除的会议分类
		return false;
	}
	for(var i = 0; i < ns.length; i++){
		if(ns[i].type == "checkbox" && ns[i].name == "id" && ns[i].checked){
			if(ns[i].getAttribute("state")==1){
				alert("${ctp:i18n('meeting.alert.ableStateToDelete')}");//启用状态的会议分类不允许删除！
				return;
			}
		}
	}	
	if(confirm("${ctp:i18n('meeting.alert.sureToDelete')}")){//你确定要删除吗？
		document.listForm.action = "meetingType.do?method=execDel";
...
function showDetail(id){
	parent.detailFrame.location = "meetingType.do?method=createAdd&id="+id+"&readOnly=true";
}

function createFormType(){
	parent.detailFrame.location = "meetingType.do?method=createAdd";
}
...
<form name="listForm" id="listForm" method="post" style="margin: 0px" target="hiddenIframe">
  ...
  <v3x:table htmlId="listTable" data="meetingType" var="bean" pageSize="${pageSize}" size="${size}" showHeader="true" showPager="true" isChangeTRColor="true">  
	...
	<v3x:column width="4%" align="center" label="<input type='checkbox' onclick='selectAll(this, \"id\")'/>">
		<input type='checkbox' name='id' value="<c:out value='${bean.id }'/>" state="${bean.state}" sortId="${bean.sortId }" />
	</v3x:column>
	
	<v3x:column width="20%" type="String" label="admin.meetingCategory.name.label" onClick="showDetail('${bean.id }')" 
		className="cursor-hand sort" alt="${bean.name }" value="${bean.name }">
	</v3x:column>
	...
	<v3x:column width="50%" type="String" label="admin.meetingtype.content.label" onClick="showDetail('${bean.id }')" 
		className="cursor-hand sort" alt="${bean.showContent }">
		<c:out value="${bean.showContent }" />
	</v3x:column>
