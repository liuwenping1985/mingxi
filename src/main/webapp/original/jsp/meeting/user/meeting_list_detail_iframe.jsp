...
<head>
<c:if test="${v3x:hasPlugin('videoconference') && joinImportUrl!=''}">
   	<jsp:include page="${joinImportUrl }"></jsp:include>	
</c:if>
...
function join_meeting(joinType) {
	...
	try{
		eval("${joinButtonClkFunName}('${joinButtonClkFunParmas}')");
	}catch(e){
		alert(v3x.getMessage("meetingLang.meeting_video_cannotOpen"));
	}
}
...
function meetingSummaryCreate(meetingId, summaryId, frameObj, closeWin) {
	...
	var url = "meetingSummary.do?method=createSummary&meetingId="+meetingId+"&summaryId="+summaryId+"&listType="+listType;
	...
	//来源-绩效
	url = "meetingNavigation.do?method=entryManager&entry="+defaultEntry+"&listType=createSummary&meetingId="+meetingId+"&summaryId="+summaryId;
	...
	if(parentWindow) {
		...
		if(parent && parent.listFrame != null) {
			url = "meetingSummary.do?method=createSummary&meetingId="+meetingId+"&summaryId="+summaryId+"&listType="+listType;
...
<body style="height:100%" onLoad="loadUE();" class="border-left border-top border-right">
...
<form style="height:100%" action="" method="post" id="dataForm" target="_self">

<iframe src="" name="tarFram" id="tarFram" style="display: none"></iframe>

<table id="t1" border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" align="center" class="CollTable">
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%" align="center">
				...
				<!-- 发起者 -->
		      	...
		      	<TD id="printSenderInfo" width="40%">
		      		<div class="detail-subject" onClick="showPeopleCard('${bean.createUser}');" style="color:#000000;cursor:pointer;font-size:12px;">
		      			${ctp:toHTML(v3x:showOrgEntitiesOfIds(bean.createUser, 'Member', pageContext))}&nbsp;&nbsp;(<fmt:formatDate value="${bean.createDate}" pattern="yyyy-MM-dd HH:mm"/>)
		      		</div>
		      	</TD>
			  	...
			  	<TD id="printAddressInfo">
			  		<div class="detail-subject" style="color:#000000; display: inline;font-size:12px;">${v3x:toHTML(roomName)}</div>
			  	</TD>
			...
			<tr id="attachment2Tr" style="display: none" height="20">
				<td nowrap valign="top" align="right">
					<div class="bg-gray detail-subject" style="font-size:12px; margin-top:5px;">
						<fmt:message key="common.toolbar.insert.mydocument.label" bundle="${v3xCommonI18N}" />
						(<span id="attachment2NumberDiv" class="font-12px"></span>)
						<fmt:message key="label.colon" />
					</div>
				</td>
				<td valign="bottom" colspan="4">
					<div id="attachment2ExportAttachment" class="attachment-single div-float detail-subject" onmouseover="exportAttachment(this)" style="height:auto;max-height:70px;">
						<div class="div-float font-12px"></div>
						<script type="text/javascript">
						<!--
			...
			<tr id="attachmentTr" style="display: none" height="20">
				<td nowrap valign="top" align="right">
					<div class="bg-gray detail-subject" style="font-size:12px; margin-top:5px;">
						...
						(<span id="attachmentNumberDiv" class="font-12px"></span>)
						<fmt:message key="label.colon"/>
					</div>
				</td>
				<td colspan="4" nowrap valign="bottom">
					<div id="attachmentTrExportAttachment" class="attachment-single div-float" onmouseover="exportAttachment(this)" style="height:auto;max-height:70px;">
						<script type="text/javascript">
						<!--
				...
				<TD nowrap class="bg-gray detail-subject" align="right">&nbsp;&nbsp;</TD>
			  	...
			  	<TD class="detail-subject" align="right" colspan="2" >
			  	 <div id="print">
                    ...
                    <c:choose>
				  		<c:when test="${((bean.state==20 || bean.state==30 || bean.state==31 || bean.state==40 || bean.state==-10) && (bean.recorderId==CurrentUser.id || (bean.recorderId=='-1' && bean.emceeId==CurrentUser.id)) && (bean.roomState!=0 && bean.roomState!=2)) && param.isQuote!='true' && fromPigeonhole!='true'}">
...
function resetHeight(){
	var t1 = document.getElementById("t1").offsetHeight;
	var top_ = document.getElementById("top_").offsetHeight;
	var middle_ = document.getElementById("middle_").offsetHeight;
	document.getElementById("floor_").style.height = (t1 - top_ - middle_) + 'px';
}
