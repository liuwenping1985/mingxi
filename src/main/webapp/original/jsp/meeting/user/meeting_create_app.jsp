...
function openLeader(){
	...
	var url = "mtAppMeetingController.do?method=openLeader&approves="+approves+"&ndate="+new Date().getTime();
	var retObj = window.showModalDialog(url,window ,feacture);
}
...
function changeMtType(obj){
	...
	var url = "mtAppMeetingController.do?method=getMtTypeContents&id=${bean.id}&meetingType=app&mtTypeId="+mtTypeId+"&date="+new Date().getTime();
...
<input type="hidden" name="roomId" value="${meetingroomId}"/>
<input type="hidden" name="roomName" value="${meetingroomName}" />
