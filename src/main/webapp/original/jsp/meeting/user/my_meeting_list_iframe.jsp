...
<c:if test="${hasDocPlug && (listType=='listMyPublishOpenedMeeting' || ((listType=='listMyJoinOpenedMeeting' || listType=='listOpenedMeeting') && pigeonholeBtnFlag))}">
	if(v3x.getBrowserFlag('hideMenu')){
		myBar.add(new WebFXMenuButton("pigeonholeBtn", "<fmt:message key='common.toolbar.pigeonhole.label' bundle='${v3xCommonI18N}' />", "mtPigeonhole('<%=com.seeyon.ctp.common.constants.ApplicationCategoryEnum.meeting.key()%>', '${listType}', 'listMyMeeting','2102');", [1,9], "", null));
	}
</c:if>
...
function openLeader(meetingId,len){
	...
	var url = "mtAppMeetingController.do?method=openLeader&fromType=2&type=hytz&ndate="+new Date().getTime();
	if(len == 1){
