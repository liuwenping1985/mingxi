...
<body class="h100b cardmini" scroll="no" style="background-color:#ffffbb">
	<div id="replayPersonsdetail" class="font_size12 adapt_w font_size12 form_area people_msg"  style="height:180px;overflow-x:visible;overflow:auto;">
			...
			<tr height="150">
				...
				<td valign="top" style="padding-left:25px;" width="${width/2}">
					...
					<c:forEach  var="user2" items="${waitJoinList}" varStatus="status2">
						<c:choose>
                          	<c:when test="${user2.subState=='31' }">
                          		<c:set value="ico16 participate_16" var="viewClass" />	
                          	</c:when>
                        ...
                        <c:choose>
                         	...
                         	<c:otherwise>
                         		<li class="margin_t_5" style="text-align:left;width: 90px; overflow: hidden; display: inline-block; white-space: nowrap; text-overflow: ellipsis; margin-left:-15px;"><span class="${viewClass }"></span><font title="${v3x:toHTML(v3x:showMemberName(user2.memberId))}" color="black">${v3x:toHTML(v3x:showMemberName(user2.memberId))}</font></li>
                          	</c:otherwise>
                        </c:choose>
					</c:forEach>
					<c:if test="${!(empty waitJoinList)}">
