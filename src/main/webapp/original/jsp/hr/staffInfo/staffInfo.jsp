...
function setSelectPeopleNameFun (selectType){
	if(parent.dept_flag || parent.post_flag || parent.level_flag || parent.secondPost_flag || parent.reporter_flag){
		if (selectType == 'dept') {
	    ...
	    } else if (selectType == 'Reporter') {
	    	document.getElementById("reporterStr").value = parent.Full_valueName ; 
            document.getElementById("reporter").value = parent.Full_valueId ;
	    }
	}
	parent.selectType = "";
}
...
function memberSelectReporter(){
	parent.selectType = "Reporter";
    parent.selectPeopleFun_reporter();
}
...
<body id="bodyID" scroll="no" bgcolor="#F6F6F6" onload="loadFun()" style="overflow:hidden">
    ...
    document.close();   
			...
			<tr>
				<td class="categorySet-head" valign="top">
					<div id="categoryBody" class="categorySet-body" style="height: 97%;padding-top: 0px;padding-right: 0px;padding-left: 0px;padding-bottom: 0px;overflow: auto;">
														...
														<td class="new-column" width="75%" nowrap="nowrap"
															align="left"><c:if test="${!ReadOnly && Manager}">
																...
																<div id="upfile">
                                                                    ...
                                                                    <input type="button" onclick="uploadPhoto()" class="button-default-2" 
                                                                    value="<fmt:message key='hr.sign.selectfile' bundle='${v3xHRI18N}' />"/>
																</div>
															</c:if></td>
													</tr>
												</table>
									...
									<table width="95%" border="0" cellspacing="0" cellpadding="0"
										...
										<tr>
											...
											<td class="new-column" width="75%" nowrap="nowrap"><input
												...
												readonly ${mdis} ${dis} validate="notNull" title="${v3x:showDepartmentFullPath(member.orgDepartmentId)}" /></td>
										</tr>
										<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
											...
											<td colspan="2">
												<div id="intenalDiv">
													<table width="100%" border="0" cellspacing="0"
														...
														<tr>
															<td class="bg-gray" width="25%" nowrap="nowrap"><fmt:message
																	key="hr.staffInfo.worklocal.label"
																	bundle="${v3xHRI18N}" />:</td>
															<td class="new-column" width="75%" nowrap="nowrap">
																<input class="input-100per" type="text" id="workspace" name="workspace" value="${webMember.worklocalStr}" readonly disabled="disabled"/>
															</td>
														</tr>
														<tr>
															<td class="bg-gray" width="25%" nowrap="nowrap"><fmt:message
																	key="hr.staffInfo.reporter.label"
																	bundle="${v3xHRI18N}" />:</td>
															<td class="new-column" width="75%" nowrap="nowrap">
																<input type="hidden" name="reporter"
																id="reporter"
																value="${webMember.reporter}"> 
																<input type="text" id="reporterStr"
																name="reporterStr" class="input-100per"
																value="${webMember.reporterStr}"
																onclick="memberSelectReporter()" readonly ${mdis} ${dis}/>
															</td>
														</tr>
	...
	<script type="text/javascript">    //document.getElementById("name").focus();个人信息会报错误，姓名是置灰的。所以就设置不了焦点
	    ...
	    function loadFun(){
	    	...
	    	document.getElementById("categoryBody").style.height=document.getElementById("bodyID").clientHeight - 70 - 50;
	    }
	</script>
</body>
</html>