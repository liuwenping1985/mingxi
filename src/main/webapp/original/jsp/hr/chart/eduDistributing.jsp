...
<html>
	...
	<%@ include file="simpleheader.jsp" %>
		...
		var indexFlag = 0;
        var xTicks = [{label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[0]}' bundle='${v3xHRI18N}' />", v: 0},
        			  {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[1]}' bundle='${v3xHRI18N}' />", v: 1}, 
        			  {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[2]}' bundle='${v3xHRI18N}' />", v: 2},
                      {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[3]}' bundle='${v3xHRI18N}' />", v: 3}, 
                      {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[4]}' bundle='${v3xHRI18N}' />", v: 4},
                      {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[5]}' bundle='${v3xHRI18N}' />", v: 5},
                      {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[6]}' bundle='${v3xHRI18N}' />", v: 6},
                      {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[7]}' bundle='${v3xHRI18N}' />", v: 7},
                      {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[8]}' bundle='${v3xHRI18N}' />", v: 8},
                      {label: "<fmt:message key='hr.staffInfo.edulevel.${educationKeys[9]}' bundle='${v3xHRI18N}' />", v: 9}
                      ];
		...
		function easyDrawGraph() {
			...
			var opts = {
   			...
   			var data1 = [[0, ${education[0]}],[1, ${education[1]}], [2, ${education[2]}], [3, ${education[3]}], [4, ${education[4]}],[5, ${education[5]}],[6, ${education[6]}],[7, ${education[7]}],[8, ${education[8]}],[9, ${education[9]}]];
   			if (hasCanvas) {
...
<form id="statisticForm" name="statisticForm" method="post" action="${hrStatisticURL}?method=eduDistributing&newOrChanged=changed">
		...
		<td class="categorySet-head" height="23">
			<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td class="categorySet-title" width="80" nowrap="nowrap"><fmt:message key="hr.statistic.edu.label" bundle="${v3xHRI18N}" /></td>
		   ...
		   <tr>
		    <td class="categorySet-body" align="center" style="padding-top: 5px;padding-bottom: 5px;">
				...
				<c:choose>
					...
					<option value="pie" selected><fmt:message key='hr.statistic.caky.label' bundle='${v3xHRI18N}' /></option>
					...
					<option value="bar" selected><fmt:message key='hr.statistic.histogram.label' bundle='${v3xHRI18N}' /></option>
					</c:otherwise>
				</c:choose>
				</select>
			</td>
		  ...
		  <table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
		   <tr>
		    <td class="categorySet-body" align="center">
	 		  <div class="scrollList padding_t_10" id="graphFm" style="height:160px; overflow:auto;"><div id="graph" height="250" width="400"></div></div>
	 		</td>
		   </tr>
		 </table>   
		</td>
