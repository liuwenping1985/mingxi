...
(function(){
    ...
    lx.use(["jquery","laypage","col"],function(){
        ...
        exportObject.dbps_click=function(id){
            ...
            window.open("/seeyon/duban.do?method=showDbps&sid="+id);
        ...
        exportObject.dbhb_click=function(id){
            window.open("/seeyon/collaboration/collaboration.do?method=newColl&templateId=7672869598940252106&data_id="+id);
        };
        exportObject.dbbj_click=function(id){
            window.open("/seeyon/collaboration/collaboration.do?method=newColl&templateId=-7849725267052772823&data_id="+id);
        };
        exportObject.dbyq_click=function(id){
            window.open("/seeyon/collaboration/collaboration.do?method=newColl&templateId=5568002767950727683&data_id="+id);
        };
        ...
        function renderRow(container,item){
            ...
            if("leader"==mode){
                ...
                htmls.push('<td><div class="layui-row layui-col-space10"> <div class="layui-col-md12"> <button  onclick="db_click(\''+item.uuid+'\')" type="button" value="'+item.uuid+'" class="layui-btn layui-btn-xs  layui-btn-warm dbps">发起督办</button> </div> </td>');
            }else if("xieban"==mode){
                htmls.push('<td><div class="layui-row layui-col-space10">');
                htmls.push('<div class="layui-col-md3"><button  onclick="dbhb_click(\''+item.uuid+'\')" type="button" value="'+item.uuid+'" class="layui-btn layui-btn-xs  layui-btn-warm dbps">任务汇报</button></div>');
                htmls.push('<div class="layui-col-md3"><button  onclick="dbbj_click(\''+item.uuid+'\')" type="button" value="'+item.uuid+'" class="layui-btn layui-btn-xs  layui-btn-warm dbps">办结申请</button></div>');
                htmls.push('<div class="layui-col-md3"><button  onclick="dbyq_click(\''+item.uuid+'\')" type="button" value="'+item.uuid+'" class="layui-btn layui-btn-xs  layui-btn-warm dbps">延期申请</button></div>');
