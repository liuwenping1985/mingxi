...
<<<<<<< HEAD:src/main/webapp/apps_res/duban/js/dash.js
            $.get("/seeyon/duban.do?method=checkCanFinish&recordId="+data_id,function(res){
                if(res.status=="0"){
                    if(res.data=="YES"){
                        ...
                        layer.alert("配合部门还有任务没有办结,无法办结，请与配合部门沟通相关");
                    }
                }else{

                    layer.alert("无法获取配合部门完成情况");
                }
...
=======
            window.open(DB_DAO.getUrlPrefix() + "/seeyon/collaboration/collaboration.do?method=newColl&templateId=" + templateMap["DB_DONE_APPLY"] + "&data_id=" + data_id + "&linkToType=" + data_mode);
>>>>>>> 63ad5d083d5a37ffc7d3ef8c9ec5cd4624c76cc4:src/main/webapp/app_res/duban/js/dash.js
        }
