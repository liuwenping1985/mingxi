...
<div class="dialog_style">
	<fieldset style="border:1px solid #ccc">
		...
		<table width="100%" style="border-spacing: 5px;" class="table_column category_default">
			...
			<c:forEach var="list" items="${govdocDoneAll}" varStatus="status">
				...
				<td>
					<label for=""><input type="checkbox" onclick="changeCheck()" checked="checked" id="check_in"  name="${ctp:i18n("edoc.receive.appending") }"><span class="padding_l_5">${ctp:i18n("edoc.receive.appending") }</span></label>
				</td>
				</c:if>
				<c:if test="${list ne 1 && status.index==1}">
				<td>
					<label for=""><input type="checkbox" onclick="changeCheck()"  id="check_in"  name="${ctp:i18n("edoc.receive.appending") }"><span class="padding_l_5">${ctp:i18n("edoc.receive.appending") }</span></label>
				</td>
				</c:if>
				<c:if test="${list eq 1 && status.index==2}">
				<td>
					<label for=""><input type="checkbox" onclick="changeCheck()" checked="checked" id="check_finish"  name="${ctp:i18n("edoc.supervise.transacted.done") }"><span class="padding_l_5">${ctp:i18n("edoc.supervise.transacted.done") }</span></label>
				</td>
				</c:if>
				<c:if test="${list ne 1 && status.index==2}">
				<td>
					<label for=""><input type="checkbox" onclick="changeCheck()"  id="check_finish"  name="${ctp:i18n("edoc.supervise.transacted.done") }"><span class="padding_l_5">${ctp:i18n("edoc.supervise.transacted.done") }</span></label>
				</td>
				</c:if>
				</c:forEach>
			</tr>
	...
	<form id="permissionForm"  method="post" action="${path}/govDoc/govDocController.do?method=savePermission&ListType=2">
	...
	<table width="100%" style="border-spacing: 5px;" class="table_style1">
		...
		<tr>
			...
			<td>
				<label for=""><input type="checkbox" name="${permission.flowPermId}" class="checkbox1"><span class="font_size12 margin_l_5">${permission.label}</span></label>
			</td>
...
if(newName!=""&&str!=""){
	...
	$("#govdocpending_name").val("govdoc.done.all");
