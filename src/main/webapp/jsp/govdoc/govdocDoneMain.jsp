...
function addColumn(){
	var dialog = $.dialog({
         ...
         url:"${path}/govDoc/govDocController.do?method=doneConfig", 
         width: 570,
          //height: 300,
          title:'分类配置',
          buttons: [{
                	  ...
                	  rv.jsonSubmit({
              			 callback:function(data){
              				 ...
              				 if(data=="false"){
                   				...
                   				add_columnDiv.before("<span class=\"resCode\" style=\"display: inline;\" resCodeParent=\"F07_edocSystem\"><div class=\"tab-tag-left\" style=\"clear: right; white-space: nowrap; word-break: keep-all;\"></div><div class=\"tab-tag-middel\" id=\"firstTab_" + resCodeId + "\" style=\"clear: right; white-space: nowrap; position: relative; word-break: keep-all;\"onclick=\"javascript:changeMenuTab(this);showEdocLocation();\"url=\"/seeyon/govDoc/govDocController.do?method=listDone&amp;app=4&amp;sub_app=1,2,4&amp;listCfgId=" + resCodeId + "\">" + resCodeName + "<span title=\"删除\" class=\"ico16 canceled_16\" style=\"top: 0px; right: 0px; display: none; position: absolute;\"onclick=\"label_click('" + resCodeId + "')\"></span></div><div class=\"tab-tag-right\" style=\"clear: right; white-space: nowrap; word-break: keep-all;\" url=\"dd\"></div><div class=\"tab-separator\" style=\"clear: right; white-space: nowrap; word-break: keep-all;\"></div></span>");
                   				showIcon(); */
                   			 }
              			 }
              		 });
				...
				<td style="padding: 0; margin: 0">
					<div id='scrollborder' style='overflow:hidden; height: 26px;width:100%;padding: 0; margin: 0'>
						<div id="menuTabDiv" class="div-float" style="word-break:keep-all;white-space:nowrap; width: auto">
							...
							<!-- 全部 -->
							<c:forEach var="list" items="${govdocDoneAll}" varStatus="status">
							<c:if test="${list eq 1 && status.index==0}">
							<span class="resCode" resCodeParent="F20_govDocDone">
								<div class="tab-tag-left" style="word-break:keep-all;white-space:nowrap;clear: right;"></div>
								<div id="firstTab" class="tab-tag-middel" style="word-break:keep-all;white-space:nowrap;clear: right;" onclick="javascript:changeMenuTab(this);showEdocLocation();" url="<html:link renderURL='/govDoc/govDocController.do?method=listDone&app=4&sub_app=1,2,4' />">全部</div>
							...
							<span class="resCode" resCodeParent="F20_govDocDone">
								<div class="tab-tag-left" style="word-break:keep-all;white-space:nowrap;clear: right;"></div>
								<div class="tab-tag-middel" style="word-break:keep-all;white-space:nowrap;clear: right;" onclick="javascript:changeMenuTab(this);showEdocLocation();" url="<html:link renderURL='/govDoc/govDocController.do?method=listDone&app=4&sub_app=1,2,4&workflowState=0' />">${ctp:i18n("edoc.receive.appending") }</div>
								<div class="tab-tag-right" style="word-break:keep-all;white-space:nowrap;clear: right;"></div>
								<div class="tab-separator" style="word-break:keep-all;white-space:nowrap;clear: right;"></div>
							</span>
							</c:if>
							...
							<span class="resCode" resCodeParent="F20_govDocDone">
								<div class="tab-tag-left" style="word-break:keep-all;white-space:nowrap;clear: right;"></div>
								<div class="tab-tag-middel" style="word-break:keep-all;white-space:nowrap;clear: right;" onclick="javascript:changeMenuTab(this);showEdocLocation();" url="<html:link renderURL='/govDoc/govDocController.do?method=listDone&app=4&sub_app=1,2,4&workflowState=1' />">${ctp:i18n("edoc.supervise.transacted.done") }</div>
							...
							<c:if test="${list.name ne 'govdoc.done.all'}">
							<span class="resCode" resCodeParent="F20_govDocDone">
								<div class="tab-tag-left" style="word-break:keep-all;white-space:nowrap;clear: right;"></div>
								<div id="firstTab_${list.id}" class="tab-tag-middel" style="word-break:keep-all;white-space:nowrap;clear: right;position:relative;" onclick="javascript:changeMenuTab(this);showEdocLocation();" url="<html:link renderURL='/govDoc/govDocController.do?method=listDone&app=4&sub_app=1,2,4&listCfgId=${list.id}' />">${list.name} <span class="ico16 canceled_16" style="position:absolute;top:0;right:0;display:none" title="删除" onclick="label_click('${list.id}', event)"></span></div>
								<div class="tab-tag-right" style="word-break:keep-all;white-space:nowrap;clear: right;" url="dd"></div>
								<div class="tab-separator" style="word-break:keep-all;white-space:nowrap;clear: right;"></div>
							</span>
							</c:if>
