...
function setDisplayCol(){
	...
	var conditionArr = initW.$(".addConditionElement");
	var conditionResult = new Array();
	for(var i = 0; i < conditionArr.length; i++){
		conditionResult[i] = conditionArr[i].id + "," + conditionArr[i].value;
	}
	initW.document.getElementById('addCondition').value=opvalue;
	initW.document.getElementById('addConditionName').value=opname;
	var addConditionTd = initW.document.getElementById('addConditionTd');
	addConditionTd.innerHTML = "";
	if(opvalue != ""){
		var opvalueArr = opvalue.split(",");
		var opnameArr = opname.split(",");
		var htmlStr = "<TABLE cellspacing='0' border='0'>";
		for(var i = 0; i<opvalueArr.length;i++){
			if(i % 2 == 0){
				htmlStr += "<tr name='addConditionTr'>";
			}
			htmlStr += "<td align='right' width='55px'>"+opnameArr[i]+":</td>";
			htmlStr += "<td align='left' width='260px'>";
			switch (opvalueArr[i]){
				case "receiptDate":
					htmlStr += "<input class='addConditionElement' type='text' readonly='readonly' onClick='whenstart(\"${pageContext.request.contextPath}\",this,575,140);' name='recieveDateB' id='recieveDateB' style='width:115px;'>-<input class='addConditionElement' type='text' readonly='readonly' onClick='whenstart(\"${pageContext.request.contextPath}\",this,575,140);' name='recieveDateE' id='recieveDateE' style='width:114px;'></TD>";
					break;
				case "registrationDate":
					htmlStr += "<input class='addConditionElement' type='text' readonly='readonly' onClick='whenstart(\"${pageContext.request.contextPath}\",this,575,140);' name='registerDateB' id='registerDateB' style='width:115px;'>-<input class='addConditionElement' type='text' readonly='readonly' onClick='whenstart(\"${pageContext.request.contextPath}\",this,575,140);' name='registerDateE' id='registerDateE' style='width:114px;'></TD>";
					break;
				case "packdate":
					htmlStr += "<input class='addConditionElement' type='text' readonly='readonly' onClick='whenstart(\"${pageContext.request.contextPath}\",this,575,140);' name='packdateB' id='packdateB' style='width:115px;'>-<input class='addConditionElement' type='text' readonly='readonly' onClick='whenstart(\"${pageContext.request.contextPath}\",this,575,140);' name='packdateE' id='packdateE' style='width:114px;'></TD>";
					break;
				case "undertakenoffice":
					htmlStr += "<input class='addConditionElement' type='text' name='undertakenoffice'  id='undertakenoffice' onClick='selectPeopleFun_undertakenoffice();'><input class='addConditionElement' type='hidden' name='undertakenofficeId'  id='undertakenofficeId' ></TD>";
					break;
				case "printUnit":
					htmlStr += "<input class='addConditionElement' type='text' name='printUnit'  id='printUnit' onClick='selectPeopleFun_printUnit();'><input class='addConditionElement' type='hidden' name='printUnitId'  id='printUnitId' ></TD>";
					break;
				case "urgentLevel":
					htmlStr += "<select class='addConditionElement' id='urgentLevel' name='urgentLevel'>";
					var urgentLevelList = '${urgentLevelList}';
			    	var arr = new Array();
			    	arr = eval("(" + urgentLevelList + ")");
			    	for(var j = 0; j<arr.length; j++){
						htmlStr += "<option value='"+arr[j].value+"'>"+arr[j].label+"</option>";
			    	}
					htmlStr += "</select></TD>";
					break;
				case "secretLevel":
					htmlStr += "<select class='addConditionElement' id='secretLevel' name='secretLevel'>";
					...
					break;
				case "unitLevel":
					htmlStr += "<select class='addConditionElement' id='unitLevel' name='unitLevel'>";
					var unitLevelList = '${unitLevelList}';
			    	var arr = new Array();
			    	arr = eval("(" + unitLevelList + ")");
			    	for(var j = 0; j<arr.length; j++){
						htmlStr += "<option value='"+arr[j].value+"'>"+arr[j].label+"</option>";
			    	}
					htmlStr += "</select></TD>";
					break;
				case "keepPeriod":
					htmlStr += "<select class='addConditionElement' id='keepPeriod' name='keepPeriod'>";
					var keepPeriodList = '${keepPeriodList}';
			    	var arr = new Array();
			    	arr = eval("(" + keepPeriodList + ")");
			    	for(var j = 0; j<arr.length; j++){
						htmlStr += "<option value='"+arr[j].value+"'>"+arr[j].label+"</option>";
			    	}
					htmlStr += "</select></TD>";
					break;
				default:
					htmlStr += "<input class='addConditionElement' type='text' name='"+opvalueArr[i]+"' id='"+opvalueArr[i]+"'></td>";
					break;
			}
			if(i % 2 == 1 || i == (opvalueArr.length - 1)){
				htmlStr += "</tr>";
			}
		}
		htmlStr += "</table>";
		addConditionTd.innerHTML = htmlStr;
	}
	for(var i = 0; i < conditionResult.length; i++){
		var result = conditionResult[i].split(",");
		var element = initW.document.getElementById(result[0]);
		if(element){
...
function onLoad(){
		var initW = transParams.parentWin; //获得父窗口对象
		//获取选择情况
		var displayCol = initW.document.getElementById('addCondition').value;
		var displayColName = initW.document.getElementById('addConditionName').value;
...
<body scroll="no" onload="onLoad()">
	...
	<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" class="popupTitleRight">
     <tr>
       <td height="20" class="PopupTitle"><fmt:message key='edoc.custom.conditionSetting'/>
		</td>
     </tr>
	  <tr>
