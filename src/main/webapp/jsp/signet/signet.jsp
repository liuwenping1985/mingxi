...
//选择查看人
	 ...
	 */
	 ...
	 var supportEndNames = ["jpg","png","bmp"];
	 function endNamePicture(){
	 	...
	 	if (showImg==0){
			...
			obj = obj[obj.length-1].toLowerCase();
		 	if($.inArray(obj,supportEndNames)==-1){
	   ...
	   function submitCheck(){
		...
		$('#icon').ctpImgcut({
	        ...
	        uploadCallback: function(instantces){ 
	        	if($.inArray(instantces[0].extension,supportEndNames)==-1){
	        		setTimeout(function(){
...
<form enctype="multipart/form-data" id="signetForm" name="signetForm" method="post" action="<html:link renderURL='/signet.do' />?method=${signetManagerMethod }" onsubmit="return submitCheck();" target="tempIframe">
 ...
 <input type="hidden" id="markNameHidden" name="markNameHidden" value="${signet.markName }">
			...
			<table width="45%" border="0" cellspacing="0" cellpadding="0" align="center">
				...
				<tr>
					...
					<td class="new-column" width="80%">
						<input name="markName" type="text" id="markName" class="input-100per" ${dis} value="${signet.markName}" inputName="<fmt:message key="signet.menu.signetname" />" maxSize="60" validate="notNull" />
					</td>
				</tr>
				<tr>
					<td class="bg-gray" width="20%" nowrap="nowrap">
...
<script type="text/javascript">
    bindOnresize('scrollListDiv',40,100)
</script>
</body>
</html>