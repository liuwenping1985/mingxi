...
var canSelfCreateFlow='${canSelfCreateFlow}';
...
var currentUserAccount = "${currentUserAccountId}";
...
function initQWQP(){
		var initCount = 0;
		if(qwqp_file_type=="pdf" || qwqp_file_type=="aip"){
			var interv = setInterval(function(){
				if(document.readyState=='complete'){
						if(initCount == 5){
							...
							if(qwqp_file_type=="aip" && HWPostil1 &&HWPostil1.lVersion){
								clearInterval(interv);
								...
								var select = $("#edoctable").val();
								loadPDF(select);
							}
							if(qwqp_file_type=="pdf" && iWebPDF2015 && iWebPDF2015.Version){
								clearInterval(interv);
								var select = $("#edoctable").val();
								loadPDF(select);
							}
...
// 	var HWPostil1;
// 	var isSave=0;
	var qwqp_file_type= "${fileType}";
	var url1 = "<%=basePath%>PDFServlet.jsp";
	var url2 = "<%=basePath%>AipServlet.jsp";
	
// 	function loadPDF(select){
// 		$.ajax({
// 				url:jsContextPath+"/edocForm.do?method=getPDFId&dd=" + new Date().getTime(),
...
<form name="sendForm" id="sendForm" method="post">
	...
	<input type="hidden" id="saveAip" name ="saveAip" value="false"/>
	<%-- 代理相关参数 --%>
	...
	<input id="isFromHome" name="isFromHome" type="hidden" value="${isFromHome}"/>
	...
	<input type="hidden" id="sub_app" name="sub_app" value="${sub_app }" /> 
...
<div name="edocContentDiv" id="edocContentDiv" style="width:0px;height:0px;overflow:hidden; position: absolute;">
    ...
    <v3x:editor htmlId="content" editType="${canUpdateContent ? '1,0' : '0,0'}"
	  ...
	  contentName="${formModel.edocBody.contentName}" />
</div>
<script type="text/javascript"> 
var newEdocBodyId=fileId;
//var newEdocBodyId;
//调用模板的时候使用。
if((typeof(isFromTemplate)!="undefined" && isFromTemplate==true) || ("${ctp:escapeJavascript(comm)}"=="distribute"||"${ctp:escapeJavascript(comm)}"=="register") ) {
...
<%--
    //myBar.add(new WebFXMenuButton("send", "<fmt:message key='common.toolbar.send.label' bundle='${v3xCommonI18N}' />", "${fromSendBack=="true" ? "showDraftChoose()" : "sendCallBack()"}", [1,4], "", null));
    
    //changyi modify 退稿箱中编辑一公文进入拟文页面 没有保存进草稿箱，显示为保存按钮
--%>
...
<c:if test="${archivedModify==null}"> 
    ...
    <c:if test= "${(appType eq 19 or appType eq 20)}" >
         myBar.add(new WebFXMenuButton("isQuickS", "<input id='isQuickSend' name='isQuickSend' type='checkbox' onclick='showQuickSend();' <c:if test='${formModel.edocSummary.isQuickSend}'>checked='true'</c:if> />"+_label, "","" , "", null));
    </c:if>
</c:if>

var categroy2Forms = '${categroy2Forms}';
//--------判断是否有PDF插件-----------xiex 2015年9月21日 正文增加pdf格式
var mObject=true;
try{
	if(v3x.isMSIE){
...
if(mObject && edocType == 0) bodyTypeSelector.add(new WebFXMenuItem("menu_bodytype_Pdf", v3x.getMessage("V3XLang.common_body_type_pdf_label"), "chanageBodyType('Pdf')",[9,10]));
//--------判断是否有PDF插件完毕-----------
</script>
...
<table border="0" cellspacing="0" cellpadding="0" class="w100b">
	...
	<tr id="form_height">
		...
		<table border="0" height="10" cellpadding="0" cellspacing="0" width="100%" align="center" id="contentTable">
			<tr class="bg-summary1">
				<!-- 发送 -->
				<td id="send_td" rowspan="2" width="1%" nowrap="nowrap">
				  <c:if test="${archivedModify==null}">
					<a href="javascript:void(0)" id='send'
						onClick='${(archivedModify==null && fromSendBack=="true") ? "showDraftChoose()" : "existPDF()"};return false;'
				...
				<div class="padding_l_10" id="workflowInfo_div">
				  ...
				  <a class="common_button common_button_icon comp"
			...
			<tr class="bg-summary1">
				...
				<c:if test="${isFromTemplate}">
                  ...
                  <select id="selectPigeonholePath" title="${archiveName}"
              ...
              <c:choose>
                ...
                <div id="genzong_label">
                  ...
                  <c:choose>
                    ...
                    <%-- 这里加上基准时间不为0时 就显示 --%>
                    <c:when test="${isSystem || isFormParentId || (!empty standardDuration && standardDuration != 0)||isFromTemplate}">
                      <th nowrap="nowrap" align="right" class="padding_r_5">
			...
			<tr id="attachment2TR" class="bg-summary1" style="display: none;"
				height="18">
			...
			<tr id="attachmentTR" class="bg-summary1" style="display: none;" height="18">
				<td nowrap="nowrap" colspan="2" class="bg-gray" align="right">
		...
		<table width="100%" height="100%" border="0" cellspacing="0"
			...
			<tr valign="top">
				...
				<c:if test="${showTurnRecInfo == 'true' }">
					<div id="turnRecInfo" style="position: absolute; right: 100px;">
				...
				<object classid="clsid:7017318C-BC50-4DAF-9E4A-10AC8364C315" codebase="iWebPDF2015.cab#version=1,0,3,1080" id=iWebPDF2015 height=0  width=0%></object>
				</c:if>
...
parent.parent.parent.openTemplateById('${templeteId1}');

$(document).ready(function(){	
	$("#process_xml").val(caseProcessXML);
});
</script>
</body>
</html>