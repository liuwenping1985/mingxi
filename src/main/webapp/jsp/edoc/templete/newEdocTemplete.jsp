...
var defaultPerm = "${defaultPerm}";
</script>
<script type="text/javascript" charset="UTF-8" src="<c:url value="/apps_res/edoc/js/newEdocTemplete.js${v3x:resSuffix()}" />"></script>
</head>
<body class="w100b h100b" scroll="no" onLoad="clearWorkflowResource(window.parent.parent);edocFormDisplay();" onbeforeunload="onbefore();">
...
<table border="0" cellpadding="0" cellspacing="0" style="background-color:#ededed;width: 100%;height: 100%">
    ...
    <td colspan="7" height="22" valign="top">
		...
		*/		
		...
		if("${edocType}" == "0"){
			...
			if(mObject) bodyTypeSelector.add(new WebFXMenuItem("menu_bodytype_Pdf", v3x.getMessage("V3XLang.common_body_type_pdf_label"), "chanageBodyType('Pdf')",[9,10]));
		}
		//--------判断是否有PDF插件完毕-----------
		</script></td>
  </tr>
  <tr class="bg-summary">
    ...
    <td nowrap="nowrap"  width="25%" colspan="1">
        <input name="templatename" maxlength='80' class="input-100per " value="${templete.subject==null?'':v3x:toHTML(templete.subject)}" />
    </td> 
    <%--公文单--%>
  ...
  <%--更多第二行 --%>
  <tr id="show_more_tr2" class="newinfo_more" style="display: none">
  	<%-- 基准时长 --%>
    <td align="right">
  ...
  <tr id="attachmentTR" class="bg-summary" style="display:none;">
      ...
      <td colspan="6" valign="top"><div class="div-float">(<span id="attachmentNumberDiv"></span>个)</div>
...
$(function(){
	// IE9获取浏览器高度赋值给文单显示区域
	$("#formTableHeight").height($("#formTableHeight").parent().height());
})
</script>
