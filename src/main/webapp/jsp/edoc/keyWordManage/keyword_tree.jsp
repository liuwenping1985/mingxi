	...
	<body onresize="resizeBody(150,'treeframeset','min','left')">
		<div class="scrollList padding5tree">
			<script type="text/javascript">
				...
				/*try{
					if(${parentId!=null} && ${accountId!=null}){
						if('${parentId}' != '-1'){
							if(node${fn:replace(accountId,'-','_')}!=null && node${fn:replace(parentId,'-','_')}!=null){
								webFXTreeHandler.expentSelect(node${fn:replace(accountId,'-','_')},node${fn:replace(parentId,'-','_')});
							}
						}else {
							webFXTreeHandler.select(node${fn:replace(accountId,'-','_')});
						}
						
					}else{
						node${fn:replace(groupAccountId,'-','_')}.expand();
					}
				}catch(e){
					webFXTreeHandler.select(root);
				}*/
