...
<title><fmt:message key="editorial.no.input" /></title>
...
function doIt() {
	...
	if (handWriteValue == "") {
		alert(v3x.getMessage('edocLang.edoc_mark_alter_not_write'));
		handWrite.focus();
		return false;
	}

	if(!checkSpecialChar(handWrite)){
		return false;
	}
	...
	arr[2]="recNo";
	transParams.parentWin.editorialChangeCallback(arr);
	...
	<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="popupTitleRight">
		<tr>
			<td height="20" class="PopupTitle">
				<fmt:message key="editorial.no.input" />
			</td>
		</tr>
		<tr>
			<td class="padding010">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td class="new-column" nowrap="nowrap">
							<div><fmt:message key='editorial.no.input.plea' />:</div>
							<input type="text" name="hand_write" id="hand_write" onkeypress="" class="input-300px" maxlength="66">
						</td>	
					</tr>
				</table>
			</td>
		...
		<tr valign="middle">
			<td height="35" align="right" class="bg-advance-bottom">
				...
				<input type="button" value="<fmt:message key='common.button.cancel.label' bundle="${v3xCommonI18N}" />" class="button-default-2" onclick="commonDialogClose('win123');">
			</td>
		</tr>
	</table>
