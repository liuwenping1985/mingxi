...
<script type="text/javascript">
	...
	var initProjectSelect=function(selectProperties,location_id,relateProjectId,fn){
		...
		//第三步:最后一列-更多
		if(retObj.length>=13){
			proHtml+="<option title='${ctp:i18n('project.body.more.label')}' id='allMore' dataType=''>${ctp:i18n('project.body.more.label')}</option>";
		}
		...
		if(relateProjectId!=""&&relateProjectId!=undefined&&relateProjectId!=-1){
			...
			//回填操作
			if(isList==true){
				//在下拉列表中已经存在,点击下拉框选中对应项目
				$("#"+selectProperties['id']+" #none").attr("selected",false);
				$("#"+selectProperties['id']+"").val(relateProjectId);
			}else{
				//在下拉列表中不存在,在更多下新增一项
				var hh="<option value='"+relateProjectId+"' title='"+relateProjectName+"' id='selProject'>"+relateProjectName+"</option>";
	...
	function initModify(selectProperties,location_id,relateProjectId){
		...
		relateProjectName=getProjectName(relateProjectName);
		var proHtml="<select id='"+selectProperties['id']+"' disabled><option value='"+relateProjectId+"'>"+relateProjectName+"</option></select>";
		$("#"+location_id+"").html(proHtml);
	}
	//回填所选中项目
	...
	function showProject(select_id,fn){
		...
		var projectDialog=$.dialog({
    	    id: 'projectDialog',
    	    url: "${path}/project/project.do?method=showProjectList",
    	    width: 780,
    	    height: 550,
              ...
              handler : function() {
              	...
              	if(retObj!=null){
              		...
              		if(len>0){
              			...
              			var hh="<option value='"+projectId+"' title='"+projectName+"' id='selProject'>"+projectName+"</option>";
              			fillProject(hh,select_id,projectId,projectName);
              		}
              	}
              	if(fn){
              		fn();
