...
function setHeight(){
    ...
    function refreshPage(){
    	...
    	$("#task_finish_rate",window.parent.document).text(finish+"%");
    	//设置父页面隐藏的项目百分比
    ...
    <div class="tab_report">
            ...
            <script type="text/javascript">
	            function getSize(data){
	        		...
	        		return {"attachmentsSize":attachmentsSize,"relSize":relSize};
	            }
        	</script>
                ...
                <li class="clearfix <c:if test="${status.count==1}">current</c:if>">
                    ...
                    <div class="tab_report_content">
                        ...
                        <div class="tab_report_content_text">
                            ...
                            <c:if test="${!empty feedback.elapsedTime}">
                        	...
                        	<div id="attachmentTR" class="left" style="display:none;"></div>
        					  <div class="clearfix" id="Attachment_${status.index}">
			                    <div class="color_black left margin_r_5"
			                                        style="margin-top: 5px;color: #8d8d8d;font-size: 12px;"><em class="ico16 affix_16" style="cursor:default"></em>(<span id="feedbackAttachmentsSize_${status.index}"></span>)</div>
        					 ...
        					 <div class="clearfix" id="RelAttachment_${status.index}">
			                    <div class="color_black left margin_r_5"
			                                        style="margin-top: 5px;color: #8d8d8d;font-size: 12px;"><em
			                                        class="ico16 associated_document_16"></em>(<span id="feedbackRelSize_${status.index}"></span>)</div>
        						<div name="attachmentResourceTR" class="comp" comp="type:'assdoc',applicationCategory:'30',attachmentTrId:'_${status.index}',canDeleteOriginalAtts:false, modids:'1,3'" attsdata='${feedback.attachments}'></div>
        					</div>
        				</div>
        				<script type="text/javascript">
        						var result_${status.index} = getSize('${ feedback.attachments}');
        						$("#feedbackAttachmentsSize_${status.index}").html(result_${status.index}.attachmentsSize);
        						$("#feedbackRelSize_${status.index}").html(result_${status.index}.relSize);
        						if(result_${status.index}.attachmentsSize==0){
            						$("#Attachment_${status.index}").hide();
        						}
        						if(result_${status.index}.relSize ==0 ){
            						$("#RelAttachment_${status.index}").hide();
        						}
        				</script>
        				</c:if>
                    </div>
