<%--
 $Author:  xiangq$
 $Rev:  280$
 $Date:: 2012-12-18 13:17:19#$:
  
 Copyright (C) 2012 Seeyon, Inc. All rights reserved.
 This software is the proprietary information of Seeyon, Inc.
 Use is subject to license terms.
--%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" isELIgnored="false"%>
<%@ include file="/WEB-INF/jsp/common/common.jsp"%>

<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<title>进展汇报</title>
<script type="text/javascript" src="${path}/ajax.do?managerName=taskAjaxManager"></script>
<%@ include file="listTaskFeedbackEvent.js.jsp" %>
<%@ include file="listTaskFeedback.js.jsp" %>
<script type="text/javascript">
    $(document).ready(function() {
        var viewType = "${param.viewType}";
        initToolBar();
        initSearchDiv();
        initListData();
        checkOperaPurview();
        showTaskFeedbackDescription();
        $("body").bind("click", window.parent.validateTask);
        showTaskFeedback();
    }); 
</script>
</head>
<body>
    <div id='layout' class="comp page_color" comp="type:'layout'">
        <div id="north" class="layout_north" layout="height:30,sprit:false,border:false">
            <div id="toolbar"></div>
            <input type="hidden" id="createUserValueText" name="createUserValueText" />
            <input type="hidden" id="conditionText" name="conditionText" />
            <input type="hidden" id="firstQueryValueText" name="firstQueryValueText" />
            <input type="hidden" id="secondQueryValueText" name="secondQueryValueText" />
        </div>
        <div id="center" class="layout_center page_color over_hidden" layout="border:false">
            <table id="task_feedback_list" class="flexme3" style="display: none"></table>
            <div id="grid_detail">
                <iframe src="" id="task_feedback_iframe" width="100%" height="100%" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</body>
</html>
