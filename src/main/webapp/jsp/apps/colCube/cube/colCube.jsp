...
<c:forEach var="digest" items="${digestMap}">     
var digestObject = new Object();
digestObject.key = "${digest.key}";
digestObject.value = "${digest.value}";
digestList.push(digestObject);
</c:forEach>

function getdigestValue(key){
  for(var i = 0;i<digestList.length;i++){
    var digestObject = digestList[i];
    if(key == digestObject.key){
...
function clicks(id,w) { //单击相关人员触发的事件
   ...
   manager.checkAuthByMember(json[id].userId2,authMember,{success:function(returnValue){
       if(returnValue){
           ...
           $("#cube_mini").click(function(){
             if('1' == '${ctp:toHTML(fromPortal)}'){//来自portal从portal跳转到360
               getCtpTop().$("#main").attr("src",url_colCube_colCube + "&userId="+json[id].userId2+"&v="+getdigestValue(json[id].userId2)+"&fromPortal=${ctp:toHTML(fromPortal)}");
             }else{
               window.location = url_colCube_colCube + "&userId="+json[id].userId2+"&v="+getdigestValue(json[id].userId2)+"&fromPortal=${ctp:toHTML(fromPortal)}";
             }
           });
           $("#360_mini").click(function(){
             if('1' == '${ctp:toHTML(fromPortal)}'){//来自portal从portal跳转到360
               getCtpTop().$("#main").attr("src", url_colCube_colAllRound + "&userId="+json[id].userId2+"&v="+getdigestValue(json[id].userId2)+"&fromPortal=${ctp:toHTML(fromPortal)}");
             }else{
               window.location = url_colCube_colAllRound + "&userId="+json[id].userId2+"&v="+getdigestValue(json[id].userId2)+"&fromPortal=${ctp:toHTML(fromPortal)}";
             }
           });
       }
   }});
	...
	$("#sendmsg").click(function() {
		var message = getA8Top().getUCStatus();
		if (message != '') {
			$.alert(message);
			return;
		}
...
function goColAllRound(id) {
    //点击中心头像
    if('1' == '${ctp:toHTML(fromPortal)}'){//来自portal从portal跳转到360
      getCtpTop().$("#main").attr("src", url_colCube_colAllRound+"&userId=${userId}&v=${ctp:digest_1(userId)}"+"&fromPortal=${ctp:toHTML(fromPortal)}");
    }else{
      window.location = url_colCube_colAllRound+"&userId=${userId}&v=${ctp:digest_1(userId)}"+"&fromPortal=${ctp:toHTML(fromPortal)}";
    }
}
...
$(function(){
    ...
    try{
        json = (new Function("return " +'${cubeViewList}'))();
    }catch(e){
        //alert(e);
    }
...
function initButton(){
    ...
    $("#dateType").change(function(){
        ...
        window.location = url_colCube_colCube + "&userId=${centerUserId}&v=${ctp:digest_1(centerUserId)}&numberId="+numberId+"&dateType=" + $("#dateType").val()+"&fromPortal=${ctp:toHTML(fromPortal)}";
    });
}
//搜索人员查询按钮事件
function searchPerson(){
    ...
    if(!$.isNull(userId)){
      ...
      window.location = url_colCube_colCube + "&userId=" + userId+"&v="+v+"&dateType="+$("#dateType").val()+"&fromPortal=${ctp:toHTML(fromPortal)}";
    }
}
</script>
</head>
<body class="h100b page_color">
    ...
    <c:if test="${fromPortal != '1'}"><%-- 首页portal隐藏 --%>
      <div class="comp"comp="type:'breadcrumb',comptype:'location',code:'F08_colCube',suffix:'${userName}${ctp:i18n('colCube.auth.show.colCube')}'"></div>
    </c:if>
      <div class="absolute" style="top:2px; right:10px;z-index:2;">  
               <c:if test="${fromPortal != '1'}"><%-- 首页portal隐藏 --%>
               <input id="choosePerson" name="name" class="comp" style="height:18px" userId=""  type="text" comp="type:'search',title:'${ctp:i18n('colCube.list.detail.choosePerson')}',fun:'searchPerson'">  
               </c:if>
    ...
    <div id='layout' class="comp" comp="type:'layout'">
        ...
        <div class="layout_center bg_color_white over_hidden" style="background:#cce4ff;" <c:if test="${fromPortal != '1'}">layout="border:true"</c:if>>
            ...
            <%-- 人员卡片界面 --%>
            <div class="relative_detail border_all absolute font_size12 cardmini hidden" style="width: 300px;" id="relative_content" ><!--关系详情-->
