...
<script type="text/javascript" src="${path}/ajax.do?managerName=conPostManager,orgManager,iOManager,roleManager"></script>
          ...
          ok_fn: function() {
              imanager.canIO({
                success: function(rel) {
                  if ('ok' == rel) {
                    var downloadUrl_e = "${path}/organization/conPost.do?method=exportCnt&isIn=${method}";
...
var __sAccountId = "";
  function editConPost() {
    ...
    $("#form_area").show();
    $("#welcome").hide();
    cPostDetail = cManager.viewOne(boxs[0].value);
    if(!cPostDetail){
      $.messageBox({
              'title': "${ctp:i18n('common.prompt')}",
              'type': 0,
  ...
  $("#cRoles").click(function() {
    ...
    dialog4Role = $.dialog({
      url: "<c:url value='/organization/member.do' />?method=cMember2Role",
  ...
  $("#btnok").click(function() {
    ...
    if ($("#cDeptId").val() === '' || $("#cDeptId").val() === '-1') {
        $.alert("${ctp:i18n('cntPost.select.dept')}");
        return;
      }
    if ($("#cPostId").val() === '') {
        $.alert("${ctp:i18n('department.choose.parttime.position.info')}");
        return;
      }
    if ($("#cRoleIds").val() === '') {
