...
src="${path}/ajax.do?managerName=postManager,iOManager"></script>
<script type="text/javascript">
var dialog;
var inHand=false;
$().ready(function() {
  ...
  var toolbar = $("#toolbar").toolbar({
      ...
      click: function() {
        $.selectPeople({
          ...
          callback: function(ret) {
            pManager.createPostFormBase(ret.value);
            $("#mytable").ajaxgridLoad(o);
          }
        });
      }
    ...
    },
    {
      ...
      subMenu: [{
        ...
        click: function() {
          dialog = $.dialog({
            ...
            //targetWindow: top,
            id: 'importdialog',
            url: '${path}/organization/organizationControll.do?method=importExcel&importType=post',
      ...
      },
      {
        ...
        click: function() {
          var downloadUrl = "${path}/organization/organizationControll.do?method=downloadTemplate&type=Post";
        ...
        imanager.canIO({
          success: function(rel) {
            if ('ok' == rel) {
              ...
              var downloadUrl_e = "${path}/organization/postController.do?method=exportPost1&condition=" + $.toJSON(ssss);
  ...
  $("#btnok").click(function() {
    ...
    pManager.createPost($("#addForm").formobj({
                    ...
                    <div id="welcome">
                        <div class="color_gray margin_l_20">
                            ...
                            <div class="line_height160 font_size14">
                                <c:if test="${CurrentUser.groupAdmin==true}">${ctp:i18n('organization.detail_post_group')}</c:if>
                                <c:if test="${CurrentUser.groupAdmin==false&&ctp:getSystemProperty('org.GroupPost')=='true'}">${ctp:i18n('organization.detail_post')}</c:if>
                                <c:if test="${CurrentUser.groupAdmin==false&&ctp:getSystemProperty('org.GroupPost')=='false'}">${ctp:i18n('organization.detail_post_nogroup')}</c:if>
                            </div>
                        </div>
                    </div>
                    <div id="postform">
