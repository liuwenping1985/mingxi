...
<script type="text/javascript" src="${path}/apps_res/plan/js/planPrint.js"></script>
...
function transmitEvent(){
   if(!$.ctx.resources.contains('F02_eventlistK')) {
...
function selectedOne(checkbox){
    ...
    if(checkbox.checked){
      ...
      var item = new parent.parent.refItem(param.recordId,displayName,"myPlan",'${planId}',param.subTableName,param.masterId,'${createUserId}',param.formId);
...
function initPlanContentUI() {
	...
	intert = window.setInterval(function(){
		if(getIFrameDOM("mainbodyFrame").readyState=="complete"){
			//调用正文组件的方法判断正文是否加载完成
			var isComplete = getIFrameWindow("mainbodyFrame").content_isComplete();
