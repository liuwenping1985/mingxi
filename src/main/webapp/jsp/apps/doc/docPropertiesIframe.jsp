...
<c:set var="isExtend" value="${extandexit}"/>
	<tr>
		<td valign="bottom" height="26" class="panel_tab_div tab-tag border_b">
			<script type="text/javascript">
				// 页签是否显示标记
		...
		<td align="right" height="26" class="panel_tab_div tab-tag border_b">		
		  <c:if test="${v3x:getBrowserFlagByRequest('HideOperation', pageContext.request)}">
	...
	<tr>
		<td class="tab-body-bg" colspan="2">
			...
			<script type="text/javascript">
				...
				docPropertyIframe.window.location.href = jsURL + "?method=docProperty&isP=${v3x:escapeJavascript(param.isP)}&isC=${v3x:escapeJavascript(param.isC)}&isM=${v3x:escapeJavascript(param.isM)}" + 
							...
							"&_parentCommentEnabled=" + _parentCommentEnabled + "&_flag=" + _flag+"&isPig=${v3x:escapeJavascript(param.isPig)}&from=${param.from}&isPersonalShare=${param.isPersonalShare}&v=${v3x:escapeJavascript(v)}";
			</script>
		</td>
	</tr>
	<tr>
