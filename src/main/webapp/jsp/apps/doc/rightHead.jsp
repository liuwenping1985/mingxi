...
<!--
	...
	hiddenPostOfDepartment_perLearn = true;
//-->
</script>
...
<v3x:selectPeople id="perLearn" panels="Department,Team,Post,Level,Outworker" selectType="Member,Team,Post,Level"
	departmentId="${sessionScope['com.seeyon.current_user'].departmentId}" 
...
<script>
//fb---这个jsp没发通过$.ctx.plugins.contains('secret')获取，采用这种方式
var hasSecretPlugins = <%=AppContext.hasPlugin("secret")%>;
    <!--
		...
		menu.AddItem("groupLearn","<fmt:message key='doc.menu.sendto.group.learning'/>","","sendto","","groupLearn");
		menu.AddItem("link","<fmt:message key='doc.menu.sendto.other.label'/>","","sendto","","link");

		if(hasSecretPlugins){
			menu.AddItem("secretLevel","<fmt:message key='secret.doc.secretLevel'/>","<c:url value='/apps_res/doc/images/sendotherto.gif'/>","rbpm","${detailURL}?method=setDocSecretLevel","secretLevel");
		}
		...
		if(v3x.getBrowserFlag("hideMenu") == true){
			menu.AddItem("download","<fmt:message key='doc.menu.download.label'/>",[17,5],"rbpm","","download","${vForDocFileDownload}");
		}
		menu.AddItem("separator","","","rbpm",null);
		if(v3x.getBrowserFlag("hideMenu") == true){
			menu.AddItem("edit","<fmt:message key='doc.menu.edit.label'/>",[1,2],"rbpm","","edit");
		}
		<c:if test="${docCollectFlag eq 'true'}">
		  menu.AddItem("collect","<fmt:message key='doc.menu.collect.label'/>","<c:url value='/apps_res/doc/images/collect.gif'/>","rbpm","","collect");
		  menu.AddItem("uncollect","<fmt:message key='doc.menu.uncollect.label'/>","<c:url value='/apps_res/doc/images/uncollect.gif'/>","rbpm","","uncollect");
		</c:if>
		<c:if test="${docRecommendFlag eq 'true'}">
		menu.AddItem("recommend","<fmt:message key='doc.menu.recommend.label'/>","<c:url value='/apps_res/doc/images/docIcon/recommend_16.gif'/>","rbpm","","recommend");
		</c:if>
		menu.AddItem("move","<fmt:message key='doc.menu.move.label'/>",[2,1],"rbpm","","move");
		if(v3x.getBrowserFlag("hideMenu") == true){		
			menu.AddItem("replace","<fmt:message key='doc.menu.replace.label'/>",[17,4],"rbpm","","replace");
		}
		menu.AddItem("del","<fmt:message key='common.toolbar.delete.label' bundle='${v3xCommonI18N}' />",[1,3],"rbpm","","del");
		menu.AddItem("rename","<fmt:message key='doc.menu.rename.label'/>",[17,7],"rbpm","${detailURL}?method=reName","rename");		
			
		menu.AddItem("lock","<fmt:message key='doc.menu.lock.label'/>","<c:url value='/apps_res/doc/images/lock.gif'/>","rbpm","","lock");
		menu.AddItem("unlock","<fmt:message key='doc.menu.unlock.label'/>","<c:url value='/apps_res/doc/images/unlock.gif'/>","rbpm","","unlock");
		  
		
		if(v3x.getBrowserFlag("hideMenu") == true){
			menu.AddItem("separator","","","rbpm",null);	 		  
			menu.AddItem("share","<fmt:message key='doc.menu.share.label'/>",[8,1],"rbpm","${detailURL}?method=docPropertyIframe&isP=false&isB=false&isM=true&isC=false&docLibType=${docLibType}&docLibId=${param.docLibId}&resId=${param.resId}&frType=${param.frType}&parentRecommendEnable=${param.parentRecommendEnable}&parentCommentEnabled=${param.parentCommentEnabled}&flag=${param.flag}&isShareAndBorrowRoot=${isShareAndBorrowRoot}","properties");
			menu.AddItem("share_pub","<fmt:message key='doc.menu.share.label'/>",[8,1],"rbpm","${detailURL}?method=docPropertyIframe&isP=false&isB=false&isM=false&isC=true&docLibType=${docLibType}&docLibId=${param.docLibId}&resId=${param.resId}&frType=${param.frType}&parentRecommendEnable=${param.parentRecommendEnable}&parentCommentEnabled=${param.parentCommentEnabled}&flag=${param.flag}&isShareAndBorrowRoot=${isShareAndBorrowRoot}","properties");
			menu.AddItem("lend","<fmt:message key='doc.menu.lend.label'/>",[17,2],"rbpm","${detailURL}?method=docPropertyIframe&isP=false&isB=true&isM=false&isC=false&docLibType=${docLibType}&isShareAndBorrowRoot=${isShareAndBorrowRoot}","properties");
		}
		<c:if test="${docOpenToZoneFlag eq 'true'}">
		menu.AddItem("openToZone","<fmt:message key='doc.menu.openToZone.label'/>","<c:url value='/apps_res/doc/images/docIcon/publicSquare_16.gif'/>","rbpm","","openToZone");
		menu.AddItem("openToZoneCancel","<fmt:message key='doc.menu.openToZone.cancel.label'/>","<c:url value='/apps_res/doc/images/docIcon/unPublicSquare_16.gif'/>","rbpm","","openToZone");
		</c:if>
		...
		menu.AddItem("learnHistory","<fmt:message key='doc.menu.learn.history.label'/>",[21,6],"rbpm","","learnHistory");
		menu.AddItem("alert","<fmt:message key='doc.menu.subscribe.label'/>",[13,9],"rbpm","","alert");
		  		  
		menu.AddItem("separator","","","rbpm",null);
		menu.AddItem("property","<fmt:message key='doc.menu.properties.label'/>",[17,6],"rbpm","${detailURL}?method=docPropertyIframe&isP=true&isB=false&isM=false&isC=false&docLibType=${param.docLibType}&docLibId=${param.docLibId}&resId=${param.resId}&frType=${param.frType}&parentRecommendEnable=${param.parentRecommendEnable}&parentCommentEnabled=${param.parentCommentEnabled}&flag=${param.flag}&isShareAndBorrowRoot=${param.isShareAndBorrowRoot}","properties");  
...
function docSelectAll(allButton, targetName){
	...
	ctrlDocMenuByAclMap(appData.doc);
}
</script>