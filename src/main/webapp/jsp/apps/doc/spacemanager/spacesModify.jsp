...
<!--
	...
	//	alert("docSpaceId =" + docSpaceId);
		...
		if (_blogSize && _blogSize != null) {
			if(validateInteger(_docSize.value)  || (_mailSize!=null&&validateInteger(_mailSize.value)) || validateInteger(_blogSize.value)){
				alert(v3x.getMessage("DocLang.doc_space_total_size_not_integer"));
				return ;
			}
		} else {
			if(validateInteger(_docSize.value)  || (_mailSize!=null&&validateInteger(_mailSize.value))){
				alert(v3x.getMessage("DocLang.doc_space_total_size_not_integer"));
				return ;
			}
		}
...
//var deptId = parent.getElementById("deptId").value;
//alert(deptId);
...
<tr>
	...
	<c:if test="${v3x:getSysFlagByName('email_notShow') == false}">
	<tr>
	<td align="center" valign="top">
