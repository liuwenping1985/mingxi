...
//执行查询后返回的参数
  ...
  function breadcrumb(){
	  if(${ctp:getSystemProperty('system.ProductId')} ==0 ){
		//  resetCtpLocation() ;
	  ...
	  if(${ctp:getSystemProperty('system.ProductId')} ==0 || ${ctp:getSystemProperty('system.ProductId')}== 7){
	  	 $("#queryMainCrumbs").hide();
	  }
  }
  //判断菜单资源
  ...
  function reportForwardCol(){
     ...
     if(tableChartTab==2){
    	...
    	if(Constants_report_id==COMPREHENSIVEANALYSIS){
    		...
    		var imgId = performanceQueryManager_.saveImage(comprehensiveChart.chart.getPNG(),1);
    		...
    		if(chartObject!=''&&chartObject!=null){
    			...
    			var imgId = performanceQueryManager_.saveImage(chartObject.chart.getPNG(),1);
    			contentHtml+="<img src='${path}/performanceReport/performanceQuery.do?method=toImageForIe&imgId="+imgId+"'/>";
    		}
    	}else{
	 ...
	 $("#onlineAnalysis").css("border-left-color","#b6b6b6");
	  var printSubject ="";
		...
		if(Constants_report_id==COMPREHENSIVEANALYSIS){
			...
			var imgId = performanceQueryManager_.saveImage(comprehensiveChart.chart.getPNG(),2);
			...
			if(chartObject!=''&&chartObject!=null){   		
				...
				var imgId = performanceQueryManager_.saveImage(chartObject.chart.getPNG(),2);
				colBody+="<img src='${path}/performanceReport/performanceQuery.do?method=toImageForIe&imgId="+imgId+"'/>";
			}
    	}else{
		if(chartObjS!=null&&chartObjS.length>0){
    	...
    	//OA-44998 解决ie7下打印和转发协同的问题
		for(var i=0;i<chartObjS.length;i++){
  				var imgId = performanceQueryManager_.saveImage(chartObjS[i].chart.getPNG(),2);
    			//colBody+="<img src='${path}/performanceReport/performanceQuery.do?method=toImageForIe&imgId="+imgId+"'/>";
