...
//生成表
 function drawGrid() {
    ...
    if ($.isNull(result)) {
        ...
        var hh = "";
        if (reportId == ONLINETIMESTATISTICS) {
            hh = drawGridOnline();
            $("#queryResult").html(hh);
        } else if (reportId == KNOWLEDGESCORESTATISTICS||reportId===EFFICIENCYANALYSIS||reportId===NODEANALYSIS
        ||reportId===OVERTIMEANALYSIS||reportId===IMPROVEMENTANALYSIS||reportId===COMPREHENSIVEANALYSIS||reportId===PROCESSPERFORMANCEANALYSIS) {
            pagingGrid();
        } else {
            ...
            for (var i = 0; i < headList.length; i++) {
                ...
                for (var j = 0; j < sHead.length; j++) {
                    ...
                    if(isAdministrator==='true'&&Constants_report_id==PROCESSPERFORMANCEANALYSIS&&headItem.title==='${ctp:i18n('performanceReport.业务类型')}'){
                ...
                for (var j = 0; j < dataObject.length; j++) {
                    ...
                    if (throughQueryUrl != '' && throughQueryUrl != undefined && !$.isNull(disVal) && disVal != '0') {
                    	if((Constants_report_id==EFFICIENCYANALYSIS||Constants_report_id==OVERTIMEANALYSIS||Constants_report_id==PROJECTSCHEDULESTATISTICS)&&j==0){
                    		...
                    		if(Constants_report_id==PROCESSPERFORMANCEANALYSIS&&headItem.title==='${ctp:i18n('performanceReport.业务类型')}'){
...
function drawGridOnline(){
	  ...
	  for(var j=0;j<sHead.length;j++){
	   ...
	   hh+="</thead>"+   
	   "<tbody>";
	   if($.isNull(reportParams.gridHtml)){
		   hh += "<tr><td align='center' nowrap='nowrap' colspan='"+sHead.length+"'>没有可以显示的数据！</td></tr>";
		   }else{
			   hh+=reportParams.gridHtml;  
		   }
...
function pagingGrid(){
	  ...
	  if(!$.isNull(dataList)){
		   ...
		   hh="<table class='only_table edit_table' border='0' cellSpacing='0' id='knowledgeLeader' cellPadding='0' width='100%' style='overflow:hidden;min-width:480px;'>"+   
		  "<thead>"; 
		 for(var i=0;i<headList.length;i++){
			 var sHead=headList[i];
		  hh+="<tr>";                             
