...
<html style="border:1px #fff solid">
<head>
...
<body class="over_auto_hiddenX w100b h100b" style="_width:100%;" onload="loadData(${calEventInfoBO.calEvent.shareType },${calEventInfoBO.calEvent.id});">
  <div id="colorgray" >
    ...
    <form id="createCalEvent" action="calEvent.do?method=saveCalEvent" method="post">
      <div id="domaincalEvent">
        ...
        <table border=0 cellSpacing=0 cellPadding=0 align=center width="100%" style="table-layout: fixed;">
            <tr>
                <th nowrap="nowrap" width="95px"><label class=margin_r_5><font color="red">*</font>事件:</label></th>
        ...
        <table border=0 cellSpacing=0 cellPadding=0 align=center width="550" style="table-layout: fixed;">
            <tr style="display:none">
                ...
                <td>
                  <DIV class=common_selectbox_wrap>
                    <select id="calEventType" style="font-size:12px;" 
                      <c:if test='${calEventInfoBO.calEvent.id!=-1}'> 
                        disabled </c:if>
                      name="calEventType" class="codecfg"
                      codecfg="codeId:'cal_event_type',defaultValue:${calEventInfoBO.calEvent.calEventType }">
                    </select>
                  </DIV>
                </td>
              ...
              <TD>
                  <div id="beginDateDIV" class="common_txtbox_wrap <c:if test='${calEventInfoBO.calEvent.id!=-1}'>common_txtbox_wrap_dis</c:if>">
                    <input onchange="doDate();" readonly 
                      ...
                      validate='name:"${ctp:i18n('calendar.event.create.beginDate')}",notNull:true,type:5'
                      comp="type:'calendar',ifFormat:'%Y-%m-%d %H:%M',showsTime:true"
                      value="${ctp:formatDateTime(calEventInfoBO.calEvent.beginDate)}" />
                  </div>
              </TD>
              
			  <TH nowrap="nowrap" width="75px" style="display:none"><LABEL class="margin_r_5" for=text>${ctp:i18n('calendar.event.create.signifyType')}:</LABEL></TH>
          ...
          <tr>
              ...
              <TD><div id="endDateDIV" class="common_txtbox_wrap <c:if test='${calEventInfoBO.calEvent.id!=-1}'>common_txtbox_wrap_dis</c:if>">
                  <input onchange="doDate('endDate');"
                    value="${ctp:formatDateTime(calEventInfoBO.calEvent.endDate)}"
                    id="endDate" readonly
                    type="text" class="comp validate <c:if test='${calEventInfoBO.calEvent.id!=-1}'> color_gray2</c:if>" validate='name:"${ctp:i18n('calendar.event.create.endDate')}",notNull:true,type:5'
                    comp="type:'calendar',ifFormat:'%Y-%m-%d %H:%M',showsTime:true" />
                </div></TD>
		      
              <TH nowrap="nowrap" style="display:none"><LABEL class="margin_r_5" for=text>${ctp:i18n('calendar.event.create.states')}:</LABEL></TH>
              <TD style="display:none">
                <DIV class=common_selectbox_wrap>
                  <select style="font-size:12px;" 
                    <c:if test='${calEventInfoBO.calEvent.id!=-1}'> disabled </c:if>
          ...
          <tr>
              ...
              <TH style="display:none" nowrap="nowrap" class="margin_r_5" style="none"><LABEL class=margin_r_5 for=text>${ctp:i18n('calendar.event.create.alarmFlag')}:</LABEL></TH>
              <TD style="display:none">
                <DIV class=common_selectbox_wrap>
                  <select
                    <c:if test='${calEventInfoBO.calEvent.id!=-1}'> disabled </c:if>
              ...
              <td>
                <DIV class=common_selectbox_wrap>
                    <select style="font-size:12px;" <c:if test='${calEventInfoBO.calEvent.id!=-1}'> disabled </c:if>
                      id="shareType" name="shareType" class="codecfg" onchange="choice(this);" codecfg="codeType:'java',codeId:'com.seeyon.apps.calendar.enums.ShareTypeEnum',defaultValue:${calEventInfoBO.calEvent.shareType }"></select>
                </DIV>
              </td>
            ...
            <tr>
                ...
                <div class="align_right">
                    <label class=margin_r_10>主要内容</label>
                </div>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <c:if test="${calEventInfoBO.calEvent.fromType==1||calEventInfoBO.calEvent.fromType==4}">
                    <tr><td>&nbsp</td></tr>
                    <td width="100%" colspan="1">
                        ...
                        <c:if test="${govdocType!=4 }">
                        	${ctp:i18n("calendar.arrangeTime.collaboration")}
                        </c:if>
                        <c:if test="${govdocType==4 }">
                        	${ctp:i18n("section.app.edoc.label")}
                        </c:if>
                        	[<a href="javascript:showCollation();">${ctp:toHTMLWithoutSpace(showTitle)}</a>]
                        </c:if>
                        </td>
            ...
            <tr><td>&nbsp</td></tr>
      		<tr>
      			<td width="100%" colspan="1">
      				<div id="attarea1" class="align_right">
      				  <a id="attaDiv2" href="javascript:void(0)" style="cursor: pointer;margin-right: 5px"><span class="ico16 affix_16"></span>${ctp:i18n("common.toolbar.insertAttachment.label")}(<span id="attachmentNumberDiv">0</span>):</a>
         	...
         	<tr>
         		<td width="100%" colspan="1">
         			<div id="docarea1" class="align_right">
         			    <a id="docDiv2"  href="javascript:void(0)" style="margin-right: 5px"><span class="ico16 relate_file_16"></span>${ctp:i18n("common.toolbar.insert.mydocument.label")}(<span id="attachment2NumberDivposition1">0</span>):</a>
