                          ...
                          },
                          {
                            ...
                            click : function() {
                              $("#deleteCalEvent").attr("action",
                                  "calEvent.do?method=deleteCalEvent");
                              var vid = entrust();
                              var ids = toAllIds(vid);
                              if (ids == null) {
                                ...
                                var curId = ids.split(",");
                                if (curId.length == 1
                                    && vid[0].calEvent.periodicalStyle != 0) {
                                    if(vid.length > 0) {
                                              ...
                                              .jsonSubmit(
                                                  {
                                                    callback : function(res) {
                                                      if (res != null
                                                          && res != "") {
                                                        if (res
                                                            .indexOf("${ctp:i18n('calendar.event.create.had.delete')}") > 0) {
                                                          $.alert({
  ...
  <div id='layout' class="comp stadic_layout" comp="type:'layout'">
    ...
    <div class="stadic_layout_body stadic_body_top_bottom over_hidden" layout="border:false" id="id">
      ...
      <form id="deleteCalEvent" action="calEvent.do?method=deleteCalEvent"
        method="post">
        <input type="hidden" id="formId" name="formId" /> <input type="hidden"
          id="checkVal" name="checkVal" value="1" />
      </form>
    </div>
  </div>
</body>
