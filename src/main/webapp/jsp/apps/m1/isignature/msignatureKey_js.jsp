<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<script type="text/javascript"src="${path}/ajax.do?managerName=mIsignatureManager"></script>
...
$().ready(function() {
  ...
  var misignature = new mIsignatureManager();
  ...
  var toolbar = $("#toolbar").toolbar({
    ...
    },
    {
      ...
      click: function() {
        ...
        if (v.length < 1) {
            $.alert("${ctp:i18n('m1.isignature.userkey.selectInfoMore')}");
          } else {
            var ids = new Array();
          ...
          $.confirm({
            ...
            'msg': "${ctp:i18n('m1.isignature.userkey.deleteConfirm')}",
            ok_fn: function() {
...
//列表
   grid = $("#mytable").ajaxgrid({
    ...
    colModel: [{
      ...
      {
        display: "${ctp:i18n('m1.isignature.userkey.userName')}",
        name: 'userName',
        sortable: true,
        width: '10%'
      },
      {
        display: "${ctp:i18n('m1.isignature.userkey.keysn')}",
        name: 'key',
        sortable: true,
        width: '60%'
      },
      {
        display: "${ctp:i18n('m1.isignature.userkey.createDate')}",
        name: 'createDate',
        sortable: true,
        width: '15%'
      }],
      managerName: "mIsignatureManager",
  ...
  function griddblclick() {
    ...
    if (v.length < 1) {
      $.alert("${ctp:i18n('m1.isignature.userkey.selectInfoOne')}");
    } else if (v.length > 1) {
        ...
        try {
          if (getCtpTop() && getCtpTop().endProc) {
            ...
            if(success == true) {
            	...
            	if("add" == type) {
            		$.alert("${ctp:i18n('m1.isignature.userkey.addSuccess')}");
            		
            	}else if ("modify" == type){
            		$.alert("${ctp:i18n('m1.isignature.userkey.modifySuccess')}");
            	}
            }else{
				$.alert(result.message);
            }
  ...
  function checkUser(obj) {
	  ...
	  if(mdetil != null) {
		  //表示有数据 。提示用户是否修改
		   $.confirm({
				title: "${ctp:i18n('common.prompt')}",
	            'msg': "${ctp:i18n('m1.isignature.userkey.exists')}",
	            ok_fn: function() {
	            	$("#signatureKeyForm").clearform({
	                    clearHidden: true
	                  });
