...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
          getCtpTop().hideLocation();
            ...
            var grid = $('#moreList').ajaxgrid({
                ...
                managerMethod : "getSuperviseList"
            });
            //回调函数
            function rend(txt, data, r, c, col) {
                if(col.name == "id"){
                    ...
                    txt = "<a class='color_black' href='javascript:void(0)' onclick='linkToSummary(\""+data.affairId+"\",\""+escapeStringToHTML(escapeStringToHTML(data.title))+"\",\""+data.summaryId+"\","+data.status+","+data.entityType+")'>"+txt+"</a>";
                }else if(col.name == "appName"){
                	//判断是否有资源菜单
                	var isHave = false;
                	if(data.entityType == 1 && data.appType != 4){
                		//判断协同
                		if($.ctx.resources.contains('F01_supervise')){
                			txt = "<a href='javascript:void(0)' onclick='linkToSupervise("+data.appType+")'>"+txt+"</a>";
                		}
                	}else {
                		//判断公文
                		txt = "<a href='javascript:void(0)' onclick='linkToSupervise("+data.appType+")'>"+txt+"</a>";
                    ...
                    txt = "<a class='color_black' href='javascript:void(0)' onclick='showFlowChart(\""+ data.caseId +"\",\""+data.processId+"\",\""+data.templeteId+"\",\""+data.activityId+"\",\""+data.appType+"\",\"true\")'>"+txt+"</a>";
        ...
        function linkToSupervise(app){
        	...
        	var _url=_ctxPath + "/supervise/supervise.do?method=listSupervise&app=" + app;
            if(edocType){
            	_url+="&edocType="+edocType;
            }
    ...
    <div id='layout' class="font_size12">
        <div class="layout_north bg_color" id="north">
            <div class="padding_b_10">
              <div class="clearfix">
                  <span class="left color_gray2 font_size24 padding_l_10 padding_t_5 font_bold">${ctp:toHTML(columnsName)}${ctp:i18n_1('common.items.count.label',total)}</span><!-- 督办事项 -->
              </div>
              <div class="common_tabs clearfix margin_l_5">
                   <ul class="left">
                       <li id="allSupervise">
