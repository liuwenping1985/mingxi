...
<script type="text/javascript">
  ...
  function categoryManager(act, categoryType) {
    ...
    if (act == 'delete') {
      ...
      if (!businessId || businessId == 0 || businessId == 1 || businessId == 2
          || businessId == 3 || businessId == 4 || businessId == 5) {
          $.alert("collaborationLang.templete_alertSelectTreeNode");
        return;
      }
      //TODO 判断是否可以删除模板分类
      ...
      var dialog = new MxtDialog({
        html : "${ctp:i18n('collaboration.template.pub.deleteTemplateClass')}", //确认要删除该模板分类? 该操作不能恢复!
        targetWindow:getCtpTop(),
        buttons : [
            {
              text : "${ctp:i18n('collaboration.pushMessageToMembers.confirm')}", //确定
              handler : function() {
                templeteTreeFrame.hiddenIframe.location.href = templeteURL
                    + "?method=deleteCategory&id=" + selectId;
                dialog.close();
              }
            }, {
  ...
  function deleteFormCategory(act, categoryType) {
    ...
    var dataUrl = v3x.baseURL
        + encodeURI(encodeURI("/formappMgrController.do?method=judgeuerscond&categoryid="
            + selectId));
  ...
  function init(aUrl) {
    ...
    httpCall.setRequestHeader('Content-Type',
  ...
  function showTempleteList(categoryId, categorType, canManage) {
    ...
    var barDelete = parent.templeteToolBarFrame.document
        .getElementById('deleteCategory');
    var barUpdate = parent.templeteToolBarFrame.document
    ...
    if (categoryId != null && parent.templeteListFrame) {
      parent.templeteListFrame.location.href = templeteURL
          + "?method=systemList&categoryId=" + categoryId + "&categoryType="
          + categorType + "&from=" + getParameter("from");
    }
  }
  ...
  function newTemplete(categoryType, category) {
    ...
    url += "?method=systemNewTemplete&categoryId=" + selectId
        + "&categoryType=" + curCategorType + "&from=" + getParameter("from");
  ...
  function editTemplete(templeteId, categoryType, from) {
    ...
    parent.location.href = url + "?method=systemNewTemplete&templeteId="
        + templeteId + "&categoryType=" + categoryType + "&from=" + from;
  }
  ...
  function showSystemDetail(templeteId, categoryType) {
    ...
    parent.templeteDetailFrame.location.href = url
        + "?method=systemDetail&templeteId=" + templeteId;
  }
    ...
    if (checkForm(theForm) && checkRepeatTempleteSubject(theForm, true)
      ...
      if (branchs) {
        for ( var i = 0, j = keys.length; i < j; i++) {
          ...
          if (branch != null) {
            var str = "<input type=\"hidden\" name=\"branchs\" value=\""
                + keys[i]
                + "↗"
                + branch.id
                + "↗"
                ...
                + "↗"
                + branch.formCondition
                + "↗"
                + branch.conditionTitle
                + "↗"
                + branch.isForce
                + "↗"
                + (branch.conditionDesc ? branch.conditionDesc.escapeQuot()
      ...
      /* try {
        var requestCaller = new XMLHttpRequestCaller(this, "ajaxColManager",
            "checkNodePolicy", false);
  ...
  function showTempleteDetail(id, type, categoryType) {
    //校验模板是否存在
    var requestCaller = new XMLHttpRequestCaller(this, "ajaxTempleteManager",
        "checkTempleteIsExist", false);
    ...
    parent.templeteDetailFrame.location.href = url + "?method=detail&id=" + id
        + "&type=" + type;
  ...
  function refTemplete() {
    ...
    _parent.location.href = collaborationURL + "?method=newColl&templeteId="
        + id + '&from=' + from + "&projectId=" + projectId;
    top.window.close();
    /*}else {
  ...
  function refTempleteContent(type, id) {
    ...
    if (type != 'workflow') {
      ...
      } else if (bodyType == "OfficeWord" || bodyType == "OfficeExcel"
          || bodyType == "WpsWord" || bodyType == "WpsExcel") {
        rv[5] = {
          bodyType : bodyType,
          fileType : fileType,
          fileId : fileId,
  ...
  function deleteTemplete() {
    ...
    $listForm.attr("action", _ctxPath
        + "/template/template.do?method=deleteTemplete");
  ...
  function doAuthTemplete(elements) {
    if (elements) {
      ...
      var listForm = parent.templeteListFrame.document
  ...
  function doInvalidateTemplete(state) {
    ...
    var listForm = document.getElementsByName("listForm")[0];
    if (listForm) {
      listForm.action = "/seeyon/template/template.do?method=doInvalidateTemplete&state="
          + state;
      listForm.method = "post";
      listForm.target = "_self";
      listForm.submit();
  ...
  function checkRepeatCategoryName(form1) {
    ...
    try {
      var requestCaller = new XMLHttpRequestCaller(this,
          "ajaxTempleteCategoryManager", "checkName", false);
  ...
  function openTempleteConfigDlg() {
    var resultList = v3x
        .openWindow({
          url : genericControllerURL
              + "collaboration/templete/templeteConfigIndex&categoryType=0,1,2,3,4,5",
          height : "600",
          width : "800"
        });
    if (!resultList)
    ...
    for ( var i = 0; i < templeteIds.length; i++) {
      var element_id = document
          .createElement("<INPUT TYPE='HIDDEN' NAME='templeteIds' value='" + templeteIds[i] + "' />");
      var element_type = document
          .createElement("<INPUT TYPE='HIDDEN' NAME='templeteTypes' value='" + templeteTypes[i] + "' />");
  ...
  function openTempleteType(fromPage) {
    var resultList = v3x
        .openWindow({
          url : genericControllerURL
              + "collaboration/templete/templeteConfigIndex&categoryType=0,1,2,3,4,5&from="
              + fromPage,
          height : "600",
          width : "800"
        });
  ...
  function alertChangeType(obj) {
	...
	function setParaWhenIsWorkflowOrText(){
		 ...
		 colPigeonhole.disabled = true;
	      try{project.options[0].selected = "selected";//关联项目
	      project.disabled = true;}catch(e){}
