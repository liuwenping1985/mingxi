...
function loadInit(){
	...
	if(pw.installWps){
		attFileType.put("application/kswps","WpsWord");
		attFileType.put(".wps","WpsWord");
		attFileType.put("application/msword","WpsWord");
		attFileType.put("application/vnd.openxmlformats-officedocument.wordprocessingml.document","WpsWord");
		attFileType.put(".doc","WpsWord");
		attFileType.put(".docx","WpsWord");
	}
	if(pw.installEt){
		attFileType.put("application/kset","WpsExcel");
		attFileType.put(".et","WpsExcel");
		attFileType.put("application/vnd.ms-excel","WpsExcel");
		attFileType.put("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","WpsExcel");
		attFileType.put(".xls","WpsExcel");
		attFileType.put(".xlsx","WpsExcel");
	}
}
...
function addAtt2Body(attachment,attBody,i){
	...
	if(!attachment.reference){
		attachment.reference = '${ctp:escapeJavascript(param.reference)}';
		attachment.subReference = '${ctp:escapeJavascript(param.subReference)}';
	}
	if(!attachment.id){
	...
	if(isEdocAtt&&isEdocAtt=="true"){//公文
		innerHTMLStr = "<label title='"+escapeStringToHTML(attachment.filename)+"'  ><input name='id' onclick='isCheckAll()' type='checkbox' id='"+attachment.id+"' reference='"+attachment.reference+"' subReference='"+attachment.subReference+"' category='"+attachment.category+"' "
							...
							+" extension='"+attachment.extension+"' v='"+attachment.v+"' icon='"+attachment.icon+"' extReference='${ctp:escapeJavascript(param.reference)}' extSubReference='${ctp:escapeJavascript(param.subReference)}'" 
							+"/></label>";
	}else{
		innerHTMLStr = "<label title='"+escapeStringToHTML(attachment.filename)+"'  ><input name='id' onclick='isCheckAll()' type='checkbox' id='"+attachment.id+"' reference='"+attachment.reference+"' subReference='"+attachment.subReference+"' category='"+attachment.category+"' "
							...
							+"size='"+attachment.size+"' fileUrl='"+attachment.fileUrl+"' description='"+ escapeStringToHTML(attachment.description) +"' needClone='"+attachment.needClone+"'"
							+" extension='"+attachment.extension+"' icon='"+attachment.icon+"' v='"+attachment.v+"' extReference='${ctp:escapeJavascript(param.reference)}' extSubReference='${ctp:escapeJavascript(param.subReference)}'"
							+"/></label>";
	}
...
function delAttachment(){
	...
	for(var i = 0 ; i < delIds.length ;i++){
		if(delIds[i] && delIds[i].checked == true){
			...
			delIds1.add(delIds[i].getAttribute("fileUrl"));
		}
	}
...
function OK(){
	...
	return [attList,logList,delIds1];
}
...
function editOfficeOnline(id){
	...
	var url = _ctxServer+"/genericController.do?ViewPage=ctp/common/fileUpload/officeEdit&id="+id+"&filename="+encodeURIComponent(filename)+"&content="+fileUrl+"&bodyType="+bodyType+"&createDate="+createDate+"&category="+category+"&_isModalDialog=true&isSender=${param.isSender}";
...
function refreshAtt(att){
	...
	//修改刷新时，需要重新生成v
	if("${param.isSender!='' && param.isSender=='sender'}"){//在已办列表修改附件后,点击确定后即时生效后重新生成下载串
		//重新生成v 此参数绑定安全校验，用于下载
		var cManaager=new colManager();
		var type=att.getAttribute("typeAttach");
		if(isEdocAtt&&isEdocAtt=="true"){
			type=att.getAttribute("attType");
		}
		var vStr=cManaager.getAttDownloadV(type,att.getAttribute("fileUrl"),att.getAttribute("description"),att.getAttribute("reference"));
		if(vStr!=''){
			att.setAttribute("v",vStr);
		}
	}
...
<div class="stadic_layout">
		<div class="stadic_layout_head stadic_head_height page_color line_height180 font_size12">
			<div class="clearfix">
				<div class="left font_bold padding_l_10">${ctp:i18n('fileUpdate.page.title')}</div>
				<div class="right"><a onclick="showAbout()">${ctp:i18n('fileUpdate.page.about')}</a>&nbsp;&nbsp;</div>
			</div>
		...
		<div class="stadic_layout_body stadic_body_top_bottom">
			<TABLE class="only_table edit_table" border=0 cellSpacing=0 cellPadding=0 width="100%">
				<THEAD>
					<TR>
						<TH width="5%" align="center" style="text-align:center;"><input id="checkAll" onclick="selectAll(this,'id')" name="importlevelcheck" type="checkbox"/></TH>
						<TH>
		...
		<c:if test="${param.isEdocAtt eq true }">
			<div class="stadic_layout_footer stadic_footer_height page_color align_right">
	            ...
	            <a id="btncancel" class="common_button common_button_gray margin_t_5" onclick="_closeWin(true)">${ctp:i18n('common.button.cancel.label')}</a><!-- 取消 -->
	        </div>
		</c:if>
	</div>
