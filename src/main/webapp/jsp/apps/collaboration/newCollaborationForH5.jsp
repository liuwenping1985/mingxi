...
<%@ include file="/WEB-INF/jsp/common/common4coll_forH5.jsp"%>
...
<head>
	...
	<script type="text/javascript" src="${path}/ajax.do?managerName=trackManager,formManager"></script>
	...
	<script type="text/javascript" charset="UTF-8" src="${path}/apps_res/collaboration/js/newCollaborationForH5.js${ctp:resSuffix()}"></script>
...
<form method="post" id="sendForm"  name="sendForm" action='<%=ctxServer%>/collaboration/collaboration.do?method=send&from=${param.from}&reqFrom=${param.reqFrom}' class="h100b">
    <div id='newColl_layout' class="comp" comp="type:'layout'">
 				    ...
 				    <table border="0" cellspacing="0" cellpadding="0" width="100%">
			            <tr>
				            <td rowspan="2" width="1%" nowrap="nowrap" class="padding_l_25">
				            	<a id='sendId' class="margin_r_10 display_inline-block align_center new_btn">${ctp:i18n('collaboration.newcoll.send')}</a>
				            </td>
                            ...
                            <c:choose>
							    ...
							    <input type="hidden" id="projectId" name="projectId" value="${vobj.projectId}">
				           	    ...
				           	    <c:otherwise>
				           	    <th width="1%" class='color_gray2 padding_l_25' nowrap="nowrap" >
        ...
        src="${path}/content/content.do?method=index&isFullPage=true&isNew=${zwIsnew}&moduleId=${zwModuleId}&moduleType=1&rightId=${zwRightId}&contentType=${zwContentType}&&originalNeedClone=true&transOfficeId=${transOfficeId}<c:if test='${"" ne zwViewState}'>&viewState=${zwViewState}</c:if>"></iframe>
        </div>
