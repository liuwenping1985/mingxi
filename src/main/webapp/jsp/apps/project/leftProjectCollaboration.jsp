 	...
 	<ul>
	<c:forEach var="projectAffair" varStatus="status" items="${leftAffairList}">
		<c:choose> 
			...
			<c:if test="${projectAffair.senderId ne null}">
				<span name="senderId" class="left flagClass text_overflow" title="${ctp:showMemberNameOnly(projectAffair.senderId)}" style="padding-left:5px;color: #111111">
					<a onclick="displayMemberInfo(this)" value="${projectAffair.senderId}">${ctp:showMemberName(projectAffair.senderId)}</a>
				</span>
			</c:if>
