	...
	$.ajax({
		   ...
		   success: function(msg){
			...
			if (jsonbean.s2sIsRuning == "0") { 
				$.alert("OA服务器S2S端口工作不正常(建议修改A8 S2S端口，重启A8后再试)，原因可能为：<br/> 1.A8S2S端口被其他程序占用!<br/>2.webapps下存在两个seeyon目录或者存在seeyon备份文件!");
			}
...
function setValueByQuery1(jsonbean){
    ...
    //m1 是否启用
    if(jsonbean.hashM1Plug == "1" && ${ctp:hasPlugin('m1')} && !${v3x:getSysFlagByName("m1_notShow")}) { 
        $("#inputM1Host").show();
        $("#inputM1Port").show();
        $("#showM1Host").show();
        $("#showM1Port").show();
        ...
        $("#seeyonM1ServerIpI").val(jsonbean.m1ServerIp);
...
function firstModul (jsonbean) { 
	...
	function loadConfig1(){
		$('#config0').hide();
		$('#config10').hide();
		if (${v3x:getSysFlagByName("m1_notShow")}) {
			$('#seeyonM1Relation').hide();
		}
		$('#config1').show();
	}
	...
	function nextConfig(flag){
		if(flag == '1'){
			...
			if($("#deploymentModeV").val() == '1'){//选择分离回填上次的ip值
				...
				$("#seeyonnServerIp").attr("disabled","disabled");
				$("#ucServerIp").attr("disabled","disabled");
				if($("#isM1WithUCV").val()!=1 && ${ctp:hasPlugin('m1')} && !${v3x:getSysFlagByName("m1_notShow")}){//kygz 如果m1不与UC OA同在一服务器上 回填或
					var m1ServerIp = chcheBean.m1ServerIp;
               		if ("1" != $("#seeyonM1ServerIp").attr("blurNum") || $("#seeyonM1ServerIp").val() == "127.0.0.1") { 
                	    $("#seeyonM1ServerIp").val(m1ServerIp);
               		}
					...
					$("#seeyonM1ServerIp").val("127.0.0.1");
            ...
            if (hashM1Plug == "1"　&& !${v3x:getSysFlagByName("m1_notShow")}) { 
    			if (seeyonM1ServerIp == "") { 
    				$.alert("M1服务器地址不能为空");
    				return ;
    			}
    			if($("#isM1WithUCV").val()!=1){//kygz 如果m1不与UC OA同在一服务器上
    				if(seeyonM1ServerIp=="127.0.0.1"){
						$.alert("M1服务器地址不能为127.0.0.1，请填正确的IP");
                   		return ;
    				}
    				if(seeyonnServerIp=="127.0.0.1"){
						$.alert("OA服务器地址不能为127.0.0.1，请填正确的IP");
                   		return ;
    				}
                ...
                if(!checkMAX($("#seeyonM1ServerPort").val())){
                    $.alert("M1服务器S2S端口请输入1~65535之间的数字");
                    return;
                }
            }
			//如果选择了分离
			if($("input[name='deploymentMode']:checked").val() == '1'){
				...
				if (hashM1Plug == "1" && !${v3x:getSysFlagByName("m1_notShow")}) { 
					if($("#isM1WithUCV").val()!=1){//kygz 如果m1不与UC OA同在一服务器上
    					if(seeyonM1ServerIp=="127.0.0.1"){
							$.alert("M1服务器地址不能为127.0.0.1，请填内网ip");
                   			return ;
    					}
    				}
	...
	function tipReStartOASever() {
		var random = $.messageBox({
			'type': 0,
			'msg': '保存配置信息成功后必须重启协同服务和致信服务方可正常使用,否则将导致致信无法正常工作！',
			'imgType' : 2,
			'ok_fn' : function () {
			},
			'close_fn' : function () {
	...
	<div class="scrollList" style="height: 100%">
			...
			<!--第一页配置 开始-->
				<div class="ConfigPage" id="config0" style="display:none">
						...
						<div class="ConContent mb8">
							<div class="ConForm1">
								...
								<dl style="margin-left: -5em;">
									...
									<dd><span id="synTime" ></span>&nbsp;&nbsp;<input type="button" id="syn" class="button-default_emphasize" value="${ctp:i18n('uc.config.title.synchronization.button.js') }" onclick="ucsyn()"/></dd>
								</dl>
							</div>
						</div>
						...
						<div class="ConContent" id="ConContentD">
							<div class="ConForm1">								
								...
								<dl style="margin-left:-5em;" id="showM1Port">
									<dt><label>M1服务器S2S端口(离线消息推送)：</label></dt>
					...
					<!--上部内容 结束-->
					<!--底部黑色区域 开始-->
					<div id ="config00" class="ConFooter"><button type="button" id="config00_update" class="button-default_emphasize" style="margin-top:-1.5px" onclick="loadConfig1();">修改配置</button></div>
					<!--底部黑色区域 结束-->
				</div>
				<!--第一页配置 结束-->
			 ...
			 <div class="ConfigPage" id="config1" style="display:none">
					<div class="ConTopArea">
						...
						<!-- kygz 增加M1与致信的关系判断 -->
							<div class="ConTitle1 mb8">M1服务的布署方式:</div>
							<div class="ConContent">
								<div class="ConForm2">
									<dl>
										<dt>
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('1');">下一步</button>&nbsp;&nbsp;<button type="button" class="button-default-2 button_class" onclick="cancelConfig('1');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
					...
					<div class="ConTopArea">
						...
						<div class="ConContent mb8">
							...
							<div class="redtip">
								...
								<br/>3：如果选择了“分离部署”，那么两个服务器的IP地址必须要能够相互访问
								<br/>4：如果选择了“分离部署”，协同服务器上如果启用了单机防火墙，请确认协同服务器的S2S端口是否开放
								<br/>5：如果选择了”分离部署“，致信服务器上如果启动了单机防火墙，请确认40011和致信服务器的S2S端口是否开放
								</p>
							</div>
						</div>
					</div>
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('2');">上一步</button>&nbsp;&nbsp;<button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('2');">下一步</button>&nbsp;&nbsp;<button type="button" class=" button_class button-default-2" onclick="cancelConfig('2');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			  ...
			  <div class="ConfigPage" id="config3" style="display:none;">
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('3');">上一步</button>&nbsp;&nbsp;<button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('3');">下一步</button>&nbsp;&nbsp;<button class="button_class button-default-2" type="button" onclick="cancelConfig('3');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			  ...
			  <div class="ConfigPage" id="config4" style="display:none">
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('4');">上一步</button>&nbsp;&nbsp;<button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('4');">下一步</button>&nbsp;&nbsp;<button class="button-default-2 button_class" type="button" onclick="cancelConfig('4');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			  ...
			  <div class="ConfigPage" id="config5" style="display:none">
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('5');">上一步</button>&nbsp;&nbsp;<button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('5');">下一步</button>&nbsp;&nbsp;<button class="button-default-2 button_class" type="button" onclick="cancelConfig('5');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			  ...
			  <div class="ConfigPage" id="config6" style="display:none">
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('6');">上一步</button>&nbsp;&nbsp;<button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('6');">下一步</button>&nbsp;&nbsp;<button type="button" class="button-default-2 button_class" onclick="cancelConfig('6');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			  ...
			  <div class="ConfigPage" id="config7" style="display:none">
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('6');">上一步</button>&nbsp;&nbsp;<button type="button"  class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('7');">下一步</button>&nbsp;&nbsp;<button  class="button-default-2" type="button" onclick="cancelConfig('7');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			  ...
			  <div class="ConfigPage" id="config8" style="display:none;">
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('8');">上一步</button>&nbsp;&nbsp;<button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('8');">下一步</button>&nbsp;&nbsp;<button class="button_class button-default-2" type="button" onclick="cancelConfig('8');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			  <div class="ConfigPage" id="config9" style="display:none">
					...
					<div class="ConFooter"><button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="backConfig('9');">上一步</button> &nbsp;&nbsp;<button type="button" class="button-default_emphasize" style="margin-top:-7px" onclick="nextConfig('9');">保存设置</button>&nbsp;&nbsp;<button class="button-default-2" type="button" onclick="cancelConfig('9');">取&nbsp;&nbsp;&nbsp;消</button></div>
				</div>
			</td></tr>
			</table>
	 </div>
