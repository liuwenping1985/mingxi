...
<head>
	...
	<script type="text/javascript">
			...
			if($.ctx.hotSpots){
			  for(var i = 0; i < $.ctx.hotSpots.length; i++){
		        if($.ctx.hotSpots[i].hotspotkey == "note"){
		          ...
		          if(picHotSpot.hotspotvalue != "default-A6s/images/main_content_ie6.jpg" && picHotSpot.hotspotvalue != "default-A6s/images/main_content_ie6.png") {
	                $(".showArea").hide();
	                $(".logoArea").hide();
	              }
		        } else if($.ctx.hotSpots[i].hotspotkey == "mainbgc") {
		          colorHotSpot = $.ctx.hotSpots[i];
	              if(colorHotSpot.hotspotvalue != ""){
	              ...
	              //if(colorHotSpot && colorHotSpot.hotspotvalue == ""){
	                picHotSpot = $.ctx.hotSpots[i];
	                if(picHotSpot && picHotSpot.hotspotvalue && picHotSpot.hotspotvalue != ""){
		            	  ischangeBgImg = true;
		              }else{
		...
		function showArea(){
			...
			} else if ($(window).width() >= 1550 && $(window).width() < 1680) {
				$(".showArea").css("left", "28%");
				$(".loginArea").css("right", "15%");
			}else if ($(window).width() >= 1680){
				$(".showArea").css("left", "22%");
				$(".loginArea").css("right", "15%");
			} else {
				$(".showArea").css("left", "");
				$(".loginArea").css("right", "");
	...
	<link rel="stylesheet" type="text/css" href="${path}/main/login/default-A6s/css/login.css${ctp:resSuffix()}" />
</head>
					...
					<div id="loginArea" class="loginArea">
						<div class="loginForm_area">
							...
							<div class="qrCode_area">
								<div id="qrcode1"><div id="qrcode" class="qrcode"></div></div>
								<div><a id="flashQrcode">${ctp:i18n("portal.login.flashQrcode")}</a><span class="padding_lr_10">|</span><a href="http://weixin.seeyon.com/help.jsp">${ctp:i18n("portal.login.wechat.help")}</a></div>
							</div>
						</div>
