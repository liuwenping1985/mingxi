...
<script type="text/javascript">
	...
	function openDetails(url) {
		getA8Top().openReplateMoreWin = getA8Top().$.dialog({
            title:" ",
				...
				<form action="">
					<v3x:table htmlId="pending" data="senderList" var="col" className="sort ellipsis">
						<v3x:column width="60%" type="String" className="cursor-hand1 sort" label="common.subject.label" hasAttachments="${col.hasAttsFlag}" importantLevel="${col.importantLevel}" bodyType="${col.bodyType}" flowState="${col.finish ? 3 : 0}">
							<a class="title-more" href="javascript:openDetails('${col.subjectUrl}')" title="${v3x:toHTML(col.subject)}">${v3x:toHTML(v3x:getLimitLengthString(col.subject, 800, '...'))}</a>
						...
						<c:if test="${hasNewColl}">
							<v3x:column align="left" type="String" width="10%" className="cursor-hand1 sort" label="common.toolbar.transmit.collaboration.label">
								<c:if test="${col.app==1}">
