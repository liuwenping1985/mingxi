...
<c:set var="_sscounter" value="1"></c:set>
		<div class="processing_view" style="padding:1px 0;">
            ...
            ${ctp:showMemberNameOnly(comment.createId)}
        </c:when>
        <c:otherwise><a href="javascript:void(0)" class="showPeopleCard" style="font-size: 14px;" onclick="$.PeopleCard({memberId:'${comment.createId}'})" id="commentSearchCreate${_sscounter}"> ${ctp:showMemberNameOnly(comment.createId)}</a></c:otherwise></c:choose>
			            ...
			            <ul class="clearfix">
			                <li class="content_in font_size12" style="word-break:normal">${comment.escapedContent }</li>
			                ...
			                <li id="replyContent_${comment.id}" class="color_gray ${fn:length(comment.children) > 0?'':'display_none'}">
			                    ...
			                    <c:forEach items="${comment.children}" var="childComment">
			                        <div class="comments_title_in">
			                            <span class="title">
			                            	<a href="javascript:void(0)" class="showPeopleCard" onclick="$.PeopleCard({memberId:'${childComment.createId}'})" id="commentSearchCreate${_sscounter}">${ctp:showMemberNameOnly(childComment.createId)}</a>
			                            </span>
			                        ...
			                        <div class="comments_content font_size12"  style="color:#111;">${childComment.escapedContent}
			                            <c:if test="${!empty childComment.relateInfo}">
		                                                ...
		                                                <td nowrap="nowrap" width="100%">
		                                                      <span class="bt" style="float:right;vertical-align:bottom;padding-right:30px;">
		                                                          ...
		                                                          <span>
		                                                            <label class="margin_r_10 hand">
		                                                              <input id="hidden" class="radio_com" name="hidden" value="true" type="checkbox">${ctp:i18n('collaboration.opinion.hidden.label')}
		                                                               <input type="hidden" id="showToId" name="showToId" value="Member|${comment.createId},Member|${createUserId}" />
		                                                            </label>
		                                                          </span>
		                                                          <span>
		                                                         ...
		                                                         <div style="display:none" class="common_txtbox common_txtbox_dis clearfix">
		                                                            <input id="reply_pushMessage_${comment.id}" value="${ctp:showMemberNameOnly(comment.createId)}" type="text" onclick="pushMessageToMembers($(this),$('#reply_${comment.id} #pushMessageToMembers'),null,'${comment.createId}')">
		                                                          </div>
		                                                             </span>   
		                                                      </span>
		                                                </td>
				 ...
				 <div class="content">
				 	<c:forEach var="summary" items="${summaryList}" varStatus="status">
				 	 	<h3 class="per_title">
			                ...
			                <c:if test="${canSummaryReply == true}">
			                	<span onclick="summaryCommentShowReply('${summary.id}');" class="add_new font_size12"><a href="javascript:void(0)" class="color_blue">${ctp:i18n('plan.summary.tab.reply')}</a></span>
			                </c:if>
			            </h3>
			            <ul>
			            	<li class="content_in font_size12" style="word-break:normal">${ctp:toHTMLWithoutSpace(summary.summaryText)}</li>
  			             	<li>
			                    <span class="font_bold font_size12 left">${ctp:i18n('common.attachment.label')}：</span>
			            ...
			            <input type="hidden" id="mcp_${summary.id}" value="${fn:length(summaryCommentMap[summary.id]) + 1}">
						<input type="hidden" id="cp_${summary.id}" value="pc">
						<ul>
							<%--评论列表区 --%>
							<li id="replyContent_${summary.id}" class="color_gray ${fn:length(summaryCommentMap[summary.id]) > 0?'':'display_none'}">
								<h4>${ctp:i18n('collaboration.opinion.replyOpinion')}</h4>
								<c:forEach var="childComment" items="${summaryCommentMap[summary.id]}">
									<div class="comments_title_in">
										<span class="title"><a href="javascript:void(0)" class="showPeopleCard" onclick="$.PeopleCard({memberId:'${childComment.createId}'})" id="commentSearchCreate${_sscounter}">${ctp:showMemberNameOnly(childComment.createId)}</a></span>
										<span class="add_new font_size12">${childComment.createDateStr}</span>
									</div>
									<div class="comments_content font_size12"  style="color:#111;">${childComment.escapedContent}
										<c:if test="${!empty childComment.relateInfo && childComment.hasRelateAttach && childComment.canView}">
											<div class="affix_content">
												<span class="font_bold font_size12 left">${ctp:i18n('common.attachment.label')}：</span>
												<span><div style="display:none;" class="comp" comp="type:'fileupload',applicationCategory:'${childComment.moduleType}',attachmentTrId:'${childComment.id}',canDeleteOriginalAtts:false,canFavourite:false" attsdata='${childComment.relateInfo}'></div></span>
											</div>
										</c:if>
									</div>
								</c:forEach>
							...
							<%--评论回复区 --%>
							<%--由于附件上传组件在页面中只支持一个实例，顾采用javascript模板方式，每次打开的时候再渲染出组件，关闭的时候清空HTML内容--%>
							<script type="text/html" id="tpl_${summary.id}">
								<input type="hidden" id="pid" value="0">
								<input type="hidden" id="clevel" value="0">
								<input type="hidden" id="path">
								<input type="hidden" id="moduleType" value="5">
								<input type="hidden" id="moduleId" value="${summary.id}">
								<input type="hidden" id="ctype" value="1">
								<input type="hidden" id="affairId" value="${contentContext.affairId}">
								<div class="common_txtbox clearfix">
									<textarea id="content" name="content" class="validate" validate="notNull:false,maxLength:500" cols="20" rows="5"></textarea>
								</div>
								<input type="hidden" id="relateInfo" name="relateInfo" />
								<div class="chearfix">
									<table style="border:0;width: 100%">
										<tbody>
										  <tr>
											<td nowrap="nowrap" width="90%">
											  <table style="border:0">
												<tbody>
												  <tr>
													<td nowrap="nowrap"><span class="left green">${ctp:i18n('collaboration.sender.postscript.lengthRange')}</span> </td>
													<td nowrap="nowrap">
														<div class="scrollList margin_tb_5">
															<a href="javascript:void(0)" onclick="insertAttachment()" class=""><em class="ico16 affix_16"></em>${ctp:i18n('plan.summary.tab.uploadatt')}</a>
														</div>
														<div id="attachmentTR" style="display: none;">
															<div class="comp" comp="type:'fileupload',applicationCategory:'1',canDeleteOriginalAtts:false,originalAttsNeedClone:false,canFavourite:false"attsdata=''></div>
														</div>
													</td>
													<td nowrap="nowrap" width="100%">
														<span class="bt" style="float:right;vertical-align:bottom;padding-right:30px;">
														  <span><label class="margin_r_10 hand"><input id="hidden" class="radio_com" name="hidden" value="true" type="checkbox">${ctp:i18n('collaboration.opinion.hidden.label')}<input type="hidden" id="showToId" name="showToId" value="Member|${summary.refUserId},Member|${createUserId}" /></label></span><span>
														  <a class="common_button common_button_gray margin_r_5" onclick="summaryCommentReply(this, '${summary.id}');" href="javascript:void(0)" style="margin-bottom: 7px;">${ctp:i18n('collaboration.sender.postscript.submit')}</a> 
														  <a onclick="commentHideReply('${summary.id}')" class="common_button common_button_gray" href="javascript:void(0)"  style="margin-bottom: 7px;">${ctp:i18n('collaboration.sender.postscript.cancel')}</a>
														  <div style="display:none" class="common_txtbox common_txtbox_dis clearfix"><input id="reply_pushMessage_${summary.id}" value="${ctp:showMemberNameOnly(summary.refUserId)}" type="text" onclick="pushMessageToMembers($(this),$('#reply_${summary.id} #pushMessageToMembers'),null,'${summary.refUserId}')"></div></span>   
														</span>
													</td>
												  </tr>
											  </table>
							...
							<%--此处调用summaryCommentShowReply将script模板填充到里面，关闭时候调用summaryCommentHideReply清空里面内容 --%>
							<li id="reply_${summary.id}" class="form_area display_none clearFlow"></li>
						</ul>
				 	</c:forEach>
				 </div>
			</li>
...
<script type="text/javascript">
  ...
  <%--
  	6.0SP1需求：计划总结后可以对计划的总结做回复
  --%>
  function summaryCommentReply(that, rid) {
	  /* 确定按钮设置为灰色 */
	  $(that).addClass("common_button_disable");
	  var dm = $("#reply_" + rid), 
	  	  mcp = $("#mcp_" + rid).val()+'', 
	  	  cp = $("#cp_" + rid).val();
	  var pt = cp;
	  ...
	  var attachmentObj = $("#attachmentTR").formobj();
	  if (attachmentObj.length != 0) {
		  obj.relateInfo = $.toJSON(attachmentObj); //保存附件信息  
	  }
	  ...
	  if(attachmentObj.length == 0 && (typeof(content) == "undefined" || content.length == 0)){
		  <%--暂时不加此提示--%>
		  $.alert("${ctp:i18n('collaboration.common.deafult.commonNotNull')}");
		  
		  unDisablebutton(rid);
		  return;
	  }
	  obj.pushMessage = true;
	  var mgr = new planManager();
	  mgr.savePlanSummaryReply(obj,{success : function(ret) {
	            	if(typeof ret != "object"){
	            		 ret = $.parseJSON(ret);
	            	}
	            	var html = '<div class="comments_title_in">';
	            	html += '<span class="title"><a href="javascript:void(0)" class="showPeopleCard" onclick="$.PeopleCard({memberId:\'${CurrentUser.id}\'})" id="commentSearchCreate'+ ret.id +'">${ctp:showMemberNameOnly(CurrentUser.id)}</a></span>';
	            	html += '<span class="add_new font_size12">'+ ret.createDateStr +'</span>';
	            	html += '</div>'
	            	html += '<div id=att'+ ret.id +' class="comments_content font_size12" style="color:#111;">' + ret.escapedContent;
	            	if (ret.relateInfo) {
	            		html += '<div class="affix_content">';
	            		html += '<span class="font_bold font_size12 left">${ctp:i18n("common.attachment.label")}：</span>';
	            		html += '<span>'
	            		html += '<div style="display:none;" class="comp" comp="type:\'fileupload\',applicationCategory:\'' + ret.moduleType + '\',attachmentTrId:\''+ ret.id +'\',canDeleteOriginalAtts:false,canFavourite:false"></div>';
	            		html += '</span>';
	            	}
	            	html += '</div>';
	            	$("#replyContent_" + rid).removeClass("display_none").append(html);
	            	if (ret.relateInfo) {
	            		$("#att"+ret.id + ' .comp', $("#replyContent_" + rid)).attr("attsdata", ret.relateInfo).compThis();
	            	}
	                $("#content", dm).val('');
	                summaryCommentHideReply(rid);
	              }
	  });
  }
  <%--打开总结评论窗口 --%>
  function summaryCommentShowReply(rid) {
	  var $reply = $("#reply_" + rid), tpl = $("#tpl_" + rid).html();
	  $reply.html(tpl);
	  $reply.show();
	  $(".comp", $reply).compThis(); //初始化附件上传组件
	  $("#content", $reply).focus();
  }
  <%-- 关闭总结评论窗口 --%>
  function summaryCommentHideReply(rid) {
	  $("#reply_" + rid).hide().html('');
    ...
    mgr.savePlanReply(
            obj,
            {
              success : function(ret) {
                ...
                $("#replyContent_" + rid).append('<div class="comments_title_in"> <span class="title"><a href="javascript:void(0)"  class="showPeopleCard" onclick="$.PeopleCard({memberId:\'${CurrentUser.id}\'})">${CurrentUser.name}</a></span><span class="add_new font_size12"></span></div>');
                $("#replyContent_" + rid).find(".add_new").text(ret.createDateStr);
