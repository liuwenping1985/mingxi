...
function checks(){
	//fb---上传附件进行提示---Start
	var attachmentTrId = '${param.attachmentTrId}';
	var mod = window.parent.location.pathname.indexOf("collaboration");
	//附件控件默认会传打开空间的window
	if(typeof(transParams)!='undefined'){
		if(-1==mod&&transParams.parentWin){
			mod = transParams.parentWin.location.pathname.indexOf("collaboration");
		}
	}
	var cur = attachmentTrId !='biz' && attachmentTrId !='xsn'&& attachmentTrId !='bizMapImage';
	if($.ctx.plugins.contains('secret') && cur && attachmentTrId !='xsn1' && (mod>-1)){//协同、公文上传需要密级提醒,上传文单不提示
		 var confirm = $.confirm({
		     'msg': "${ctp:i18n('secret.upload.remind')}",
		     ok_fn: function () {
				 afterCheckOk();
		     }
		 });
...
function addAttachmentPoi(type, filename, mimeType, createDate, size, fileUrl, canDelete, needClone,extension,icon,poi, reference, category, onlineView, width, embedInput,hasSaved,isCanTransform,v,canFavourite, isShowImg){
  ...
  if( typeof(_parent)=='undefined' || _parent == null){
        if(getCtpTop().window.document.getElementById("zwIframe")){
            _parent = getCtpTop().window.document.getElementById("zwIframe").contentWindow;
        }else if(getCtpTop().window.document.getElementById("componentDiv")){
            if(getCtpTop().window.document.getElementById("componentDiv").contentWindow.document.getElementById("zwIframe")){
                _parent = getCtpTop().window.document.getElementById("componentDiv").contentWindow.document.getElementById("zwIframe").contentWindow;
            }
        }
  }
...
function callforward(fileurls,repeat){
  ...
  <c:if test="${not empty param.targetAction}" >
     ...
     <c:if test="${not empty param.callMethod}" >
         if("${ param.callMethod}" == "fileValueChangeCallBack"){
            if(getCtpTop().window.document.getElementById("zwIframe")){
                 _parent = getCtpTop().window.document.getElementById("zwIframe").contentWindow;
...
function onUpload(){
	...
	var isCol = pathname.indexOf("collaboration")>-1;
	if(!isCol){
		isCol = transParams.parentWin.location.pathname.indexOf("collaboration")>-1;
	}
	var cur = attachmentTrId !='biz' && attachmentTrId !='xsn'&& attachmentTrId !='bizMapImage';
	if($.ctx.plugins.contains('secret') && cur && attachmentTrId !='xsn1' && isCol){//协同、公文上传需要密级提醒,上传文单不提示
		 var confirm = $.confirm({
		     'msg': "${ctp:i18n('secret.upload.remind')}",
		     ok_fn: function () {
				 curCheckOk();
...
<body  onkeydown="listenerKeyPress()" style="height: 100%;overflow: hidden" class="page_color">
    <form style="height: 100%;display: block;" enctype="multipart/form-data" name="uploadForm" method="post" id="form_upload" action="${pageContext.request.contextPath}/fileUpload.do?method=processUpload"   target="uploadIframe">
        <input type="hidden"  id="type" name="type" value="<%=type%>"> <input type="hidden" name="extensions"
            id="extensions" value="<%=extensions%>"> <input type="hidden" name="applicationCategory"
            id="applicationCategory" value="<%=applicationCategory%>"> <input type="hidden"
            name="destDirectory" id="destDirectory" value="<%=destDirectory%>"> <input type="hidden"
            name="destFilename" id="destFilename" value="<%=destFilename%>"> <input type="hidden"
            name="maxSize" id="maxSize" value="<%=maxSize%>"> <input type="hidden" name="isEncrypt"
            id="isEncrypt" value="<%=isEncrypt%>"> <input type="hidden" id="form_action"
