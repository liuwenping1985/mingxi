...
<script type="text/javascript">
	...
	try{
		...
		//新建时校验重复
			...
			if(labelObj && existMetadataNames  && orgLabel.value != labelObj.value){
...
<c:set value="${(param.disabled=='true' || metadataItem.isRef==0 )? 'disabled':''}" var="setIsEnable"/>
...
<form name="editForm" method="post" action="${metadataMgrURL}" onsubmit="return (checkForm(this) && checkIsNameDuple())">
	<input type="hidden" name="method" value="updateUserDefinedItem">
	...
	<input type="hidden" name="orgItemLabel" value="<c:out value="${metadataItem.label}"/>">
	<input type="hidden" name="isRef" value="${metadataItem.isRef}">
			...
			<table  width="80%" height="60%" border="0" cellpadding="0"  cellspacing="0">
				<tr>
					...
					<td class="new-column" width="50%" nowrap="nowrap">
					 <c:choose>
					 <c:when test="${metadataItem.isRef == '0'}">
					  	<input type="text" name="label" class="input-100"  disabled="disabled" value="<c:out value="${metadataItem.label}"/>" maxSize="85" maxlength="85" inputName="<fmt:message key='metadata.manager.displayname'/>" validate="notNull,maxLength,isWord"  character="\\/|><*?'&%$" />
					 </c:when>
					 <c:otherwise>					 
					  	<input type="text" name="label" class="input-100" ${setIsEnable} value="<c:out value="${metadataItem.label}"/>" maxSize="85" maxlength="85" inputName="<fmt:message key='metadata.manager.displayname'/>" validate="notNull,maxLength,isWord"  character="\\/|><*?'&%$" />
					 </c:otherwise>			
					 </c:choose>
				...
				<tr>			
				 ...
				 <c:choose>
					 <c:when test="${metadataItem.isRef == '0'}">									
 						<input class="input-100" type="text" disabled="disabled" name="value" ${setIsEnable} maxlength="8" maxSize="8" value="${metadataItem==null? valueNumber:metadataItem.value}" inputName="<fmt:message key='metadata.manager.value'/>" validate="notNull,isNumber,maxLength,isInteger"  ${hasRefValue==true?"READONLY":""}/> 	
					 </c:when>
						 <c:otherwise>	
						<input class="input-100" type="text" name="value" ${setIsEnable} maxlength="100" maxSize="100" integerDigits="100" value="${metadataItem==null? valueNumber:metadataItem.value}" inputName="<fmt:message key='metadata.manager.value'/>" validate="notNull,isNumber,maxLength,isInteger"  ${hasRefValue==true?"READONLY":""}/> 	 					
						 </c:otherwise>			
					 </c:choose>					
					</td>
					<td  width="20%">&nbsp;</td>
				...
				<tr>
					...
					<td class="new-column" width="50%">
						...
						<input type="radio" name="inputSwitch" id="inputSwitch1" value="1" <c:if test="${metadataItem.state == '1'}">checked</c:if> onclick="fixStatus();"  <c:if test="${param.disabled == 'true' }" >disabled</c:if> >
				...
				<tr>
					...
					<td class="new-column" width="50%">
						...
						<input type="radio" name="outputSwitch" id="outputSwitch1" value="1" <c:if test="${metadataItem.outputSwitch == '1'}">checked</c:if> <c:if test="${param.disabled == 'true' }" >disabled</c:if>>
						<fmt:message key="common.state.normal.label" bundle="${v3xCommonI18N}"/>
						</label>
						<label for="outputSwitch2">
						<input type="radio" name="outputSwitch" id="outputSwitch2" value="0" <c:if test="${metadataItem.outputSwitch == '0'}">checked</c:if> <c:if test="${param.disabled == 'true' }" >disabled</c:if>>
						<fmt:message key="common.state.invalidation.label" bundle="${v3xCommonI18N}"/>
						</label>
						</td>
						</tr>
