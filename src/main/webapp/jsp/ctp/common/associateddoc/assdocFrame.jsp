...
if($.ctx.plugins.contains('secret')){
	if(typeof(transParams) !="undefined"){
		secretO = transParams.parentWin.document.getElementById("secretLevel");
	}
	if(secretO){
		secretLevel = secretO.value == ""?'-1':secretO.value;
	}
}
//fb---获取关联信息的密级---End
...
for(String key: assBeanMap.keySet()){
    %>
    appurlMap.put("<%= key%>","<%= assBeanMap.get(key).getUrl()%>&referenceId=${param.referenceId}&secretLevel="+secretLevel);
    <%
}
%>
...
<c:set var="canCol"  value="<%= AppContext.getCurrentUser().hasResourceCode(\"F01_listDone\") %>" />
<%--a6s去掉公文模块--%>
<c:set var="canOffice"  value="${ctp:getSystemProperty('system.ProductId') eq '7'}?false:<%= AppContext.getCurrentUser().hasResourceCode(\"F07_sendDone\") %>" />
<c:choose>
	<c:when test="${ctp:getSystemProperty('system.ProductId') eq '7'}">
		<c:set var="canOffice"  value="false" />
	</c:when>
	<c:otherwise>
		<c:set var="canOffice"  value="<%= AppContext.getCurrentUser().hasResourceCode(\"F07_sendDone\") %>" />
	</c:otherwise>
</c:choose>
...
<%-- <c:set var="canDoc"  value="true" />--%> <%-- 因A6-S 版的资源权限不能确认，暂时设置成不判断 --%>

<c:set var="canMeeting"  value="<%= AppContext.getCurrentUser().hasResourceCode(\"F09_meetingPending\") %>" />

    <div id='layout' class="comp" comp="type:'layout'">
        <div class="layout_north" layout="sprit:false,border:false,height:72,spiretBar:{show:false}">
            <div class="popup_title"><fmt:message key='common.toolbar.insert.label' bundle='${v3xCommonI18N}' /> - <fmt:message key='common.toolbar.insert.mydocument.label' bundle='${v3xCommonI18N}' /></div>
            <div class="common_tabs clearfix padding_l_10">
                <ul class="left">
                    ...
                    <c:forEach var="appid" items='${fn:split(allAssModules,",")}'>
                        <c:if test="${ appid ne 'rm' && assMap[appid] != null }">
                            <c:if test="${ (fn:trim(appid) eq '1' && canCol) || (fn:trim(appid) eq '4' && canOffice) || (fn:trim(appid) eq '3' && canDoc) || (fn:trim(appid) eq '6' && canMeeting)  }">
        ...
        <div class="layout_south over_hidden" layout="sprit:false,border:false,height:120,maxHeight:120,minHeight:120,spiretBar:{show:false}">
               <table width="100%" height="110" border="0" cellspacing="0" cellpadding="0" class="border_t">
                ...
                <c:when test="${from == 'formTable'}">
                        <td height="40" align="right" class="bg-advance-bottom">
...
/**
 * 页面加载方法放在最后，目的是需要获取${defaulturl.url}
 */
function cruuentAddLoad(){
    $(".common_tabs li:first").addClass('current');
    
    //页面加载完成后再进行iframe加载
    quoteDocumentFrame.location.href = "${pageContext.request.contextPath}${defaulturl.url}&secretLevel="+secretLevel;
}
