...
<%@page import="com.seeyon.ctp.common.constants.ProductEditionEnum"%>
<%@page import="com.seeyon.ctp.common.constants.ProductVersionEnum"%>
...
writer.println("</head>");
writer.println("<body>");
writer.println("<div class='comp'"+" comp="+"type:'breadcrumb',code:'T01_systemview'"+"></div>");
...
writer.print("<h4>" + ResourceUtil.getString(ProductEditionEnum.getCurrentProductEditionEnum().getName()) + " " + ProductVersionEnum.getCurrentVersion().getCanonicalVersion() + " (B" + Datetimes.format(SystemEnvironment.getProductBuildDate(), "yyMMdd") + "." + SystemEnvironment.getProductBuildVersion() + ".CTP" + SystemEnvironment.getCtpProductBuildVersion() + ")</h4>");
writer.println("<pre>");
writer.print("Online Users: " + OnlineRecorder.getOnlineUserNumber4ALL());
writer.print("; Peak Online Users: " + OnlineRecorder.getPeakOnlineUserNumber4ALL() + " (" + Datetimes.formatDatetime(new Date(OnlineRecorder.getPeakOnlineUserTimestamp4ALL())) + ")");
writer.println("");

writer.print("PC Users: " + OnlineRecorder.getOnlineUserNumber4Server());
writer.print("; Peak PC Users: " + OnlineRecorder.getPeakOnlineUserNumber4Server() + " (" + Datetimes.formatDatetime(new Date(OnlineRecorder.getPeakOnlineUserTimestamp4Server())) + ")");
writer.println("");

writer.print(MxVersion + " Users: " + OnlineRecorder.getOnlineUserNumber4M1());
writer.print("; Peak " + MxVersion + " Users: " + OnlineRecorder.getPeakOnlineUserNumber4M1() + " (" + Datetimes.formatDatetime(new Date(OnlineRecorder.getPeakOnlineUserTimestamp4M1())) + ")");
writer.println("");
...
try{
    ObjectName sessionON = new ObjectName("Catalina:type=Manager,context=/seeyon,host=localhost");
    ObjectInstance sessionMB = mBeanServer.getObjectInstance(sessionON);

    writer.println("<h4>Session</h4>");
    writer.println("<pre>");
    writer.println("Active: " + mBeanServer.getAttribute(sessionON, "activeSessions"));
    writer.println("</pre>");
}
catch (Exception e) {
    e.printStackTrace(writer);
...
for(DataSourceConProvider entry : map){
	writer.println("<h5>" + entry.getName() + "</h5>");
...
%>
<%
	com.seeyon.ctp.cluster.ClusterConfigBean ccb = com.seeyon.ctp.cluster.ClusterConfigBean.getInstance();
	if(ccb.isClusterEnabled()){
		writer.println("<h4>Cluster</h4>");
		writer.println("<pre>");
		writer.print(" Node Index : " + ccb.getClusterHostIndex());
		writer.print(" MainNode : " + ccb.isClusterMain());
		writer.print(" Name : " + ccb.getClusterName());		
		writer.print(" Hosts : " + ccb.getHosts());
		writer.println("</pre>");
	}
%>
...
writer.println("&nbsp;&nbsp;|&nbsp;&nbsp;");
writer.println("&nbsp;&nbsp;|&nbsp;&nbsp;");
writer.println("<a target='_blank' href='propertiesDump.do'>Properties Dump</a>");
writer.println("&nbsp;&nbsp;|&nbsp;&nbsp;");
writer.println("<a target='_blank' href='eventDump.do?action=resetRestPassword'>重置REST动态密码</a>");
writer.println("&nbsp;&nbsp;|&nbsp;&nbsp;");
writer.println("<a target='#' href='javascript:void(0)' onclick='LogUpload();'>日志一键上传</a>");
writer.println("&nbsp;&nbsp;|&nbsp;&nbsp;");
writer.println("<a target='_blank' href='log.do?method=showOperationPage'>运行态改变日志级别</a>");
writer.println("</div>");
...
writer.println("</html>");
%>
<div id="uploadLogDivContainer">
    <div id="uploadLogDiv" style="margin-left:100px;display: none;">
	<span style="color:red">Date format:yyyy-MM-dd,eg:2015-04-03</span><br/><br/>
    		    ...
    		    buttons : [ {
    		      ...
    		      handler : function() {
    		    	  var logDateStr=$("#logDate").val();
    			         ...
    			         var url="${path}/sensor.do?method=checkLog&logDateStr=" + logDateStr +"&random=" + random;
    			         $.ajax({
    			        	 url:url,
    			        	 dataType:"html",
    			        	 success:function(data)
    			        	 {
    			        		 if(trim(data)=="")
    			        			 {
    			        			    var uploadUrl="${path}/sensor.do?method=uploadLog&logDateStr=" + logDateStr + "&random=" + random;
    			        			    $.messageBox({
    			        			    	'type':1,
    			        			    	'imgType':'3',
