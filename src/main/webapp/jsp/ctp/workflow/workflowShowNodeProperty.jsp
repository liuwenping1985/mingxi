    ...
    <div class="form_area relative margin_l_10">
        <table border="0" cellspacing="0" cellpadding="0" width="100%" class="my_th">
            <tr>
                ...
                <td width="140" colspan="2" class="padding_l_5">${ctp:toHTML(nodeName)}</td>
            </tr>
                ...
                <td width="140" class="padding_l_5" colspan="2">
                    ...
                    <c:choose>
                        <c:when test="${matchScope eq '5'}"><%-- 发起者上级单位 --%>
                            ${ctp:i18n("node.matchScope.5")}
                        </c:when>
                        <c:when test="${matchScope eq '4'}"><%-- 表单控件决定 --%>
                            ${ctp:i18n("workflow.designer.node.matchScope.4.1")}
                        </c:when>
                        <c:when test="${matchScope eq '3'}"><%-- 发起者部门 --%>
                            ${ctp:i18n("node.matchScope.3")}
                        ...
                        <c:when test="${matchScope eq '2'}"><%-- 集团标准岗 --%>
                            <c:if test="${v3xSuffix eq '.GOV' }">${ctp:i18n("node.matchScope.2.GOV")}</c:if>
                            <c:if test="${v3xSuffix ne '.GOV' }">${ctp:i18n("node.matchScope.2")}</c:if>
                        </c:when>
                        <c:when test="${matchScope eq '1'}"><%-- 单位岗 --%>
                            <c:if test="${matchScopeSuffix eq '.A' }">${ctp:i18n("node.matchScope.1.A")}</c:if>
                            <c:if test="${matchScopeSuffix ne '.A' }">${ctp:i18n("node.matchScope.1")}</c:if>
                        </c:when>
                    </c:choose>
                </td>
            </tr></c:if>
            ...
            <tr>
                ...
                <c:if test="${na=='2' }">${ctp:i18n('workflow.designer.node.nouser.action.label2')}</c:if>
                </td>
            </tr>
            </c:if>
...
<fieldset class="margin_10"><%-- 表单绑定，只有display不为空的时候 --%>
    ...
    <div class="form_area relative margin_l_10">
        <table border="0" cellspacing="0" cellpadding="0" width="100%">
            ...
            <tr>
                <th style="line-height: 24px;" nowrap="nowrap" width="80"><label class="my_label"  for="text">${ctp:i18n('workflow.designer.node.bindq')}:</label></th>
                <td width="230" class="padding_l_5 toomore">
                <div class="toomore" title="${queryIdsText}">${queryIdsText}</div>
                </td>
            </tr>
            <tr>
                <th style="line-height: 24px;" nowrap="nowrap" width="80"><label class="my_label"  for="text">${ctp:i18n('workflow.designer.node.binds')}:</label></th>
                <td width="230" class="padding_l_5" >
                <div class="toomore" title="${statisticsIdsText}">${statisticsIdsText}</div>
                </td>
            </tr>
            <tr style="display: none;">
                <th style="line-height: 24px;" nowrap="nowrap" width="80"><label class="my_label"  for="text">${ctp:i18n('govdoc.content.show')}:</label></th>
                <td width="230" class="padding_l_5" >
               <!--  <div class="toomore" id="isShowContent"></div> -->
                </td>
            </tr>
         </table>
    </div>
