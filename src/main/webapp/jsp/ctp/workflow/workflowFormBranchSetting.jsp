...
$(function() {
    ...
    $("a.workflow_form_sammif").click(function(){
        ...
        var title = "重复表分类求和";
        switch(functionName){
        	case "sumif":{title="重复表分类合计";break;}
        	case "averif":{title="重复表分类平均";break;}
        	case "maxif":{title="重复表分类最大";break;}
        	case "minif":{title="重复表分类最小";break;}
        }
        //调用extend功能函数
    ...
    $("a.workflow_form_ymrw").click(function(){
        //没有选中的表单表单域，什么也不做
        var option = $("#sheetselectForm option:selected");
        if(option.size()<=0){$.alert("请选择主表的表单域！");return}
        var fieldName = option.val();
        var fieldObj = fieldMap[fieldName];
        //是重复项表单域，什么也不做
        if(fieldObj.ownerTableName.toLowerCase().indexOf('formson')>-1){$.alert("${ctp:i18n('workflow.formBranch.validate.5')}");return;}
...
function openWindowByType(fieldObj,formShortName){
    ...
    //只有主表字段才能直接显示在条件设置框里
    var tempInputType = fieldObj.inputType.toLowerCase(), tempFieldType = fieldObj.fieldType.toLowerCase();
    if((tempInputType=="text"||tempInputType=="lable"||tempInputType=="relation"
        ||tempInputType=="relationform")
...
<table width="100%" border="0" align="center">
    <tr>
        <td width="60%" valign="top">
            <select id="sheetselectForm" name="sheetselectForm" size="10" ondblclick="showFormfieldExpresion()"  style="width: 300px; height: 300px;" multiple="multiple" class="font_size12">
