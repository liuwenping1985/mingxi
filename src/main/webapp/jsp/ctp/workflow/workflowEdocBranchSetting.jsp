        ...
        <td width="60%" valign="top">
            <select id="edocFieldSelect" style="width: 300px; height: 260px;" multiple="multiple" class="font_size12" onclick="showFormfieldExpresion()">
                <c:forEach items="${fieldList}" var="field">
                    <option value="${field.name}">${field.name}<c:if test="${field.inputType=='select'}">(枚举类型)</c:if></option>
                </c:forEach>
            </select>
        </td>
                ...
                ,handler : function() {
                    ...
                    if(text!=null){
                        try{
                            ...
                            addToTextArea(getFormConditionTextArea(), fieldName + text);
                            dialog.close();
                            createCode.focus();
                            handInvokeTranslate();
                            createCode = null;
...
function openWindowByType(fieldObj,formShortName){
    ...
    //只有主表字段才能直接显示在条件设置框里
    if(fieldObj.fieldType.toLowerCase()=="decimal"){
        return formShortName+fieldObj.name;
    } else {
        return "";
    }
}
