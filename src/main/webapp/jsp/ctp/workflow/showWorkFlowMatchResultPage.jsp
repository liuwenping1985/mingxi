...
//fb 获取涉密信息的流程密级
var secretLevel;
var sl = parent.document.getElementById("secretLevel");
if(!sl && parent.main){
	var sl = parent.main.document.getElementById("secretLevel");
}
if(sl){
	secretLevel = sl.value;
}
...
function showWorkflowMatchResult(cpMatchResult){
  ...
  var allCanSeeTr = [];
  $.each(condtionMatchMap,function(key,valueObject){
    ...
    var conditionDesc= escapeSpecialChar(valueObject["conditionDesc"]+"", true);
      ...
      if(!(!hand && !matchResult)){
        if(hasBranch){
          ...
          row.find("#workflow_branch_matchresult").html("<textarea id=\"conditionTitle_"+toNodeId+"\" style=\"display:none\">"
              +conditionTitle+"</textarea><a  class=\"color_blue\"  id=\"conditionTitle_"+toNodeId+"_a\"  onmouseover=\"showConditionTitle('conditionTitle_"+toNodeId+"')\""
              +">"+matchResultName+"</a>"+(conditionDesc==""?'':'<br/><br/>')+'<font class="color_2b2b2b" style="line-height:16px;">'+conditionDesc+'</font>');
          /*row.find("#workflow_branch_matchresult").html("<textarea id=\"conditionDesc_"+toNodeId+"\" style=\"display:none\">"
              +conditionDesc+"</textarea><a id=\"conditionDesc_"+toNodeId+"_a\"  onmouseover=\"showConditionTitle('conditionTitle_"+toNodeId+"')\""
              +">"+matchResultName+"</a>");*/
        }else{
          row.find("#workflow_branch_tonodename").html(toNodeNameHtml);
          row.find("#workflow_branch_processmode").text("("+processModeName+")");
          ...
          if(processMode=="single"){//单人，下拉选择
            ...
            for(var i=0; i<pSize; i++){
              ...
              if(a_people!=null){
                  ...
                  //fb 获取用户的密级,根据流程密级进行显示---Start
                  if($.ctx.plugins.contains('secret')){
	                  var orggManager = new orgManager();
					  var member = orggManager.getMemberById(people_id);
					  var usecret = member.secretLevel;
					  if(usecret == null && secretLevel != null){
						  continue;
					   }
					  if(usecret != null && secretLevel != null && usecret < secretLevel){
						  continue;
					  }
                  }
            ...
            for(var i=0; i<pSize; i++){
              ...
              var depName = a_people["depName"];
              if(depName.length>15){
                depName = depName.substr(0,14)+"...";
              }
              var postName = a_people["postName"];
  ...
  if(hasSubProcess){
    ...
    $.each(subProcessMatchMap,function(key,valueObject){
      ...
      var conditionTitle= valueObject["triggerConditionTitle"]||"";
      ...
      row.find("#workflow_subprocess_name").html("<textarea id=subflowtitle"+index+" style=\"display:none\">"
              +conditionTitle+"</textarea>"+subProcessTempleteName+"<a  class=\"color_blue\"  id=\"subflowtitle"+index+"_a\"  onmouseover=\"showConditionTitle('subflowtitle"+index+"')\""
              +">           ["+triggerResultName+"]"+"</a>");
      ...
      if(null== peoples || peoples.length== 0){//弹出选人界面
        ...
        for(var i=0; i<pSize; i++){
          var a_people= peoples[i];
          //fb 获取用户的密级,根据流程密级进行显示
          if($.ctx.plugins.contains('secret') && typeof(people_id) != "undefined"){
	          var orggManager = new orgManager();
			  var member = orggManager.getMemberById(people_id);
			  var usecret = member.secretLevel;
			  if(usecret == null && secretLevel != null){
  ...
  */
   ...
   */
   function showConditionTitle(conditionDescId){
     ...
     msgText= (msgText==null||msgText==""||msgText=="null")?"${ctp:i18n('workflow.matchResult.msg5')}":msgText;
    ...
    */
     ...
     */
     function selectPeople(inputObj,singleOrMany, nodeId){
       ...
       var selectPanels= "Department,Team,Post,RelatePeople,Outworker";
