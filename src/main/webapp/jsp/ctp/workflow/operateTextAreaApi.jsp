...
function inFunction(fieldObj, formShortName, createCode){
    ...
    startPos = result.startPos;
    endPos = result.endPos;
    dialogUrl = "<c:url value='/workflow/designer.do?method=inFunc'/>&fieldName="+fieldObj.name+"&formApp=${formBeanId}";
    window.inDialog = $.dialog({
        url:dialogUrl,
//        title : 'in设置',
        title : '${ctp:i18n("workflow.formBranch.validate.27")}',
        width:500,
        height:300,
        targetWindow:getCtpTop(),
        transParams:window,
...
function extendFunction(fieldObj, formShortName, createCode){
    ...
    if(RealType_Multi[realType]){
        dialogUrl = "<c:url value='/workflow/designer.do?method=formulaExtendMulti'/>&fieldName="+fieldObj.name+"&formApp=${formBeanId}";
    }else{
        dialogUrl = "<c:url value='/workflow/designer.do?method=formulaExtend'/>&fieldName="+fieldObj.name+"&formApp=${formBeanId}";
    }
    window.extendDialog = $.dialog({
        url:dialogUrl,
        //title : 'extend设置',
...
function getNormalConditionValue(formShortName, fieldObj, result){
    ...
    if(result[1]!=null && $.trim(result[1])!="{}"){
    	...
    	temp.push(operator);
        temp.push("',");
        var tempField = getFieldName(formShortName, fieldObj.display);
        temp.push(tempField);
    	if(result[2]=="1"){
            temp.push(", '");
            temp.push(result[1]);
    	   ...
    	   //角色使用role表达式
            ...
            for(var i = 0; i < DEPT_ROLES.length; i++){
                if(DEPT_ROLES[i].toLocaleLowerCase() == result[1].toLocaleLowerCase()){
                    isSysDeptRole = true;
                    break;
                }
            } --%>
            var tempScript = "";
             <%--if(isSysDeptRole){ --%>
                tempScript = "'"+result[1]+"'," + result[5] + "," + result[6] + "," + result[7] + "," + result[4] ;
             <%-- } else {
               tempScript = "'"+result[1]+"'";
            }  --%>
            temp.push(tempScript);
            temp.push("," + tempField)
    	}
...
function dateOrDatetimeCal(appName, urlParams, type, formShortName, createCode, paramObj){
    ...
    var result = getTextAreaPosition(createCode), startPos = 0, endPos = 0, title = "${ctp:i18n('workflow.label.dialog.dateDiffSetting')}";// '日期差设置'
    startPos = result.startPos;
    endPos = result.endPos;
	var dialog = $.dialog({
