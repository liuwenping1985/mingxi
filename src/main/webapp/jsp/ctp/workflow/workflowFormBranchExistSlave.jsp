...
$(function(){
    ...
    $("a.workflow_form_ymrw").click(function(){
        ...
        if(realType!='date' && realType!='datetime'){$.alert("workflow.label.dialog.selectDateOrDataTimeField");return;}
        //添加值
        var createCode = getFormConditionTextArea();
        var functionName = $(this).attr("functionName"), formShortName= getFormShortName();
        var addedValue = functionName + "("+getFieldName(formShortName, fieldObj.display)+") ";
...
function openWindowByType(fieldObj,formShortName){
    ...
    var tempInputType = fieldObj.inputType.toLowerCase();
    if ((tempInputType == "text" 
            || tempInputType == "lable" 
            || tempInputType == "relation" 
            || tempInputType == "relationform"
            || tempInputType=="customcontrol")
