...
<input type="hidden" id="is_form_template"  name="is_form_template" value="true">
</c:if>
<c:if test="${!isFormTemplate== true}">
<input type="hidden" id="is_form_template"  name="is_form_template" value="false">
</c:if>
...
<c:choose>
	...
	<c:otherwise>
		<tr height="10" style="display:none;"><td></td></tr>	
          ...
          <c:choose>
            ...
            <c:when test="${isShowMatchScope}">
               ...
               <table align="center" border="0" width="100%">
                  ...
                  <tr>
                      <c:choose>
                        <c:when test="${isFormTemplate == true && isFormAdvanced==true}">
                           <td rowspan="3" valign="top" width="20%" nowrap="nowrap" align="right">
                          ${ctp:i18n('workflow.designer.node.matchScope')}:
                          </td> 
                        </c:when>
                        <c:otherwise>
                           <td rowspan="2" valign="top" width="20%" nowrap="nowrap" align="right">
                          ${ctp:i18n('workflow.designer.node.matchScope')}:
                          </td> 
                        </c:otherwise>
                      </c:choose>
                    ...
                    <tr>
                      <td height="28" colspan="2" align="left">
                       ...
                       <label for="departmentPostAutoUpId"  id="departmentPostAutoUpIdLabel" style="display: ${matchScope3Checked=='checked'?'':'none'}">
                            <input ${isCopyFromDisable} type="checkbox"  ${pup=='1'|| pup=='-1' || pup=='null'?'checked':''}  name="departmentPostAutoUpId" id="departmentPostAutoUpId">${ctp:i18n('workflow.designer.node.role.autoup.label')}
                       </label>
                     </td>
                    </tr>
                 ...
                 <tr id="nouserActionTitle">
                        <td width="20%" height="28" align="right">${ctp:i18n('workflow.designer.node.nouser.action.label')}:</td>
                        <c:choose>
                        <c:when test="${isRootPost && isGroupVersion}">
                        ...
                        <select ${isCopyFromDisable}  ${disable0} style="width:200px" class="margin_l_5" name="nouserAction" id="nouserAction"  onchange="showNouserActionTitle();">
                            <%--弹选人界面 --%>
            ...
            <c:when test="${(partyType=='Node' || partyType=='Role') 
              ...
              <tr height="10"><td></td></tr>
               ...
               <fieldset width="80%" align="center"><legend>${ctp:i18n('workflow.designer.node.matchScope.node')}</legend>
               <table align="center" border="0" width="100%">
              <c:if test="${isMatchup eq true}"><tr>
                <td width="28%" height="28" align="right">${ctp:i18n('workflow.designer.node.matchScope')}:</td>
                <td width="72%" align="left" colspan="2">
                     ...
                     ${(rup=='1'||rup=='-1' || rup=='null')?'checked':''}/>${ctp:i18n('workflow.designer.node.role.autoup.label')}
                </td>
               </tr></c:if>
               <tr id="nouserActionTitle">
               <td width="20%" height="28" align="right">${ctp:i18n('workflow.designer.node.nouser.action.label')}:</td>
            ...
            <%--单人节点：默认走代理交接 --%>
            <c:when test="${partyType=='user'}"></c:when>
            <%--其他：由上节点选人和自动跳过 --%>
            <c:otherwise>
             <tr height="10"><td></td></tr>
                <tr>
               ...
               <table align="center" border="0" width="100%">
             <tr id="nouserActionTitle">
               <td width="28%" height="28" align="right">${ctp:i18n('workflow.designer.node.nouser.action.label')}:</td>
		  ...
		  <!-- 表单视图绑定部分:开始  --> 
		  <c:if test="${isFormTemplate== true&&formType!='6'  && showBindErea == 'true'}">
		  <tr height="10"><td></td></tr>
		  <tr>
		     <td>
               <fieldset width="80%" align="center">
			   ...
			   <table align="center" width="100%" border="0">
			     <tr <c:if test="${govdocFormType == 5 || govdocFormType == 6 || govdocFormType == 7 || govdocFormType == 8 }">style='display:none;'</c:if>>
         ...
         <c:if test="${isFromTemplete or _isTemplete}"><!-- 模版流程 -->
             ...
             <tr>
                ...
                <div class="common_txtbox_wrap" style="padding:0px;">
                    <textarea id="desc" ${nodeState!="1"?"disabled":"" } name="desc"  rows="5" maxSize="200" class="validate border_no font_size12" validate="isWord:true,avoidChar:'!@#$%^*()<>\\',maxLength:200,name:'处理说明'" style="width:99.5%;"></textarea>
                </div>
                </td>
             </tr>
          </c:if>
        ...
        <table width="95%" border="0" cellspacing="0" cellpadding="0" align="center">
            <tr>
                ...
                <td align="right" style="padding-top:5px;">
                	...
                	<a id="btnreset" class="common_button common_button_gray" href="javascript:void(0)" onclick="openEventAdvancedSetting();">开发高级</a>
...
var isTemplate = $("#is_form_template").val();
...
function openEventAdvancedSetting(){
	...
	var dialog = $.dialog({
		...
		url: _ctxPath + "/workflow/designer.do?method=advancedSetting&from=node&nodeId=${nodeId}",
		title : "开发高级",
		width :dialogWidth,
		height:'300',
		transParams:{
