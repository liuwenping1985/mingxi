...
<table class="only_table edit_table" border="0" cellSpacing="0" cellPadding="0" width="100%">
     ...
     <tbody>
       ...
       <tr type="${funcParam.type }">
          ...
          <td nowrap="nowrap">
              <c:choose>
                  <c:when test="${funcParam.sameTypeField!=null }">
	                <label for="formField${status.count }" class="margin_t_5 hand display_inline">
	                <input type="radio" value="2" id="formField${status.count }" name="option${status.count }" class="radio_com">:
	                </label>
                    <select style="width:70%;" name="formFieldSelect" id="formFieldSelect">
		                <c:forEach items='${funcParam.sameTypeField}' var="field">
