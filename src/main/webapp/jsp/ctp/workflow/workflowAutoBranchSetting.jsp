...
if(dialogParentTransParams.isCircle && dialogParentTransParams.isCircle!="null"){
	isCircle= dialogParentTransParams.isCircle;
}
if(dialogParentTransParams.submitStyle && dialogParentTransParams.submitStyle!="null"){
	submitStyle= dialogParentTransParams.submitStyle;
}
...
$(document).ready(function() {
    ...
    //chorme浏览器做特殊设置OA-96998
    if($.browser.chrome){
        $("#creatcode").attr("rows","5");
        $("#creatcodeReadOnlyArea").css("height", "60px");
    }
    
  /*if($.browser.msie){
    $("#creatcode").attr("rows","12");
  }*/
  if(formCondition.search(/field\d+/)>-1 || appName=='edoc' || appName=='edocSend' || appName=='recEdoc' || appName=='signReport' || appName=='sendEdoc' || appName=='edocRec' || appName=='edocSign'  || appName=='collaboration'){
	var result = null;
	if(appName=='edoc' || appName=='edocSend' || appName=='recEdoc' || appName=='signReport' || appName=='sendEdoc'){
		result = wfAjax.conditionToFieldDisplay("${appName}","${wendanId}",formCondition);
	}else{
		result = wfAjax.conditionToFieldDisplay("${appName}","${formApp}",formCondition);
	}
  ...
  <c:choose>
    ...
    <c:when test="${appName=='edoc' || appName=='edocSend' || appName=='recEdoc' || appName=='signReport' || appName=='sendEdoc' || appName=='edocRec' || appName=='edocSign'}">
  ...
  if(isCircle == 'true'){
	  $("#isForce").disable();
	  var obj = $("#submitStyleArea");
	  var display = obj.css("display");
	  if(display=="none"){
	      obj.css("display", "");
	  }
	  var _submitStyle = submitStyle.split(",");
	
	  if(_submitStyle.indexOf('0') >= 0){
		  $("#submitStyle0").attr("checked","checked");
	  }
	  if(_submitStyle.indexOf('1') >= 0){
		  $("#submitStyle1").attr("checked","checked");
	  }
	  if(_submitStyle == '') {
		  $("#submitStyle0").attr("checked","checked");
		  $("#submitStyle1").attr("checked","checked");
...
function translateBranchInfo(){ 
	var result= null;
	if(appName=='edoc' || appName=='edocSend' || appName=='recEdoc' || appName=='signReport' || appName=='sendEdoc' || appName=='edocRec' || appName=='edocSign'){
		result= wfAjax.branchTranslateBranchExpression("${ctp:escapeJavascript(appName)}","${wendanId}",$("#creatcode").attr("value").replace(/[\r\n]+/g,''));
	}else{
		result= wfAjax.branchTranslateBranchExpression("${ctp:escapeJavascript(appName)}","${formApp}",$("#creatcode").attr("value").replace(/[\r\n]+/g,''));
	}
  if(result!=null){
...
function OK(jsonArgs){
  ...
  if(innerButtonId=='ok'){
    ...
    if(creatcodeValue==""){
      ...
      if(tempCondition.search(/{[^{}]+}/)>-1  || appName=='edoc' || appName=='edocSend' || appName=='recEdoc' || appName=='signReport' || appName=='sendEdoc' || appName=='edocRec' || appName=='edocSign' || appName=='collaboration'){
          var result= null;
      	  if(appName=='edoc' || appName=='edocSend' || appName=='recEdoc' || appName=='signReport' || appName=='sendEdoc' || appName=='edocRec' || appName=='edocSign'){
      		result = wfAjax.conditionToFieldName("${appName}","${wendanId}",tempCondition);
      	  }else{
      		result = wfAjax.conditionToFieldName("${appName}","${formApp}",tempCondition);
      	  }
          if(result!=null && result.success){
      ...
      var __submitStyle = "";
      $('input:checkbox[name=submitStyle]:checked').each(function(i){
       if(0==i){
...
<table width="100%" height="100%" border="0" align="center">
    ...
    <tr id="creatcodeReadOnlyTR" style="display:none;">
        <td colspan="2">
        <div id="creatcodeReadOnlyArea" style="border:1px solid #CCC;height:100px;width:520px;word-wrap:break-word;font-size:12px;overflow:auto;" >
        </td>
    </tr>
    <tr>
       <td>
            <div class="common_tabs clearfix">
              <ul class="left">
                  <li class="current"><a id="orgBranchBtn" class="border_b last_tab" href="javascript:void(0);" ><span title ="${ctp:i18n('workflow.condition.auto.label')}">${ctp:i18n('workflow.condition.auto.label')}</span></a></li>
                  <c:choose>
                    <c:when test="${appName=='form' }">
                        <li><a id="formBranchBtn" href="javascript:void(0);" class='border_b last_tab' ><span title ="${ctp:i18n('workflow.condition.form.label')}">${ctp:i18n('workflow.condition.form.label')}</span></a></li>
                    </c:when>
                   <c:when test="${appName=='edoc' || appName=='edocSend' || appName=='recEdoc' || appName=='signReport' || appName=='sendEdoc' || appName=='edocRec' || appName=='edocSign'}">
                        <li><a id="edocBranchBtn" href="javascript:void(0);" class='border_b last_tab'><span title="${ctp:i18n('workflow.condition.edoc.label')}">${ctp:i18n('workflow.condition.edoc.label')}</span></a></li>
                    </c:when>
                    <c:otherwise>
                        <c:if test="${isShowBranchTab==true }">
                            <li><a id="officeBranchBtn" href="javascript:void(0);" class='border_b last_tab' style="max-width: none;"><span title="${branchTabName}">${branchTabName}</span></a></li>
                        </c:if>
                    </c:otherwise>
                  </c:choose>
              </ul>
    ...
    <tr id="submitStyleArea"  style="display:none;">
       <td height="30" colspan="2">
       		<span class="font_size12" >${ctp:i18n('workflow.special.stepback.label14')}&nbsp;&nbsp;:&nbsp;&nbsp;</span>
            <input id="submitStyle0" class="radio_com" name="submitStyle" value="0" type="checkbox"><span class="font_size12" >${ctp:i18n('workflow.special.stepback.label6')}</span>
            <input id="submitStyle1" class="radio_com" name="submitStyle" value="1" type="checkbox"><span class="font_size12" >${ctp:i18n('workflow.special.stepback.label7')}</span>
       </td>
    </tr>
</table>
</form>
