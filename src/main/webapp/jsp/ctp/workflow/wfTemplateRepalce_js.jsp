...
//    window.dialogArguments.templateReplaceNodeDialog.reSize({
//		height:98
//	});
	resetOtherFunction();
}
...
function selectRePeopleFunction(){
	...
	}else if('Department'==selectedType){
					...
					var dialog = $.confirm({
						...
						,ok_fn: function() {
				      		var data = wfAjax.repalceTemplateList(templateIds,{
								...
								newEle:newNode,
								formIds:formIds
							}, "${ctp:escapeJavascript(appName)}");
							$.alert("${ctp:i18n('workflow.replaceNode.33')}".replace("xxxx", data));
				          	dialog.close();
				          }
					});
				}
...
function showResultFunction(onlyTitle){
	...
	var data = wfAjax.getNeedRepalceTemplateList({
            ...
            handler : function() {
              ...
              if(returnValue){
              	var dialog = $.confirm({
					...
					,ok_fn: function() {
		                var result = wfAjax.updateTemplateList(appName,ptemplateId, formApp, returnValue[1]);
		                if(result!=null){
			                if(result.success){
				                workfowFlashDialog.close();
