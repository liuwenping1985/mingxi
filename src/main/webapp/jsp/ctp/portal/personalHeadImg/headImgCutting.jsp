...
var cutWidth = ${empty param.cutWidth ? '104' : param.cutWidth};
var cutHeight = ${empty param.cutHeight ? '104' : param.cutHeight};
...
//预览区域宽
cuttingParam.previewWidth = cutWidth;
//预览区域高
cuttingParam.previewHeight = cutHeight;
//x轴坐标
cuttingParam.x = 0;
//y轴坐标
cuttingParam.y = 0;
...
<body style="font-size: 14px; overflow: hidden; height:400px;background:#fafafa;">
    <input id="myfile" type="hidden" />
    <table border="0" cellSpacing="0" cellPadding="0" style="margin-top: 40px; margin-left: 64px;">
        <tr>
            <td style="position: relative;"><div style="float:left; width: 250px; height:250px; text-align:center; background-color: #f5f5f5; border: 1px #e1e1e1; color: #aaaaaa" ><div id="headImgInstruction" style="margin-top: 155px">${ctp:i18n("personal.headImg.instruction")}</div></div><img id="originalImg" style="position:absolute; display:none; " /></td>
            <td valign="top">
                <div style="margin-left: 68px; margin-top: 0px; width:${empty param.cutWidth ? '104' : param.cutWidth}px; height:${empty param.cutHeight ? '104' : param.cutHeight}px; overflow: hidden;">
                    <img id="preview" src="${path}${empty param.cutImg ? '/apps_res/v3xmain/images/personal/pic.gif' : param.cutImg}" style="width:${empty param.cutWidth ? '104' : param.cutWidth}px; height:${empty param.cutHeight ? '104' : param.cutHeight}px;">
                </div>
                <div style="margin-left: 68px; margin-top: 5px; width: 104px;">${empty param.cutWidth ? '104' : param.cutWidth}*${empty param.cutHeight ? '104' : param.cutHeight}${ctp:i18n("personal.headImgPreview.instruction")}</div>
            </td>
        </tr>
    </table>
    ...
    <div class="align_right bg-advance-bottom" style="position: absolute; top: 350px; left: 0px; width: 100%;padding-top: 5px;">
