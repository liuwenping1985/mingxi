...
<script type="text/javascript" src="${url_ajax_formReportViewManager}"></script>
...
<script>
  var formReportViewManager_ = new formReportViewManager();
...
<c:if test="${fieldSize > 0}">
        	 ...
        	 if(ct && ct != '') { 
          	 ...
          	 var compObj = $(this);
         		  if(j.type=='chooseProject') {//关联项目组件重复comp有些问题   在外面处理
             		 ...
             		 compObj = clObj;
         		  }else if (j.type=='autocomplete'){//重置下拉列表
         		   var clParent = $(this).parent();
         		   $(this).width(100);
         		   clParent.empty();
  ...
  $("#delMyQuerySet").click(function(){
    var rSid=$("#rSid").val();
    var confirm = $.confirm({
        'msg': "${ctp:i18n('report.queryReport.index_right.prompt.deleteConfirm')}",//你确定删除该统计信息吗?
        ok_fn: function () { 
          //OA-27820
...
//           parent.location.reload(); firefox下的window.location.reload(true)会引起post请求，ie不会。
  ...
  function search(){
    ...
    //===========判断输入是否包含"<"和">"=====
        ...
        var userCondition = $("#userConditionTable").formobj({validate : false});//用户自定义查询
        ...
        var userFastCon = $("#userFastCondition").formobj({validate : true});
        var stateCon = $("#stateTr").formobj({checked : true}); //状态选择
       ...
       if($("#formType").val() == "1" || $("#formType").val() == "5" ||  $("#formType").val() == "6" || $("#formType").val() == "7"|| $("#formType").val() == "8"){//无流程不显示
...
function saveMyQuery(){
     var dialog = $.dialog({
        ...
        height:"120 px",
        buttons : [ {
          text : "${ctp:i18n('report.reportDesign.button.confirm')}",//确定
          id:"sure",
          handler : function() {
          	  var test = $("#name").validate();
          	  if(!test){
          	 	 $.alert("${ctp:i18n('report.queryReport.index_right.prompt.noTitleEmpty')}");//我的统计标题不能为空!
                 return false;
          	  }
          	  var valiError = false;
          	  var temp = ["\"","<",">","!",",","@","#","$","%","&","*",")","(","\'","\\"];
       	  	  $("#userConditionTable :input").each(function (){
			        var value = $(this).val();
			        if($(this).attr("type") == "text" || $(this).attr("type") == "textarea"){
				        if(!valUserCond(temp,value)){
				        	valiError = true;
				        }
			        }
			    });
			    if(valiError){
			     $.alert("${ctp:i18n('report.queryReport.index_right.prompt.condition')}");//自定义统计条件不能包含特殊字符，这样的统计条件保存后会报错
                 return false;
			    }
              $.setFieldValueSubmit();
              //清掉两边的空格
              $("#name").val($.trim($("#name").val()));
              if(!checkMaxZhCharLength($.trim($("#name").val()),[0,255])){
                 $.alert("${ctp:i18n('report.queryReport.index_right.prompt.titleTooMuchWords')}");//我的统计标题中文汉字不能超过85个，普通字符不能超过255个!
              }else{
                  //判断是否为重新保存我的统计
                  var rSid=$("#rSid").val(); 
                  var myname=$("#name").val();
               if(myname.indexOf("$")>=0||myname.indexOf("|")>=0||myname.indexOf("/")>=0||myname.indexOf(">")>=0||
                  myname.indexOf("<")>=0||myname.indexOf("&")>=0||myname.indexOf("%")>=0){
                   $.alert("${ctp:i18n('report.queryReport.index_right.prompt.specialCharacter')}");//统计名称不能包含$ | / < > & % 特殊字符
                }else{
                      if(rSid!=""&&(myname=="${reportSave.name}")){
                          var confirm = $.confirm({
                              'msg': "${ctp:i18n('report.queryReport.index_right.prompt.nameExist')}",//已存在，你确定覆盖原有信息吗?
                              ok_fn: function () { 
                                  saveReportSave();
                                  dialog.close();
                               }
						  ...
						  success : function(msg){
								if(msg){
									var win = new MxtMsgBox({
										...
										'imgType':2,
										'msg': "${ctp:i18n('report.reportResult.chongfu.label')}",//已有同名名称，请重命名
										 ok_fn:function(){
											return;
										 }
									});
...
{   
    $("body").jsonSubmit( {
        ...
        beforeSubmit:function(){
            processBar =  new MxtProgressBar({text: "${ctp:i18n('report.queryReport.index_right.prompt.saving')}"});//正在保存我的查询....
        },
...
function resetWidthUserFastCondition(){
	  ...
	  $('.comp', "#userFastCondition").each(function(){
		  ...
		  if(ct && ct != '') {
		  var j = $.parseJSON('{'+ct+'}');
		  if(j.type == 'selectPeople'||j.type=='chooseProject') {
...
<div id="myQueryTable" class="hidden">
	<table height="110px">
    		<tr>
                    <th nowrap="nowrap">
                        <label class="margin_r_10" for="text">${ctp:i18n('report.queryReport.index_right.myReport')}:</label></th>
                    <td width="100%">
                        <div>
                            <input type="text" validate="type:'string',notNull:true,notNullWithoutTrim:true,avoidChar:'\\\&#39;&quot;&lt;&gt;!,@#$%&*()',maxLength:85" class="validate" name="name" id="name" value="${reportSave.name}" />
...
<form action="#" id="queryConditionForm" method="post" target="main">
    ...
    <div class="layout_north " id="north">
        ...
        ${ctp:i18n('report.queryReport.index_right_grid.statisticConditions')}:
                        ...
                        <fieldset class="fieldset_box margin_t_5">
                            <legend>${ctp:i18n('report.queryReport.index_right.inputStatCondition')}:</legend>
                                <div style="height:80px; overflow:auto;">
                                    ...
                                    <tr id="stateTr" <c:if test="${formBean.formType != 1 && !formBean.isGovForm4View }">class='hidden'</c:if>>
                                    <td>
				              ...
				              <div class="common_checkbox_box clearfix padding_tb_5">
					              ...
					              <c:if test="${!formBean.isGovForm4View}">
            ...
            <div class="align_center clear margin_t_5">
                <a class="common_button margin_r_10" href="javascript:void(0)" id="excQuery">${ctp:i18n('report.queryReport.index_right.button.excQuery')}</a><!-- 统计 -->
    ...
    <div class="layout_center stadic_layout over_hidden" id="center">
        <div class="stadic_layout w100b h100b hidden" style="*position:absolute;" id="searchResultDiv">
            ...
            <div id="result" class="align_center border_t common_tabs_body stadic_layout_body stadic_body_top_bottom" style="top:45px;bottom:0px; background:#fff;overflow:hidden;">
    ...
    function addButtonEvent() {
        ...
        $("#leadingOut").unbind("click").bind("click").click(function() {
        	...
        	if(item==1){
       		 	//$.alert("${ctp:i18n('report.queryReport.index_right.errorExportChart')}");//对不起,图表无法导出,您可以右键保存到本地
       		 	var echartData = window.frames["content"].echartData;
       		 	var performanceQueryManager_ = new performanceQueryManager();
				var imgId = performanceQueryManager_.saveImage(echartData.getDataURL(),2);
				
	       		$("#excelCondition").remove();
	       		$("#queryConditionForm").append("<div id='excelCondition'></div>");
				$("#queryConditionForm").attr("action","${path}/report/queryReport.do?method=exportToExcel&imgId="+imgId[0]+"&reportId=${reportId}");
        ...
        $("#print").unbind("click").bind("click").click(function() {
            ...
            if(printsub == undefined){
				...
				if(item==1){
				var echartData = window.frames["content"].echartData;
				 if(echartData != "" & echartData != null){
					//colBody="<img src='data:image/gif;base64,"+echartData.getDataURL()+"'>";
					//OA-44998 为了解决ie7下打印和转发协同的问题，相应的方法是写在绩效模块的。
					var performanceQueryManager_ = new performanceQueryManager;
					var imgId = performanceQueryManager_.saveImage(echartData.getDataURL(),2);
					colBody+="<img src='${path}/performanceReport/performanceQuery.do?method=toImageForIe&imgId="+imgId+"'/>";
				 }
			   }else{
				   iframeobj.find("#reportName").hide();
				   iframeobj.find("#portalHidden").attr("style","border: none;font-size: 12px;padding-left: 35px;");
				   iframeobj.find("#context").find(".dongtaiContent").remove();
				   colBody = iframeobj.find("#portalHidden tbody").html() + iframeobj.find("#context").html();
				   iframeobj.find("#portalHidden").attr("style","border: none;font-size: 12px;");
				   iframeobj.find("#reportName").show();
			   }
			   // var colBody = iframeobj.find("#context").html();
        ...
        function synergyCo(){ 
			...
			title = title + " " + currentData; 
			            	...
			            	if(item==1){ 
			            	if(echartData !="" && echartData != null){ 
			            	//context="<img src='data:image/gif;base64,"+chartObj.chart.getPNG()+"'>"; 
			            	//OA-44998 为了解决ie7下打印和转发协同的问题，相应的方法是写在绩效模块的。 
			           	var performanceQueryManager_ = new performanceQueryManager; 
			           	var imgId = performanceQueryManager_.saveImage(echartData.getDataURL(),2); 
						context+="<img src='${path}/performanceReport/performanceQuery.do?method=toImageForIe&imgId="+imgId+"'/>"; 
			            	} 
			            	}else{ 
			            	iframeobj.find("#context a").removeAttr("onClick") 
			               	//添加统计标题居中样式(默认) OA-44201 
			               	iframeobj.find("#reportName").attr("style", "font-size:20px; text-align:center;"); 
			            	...
			            	context = context + iframeobj.find("#portalHidden tbody").html() + iframeobj.find("#context").html(); 
			               	}else{ 
			           	...
			           	/* if(parent.window.location.href.indexOf("queryReport.do")==-1){ 
			           	$("#queryConditionForm").attr("target","main"); 
			            	}*/ 
			            	$("#reportContent").val(context); 
			            	$("#reportTitle").val(title); 
			            	$("#queryConditionForm").attr("action","${path}/report/queryReport.do?method=reportForwardCol"); 
			           	var tarWin = window; 
			        	...
			        	$("#queryConditionForm").jsonSubmit({targetWindow:tarWin}); 
			        	try{ 
			            	if(isModelOpen){ 
			            	window.close(); 
			            	} 
			        	}catch(e){} 
			        	<c:if test="${param.openfrom eq 'datalist'}"> 
