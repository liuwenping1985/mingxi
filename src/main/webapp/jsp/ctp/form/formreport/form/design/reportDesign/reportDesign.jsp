    	...
    	<div id='layout'>
	        <div class="layout_north" id="north">
				...
				<div class="step_menu clearfix margin_tb_5 margin_l_10">
					<%@ include file="/WEB-INF/jsp/ctp/form/design/top.jsp" %>
				</div>
				<!--向导菜单-->
...
<%-- 							<a class="common_button common_button_gray" href="javascript:void(0)" id="newquery">${ctp:i18n('report.reportDesign.form.button.set')}</a> --%>
<!-- 						</div> -->
							...
							<select onchange="changeVal();" id="statisticsC" class="common_selectbox_wrap hidden">
							    <option value="0">
									...
									<c:if test="${formBean.govDocFormType !=5 &&formBean.govDocFormType !=6 && formBean.govDocFormType !=7}">
										${ctp:i18n('report.reportDesign.data')}
									</c:if>
									<c:if test="${formBean.govDocFormType ==5 ||formBean.govDocFormType ==6 || formBean.govDocFormType ==7}">
										${ctp:i18n("edocreport.reportDesign.data")}
									</c:if>
								</option>
	       	...
	       	<div class="layout_south over_hidden" id="south">
					<%@ include file="/WEB-INF/jsp/ctp/form/design/bottom.jsp" %>
			</div>
		</div>
	</body>
	...
	$(document).ready(function(){
        ...
        if(!${formBean.newForm }){
          new ShowTop({'current':'report','canClick':'true','module':'report'});
          new ShowBottom({'show':['doSaveAll','doReturn']});
          }else{
          new ShowTop({'current':'report','canClick':'false','module':'report'});
          new ShowBottom({'show':['upStep','nextStep','finish'],'source':{'upStep':'../form/queryDesign.do?method=queryIndex','nextStep':'../form/bindDesign.do?method=index'}});
          }
...
// 	                              window.location.href=url_reportDesign_index;
		...
		$("#update").unbind('click').click(function(){
			...
			if(selectQuery.length!=1){
				$.error("${ctp:i18n('report.reportDesign.dialog.prompt.selecteEdit')}");//请选择一项记录进行修改!
				return;
			}else{	
     ...
     */
     function openFormSortField(id, title, from) {
         ...
         var sortCrossColumnDataType=$("#crossColumndDateTypeHidden").val();
     	 ...
     	 var classifyValue=$("#classifyValue").val();
     	 var url = url_reporDesign_selectFormField + "&from=" + from;
     	 var width = from == $("#setCrossColumn").attr("id") ? 450 : 580;
         var height = 400;
         var dialog = $.dialog({
             id: id,
             url: url,
	                         ...
	                         if (checkValue !== "" || checkTitle ) {
	                             ...
	                             if(isInSameMainTable(returnObj.ownerTableName,from,returnObj.displayName )){
	                                 ...
	                                 if (from === "setReportHead") {
	                                     ...
	                                     $("#crossColumn").val(returnObj.texts);
