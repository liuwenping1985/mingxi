...
<body scrolling="no">
	<div class="comp" comp="type:'layout'" id="layout">
		<%-- 顶部按钮区域 --%>
		 <div id="north" style="border-top:1px solid #b6b6b6;border-left: none;" class="layout_north" layout="width:260,height:30,maxHeight:30,minHeight:30,sprit:false,spiretBar:{show:false}">
         <div id="toolbar"></div>
        </div>
...
<script type="text/javascript">
	...
	function newFunction(){
		...
		$("#bottomIframe").attr("src","${path}/form/formula.do?method=formulaFunctionDesign&formId="+formId+"&formulaType="+formulaType+"&fieldName="+fieldName); 
	}
	//修改
	function editorFunction(){
		...
		gridObj.grid.resizeGridUpDown('middle');
		$("#bottomIframe").attr("src","${path}/form/formula.do?method=formulaFunctionDesign&pageType=editor&formId="+formId+"&formulaType="+formulaType+"&fieldName="+fieldName+"&id="+rows[0].id); 
	}
	//删除
	function delFunction(){
		...
		var confirm = $.confirm({
		    'msg': '删除函数操作将导致使用了该函数的计算式失效，是否继续？',
		    ok_fn: function () { 
		    	//进行计算式判断
	...
	function createTable(){
		var tableParam = {
	            colModel : [ {display : 'id',name : 'id',width : '10%',sortable : false,align : 'center',type : 'checkbox',hide:false,isToggleHideShow:true}, 
	                         {display : '函数名称',name : 'functionName',width : '51%',sortable : true,align : 'left',hide:false,isToggleHideShow:true},
	                         {display : '函数参数',name : 'functionParam',sortable : true,align : 'left',hide:true,isToggleHideShow:true},
	                         {display : '参数值',name : 'selectParam',sortable : true,align : 'left',hide:true,isToggleHideShow:true},
	                         {display : '代码类型',name : 'codeType',width : '20%',sortable : true,align : 'left',hide:false,isToggleHideShow:true}, 
	                         {display : '返回类型',name : 'returnType',width : '20%',sortable : true,align : 'center',hide:false,isToggleHideShow:true}],
	...
	function functionClick(data,r,c){
		gridObj.grid.resizeGridUpDown('middle');
		$("#bottomIframe").attr("src","${path}/form/formula.do?method=formulaFunctionDesign&pageType=browser&formId="+formId+"&formulaType="+formulaType+"&fieldName="+fieldName+"&id="+data.id); 
	}
	//弹出框点击确定方法
	function OK(){
		var rows = gridObj.grid.getSelectRows();
