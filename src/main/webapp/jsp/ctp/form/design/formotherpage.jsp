  ...
  <tr>
    <td valign="top">
     ...
     <tr>
	      ...
	      <tr>
	      <c:if test="${isGovDocForm}">
			<td width = "40%">${ctp:i18n('form.formotherpage.inputnewgovdocname')}</td>
		  </c:if>
		  <c:if test="${!isGovDocForm}">
		  ...
		  <c:if test="${temList!=null &&fn:length(temList)>0}">
		  	<c:if test="${isGovDocForm}">
				<tr>
				  ...
				  <fieldset>
				  <legend>
					  	${ctp:i18n('form.formotherpage.inputgovdoctemplatename')}：
				  </legend>
				  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" id="template">
				  <tr>
				  <td>
						${ctp:i18n('form.formotherpage.oldgovdoctemplatename')}
				  </td>
				  <td>
				  		${ctp:i18n('form.formotherpage.newgocdoctemplatename')}
				  </td>
				</tr>
			</c:if>
			<c:if test="${!isGovDocForm}">
