...
<html>
    ...
    <body class="page_color">
    	<div id='layout'>
	        ...
	        <form action="${path}/form/authDesign.do?method=formDesignSaveAuth" id="saveForm" style="height: 94%">
	                ...
	                <table border="0" cellspacing="0" cellpadding="0">
	                  <tr>
	                    ...
	                    <c:forEach var="view" items="${formBean.formViewList }" varStatus="status">
	                     	<option value="${view.id }">${view.formViewName }</option>
	                    </c:forEach>
	                    </select></td>
                          <!-- infopath表单才显示移动设计器-->
                          <c:if test="${formBean.formType ne 4  and formBean.infoPathForm}">
                              <td nowrap="nowrap">
                                  <a id="designPhoneView" class="common_button common_button_gray margin_l_10" href="javascript:void(0)"><em class='ico16 formPhone_16 ${viewBean.settedPhoneView ? "" : "toGray"}'></em>移动视图设计</a>
                              </td>
                          </c:if>
	                  </tr>
	                </table>
	             ...
	             <!--左右布局-->
				 <div class="layout clearfix code_list padding_l_10 margin_t_10" style="height: 90%">
	            	<div class="col2" id="authField" style="float: left;width:54%;height: 100%">
						...
						<fieldset class="form_area padding_10" id="authFieldMap" style="height: 90%">
							...
							<div style="height: 90%;margin-bottom: 5px;">
								<div  id="authFieldTable" style="height: 100%">
								<table border="0" cellspacing="0" cellpadding="0" width="100%">
				                  <tr>
				                    ...
				                    <td align="left" clospan="2">
				                    	...
				                    	<label for="conditionCkb" class="margin_r_10 hand">
								...
								<div id="radioHeight" style="width:100%; overflow:scroll; overflow-x:hidden; margin-bottom:5px;height: 65%;height:60%\9">
								...
								<tr id="${field.name }_tr">
                                    <td height="30" style="word-break:break-all;">[<c:if test="${field.masterField}">${ctp:i18n("form.base.mastertable.label")}</c:if><c:if test="${!field.masterField}">${ctp:i18n("formoper.dupform.label")}${field.extraMap.subTableIndex}</c:if>]&nbsp;${field.display}:</td>
									...
									<td align="center" width="10%">
										<c:if test="${(field.inputType eq textarea || field.inputType eq flowDealOption) && field.mappingField ne 'subject'}">
									...
									<INPUT readOnly disabled id="${field.name }_showName" fieldType="${field.inputType}" <c:if test="${field.inputType eq relationForm}">viewSelectType = ${field.formRelation.viewSelectType }</c:if> attrType="def" fieldName="${field.name }" style="width: 90%"/>
                                    </td>
                                    <td name="conFieldAuthTD" width="11%">
                                            ...
                                            <table id="groupEdit" style="display: inline" width="90%" border="0"
                                                ...
                                                <c:forEach var="table" items="${formBean.tableList }" begin="1">
                                                    <tr tableName="${table.tableName }">
                                                        <td class="bg-gray" width="37%" nowrap="nowrap" id="${table.display }" title="${ctp:i18n("formoper.dupform.label")}${table.extraMap.subTableIndex}">[${ctp:i18n("formoper.dupform.label")}${table.extraMap.subTableIndex}]</td>
                                                        <td width="32%" nowrap="nowrap"><label for="${table.display }_allowAdd">
                                                            <!-- 允许添加 -->
                                                            <input tableName="${table.tableName }" type="checkbox" name="${table.display }_allowAdd" id="${table.display }_allowAdd" value="true" />${ctp:i18n('form.authDesign.allowadd')}</label>
                                                        </td>
                                                        <td width="31%" nowrap="nowrap">
                                                            <label for="${table.display }_allowDelete">
                                                                <!-- 允许删除 -->
                                                                <input tableName="${table.tableName }" type="checkbox" value="true" name="${table.display }_allowDelete" id="${table.display }_allowDelete" />${ctp:i18n('form.operhigh.allowdel.label')}
                                                            </label>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="only_table edit_table">
		                    <thead>
		                        <tr>
                                    ...
                                    <c:if test="${ formBean.infoPathForm}">
                                        <th>移动视图设计</th>
                                    </c:if>
		                        </tr>
		                    </thead>
		                    <tbody id="authBody" isNewCode='1'>
			                    	<c:forEach var="auth" items="${authList }" varStatus="status" >
			                    		...
			                    		<tr class="hand <c:if test="${(status.index % 2) == 1 }">erow</c:if>">
		                            		<td id="selectBox"><input name="selectRight" id="authBox" type="checkbox" ${formBean.formType eq 3 and status.index ==1 ? "disabled=true" : "" } authSysType="${authSysType }"  value="${auth.id }" authName="${auth.name}"/></td>
		                            		<td id="${auth.id }">${auth.name }</td>
		                            		<td id="${auth.id }">${auth.extraMap.typeName }</td>
		                            		<td id="${auth.id }">${auth.extraMap.sysName }</td>
                                            <c:if test="${  formBean.infoPathForm}">
                                        		<td id="${auth.id }"><span view="${auth.id }" class='phone ico16 formPhone_16 ${auth.settedPhoneView ? "" : "toGray"}'></span></td>
                                            </c:if>
		                        		</tr>
			                    	</c:forEach>
		                    </tbody>
