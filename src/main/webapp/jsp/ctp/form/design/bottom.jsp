...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $("input", "#buttom_table").click(function () {
        	...
        	if ("doReturn" != $(this).prop("id")) {
        		if("${formType}" == "5" || "${formType}" == "6" || "${formType}" == "7" || "${formType}" == "8") {
        			if(!checkMarkField()) {
                		return;
                	}
        		}
            ...
            if(sourceUrl.indexOf("baseInfo")>=0 || tempObj.prop("action").indexOf("BaseInfo")>=0){ //基础应用设置
            	var govDocFormType = "${formBean.govDocFormType}";
                if(govDocFormType == "7" || govDocFormType == "5" || govDocFormType == "6"){
                	var subjectStr = ""; 
                    $("#formTable tr td input[id^='mappingField']").each(function(){
                    	subjectStr = subjectStr +  $(this).val();
		        	});
                	if(subjectStr.indexOf("subject") < 0 ){
                		var result = new formFieldDesignManager().getEdocMapping();
                		if(result == "1"){
                			hasSubject = false;
                		}else{
	                		hasSubject = true;
    ...
    function checkMarkField() {
    	var edocDocMark = $("#baseInfoForm").find("select[oldinputtype='edocDocMark']");
    	var docMarkId = edocDocMark.attr("id");
    	if(docMarkId && docMarkId!="") {
    		var index = docMarkId.substring(docMarkId.indexOf("inputType") + 9, docMarkId.length);
    		var mappingField = $("#baseInfoForm").find("input[name='mappingField"+index+"']").val();
    		if(mappingField != "doc_mark") {
    			alert("公文文号必须映射元素doc_mark，否则公文文号不能正常使用，请确认!");
    			return false;
    		}
    	}
    	var serialNoMark = $("#baseInfoForm").find("select[oldinputtype='edocInnerMark']");
    	var serialNoMarkId = serialNoMark.attr("id");
    	if(serialNoMarkId && serialNoMarkId!="") {
    		var index = serialNoMarkId.substring(serialNoMarkId.indexOf("inputType") + 9, serialNoMarkId.length);
    		var mappingField = $("#baseInfoForm").find("input[name='mappingField"+index+"']").val();
    		if(mappingField != "serial_no") {
    			alert("内部文号必须映射元素serial_no，否则内部文号不能正常使用，请确认!");
    			return false;
    		}	
    	}
    	var signMark = $("#baseInfoForm").find("select[oldinputtype='edocSignMark']");
    	var signMarkId = signMark.attr("id");
    	if(signMarkId && signMarkId!="") {
    		var index = signMarkId.substring(signMarkId.indexOf("inputType") + 9, signMarkId.length);
    		var mappingField = $("#baseInfoForm").find("input[name='mappingField"+index+"']").val();
    		if(mappingField != "sign_mark") {
    			alert("签收编号必须映射元素sign_mark，否则签收编号不能正常使用，请确认!");
    			return false;
    		}	
    	}
    	return true;
                            ...
                            if (retObj && retObj.success) {
                                $.messageBox({
                                    ...
                                    ok_fn: function () {
                                        ...
                                        winReflesh("${path}/form/formList.do?method=index&formType=${formType}&property=myForm");
                                    }, close_fn: function () {
                                        dialog.close();
                                        $("#otherFormSave").removeProp("disabled");
                                        winReflesh("${path}/form/formList.do?method=index&formType=${formType}&property=myForm");
