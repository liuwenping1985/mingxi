...
<ul id="topUl">
    ...
    <c:if test="${formBean.govDocFormType == '5' || formBean.govDocFormType == '6' || formBean.govDocFormType == '7' || formBean.govDocFormType == '8'}">
	    <li style="cursor: pointer;" class="${formBean.newForm?'':'step_complate' }" id="rightli" step="nextStep" source="/seeyon/form/authDesign.do?method=rightSettingInfo"><b> </b> <span>${ctp:i18n('form.pagesign.rightSetting.label') }</span></li>
    </c:if>
    ...
    <%-- 应用绑定  --%>
    <li style="cursor: pointer;" class="${formBean.newForm?'':'step_complate' }" id="bindli" step="nextStep" source="/seeyon/form/bindDesign.do?method=index&formType=${formType}"><b> </b> <span>${ctp:i18n('form.pagesign.appbind.label') }</span></li>
    <%-- 触发设置 --%>
    <c:if test="${enu.Enums$FormType.planForm != formBean.formType }">
