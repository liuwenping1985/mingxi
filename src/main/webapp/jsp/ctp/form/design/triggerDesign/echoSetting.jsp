                ...
                <fieldset style="width: 480px;padding: 0px;">
                    ...
                    <div style="width: 480px;">
						<div class="clearfix" style="line-height: 20px;">
							<div class="left" style="width: 150px;text-align: right;"><label class="" for="text"><font color="red">*</font>${ctp:i18n('form.echoSetting.triggername.label')}：</div>
							<div class="left common_txtbox_wrap">
								<input type="text" id="triggerName" value="${echo.name }" name="triggerName" style="width:176px;" maxlength="80" class="triggerName validate" validate="notNullWithoutTrim:true,name:'${ctp:i18n('form.trigger.triggerSet.name.label') }',type:'string',notNull:true,character:'\&#39;&quot;&lt;&gt;'">
							</div>
						</div>
						<div class="clearfix" style="line-height: 20px;">
							<div class="left" style="width: 150px;text-align: right;margin-top:5px;">${ctp:i18n('form.echoSetting.selectForm.label')}：</div>
                            <div class="left">
                                <div class="common_selectbox_wrap  showFormName" style="margin-left: 0px;margin-top:5px;">
                                 ...
                                 <c:choose>
                                     ...
                                     <c:otherwise>
                                     <select id="selectRelationForm" name="selectRelationForm" style="width: 150px;margin: 0px;" class="selectRelationForm">
                        ...
                        <div id = "echoTiemDiv" class="clearfix margin_t_5" style="line-height: 20px;">
                            ...
                            <div class="left">
                                ...
                                <label for="approved${status.index }"><input type="radio" id="approved${status.index }"  class="radio_com approved" name="echoTime${status.index }" value="2" <c:if test="${echo.flowState eq '2' }">checked="checked"</c:if>>${ctp:i18n('form.trigger.triggerSet.approvedBy2.label')}</label>
								<label for="perapproved${status.index }"><input type="radio" id="perapproved${status.index }" class="radio_com perapproved"  name="echoTime${status.index }" value="6" <c:if test="${echo.flowState eq '6' }">checked="checked"</c:if>>${ctp:i18n('form.trigger.triggerSet.approvedBy3.label')}</label>
                            </div>
                        </div>
                            ...
                            <div class="left" style="">
                                <select id="refColumNames" name="refColumNames" style="width:150px" class="refColumNames validate" validate="name:''${ctp:i18n('form.trigger.triggerSet.fillback.field') },type:'string',notNull:true">
                                    ...
                                    <c:forEach var="outField" items="${outWriteFields[status.index] }">
                                        ...
                                        <option <c:if test="${optionValue eq refColum.key }">selected=true</c:if> value="${optionValue }" uniqueTableName='${outField.extraMap.uniqueTableName }' tableName='${outField.ownerTableName }' inputType='${outField.inputType }' fieldType='${outField.fieldType }' formatType='${outField.formatType }' isMaster="${outField.masterField }">${outField.display }</option>
                                    </c:forEach>
                                </select>
                            </div>
                        ...
                        <div class="clearfix margin_t_5 radiocheck" style="line-height: 20px;padding-left: 10px;">
                            <input id="withholding" type="hidden" <c:if test='${echo.flowState eq "6"}'>value="false"</c:if><c:if test='${echo.flowState ne "6"}'>value="${echo.actions[0].param.withholding }"</c:if>>
                            ...
                            <div class="common_checkbox_box clearfix ">
                                <label class="margin_r_10 hand" for="withholding${status.index }">
                                    <input name="withholding" class="radio_com withholding" id="withholding${status.index }" type="checkbox" <c:if test='${echo.flowState eq "6"}'>disabled="disabled"</c:if><c:if test='${echo.actions[0].param.withholding eq "true" && echo.flowState ne "6" }'>checked='checked'</c:if> value="true">${ctp:i18n('form.create.withholding.label')}</label>
                ...
                <fieldset style="width: 480px;padding: 0px;">
                    ...
                    <div style="width: 480px;">
						<div class="clearfix" style="line-height: 20px;">
							<div class="left" style="width: 150px;text-align: right;"><label class="" for="text"><font color="red">*</font>${ctp:i18n('form.echoSetting.triggername.label')}：</div>
							<div class="left common_txtbox_wrap">
								<input type="text" id="triggerName" value="" name="triggerName" style="width:176px;" maxlength="80" class="triggerName validate" validate="notNullWithoutTrim:true,name:'${ctp:i18n('form.trigger.triggerSet.name.label') }',type:'string',notNull:true,character:'\&#39;&quot;&lt;&gt;'">
							</div>
						</div>
						<div class="clearfix" style="line-height: 20px;">
                            <div class="left" style="width: 150px;text-align: right;margin-top:5px;">${ctp:i18n('form.echoSetting.selectForm.label')}：</div>
                            <div class="left">
                                <div class="common_selectbox_wrap  showFormName" style="margin-left: 0px;margin-top:5px;">
                                     <select id="selectRelationForm" name="selectRelationForm" style="width: 188px;margin: 0px;" class="selectRelationForm">
                                         <c:forEach var="rel" items="${relation }">
                        ...
                        <div id = "echoTiemDiv" class="clearfix margin_t_5" style="line-height: 20px;">
                            ...
                            <div class="left">
                                ...
                                <label for="approved"><input type="radio" id="approved" class="radio_com approved"  name="echoTime" value="2">${ctp:i18n('form.trigger.triggerSet.approvedBy2.label')}</label>
								<label for="perapproved"><input type="radio" id="perapproved" class="radio_com perapproved"  name="echoTime" value="6">${ctp:i18n('form.trigger.triggerSet.approvedBy3.label')}</label>
                            </div>
                        </div>
                        <div class="clearfix margin_t_5" style="line-height: 20px;">
                            <div class="left" style="width: 150px;text-align: right;">${ctp:i18n('form.echoSetting.data.label')}：</div>
...
$(document).ready(function(){
	...
	$(".refColumNames").change(function(){
		...
		if($("#addSlaveRow",currentEchoSet).val()=="false"){
			...
			var objTr = $(this).parents(".rowdata");
	...
	$("#addNewRelationForm").click(function(){
		...
		var dialog = $.dialog({
			url:"${path}/form/triggerDesign.do?method=setFillBackForm",
			  	...
			  	isEmphasize: true,
					handler : function() {
			      	...
			      	if(ret != "error"&& typeof(ret)=="object"){
				      	...
				      	currentEchoSet = addEchoSet(currentEchoSet);
				      	$(".triggerName",currentEchoSet).val(ret.formName);
	...
	$(".calcExpression").click(function(){
		...
		if($(".refColumNames",parentTr).val()!=""){
			...
			if($("#fillType",parentTr).val()=="formula"){
				...
				var formatType = $(":selected",parentTr).attr('formatType');
				rightData.fieldType = selectedType;
				rightData.formatType = formatType;
				var showLeftDown = false;
				if(formatType == "" || formatType == "flowTitle" || formatType == "text" || formatType == "textarea"){
					showLeftDown = true;
				}
				if(formatType != "" && (formatType == "attachment" ||  formatType == "multiattachment")){
					showLeftDown = true;
				}
				rightData.IsShowLeftDown = showLeftDown;
	...
	$(".flow").click(function(){
		...
		$(".withholding",currentEchoSet).removeAttr("disabled");
	});
	...
	$(".perapproved").click(function(){
		setCurrentEchoSet(this);
		$("#pointData",currentEchoSet).val("6");
		$("#withholding",currentEchoSet).val("false");
		$(".withholding",currentEchoSet).prop("checked", false).attr("disabled", "disabled");
	});
	...
	$(".relateButton").click(function(){
	    ...
	    obj[1] = contentJson;
	    var dialog = $.dialog({
	        url: "${path}/form/triggerDesign.do?method=setFillBackForm&formId=" + formId + "&isCanChange=false",
	        title: "${ctp:i18n('form.echoSetting.form.relative')}",
	        width: 500,
	        height: 370,
	        transParams: obj,
...
function setCurrentRow(obj){
  ...
  currentRow = $(obj).closest(".rowdata");
}
function setCurrentEchoSet(obj){
  currentEchoSet = $(obj).closest(".echoSetting");
}
...
//BUG_普通_V5_V5.1sp1_北京康吉森自动化设备技术有限责任公司_在设置回写设置时，做的有条件回写，条件设置后，再次查看，条件丢失_20151218015186
...
function initOutWriteFieldShow(obj,list){
	...
	for(var i=0;i<list.length;i++){
		...
		str += "<option value='"+list[i].ownerTableName+"."+list[i].name+"' uniqueTableName='"+list[i].extraMap.uniqueTableName+"' tableName='"+list[i].ownerTableName+"' inputType='"+list[i].inputType+"' fieldType='"+list[i].fieldType+"' formatType='"+list[i].formatType+"' isMaster='"+isMasterT+"'>"+list[i].display+"</option>";
	}
	obj.empty();
	obj.html("");
	obj.append(str);
...
function addEchoSet(obj){
	...
	var perapproved = "perapproved"+count;
	...
	$("#perapproved",temObj).prop("id",perapproved).attr("name",name);
	$("label[for='flow']",temObj).attr("for",flow);
	$("label[for='approved']",temObj).attr("for",approved);
	$("label[for='perapproved']",temObj).attr("for",perapproved);
	if($.v3x.isMSIE7) {
	  var html = "<div class=\"left\" style=\"width: 150px;text-align: right;\">${ctp:i18n('form.echoSetting.echoTime.label')}：</div><div class=\"left\"><input type=\"hidden\" id=\"pointData\" value=\"1\">"
		...
		+ "<label for=\""+approved+"\"><input type=\"radio\" id=\""+approved+"\" class=\"radio_com approved\"  name=\""+name+"\" value=\"2\">${ctp:i18n('form.trigger.triggerSet.approvedBy2.label')}</label>"
			  + "<label for=\""+perapproved+"\"><input type=\"radio\" id=\""+perapproved+"\" class=\"radio_com perapproved\"  name=\""+name+"\" value=\"6\">${ctp:i18n('form.trigger.triggerSet.approvedBy3.label')}</label></div>";
...
function OK(obj){
	...
	var o = new formTriggerDesignManager();
	var result = o.checkTriggerDeadCycle(targetIds);
	if(result){
		var msg = $.i18n("form.trigger.settings.dead.cycle.label", $.i18n("form.echoSetting.label"), result);
		$.confirm({
			'msg' : msg,
			ok_fn : function() {
				var param = $("#newEchoSettingContainer").formobj({domains:['echoSetting','refColumTable'],validate:false,matchClass:true,isGrid:true});
...
function echosetValidate(){
	var result = true;
	$(".triggerName","#newEchoSettingContainer").each(function(){
		if($(this).val()==""){
			$.alert("${ctp:i18n('form.echoSetting.triggername.error1.label')}");//回写名称不能为空！
			result = false;
			return false;
		}
	});
...
function checkDeadCycle(){
	...
	var result = o.checkTriggerDeadCycle(targetIds);
	if(result){
		//此种方式为采用原生的方式处理，样式不够统一
		var ss = confirm("当前回写设置有可能导致死循环，包括表单： " + result + "，请检查配置避免宕机风险。请确认是否继续？");
		if(ss){
		}else{
			return false;
		}
