    ...
    $(document).ready(function(){
        ...
        $("#saveTrigger").click(function(){ 
            ...
            if(state == "0"){
                ...
                if(result){
                    var msg = $.i18n("form.trigger.settings.dead.cycle.label", $.i18n("form.pagesign.trigger.label"), result);
                    $.confirm({
                        'msg' : msg,
                        ok_fn : function() {
                            saveTriggerFun($(this));
        ...
        $("#content","#cloneRow").click(function(){
        	chooseTriggerTemplate(_ctxPath + "/form/triggerDesign.do?method=triggerTemplate&templateId="+$("#templateId",currentTr).val(), 
        ...
        <c:forEach var="ac" items="${actionList }" varStatus="st">
            <c:if test="${ac.hasSettingPage() }">
                _hasSettingPageAction += "${ac.id }";
            </c:if>
        </c:forEach>
    });
