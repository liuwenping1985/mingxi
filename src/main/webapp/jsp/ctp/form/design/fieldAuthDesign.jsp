...
<html>
    ...
    <body>
       <form action="" id="saveForm">
            <!--左右布局-->
           	<div id="authset" style="text-align: left;left: 0px;overflow:hidden;height:400px;">
				<div class="form_area margin_lr_10 margin_t_10 margin_b_5 border_all" id="authFieldMap" style="width: 500px;height: 350px;overflow:auto;">
					...
					<tr id="${field.name }_tr">
						<th noWrap align="right"><label class="margin_l_10 margin_r_10">[<c:if test="${field.masterField}">${ctp:i18n("form.base.mastertable.label")}</c:if><c:if test="${!field.masterField}">${ctp:i18n("formoper.dupform.label")}${field.extraMap.subTableIndex }</c:if>]&nbsp${field.display }:</LABEL> </th>
           		...
           		<div id="groupEdit">
					<c:forEach var="table" items="${fb.tableList}" begin="1">
						<!-- 允许添加 -->
						<input tableName="${table.tableName }" displayName="${table.display }" type="hidden" name="${table.display }_allowAdd" id="${table.display }_allowAdd" value=""/>
						<!-- 允许删除 -->
						<input tableName="${table.tableName }" displayName="${table.display }" type="hidden"  name="${table.display }_allowDelete" id="${table.display }_allowDelete" value=""/>
					</c:forEach>
				</div>
            </div>
		</form>
