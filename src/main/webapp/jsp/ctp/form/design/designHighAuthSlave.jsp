...
<form id="operadd" method="post" action="" onSubmit="" name="operadd">
	<table id="groupShow" width="100%" height="0px" border="0" cellpadding="0" cellspacing="0" >
		<tr>
			<td>
				<fieldset style="width: 95%; height: 90%" align="center">
				<!-- 重复表操作 -->
										...
										<c:forEach var="table" items="${fb.tableList }" begin="1">
											<tr tableName="${table.tableName }">
												...
												<td width="32%" nowrap="nowrap"><label for="${table.display }_allowAdd">
													<!-- 允许添加 -->
													<input tableName="${table.tableName }" type="checkbox" name="${table.display }_allowAdd" id="${table.display }_allowAdd" />${ctp:i18n('form.authDesign.allowadd')}</label>
												</td>
												<td width="31%" nowrap="nowrap">
													<label for="${table.display }_allowDelete">
														<!-- 允许删除 -->
														<input tableName="${table.tableName }" type="checkbox"  name="${table.display }_allowDelete" id="${table.display }_allowDelete" />${ctp:i18n('form.operhigh.allowdel.label')} 
													</label>
												</td>
											</tr>
										</c:forEach>
