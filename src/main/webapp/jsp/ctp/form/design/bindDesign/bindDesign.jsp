...
<html>
    <head>
        ...
        <script type="text/javascript" src="${path}/common/form/design/bindDesign/bindDesign.js"></script>
    </head>
    	...
    	<div id='layout'>
	        ...
	        <form action="${path }/form/bindDesign.do?method=bindLogsAndCodeSave" id="saveCode" style="height: 80%">
				 ...
				 <div class="layout clearfix code_list padding_t_5 padding_l_10" style="height: 100%">
	            	<div <c:if test="${formBean.formType!=baseInfo }">class="col2 left"</c:if> id="bindSet" class="col2 " style="width:58%;">
							...
							<legend>${ctp:i18n('form.pagesign.appbind.label')}</legend>
								<table width="100%" border="0" cellpadding="2" cellspacing="0" id="triggerNameSet">
									...
									<tr height="30px" <c:if test="${formBean.formType==baseInfo }"> class="hidden" </c:if>>
										...
										<td nowrap="nowrap">
										    <div class=" w200" >
										    <input readonly="readonly" id="searchFieldList" name="searchFieldList" class="w100b" type="text" mytype="1" hideText="searchFieldNameList">
													...
													<c:forEach var="view" items="${formBean.formViewList }"
														...
														<tr>
															<td align="left" width="35%"><input class="browse"
																...
																name="browseCheck${status.index}" value="${view.id }" /> <span
																title="${view.formViewName }">${view.formViewName }</span></td>
	                ...
	                <div class="col2 margin_l_5 left <c:if test="${formBean.formType==baseInfo }">hidden </c:if>" style="width:40%;height: 100%">
	...
	$(document).ready(function(){
        ...
        if(!${formBean.newForm }){
        	new ShowTop({'current':'bind','canClick':'true','module':'bind'},$("#saveCode"));
        	new ShowBottom({'show':['doSaveAll','doReturn']},$("#saveCode"));
        }else{
        	new ShowTop({'current':'bind','canClick':'false','module':'bind'},$("#saveCode"));
        	new ShowBottom({'show':['upStep','nextStep','finish'],'source':{'upStep':'../report/reportDesign.do?method=index','nextStep':'../form/triggerDesign.do?method=index'}},$("#saveCode"));
        }
