  ...
  $(function() {
    ...
    function rend(txt, data, r, c) {
        if(c == 0){
          ...
          var id = data.id;
          if(data.status == 1){
              return "<a href='javascript:void(0)' onclick='showSuperviseLog(\""+id+"\")'>"+txt+"${ctp:i18n('supervise.superviseDetailList.secondary')}</a>";
          }else{
              return "<a href='javascript:void(0)' onclick='showSuperviseLog(\""+id+"\")'>"+txt+"${ctp:i18n('supervise.superviseDetailList.secondary')}</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:void(0)' class='ico16 notify_16' onclick='showWFCDiagram(top,\""+data.caseId+"\",\""+data.processId+"\",false,true,\""+data.id+"\",null,\"collaboration\")'></a>";
          }
      }else if(c==6){
  ...
  function getDateTime(awake){
    ...
    if(bdate.getTime() > edate.getTime()){
        var confirm = $.confirm({
            ...
            ok_fn: function () { 
		      sup.updateAwakeDate(changeAwakeID,awake);
              loadData();
            },
            cancel_fn:function(){
                confirm.close();
