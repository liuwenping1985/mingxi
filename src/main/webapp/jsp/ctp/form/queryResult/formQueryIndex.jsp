...
<script>
	...
	var treeParams = {
		...
		asyncParam : {formType:"1,2,3,5,7,8", templateCategoryId:"${param.templateCategoryId}"},
		</c:if>
		<c:if test="${formType != ''}">
    ...
    function getQueryParam(o){
    	...
    	treeParams.nodeHandler = function(n){
    		if (n.data.id == '2'|| isQuery || n.data.id == '4') {
    		      n.open = true;
    		    }
        ...
        treeParams.onAsyncSuccess=function(){
            $(".query_count_",$($("iframe")[0].contentWindow.document)).text(count);
        };
    ...
    function getMyQueryParam(o){
    	...
    	treeParams.nodeHandler = function(n){
    		if (n.data.id == '2'||(o!=null&&o!="")|| isQuery) {
    		      n.open = true;
    		    }
        };
...
<body class="h100b overflow_hidden page_color" id='layout'>
	...
	<c:if test="${formType!='4'}">
	    ...
	    <div class="layout_center " id="center" style="overflow:hidden;">
		    <c:if test="${formType eq ''}">
			<iframe frameborder="0" width="100%" height="100%" src="${path }/form/queryResult.do?method=queryMain&formType=1,2,3,5,7,8"> </iframe>
			</c:if>
			<c:if test="${formType != ''}">
			<iframe frameborder="0" width="100%" height="100%" src="${path }/form/queryResult.do?method=queryMain&formType=${formType }"> </iframe>
			</c:if>
    	</div> 	
	</div>
</body>
