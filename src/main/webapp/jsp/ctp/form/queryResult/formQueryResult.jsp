...
<script type="text/javascript" src="${path}/ajax.do?managerName=formQueryResultManager,ctpMainbodyManager,formManager"></script>
...
<script>
    ...
    if(isMyQuery){
      myConditionData = $.parseJSON('${userCondition}');
    }
    ...
    $("#center").resize(function(){
        ...
        $("table.flexme1").ajaxgrid().grid.resizeGrid(parseInt(h-h2-h3*2));//表格高度
    });
    ...
    if(!$.ctx.resources.contains('F01_newColl')){
     ...
     <c:if test = "${param.srcFrom ne 'bizconfig' and param.hidelocation ne 'false'}">
     try{
    ...
    function search(){
        ...
        if(showFieldList!=""){
           ...
           var userCondition = $("#userConditionTable").formobj({validate : false});
            //BDGW-2318 公文文单设置的文号查询，查询条件失效 做个防护
            for(var i=0;i<userCondition.length;i++){
                var oneCondition = userCondition[i];
                if(oneCondition.hasOwnProperty("fieldName")&&oneCondition.hasOwnProperty(oneCondition.fieldName)&&!oneCondition.hasOwnProperty("fieldValue")){
                    oneCondition["fieldValue"]=oneCondition[oneCondition.fieldName];
                }
            }
    ...
    function propCurrentCondition(){
      ...
      //处理各种状态  增加新增公文类型
      <c:if test="${queryBean.formBean.formType == 1 || queryBean.formBean.formType == 5  || queryBean.formBean.formType == 7 }">
      //流程表单
      show = '';
  ...
  function saveMyQuery(){
    ...
    var dialog = $.dialog({
        ...
        buttons : [ {
          ...
          id:"sure",
          handler : function() {
            if(!$("#myQueryTable").validate({errorAlert:true,errorIcon:false})){
              return;
            }
            $.setFieldValueSubmit();
            //修改更新
            ...
            <table width="100%" height="100px" border="0" cellspacing="0" cellpadding="0" align="center" id="allDataTable">
                ...
                <tr>
                    <td width="60%" valign="top">
                      ...
                      <tr id="stateTr" <c:if test="${queryBean.formBean.formType != 1&&queryBean.formBean.formType != 5&&queryBean.formBean.formType != 7 && queryBean.formBean.formType != 8}">class='hidden'</c:if>>
                          <td>
...
<table class="hidden form_area" id="myQueryTable" width="90%" height="100%">
  <tr>
    <td width='40%' style="text-align: right;"><input id="myqueryId" class="hidden" value="${queryId }">
    <input id="sameName" class="hidden" value="0">
    <input id="myOldQueryName" class="hidden" value='${type == "myQuery" ? queryBean.name : "" }'>
    <input id="myConditionHtml" class="hidden">${ctp:i18n('form.query.myplan')}：</td>
    <td><div class="common_txtbox_wrap" ><input value='${type == "myQuery" ? queryBean.name : "" }' id="myquery" name="${ctp:i18n('form.query.myplan')}" validate="type:'string',notNull:true,maxLength:60,avoidChar:'\\\'&&quot;&lt;&gt;'" class="validate"></div></td>
  </tr>
</table>
