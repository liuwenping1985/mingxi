...
(function($) {
	...
	$.fn.compChangeCondition = function(param2) {//修改字段触发事件
		...
		if(param2!= null&&(param2.inputType == "edocDocMark" || param2.inputType == "edocInnerMark" ||param2.inputType == "edocSignMark")){
			 var htmlStr = $("<input size='29' id='"+param2.fieldName+"' name='"+param2.fieldName+"' type='text' value='"+value+"' onchange='changeFieldValue(this)'/><input hidden='hidden' id='fieldValue' value='"+value+"' name='fieldValue' type='hidden'/>");
			 $(".fieldInputValue",objTr).html(htmlStr);
			 $(".operationDiv",objTr).html("<select id=\'operation\' class=\'operation  w100b\'><option  value=\'like\'>包含</option></select>");
		}else{
	...
	$.fn.compCondition = function(options) {//初始化全部
		...
		//if($(this).html()==""){
			...
			"</td>"+
			"<td style='width: 20%'>"+
			"<DIV class=common_selectbox_wrap><select onchange='$.changeField(this);' formId='"+param.formId+"' id='fieldName' name=\"${ctp:i18n('form.query.condition.label')}\" class='fieldName validate' validate=\"type:'string',notNull:true,errorMsg:'输入查询条件不能为空!'\">"+
				"<option selected value=''></option>";
			...
			"</DIV>"+
			"</td>"+
			"<td style='width: 30%' align='left' class='fieldInputValue'>"+
			...
			"</select></DIV>"+
			"</td>"+
			"<td  style='width: 50px'><span id='delButton' class='delButton repeater_reduce_16 ico16 revoked_process_16'>"+
		...
		"</tr>";
			...
			$(".fieldInputValue","#userConditionTable").css("width",""+(fsWidth-50)+"px");
			$("#userConditionTable").attr("widthAttr",fsWidth);
			resetWidthComp($("#userConditionTable"));
			$("#userConditionTable").comp();
			$(".addButton",$("#userConditionTable")).click(function(){
		      ...
		      $(".delButton",$("#userConditionTable")).click(function(){
	...
	$('.comp', comp).each(function(){
		  ...
		  if(ct && ct != '') {
		      ...
		      if(j.type == 'selectPeople') {
		          ...
		          if($.v3x.isChrome){
		              $t.css('height', '20px').css('width', (widthAttr-40)+"px");
      	  		 }else{
          		    $t.css('height', '20px').css('width', (widthAttr-30)+"px");
      	  		}
