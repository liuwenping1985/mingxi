...
<script type="text/javascript">
	function submitForm(){
	    ...
	    checkDataSource()
	   /*  if(checkForm(form)){ */
	   parent.detailFrame.location.href = "${urlU8UserMapper}?method=updateUserMapper&id=${member.v3xOrgMember.id}&valideLogin="+valideLogin+"&u8UserCodes="+u8UserCodes+"&oper=update&serverName="+serverName+"&dataSource="+document.getElementById("dataSource").value;
	    /* } */
	}
	
	function checkDataSource(){
		var serverSelcet = document.getElementById("serverName");
		var dataSource =  serverSelcet.options[serverSelcet.selectedIndex].getAttribute("dataSource");
    ...
    <fieldset style="width:95%;border:0px;" align="center">
		<table width="50%" border="0" cellspacing="0" cellpadding="0" align="center">
			<tr>
				...
				<input type="hidden" id="dataSource" name="dataSource" value="">
			<tr>
			  	<td class="bg-gray" width="45%" nowrap="nowrap"><fmt:message key="u8.server.label"/>:</td>
				<td class="new-column" width="55%" nowrap="nowrap">
					<c:if test="${oper != 'edit'}">
					<select name="serverName" id="serverName" onchange="checkDataSource();" class="input-100per" deaultValue="${u8User.u8ServerName}"
					     	...
					     	<c:if test="${server.has_u8==1}">
					     		<c:if test="${server.u8_server_name==u8User.u8ServerName}">
					     			<c:if test="${server.dataSource!=null }">
					     				<option value="${server.u8_server_name}" selected="selected" title="${server.u8_server_name}" dataSource='${server.dataSource }'>${server.u8_server_name}</option>
					     			</c:if>
					     			<c:if test="${server.dataSource==null }">
					     				<option value="${server.u8_server_name}" disabled="disabled" selected="selected" title="${server.u8_server_name}"  dataSource='${server.dataSource }'>${server.u8_server_name}<font><fmt:message key="u8.bind.noinitial.label"/></font></option>
					     			</c:if>
					     		</c:if>
					     		<c:if test="${server.u8_server_name!=u8User.u8ServerName}">
		                 			<c:if test="${server.dataSource!=null }">
						     				<option value="${server.u8_server_name}" title="${server.u8_server_name}" dataSource='${server.dataSource }'>${server.u8_server_name}</option>
						     			</c:if>
						     			<c:if test="${server.dataSource==null }">
						     				<option value="${server.u8_server_name}" disabled="disabled" title="${server.u8_server_name}"  dataSource='${server.dataSource }'>${server.u8_server_name}<font><fmt:message key="u8.bind.noinitial.label"/></font></option>
						     			</c:if>
		                 			</c:if>
	                 		</c:if>
					     </c:forEach>
