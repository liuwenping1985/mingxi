...
function valaccyear(year,type){
  ...
  return true;
    ...
    function check_form()
    {
        ...
        }/* else if(u8ServerName.indexOf("<")!=-1 || u8ServerName.indexOf(">")!=-1 || u8ServerName.indexOf("'")!=-1 || u8ServerName.indexOf("\"")!=-1 || u8ServerName.indexOf("#")!=-1 || u8ServerName.indexOf("%")!=-1 || u8ServerName.indexOf("&")!=-1 || u8ServerName.indexOf("$")!=-1){
        	alert('<fmt:message key="u8.server.detail.nameinvalid.error"/>');
        	return false;
        } */
        if(isU8Key()){
        ...
        /* var temp = login_date.split('-');
        if(parseInt(account_year) >= parseInt(temp[0])){
        ...
        try{
            ...
            alert("初始操作员设置保存异常，请检查！");
            return false;
        }
        var action = "/seeyon/u8ServerAndUserInfoController.do?method=saveU8ServerAndU8UserInfo&u8_server_name="+$("#u8_server_name").val();
        document.getElementById("userMapperForm").action = action;
...
<form id="userMapperForm" name= "userMapperForm" target="editMemberFrame" action=""   onsubmit="return check_form();" method="post" >
...
//                                  document.getElementById("u8_set").style.display="";
  ...
  <tr>
    ...
    <tr>
        ...
        <p align="center">
            <input id="submintButton" type="submit" style="width:70px;" value="<fmt:message key='common.button.ok.label' bundle="${v3xCommonI18N}" />" >&nbsp;
        </p>
        </td>
    </tr>
