...
<script type="text/javascript">
	...
	function autoSynchron(form){
		...
		if(handRadioObj.checked){
			...
			if(count <= 0 && ${v3x:getSysFlagByName('sys_isEnterpriseVer')!=true}){
				alert("<fmt:message key='nc.org.synchron.hand.selectOrg'/>");
				return false;
			}
		       var requestCaller= new XMLHttpRequestCaller(this, "ajaxU8OrgManager", "isSyningHand",false);
				 var ds= requestCaller.serviceRequest();
				 if(ds=='true'){
					 alert("<fmt:message key='nc.org.hand.syning'/>");
					 return false;
				 }
		       var requestCaller1= new XMLHttpRequestCaller(this, "ajaxU8OrgManager", "isSyningAuto",false);
				var ds1= requestCaller.serviceRequest();
				if(ds1=='true'){
						 alert("<fmt:message key='nc.org.auto.syning'/>");
						 return false;
					 }
			...
			if(document.getElementById("isStart1").checked && document.getElementById("intervalTimeRadio").checked){
				if(document.all.intervalDay.value=='0' && document.all.intervalHour.value=='0' && document.all.intervalMin.value=='0'){
					alert("<fmt:message key='nc.org.synchron.timeSetting.timeMustGT0'/>");
					return false;
				}
			}
	...
	function checkSynchState(synchType){
		...
		if(synchHandState){
			...
			window.setInterval("intervalometer()",20000);
		}
	}
	function init(){
...
<form action="${urlNCAutoSynch}?method=synchOperation" name="autoForm" method="post" onsubmit="return autoSynchron(this)">
<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" align="center">
	<tr>
		...
		<table width="90%" height="100%" border="0" cellspacing="0" cellpadding="0" align="center">
			<tr>
				<td height="40" nowrap>
					<b><fmt:message key="nc.org.synchron.type"/>:</b>&nbsp;&nbsp;
					<label for="handRadio">
						<input id="handRadio" type="radio" name="synchType" value="hand" ${synchType=='auto'? '':'checked'} onclick="switchSynchType('hand')"><fmt:message key="nc.org.synchron.type.hand"/>
					</label>&nbsp;&nbsp;
					<label for="autoRadio">
						<input id="autoRadio" type="radio" name="synchType" value="auto" ${synchType=='auto'? 'checked':''} onclick="switchSynchType('auto')"><fmt:message key="nc.org.synchron.type.auto"/>
					</label>
				</td>
			</tr>
			<tr>
				<td height="20" nowrap>
					<font color="red"><p id="synchState" style="display: none;"><fmt:message key="nc.org.synching"/></p></font>
				</td>
			</tr>
			...
			<tr>
				<td height="100%" valign="top">
					<fieldset style="vertical-align: top">
						<legend style="height: 20px; padding-top: 5px;"><b><fmt:message key="nc.org.synchron.option"/></b></legend>
						<table width="100%" height="300" id="handSynchOption">
							...
							<tr>
						   				...
						   				<tr>
						   					<td class="padding-L" valign="top" height="100%">
												<c:forEach items="${accountlist}" var="account" varStatus="status">
													<c:if test="${account.v3xOrgAccount.path != '0000'}">
												       <label for="chk${status.index}">
														<input id="chk${status.index}" type="checkbox" checked="true" onclick="return false"  name="orgIds" value="${account.v3xOrgAccount.id}"> ${account.v3xOrgAccount.name}
														</label><br/>
													</c:if>
												</c:forEach>
												<%-- 
			   				...
			   				<td height="30" style="padding-left: 30px;">
								<font color="red">*</font><fmt:message key="nc.org.hand.starttime" />:
								<input type="text" name="startTime" id="startTime" class="cursor-hand" inputName="startTime" validate="notNull" value="2000-01-01 00:00" onclick="whenstart('${pageContext.request.contextPath}',this,300,200,'datetime');" ${v3x:outConditionExpression(edit == "1", 'disabled', 'readonly')}/>
								</td>
							</tr>
						...
						<table width="100%" id="autoSynchOption" style="display: none;">
							<tr>
								<td width="60%" class="padding-L">
									<table width="100%">
										<tr>
								    		<td width="100" height="50" nowrap="nowrap" align="right"><font color="red">*</font><fmt:message key="nc.org.synchron.auto.enable" />：</td>
								    		<td colspan="2">
								    			<label for="isStart1">
			        			  		...
			        			  		<tr>
			        			  			<td rowspan="2" nowrap="nowrap" align="right">
			        			  				<font color="red">*</font><fmt:message key="nc.org.synchron.timeSetting"/>：
			        			  			</td>
											<td width="80" nowrap="nowrap"><label for="setTimeRadio"><input id="setTimeRadio" type="radio" name="synchTimeType" ${synchTimeType==0?'checked':''} ${isStart?'':'disabled'} value="setTime" onclick="clickSetTimeRadio()"><fmt:message key="nc.org.synchron.timeSetting.0"/>：</label></td>
											<td>
												<select id="typeSel" name="type" ${isStart?'':'disabled'}>
						        			      <option value="0" ${type == '0'?'selected':''}><fmt:message key="nc.org.auto.everyday" /></option>
						        			      <option value="1" ${type == '1'?'selected':''}><fmt:message key="nc.org.auto.sunday" /></option>
						        			      <option value="7" ${type == '7'?'selected':''}><fmt:message key="nc.org.auto.saturday" /></option>
						        			      <option value="6" ${type == '6'?'selected':''}><fmt:message key="nc.org.auto.friday" /></option>
						        			      <option value="5" ${type == '5'?'selected':''}><fmt:message key="nc.org.auto.thursday" /></option>
						        			      <option value="4" ${type == '4'?'selected':''}><fmt:message key="nc.org.auto.wednesday" /></option>
						        			      <option value="3" ${type == '3'?'selected':''}><fmt:message key="nc.org.auto.tuesday" /></option>
						        			      <option value="2" ${type == '2'?'selected':''}><fmt:message key="nc.org.auto.monday" /></option>
						        			    </select>
												...
												<fmt:message key="nc.org.auto.hour" />
						        			    ...
						        			    <fmt:message key="nc.org.auto.min" />
									        </td>
										</tr>
										<tr>
											<td nowrap="nowrap"><label for="intervalTimeRadio"><input ${isStart?'':'disabled'} id="intervalTimeRadio" type="radio" name="synchTimeType" ${synchTimeType!=0?'checked':''} value="intervalTime" onclick="clickIntervalTimeRadio()"><fmt:message key="nc.org.synchron.timeSetting.1"/>：</label></td>
											<td>
												...
												<fmt:message key="nc.org.synchron.timeSetting.day"/>
												...
												<fmt:message key="nc.org.synchron.timeSetting.hour"/>
						        			    ...
						        			    <fmt:message key="nc.org.synchron.timeSetting.min"/>
									        </td>
										</tr>
										</table>
										</td>
	...
	<tr>
		<td height="20" align="center" class="bg-advance-bottom" style="border: 0px">
			...
			<input type="button" name="cancelButton" id="cancelButton" onclick="getA8Top().backToHome()" value="<fmt:message key='common.button.cancel.label' bundle='${v3xCommonI18N}' />" class="button-default-2">
		</td>
	</tr>
</table>
</form>
