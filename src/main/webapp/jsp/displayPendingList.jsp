			...
			<c:forEach items="${columnHeaderList}" var="colHeader">
				...
				<c:if test="${colHeader eq 'theConferenceHost'}">
					<span class="left flagClass hand  text_overflow" id="${row.id}theConferenceHostId" title="${row.theConferenceHost}" style="padding-left:5px;width:54px;color: #111111">
                       	...
                       	<c:if test="${not empty row.theConferenceHost}"> 
				...
				<c:if test="${colHeader eq 'policy'}">
					<c:choose>
						<c:when test="${param.locale eq 'en'}">
							<span class="left flagClass text_overflow pendingNodeName" title="${row.policyName}" style="padding-left:8px;min-width:85px;color: #111111">
						</c:when>
						<c:otherwise>
				...
				<c:if test="${colHeader eq 'category'}">
					<c:choose>
						<c:when test="${param.locale eq 'en'}">
							<span class="left flagClass" title="${ctp:i18n(row.categoryLabel)}" style="padding-left:5px;width:75px;">
						</c:when>
						<c:otherwise>
							<span class="left flagClass" title="${ctp:i18n(row.categoryLabel)}" style="padding-left:5px;width:35px;">
						</c:otherwise>
					</c:choose>
					
						<c:choose>
							<c:when test="${row.hasResPerm eq true && not empty row.categoryLink}">
                                <c:choose>
                                    <c:when test="${row.categoryOpenType == 3}"><%-- 新窗口 --%>
	                                    <c:choose>
											<c:when test="${param.locale eq 'en'}">
												<a class="sendFlagClass text_overflow" style="width:80px;" onclick="javascript:openCtpWindow({'url':'${path}${row.categoryLink}'})">${row.categoryLabel}</a>
											</c:when>
											<c:otherwise>
												<a class="sendFlagClass text_overflow" style="width:40px;" onclick="javascript:openCtpWindow({'url':'${path}${row.categoryLink}'})">${row.categoryLabel}</a>
											</c:otherwise>
										</c:choose>
                                    </c:when>
                                    <c:otherwise>
                                    	<c:choose>
											<c:when test="${param.locale eq 'en'}">
												<a class="sendFlagClass text_overflow" style="width:80px;" href="javascript:openLink('${row.categoryLink}')">${row.categoryLabel}</a>
											</c:when>
											<c:otherwise>
												<a class="sendFlagClass text_overflow" style="width:40px;" href="javascript:openLink('${row.categoryLink}')">${row.categoryLabel}</a>
											</c:otherwise>
										</c:choose>
                                    </c:otherwise>
                                </c:choose>
				...
				<c:if test="${colHeader eq 'sendUser'}">
					<c:choose>
						<c:when test="${param.locale eq 'en'}">
							<span class="left flagClass text_overflow senderflag" style="padding-left:5px; min-width:80px;">
						</c:when>
						<c:otherwise>
							<span class="left flagClass text_overflow senderflag" style="padding-left:5px;">
						</c:otherwise>
					</c:choose>
						<span class='left hasIconSpan hand' style='padding:0px;margin:0;'>
	                        <span class="senderTitle sendUserName left text_overflow" style="text-align:left;" onclick="$.PeopleCard({'memberId':'${row.createMemberId}'})" id="${row.id}senderId" title="${ctp:toHTML(row.createMemberName)}" style="margin:0;">
                ...
                <c:if test="${colHeader eq 'preApproverName'}">
					<span class="left flagClass text_overflow senderTitle sendUserName" title="${ctp:toHTML(row.preApproverName)}" style="padding-left:5px;width:78px;">
	                       	${ctp:toHTML(row.preApproverName)}
                     </span>
				</c:if>
				...
				<c:if test="${colHeader eq 'subject'}">
					<span class="channel_title left" style="width:100px; display:inline-block;overflow:hidden;color: #111111">
                        ...
                        <%-- 测试字符3： ”“”“""""""ds"    --%>
						<a id="row${row.id}" class="color_black left hand ${row.subState eq 11 ? 'font_bold':''}" title="${ctp:toHTML(row.addition)}"
