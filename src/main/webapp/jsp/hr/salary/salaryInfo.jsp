			...
			<table id="salarylist" class="sort docellipsis" width="100%"  border="0" cellspacing="0" cellpadding="0" onClick="sortColumn(event, true)" dragable="true">
				...
				<tbody class="mxt-grid-tbody">
					<c:forEach items="${salarys}" var="sal">
						...
						<tr class="sort">
							<td width="30" align="center" class="cursor-hand" style="border-bottom: solid 1px #D7D7D7;"><input type="checkbox" name="id" value="${sal.salary.id}"></td>
...
try{
	 $(window).resize(function(){
	        if ($("body").height()-26 > 0) {
	            $(".scrollList").height($("body").height()-26);
	        }
	    });
	    if ( $("body").height()-26 > 0) {
	        $(".scrollList").height($("body").height()-26);
