...
//	getA8Top().showLocation(null, getA8Top().findMenuName(12), getA8Top().findMenuItemName(1202), "<fmt:message key='hr.staffInfo.postchange.label' bundle='${v3xHRI18N}'/>");
//}
//else{
//    getA8Top().showLocation(null, getA8Top().findMenuName(8), getA8Top().findMenuItemName(801), "<fmt:message key='hr.staffInfo.postchange.label' bundle='${v3xHRI18N}'/>");
//}
</script>
...
function add(){
     ...
     parent.detailFrame.window.location="${hrStaffURL}?method=initDetail&staffId=${staff.id}&isManager=${isManager}&isNew=New&infoType=6";
}
...
function modify(){ 
    ...
    /* if(getSelectId(this)){
		...
		parent.detailFrame.window.location='${hrStaffURL}?method=initDetail&staffId=${staff.id}&id='+id+'&isManager=${isManager}&infoType=6';
	 }else{
...
function viewPostChange(id){
	 parent.detailFrame.window.location='${hrStaffURL}?method=initDetail&id='+id+'&isReadOnly=ReadOnly&isManager=${isManager}&infoType=6';
} 
function del(){
    if("${isManager}"!="Manager"){
    ...
    if(getSelectIds(this)){
		...
		window.location='${hrStaffURL}?method=deletePostChange&staffId=${staff.id}&ids='+ids+'&isManager=${isManager}&infoType=6';
	}else{
		...
		<td colspan="2">
			<div class="scrollList">
				<form id="form1" method="post">
			        ...
			        <c:set var="click" value="viewPostChange('${bean.id}');"/>			       
					...
					></v3x:column>	
                    --%>
					<v3x:column width="10%" label="hr.staffInfo.postchange.startdate.label" type="String" onClick="${click}" className="cursor-hand sort">
					 		<fmt:formatDate value="${bean.start_time}" type="both" dateStyle="full" pattern="yyyy-MM-dd"  />
                    </v3x:column>
                    <v3x:column width="10%" label="hr.staffInfo.postchange.enddate.label" type="String" onClick="${click}" className="cursor-hand sort">
					 		<fmt:formatDate value="${bean.end_time}" type="both" dateStyle="full" pattern="yyyy-MM-dd"  />
					</v3x:column>
					<v3x:column width="25%" label="hr.staffInfo.postchange.post.label" type="String"
							value="${bean.post_name}" onClick="${click}" className="cursor-hand sort"
					 		alt="${bean.post_name}"
					></v3x:column>					
					<v3x:column width="20%" label="hr.staffInfo.postchange.wordnumber.label" type="String"
							value="${bean.wordnumber}" onClick="${click}" className="cursor-hand sort"
					 		alt="${bean.wordnumber}"
					></v3x:column>					
					<v3x:column width="25%" label="hr.staffInfo.postchange.organization.label" type="String"
...
<script type="text/javascript">
	showDetailPageBaseInfo("detailFrame", "<fmt:message key='hr.staffInfo.postchange.label' bundle='${v3xHRI18N}'/>", [2,4], pageQueryMap.get('count'),  v3x.getMessage("HRLang.detail_hr_120206"));
	</script>
</body>
</html>