...
<title><fmt:message key="system.password.protecd"></fmt:message> </title>
...
<form action="${urlHrViewSalary}?method=updatePassWord" target="submitFrame" name="setPassWord" id="setPassWord" method="post" onsubmit="return checkWrod()">
<table width="100%" height="100%" cellpadding="0" cellspacing="0" border="0" class="popupTitleRight" >
	<tr class="PopupTitle" height="20">
	  <td class="PopupTitle" colspan="2"><fmt:message key="system.password.protecd"></fmt:message></td>
	</tr>
	<tr>
	   <td height="20" colspan="2"></td>
	</tr>
	<tr>
		<td class="bg-gray" width="30%" nowrap="nowrap" align="right"><label for="post.code"><font color="red">*</font><fmt:message key="manager.formerpassword.notnull" />:</label></td>
		<td class="new-column"><input type="password" width="70%" id="oldPassWord" class="input-60per" inputName="<fmt:message key="manager.formerpassword.notnull" />" name="oldPassWord" minLength="6" maxLength="50" validate="notNull,minLength,maxLength" ></td>
	</tr>
	<tr>
		<td class="bg-gray" nowrap="nowrap" align="right"><label for="post.code"><font color="red">*</font><fmt:message key="manager.password.notnull" />:</label></td>
		<td class="new-column"><input type="password" id="newPassword" 
			<c:if test="${pwdStrengthValidation==1 }">
			onKeyUp="EvalPwdStrength(document.forms[0],this.value);" 
			</c:if>
		class="input-60per" inputName="<fmt:message key="manager.password.notnull" />" name="newPassword" minLength="6" maxLength="50" validate="notNull,isCriterionWord,minLength,maxLength"></td>
	</tr>
	<c:if test="${pwdStrengthValidation==1 }">
	<tr>
		<td class="bg-gray"  nowrap="nowrap" align="right"><label for="post.code"><fmt:message key="common.pwd.pwdStrength.label" bundle="${v3xCommonI18N }"/>:</label></td>
		<td class="new-column">
	...
	<tr>
		<td class="bg-gray" align="right" nowrap="nowrap"><label for="post.code"> <font color="red">*</font><fmt:message key="manager.validate.notnull" />:</label></td>
		<td class="new-column" >
	...
	<tr>
	     <td align="center" class="bg-advance-bottom" colspan="2">
	      <input type="submit"  class="button-default_emphasize" value="<fmt:message key='common.button.ok.label' bundle="${v3xCommonI18N}" />" id="b1" name="b1" />&nbsp;&nbsp;&nbsp;&nbsp;
