...
function doIt() {
	...
	if(description.length>80){
		alert("用途不能多于80个字,当前共有"+description.length+"个字.");
		return;
	}
		roomAppForm.submit();
}
...
<title><fmt:message key='mr.tab.app' bundle='${v3xMeetingRoomI18N }' /></title>
</head>
	...
	<table width="100%" height="100%"  border="0" cellspacing="5" cellpadding="0" align="center" style="font-size:12px" class="w100b h100b bg_color_white">
		...
		<tr height="30">
			<td valign="top" class="padding_l_30" colspan="2">
				<fmt:message key='mr.add.allowed' bundle='${v3xMeetingRoomI18N }' /><a class="color_blue" href="javascript:addRoomDesc()"><fmt:message key='mr.add.description' bundle='${v3xMeetingRoomI18N }' /></a>
                <c:if test="${hasMeetingArrangeMenu}">
                	<fmt:message key='mr.add.or' bundle='${v3xMeetingRoomI18N }' /><a class="color_blue" href="javascript:addMeetingByRoom()"><fmt:message key='mr.add.meeting' bundle='${v3xMeetingRoomI18N }' /></a>。
                </c:if>
			</td>
		</tr>
		...
		<tr height="140">
			<td valign="top" colspan="2" class="padding_l_10">
				<div>
					<fmt:message key='mr.add.description' bundle='${v3xMeetingRoomI18N }' /><span id="useIcon" onClick="javascript:showRoomDesc()" class="arrow_1_b"></span> <br />
				</div>
...
<div class="dialog_main_footer bg_color_gray w100b padding_t_5 margin_r_10 absolute align_right" style="width:97%">
	...
	<input type="button" value="<fmt:message key='common.button.cancel.label' bundle="${v3xCommonI18N}" />" class="button-default-2" onclick="exit()">		
</div>
