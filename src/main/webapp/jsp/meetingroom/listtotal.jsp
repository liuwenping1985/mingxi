...
<html>
	<head>
		...
		<script type="text/javascript">
		    //showCtpLocation("F09_meetingRoom",{surffix:"使用统计"});
		...
		function doExport(){
			...
			document.getElementById("hiddenIframe").contentWindow.location.href = "<html:link renderURL='/meetingroom.do?method=listTotalExport&startDatetime=${startDatetime}&endDatetime=${endDatetime}'/>";
		}
		</script>
		...
		<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
	   		<tr>
				<td>
	   			...
	   			<form name="searchForm" action="${mrUrl }?method=listTotal" method="post" onsubmit="">
    ...
    <v3x:table htmlId="listTable" data="list" var="bean" pageSize="${pageSize }" size="${size }" showHeader="true" showPager="true" isChangeTRColor="true">
		...
		<v3x:column width="36%" type="String" 
			...
			<c:out value="${bean.MeetingRoomRecord.meetingRoom.name }" />
			<c:if test="${bean.MeetingRoomRecord.meetingRoom.needApp == MRConstants.Type_MeetingRoom_NoNeedApp }">※</c:if>
		</v3x:column>
		...
		<v3x:column width="30%" type="String" align="center"
		label="${datefrom }${startDatetime }${dateto }${endDatetime }" className="cursor-hand sort" alt="">
