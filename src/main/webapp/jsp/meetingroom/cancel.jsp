...
<% } %>
<c:set value="listMyApp" var="listMethod" />
...
<frame  frameborder="no" src="${mrUrl}?method=meetListLeft&from=myApp&select=${select}" name="treeFrame" scrolling="yes" id="treeFrame"/>
	<frameset rows="40%,*" id="sx" name="sx" cols="*" framespacing="3" frameborder="no" border="0">
	  <frame frameborder="no" src="${mrUrl }?method=${listMethod }&select=${select}&flag=${flag==null?'yesApp':flag}" name="listFrame" scrolling="auto" id="listFrame" />
