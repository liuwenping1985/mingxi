    ...
    function getOperationTypeValue(){
    	...
    	function validate(){
			...
			function repealWorkflow(tWindow,caseId,processId,isTemplate,vWindow,appName,dialog){
	                ...
	                var lockWorkflowRe = lockWorkflow(processId, '${CurrentUser.id}', 12);
		                ...
		                if((isAdmin == true && (condition == "2" || condition =="19"))){
		                    ...
		                    if(canDealCancel.msg == "${ctp:i18n('collaboration.cannotRepeal_workflowHasDelivery')}" ){
		                    	$.alert("${ctp:i18n('collaboration.cannotRepeal_workflowHasDelivery')}");//该流程已分送，不能撤销！
		                    	releaseWorkflowByAction(processId, '${CurrentUser.id}', 12);
		                    	return;
		                    }
		                }
    	...
    	function showDetailLogDialog(summaryId,processId,showFlag){
    	    var dialog = $.dialog({
    	        ...
    	        url : _ctxPath+'/detaillog/detaillog.do?method=showDetailLog&summaryId='+summaryId+'&processId='+processId+"&showFlag="+showFlag+"&isAdminCheck=1",
    	        width : 800,
    	        height : 400,
    	...
    	function rend(txt,data, r, c) {
    		if(c===4){
    	        ...
    	        var name = "";
                var bianma = "";
				searchItem = data.edocType;
				if(searchItem == -1){
					searchItem = "\'\'";
				}
                var appEnumStr = data.appEnumStr;
                if(data.appEnumStr == 'govdocSend' || data.appEnumStr == 'govdocRec' ||data.appEnumStr =='govdocSign'){
                	name = "${ctp:i18n('edoc.element.shenpi')}";
	    	        bianma = "shenpi";
	    	        appEnumStr = 'collaboration';
                }else{
	    	        if(data.appEnumStr == 'edocSend' || data.appEnumStr=='signReport'||data.appEnumStr=="edocSign"){
	    	            name = "${ctp:i18n('edoc.element.shenpi')}";
	    	            bianma = "shenpi";
	    	        }else if(data.appEnumStr == 'recEdoc'||data.appEnumStr=="edocRec"){
	    	            name = "${ctp:i18n('edoc.element.yuedu')}";
	    	            bianma = "yuedu";
	    	        }
                }
                txt="<span class='ico16 process_16' onclick=\"$(\'#summaryIdValue\').val(\'"+data.summaryId+"\');$(\'#vouchValue\').val(\'"
	    	        	+data.vouch+"\');searchItem="+searchItem+";secretLevel="+data.secretLevel+";adminWFCDiagram(window.parent,\'"+data.caseId+"\',\'"+data.processId+"\',"+data.isFromTemplete+",window.parent,\'"
    	    			+appEnumStr+"\',repealWorkflow,stopWorkflow,\'"+data.accountId+"\',\'"+bianma+"\',\'"+name+"\',null,\'"+data.secretLevel+"\');\" title=''></span>";
    	    	return txt;
    	    }else if(c==7){
        ...
        $(document).ready(function () {
            ...
            $("#templateName").click(function(){
            	var item = $("#condition").find("option:selected").attr("item1");
				if(-1 == item){
					item = null;
				}
            	templateChoose(templateChooseCallback,item,true,"","MaxScope","",null);
           });
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="font_size12 margin_l_20 margin_t_10">
            ...
            <td width="220">
                <div class="common_selectbox_wrap">
                    <select id="condition" name="condition">
                        <option value="1" title="${ctp:i18n('application.1.label')}" item1='1'>${ctp:i18n("application.1.label")}</option><!-- 协同 -->
                        <option value="2" title="${ctp:i18n('collaboration.common.workflow.form')}" item1='2'>${ctp:i18n('collaboration.common.workflow.form')}</option><!-- 表单 -->
                        <!-- 判断是否有公文插件 -->
                        <c:if test ="${v3x:getSysFlagByName('edoc_notShow')!='true'}">
	                        <c:if test="${ctp:hasPlugin('edoc')}">
	                        	<option value="19" title="${ctp:i18n('collaboration.pending.lable2')}" item1='-1'>${ctp:i18n('collaboration.pending.lable2')}</option><!-- 发文 -->
	                        	<option value="20" title="${ctp:i18n('collaboration.pending.lable1')}" item1='-1'>${ctp:i18n('collaboration.pending.lable1')}</option><!-- 收文 -->
	                        	<!-- <option value="401" title="${ctp:i18n('collaboration.pending.lable11')}">${ctp:i18n('collaboration.pending.lable11')}</option> 发文 -->
	                        	<!-- <option value="402" title="${ctp:i18n('collaboration.pending.lable12')}">${ctp:i18n('collaboration.pending.lable12')}</option> 收文 -->
								<%--BDGW-1404 流程管理的应用类型列表中有签报--%>
	                        	<option value="21" title="${ctp:i18n('collaboration.pending.lable6')}" item1='-1'>${ctp:i18n('collaboration.pending.lable6')}</option><!--  签报 屏蔽签报 -->
	                        </c:if>
                        </c:if>
                    </select>
                </div>
        ...
        <tr style="height: 60px;">
            <td colspan="4" class="align_center">
            	<a href="javascript:void(0)" class="common_button common_button_gray" id="queryBtn">${ctp:i18n('template.templateChoose.inquiry')}<!-- 查询 --></a>
