...
<div class="clearfix padding_t_15"><span class="left"><em
...
<c:if test="${ctp:containInCollection(basicActionList, 'Opinion')}">
	<textarea id="content_deal_comment" name="content_deal_comment"
		class="padding_5 margin_t_5" errorIcon="false" style="width: 95%; height: 200px;font-size:14px;">${commentDraft.content}</textarea>
</c:if>
<div class="clearfix margin_t_10" id="attachmentAndDoc">
    ...
    <span class="right"><%-- 消息推送 --%>
        <a id="pushMessageButton" style="display: none;">
...
<div class="hr_heng margin_t_10">&nbsp;</div>
<div class="newinfo_area margin_t_5" id="fenfadanwei" style="display:none">${ctp:i18n('collaboration.msg.distributeCompany')}:&nbsp;
<input  type = "hidden"  id="fenfa_input_value" name="fenfa_input_value"/>
<input type="hidden" value="" id="isNotEdit" name = "isNotEdit"/>
<input class="w80b cursor-hand" value="<--${ctp:i18n('collaboration.msg.chooseCompany') }-->" type = "text" readOnly="true" id="fenfa_input" name="fenfa_input"/></div>
<c:if test="${ctp:containInCollection(basicActionList, 'Opinion')}">
...
<div align="right"><%--存为草稿 --%> <c:if
	...
	<c:choose>
		<c:when test="${ctp:containInCollection(basicActionList, 'ReSign')}">
			<!-- 签收 -->
			<a id="_dealSubmit"
