    ...
    */
    ...
    function OK(){
		...
		if(!(rule.test(superviseTitle))) {
            //督办主题包含特殊字符(\|\"<>')请重新录入!
			$.alert("${ctp:i18n('collaboration.common.common.supervise.specialCharacters')}");
			return false;
		}
        ...
        var returnValue = "";
		//BDGW-2714 回传unCancelledVisor，防止第二次进入督办设置时，系统自带的模板unCancelledVisor参数丢失
		var uncancel = $("#unCancelledVisor").val();
		if("true" == isSubmit){
		    ...
		    returnValue = "{\"supervisorIds\":\""+supervisorIds+"\",\"supervisorNames\":\""+supervisorNames+"\",\"awakeDate\":\""+superviseDate+"\",\"title\":\""+superviseTitle +
		        "\",\"detailId\":\""+detailId+"\",\"isSubmit\":\""+isSubmit+"\",\"appKey\":\""+superviseType+"\",\"entityId\":\""+entityId+"\",\"unCancelledVisor\":\""+uncancel+"\"}";
		    return returnValue;
		}else{
			...
			if(isTemplate && "true" == isTemplate){
		        ...
		        +"\",\"templateDateTerminal\":\""+superviseDate+"\",\"title\":\""+superviseTitle+"\",\"role\":\""+role+"\",\"roleNames\":\""+roleNames+"\",\"unCancelledVisor\":\""+uncancel+"\"}";
			}else{
			    returnValue = "{\"supervisorIds\":\""+supervisorIds+"\",\"supervisorNames\":\""+supervisorNames
		        +"\",\"awakeDate\":\""+superviseDate+"\",\"title\":\""+superviseTitle+"\",\"role\":\""+role+"\",\"roleNames\":\""+roleNames+"\",\"unCancelledVisor\":\""+uncancel+"\"}";
			}
	        return returnValue ;
		}
...
<form name="sendForm" id="sendForm" method="post">
	...
	<!-- 用该js文件中的隐藏域 -->
	<table border="0" cellspacing="0" cellpadding="0" class="font_size12" align="center">
	    <tr>
