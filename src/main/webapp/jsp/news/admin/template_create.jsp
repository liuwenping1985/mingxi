...
<head>
	<title>
		<c:if test="${bean.id!=null}"><fmt:message key="oper.edit" /></c:if><c:if test="${bean.id==null}"><fmt:message key="oper.new" /></c:if><fmt:message key="news.template" />
	</title>
...
<script type="text/javascript">
    	...
    	myBar.add(new WebFXMenuButton("submit", "<fmt:message key='common.toolbar.save.label' bundle='${v3xCommonI18N}' />", "if($('dataForm').onsubmit()){saveOffice(); isFormSumit = true;  validate(); $('dataForm').submit();}", "<c:url value='/common/images/toolbar/save.gif'/>", "", null));
    	...
    	myBar.add(new WebFXMenuButton("return", "<fmt:message key='common.toolbar.back.label' bundle='${v3xCommonI18N}' />", "location.href='${newsTemplateURL}?method=listMain';", "<c:url value='/common/images/toolbar/back.gif'/>", "", null));    	
...
<div class="newDiv">
<form id="dataForm" name="dataForm" action="${newsTemplateURL}" method="post" onsubmit="return checkForm(this)">
...
<input type="hidden" name="templateFormat" id="templateFormat" value="${bean.templateFormat}" />
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" align="center" class="CollTable">
		...
		<tr>
			<td class="label"><fmt:message key="news.template.templateName" /><fmt:message key="label.colon" /></td>
			<td class="value" colspan="3">
				<jsp:include page="../include/inputDefine.jsp">
					<jsp:param name="_property" value="templateName" />
					<jsp:param name="_key" value="news.template.templateName" />
					<jsp:param name="_validate" value="notNull,isDefaultValue" />
				</jsp:include>
			</td>
			<%-- 
			<td class="label"><fmt:message key="news.template.usedFlag" /><fmt:message key="label.colon" /></td>
			<td class="value">			
				<label for="usedFlag1">
		...
		<tr>
			...
			<td class="label"><fmt:message key="news.template.createDate" /><fmt:message key="label.colon" /></td>
			<td class="value"><fmt:formatDate value="${bean.createDate}" pattern="${datePattern}"/></td>
		</tr>
		</c:if>
		<tr>
			<td class="label"><fmt:message key="news.template.description" /><fmt:message key="label.colon" /></td>
			<td class="value" colspan="3">	
				<jsp:include page="../include/inputDefine.jsp">
					<jsp:param name="_property" value="description" />
					<jsp:param name="_key" value="news.template.description" />
