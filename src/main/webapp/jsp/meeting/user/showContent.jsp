...
<head>
	...
	<script type="text/javascript">
		...
		function summaryToCol(app){
            ...
            if(app == 'edoc'){
            	//会议纪要转新公文  ltao
				url = "collaboration/collaboration.do?method=newColl&app=4&sub_app=1&_resourceCode=F20_newSend&meetingSummaryId=${summary.id}";
				window.open(url,'_blank');
				return;
            	//url = "edocController.do?method=entryManager&entry=sendManager&toFrom=newEdoc&meetingSummaryId=${summary.id}";
            }else if(app == 'coll'){
            	url = "${mtSummaryURL}?method=summaryToCol&id=${summary.id}";
            }
...
<body class="body-bgcolor" style="overflow:auto;" onload="init()">
								...
								<div class="div-clear" style="width: 100%;">
									<div class="optionWriterName">
													<div class="div-float font-12px"
														style="padding-left: 20px; padding-bottom: 2px; color: #335186;">
														<%-- <span class="cursor-hand" id="replySpan${mtReply.id }"
															replyUserId="${mtReply.userId}"
															onclick="showV3XMemberCardWithOutButton('${mtReply.userId}')">
                                                            <b><c:out value="${mtReply.userName}" /></b>
														</span> --%>
                                                        <span class="cursor-hand" id="replySpan${mtReply.id }"
                                                            replyUserId="${mtReply.userId}"
                                                            onclick="displayPeopleCard('${mtReply.userId}', '${mtReply.validByUser}')">
                                                            <b><c:out value="${mtReply.userName}" /></b>
                                                        </span>
														...
														&nbsp;
														<b><c:if test="${!isImpart && mtReply.feedbackFlag !=3 }">
														<fmt:message
																key="mt.mtReply.feedback_flag.${mtReply.feedbackFlag }" />
														</c:if>
														</b> &nbsp;&nbsp;
														<fmt:formatDate value="${mtReply.readDate}"
															pattern="${datePattern}" />
													</div>
													<c:if test="${(meetingState eq 10 || meetingState eq 20) && param.isQuote!='true'}">
														<div class="div-float-right" style="padding-right: 20px;">
				                                            <%-- <span class="like-a" onclick="reply('${opinion.id}', '${opinion.writeMemberId}', ${isUploadAtt}, ${isUploadDoc},null,'${opinionMemberName}','${opinion.affairId}','${writeMemberId }')"> --%>
				                                            <span class="like-a" onclick="opinion_reply('${mtReply.id}');">
				                                               <font size="2"><fmt:message key="topic.reply.label"/></font>
				                                            </span>
														</div>
                                        			</c:if>
									</div>
									<div class="optionContents" style="width:678px;padding: 2px 20px 2px 20px; word-break:break-all;word-wrap:break-word;">
										${v3x:toHTML(mtReply.feedback)}&nbsp;
									</div>
									<table border="0" cellpadding="0" cellspacing="0" width="100%" align="center">
									<tr>
									 <td class="align_right" width="80" style="display:none;" id="attachment1Tr${mtReply.id}">
									 <div class="atts-label" style="width:100%;" align="right">${attachmentLabel}:(<span class="font-12px" id="replyAttachmentNumberDiv${mtReply.id}"></span>)</div></td>
									 <td>
									...
									<div class="atts-label"  style="width:100%;" align="right">${mydocumentLabel}:(<span class="font-12px" id="replyDocachmentNumberDiv${mtReply.id}"></span>)</div>
									</td>
									<td>
									<div class="div-float"  style="float:none;">
											<script type="text/javascript">
                                    ...
                                    <!-- <div style="display: inline-block;"></div> --><%--解决回执信息的附件DIV层与意见回复层合并的错误 xiangfan --%>
                                    <%-- 回复意见 --%>
                                    <%-- <c:if test="${!empty comments[mtReply.id]}"> --%>
                                     ...
                                     <%-- <div class="comment-div-mercury" id="opinDiv${mtReply.id}" style="padding-bottom: 10px;"> --%>
                                     <!-- <div class="comment-div-mercury" style="height:100%; padding-bottom: 6px;"> -->
                                     ...
                                     <div id="replyCommentDiv${mtReply.id}">
                                        <c:forEach items="${comments[mtReply.id]}" var="comment">
                                         <div class="reply_message_con">
                                            <div class="comment4-div-mercury" id="Anchor${comment.id}" title="${v3x:showMemberName(comment.createUserId)}">
                                            <c:choose>
                                            <c:when test="${!empty comment.proxyId}">
                                                <span id="smember${num}"  class="reply_member" name="opinion${comment.createUserId}" onclick="displayPeopleCard('${comment.proxyId}','${comment.validByUser}')" class="reply_member">${v3x:showMemberName(comment.proxyId)}
                                                <font color="red" style="font-weight: lighter;">(<fmt:message key="mt.agent.label1"><fmt:param value="${v3x:showMemberName(comment.createUserId)}" /></fmt:message>)</font>:
                                            </c:when>
                                            <c:otherwise>
                                                <span id="smember${num}"  class="reply_member" name="opinion${comment.createUserId}" onclick="displayPeopleCard('${comment.createUserId}','${comment.validByUser}')" class="reply_member">${v3x:showMemberName(comment.createUserId)}:
                                            </c:otherwise>
                                            </c:choose>
                                            </span>
                                            <c:set var="num" value="${num+1}"/>
                                            <span  class="reply_data"><fmt:formatDate value="${comment.createDate}" pattern="${datePattern}"/></span>
                                            </div>
                                            <div class="comment-content-cols wordbreak clearFloat" style="width: 678px; word-break: break-all; word-wrap: break-word;">
        ...
        <table width="98%" height="100%" border="0" cellspacing="0" cellpadding="0" id="reply-table" style="margin: auto;font-family:'宋体'">
           ...
           <tr>
              <td align="right" colspan="2" height="25">
                <div style="float: left;margin-top:3px;*margin-top:5px;" class="description-lable font-12px"><fmt:message key="common.charactor.limit.label" bundle="${v3xCommonI18N}"><fmt:param value="500" /></fmt:message></div>
                ...
                <span id="moreHiddenOption" class="font-12px" style="display: none">
                    ...
                    <input id="showToIdInput" type="text" readonly="readonly" size=15 style="height: 21px" value="${v3x:showMemberNameOnly(MtCreateUserId)}" name="showToIdInput" onclick="selectPeopleFun_commentHidden()">
                    ...
                    <v3x:selectPeople panels="Department,Team,Post,Outworker,RelatePeople"
                        ...
                        selectType="Member"
                        originalElements="${senderids}"
                        id="commentHidden" minSize="1" />
                        &nbsp;
                </span>
               <%-- 消息推送 --%>
                <label for="" id="" style="display:inline" class="font-12px">
                    <input type="checkbox" onclick="showMoreHiddenOption(this,'moreHiddenMessage')"><fmt:message key="common.message.push.label"/>
                </label>
                <span id="moreHiddenMessage" class="font-12px" style="display: none">
                    &nbsp;
	...
	<table align="center" border="0" cellspacing="0" cellpadding="0" class="body-detail">
		<tr>
			<td height="30" width="10%" align="">
				<div class="body-detail-su" style="padding-left: 15px;"><fmt:message key="mt.meetingrecord"/>
		...
		<!-- xiangfan 2012-04-01 添加纪要附件和关联文档start -->
		<tr >
				<td class="bg-gray" >
		...
		<tr >
		 ...
		 <!-- xiangfan 2012-04-01 添加纪要附件和关联文档end -->
