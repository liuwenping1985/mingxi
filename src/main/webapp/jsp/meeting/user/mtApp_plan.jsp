...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<title>
	<c:choose>
		<c:when test="${param.readonly eq 'readonly'}">
...
<table class="popupTitleRight" width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td class="bg-advance-middel" style="padding:26px 15px 12px;">
			<textarea  <c:if test="${param.readonly ne 'readonly'}">onfocus="if(this.innerHTML=='${planInput}'){this.innerHTML='';this.style.color='#000'}" onblur="if(this.innerHTML==''){this.innerHTML='${planInput}';this.style.color='#D1D1D1'}" </c:if> style="height:84%;width: 470px;padding:6px;" cols="90" id="plan" name="plan" <c:if test="${param.readonly=='readonly'}">readonly="readonly"</c:if> inputName="<fmt:message key='meeting.mtMeeting.plan' />">${content}</textarea>
		</td>
	</tr>
	<tr>
