...
function openNotice() {
	...
	var url = "mtAppMeetingController.do?method=openNotice&type=view&readonly=readonly&id=${bean.id}&mtType=meeting_app&ndate="+new Date();//xiangfan 修改了参数，修复GOV-3025
...
function openPlan(){
	...
	feacture = feacture + "directories:no; localtion:no; menubar:no; status:no;";
	feacture = feacture + "toolbar:no; scroll:no; resizeable:no; help:no";
	var url = "mtAppMeetingController.do?method=openPlan&type=view&readonly=readonly&id=${bean.id}&mtType=meeting_app&ndate="+new Date();//xiangfan 修改了参数，修复GOV-3025
	var retObj = window.showModalDialog(url,window ,feacture);
}
