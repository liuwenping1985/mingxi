	...
	<td align="center" valign="top">
		<div class="scrollList">
			<table width="60%" height="100%" align="center" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td valign="middle">
					<fieldset><legend><fmt:message key="mt.mtMeeting.Conference.review.label"/></legend>
						<% if(com.seeyon.v3x.meeting.contants.MeetingConfig.HAS_MEETING_APP) { %>
						<table width="100%" cellpadding="4" cellspacing="6" border=0>
							<tr>
								<td width="100px"><fmt:message key="mt.mtMeeting.review.label"/>：<br><fmt:message key="mt.admin.lable.people"/></td>
						...
						<table width="100%" cellpadding="4" cellspacing="6" border=0>
							<tr>
								<td width="100px"><fmt:message key="mt.mtMeeting.access.leadership.label"/>：</td>
								<td><textarea id="edocSendCreatesName" rows="5" class="new-column cursor-hand" name="edocSendCreatesName" readonly style="width:100%" onclick ="openSelectDepartWin();">${v3x:showOrgEntitiesOfTypeAndId(edocSendCreates,pageContext)}</textarea></td>
							</tr>
						</table>
						<<% } %>
...
<v3x:selectPeople id="edocSendAcc" panels="Department,Post,Level" selectType="Account,Department,Member,Post,Level,Team" departmentId="${sessionScope['com.seeyon.current_user'].departmentId}" jsFunction="setEdocCreateFields(elements)" viewPage=""  showAllAccount="false" />
