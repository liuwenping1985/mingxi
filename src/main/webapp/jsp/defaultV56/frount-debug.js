...
var frountSpace = {
	headerHeight:50,//banner高度
	menuHeight:48,//菜单高度
	locationHeight:48,//当前位置高度
	secondMenuTop:98//(headerHeight+menuHeight)
}
jQuery.extend({
	...
	initLayout:function(){
        var _w = $('.warp').height();
        frountSpace.wrapeSpace = _w;
		if ($.browser.msie) {
            if ($.browser.version == '7.0') {
				$('.layout_content').height(frountSpace.wrapeSpace-frountSpace.headerHeight-frountSpace.menuHeight);
				$('.layout_content_main_content').height(frountSpace.wrapeSpace-frountSpace.menuHeight-frountSpace.headerHeight-frountSpace.locationHeight);
				$('#main').height(frountSpace.wrapeSpace-frountSpace.headerHeight-frountSpace.menuHeight-frountSpace.locationHeight);
            }
        }
	},
	pageMenu:function(menus){
		
		//alert(window.screen.width)//分辨率区分
		var _width = $('body').width();
		var _shortCutWidth = _locale == 'en'?130:96;//我的快捷
		var _agentWidth = hasAgent=='true'?_shortCutWidth:0;//代理事项
		if("true" == isAdmin){
			_shortCutWidth = _agentWidth = 0;
		}
		var _prevWidth = 32;//前一页
		var _nextWidth = 32;//下一页
		var _navWidth = _width-200;//200为左侧人员头像和在线人数宽度
		var _menuNavWidth = _navWidth-_shortCutWidth-_agentWidth-_prevWidth-_nextWidth;
		if(menus == null || menus.length == 0){
			$('#nextli').hide();
			$('#prevli').hide();
			$('#layout_nav_right').width(_navWidth);//设置右侧菜单区域宽度
			return;
		}
		var _menuWidth = {1:56,2:56,3:56,4:56,5:70,6:90,7:100};//字数和对应的宽度
		var _paddingLR = 20;
		var _isPager = true;
		var _pager = $('#mainNavUi').attr("pager");
		var _pages = [0];
		var _wholeWh = 0;
		var _pageNum = 1;
		if(_locale == 'en'){
			$('#shortcutli').width(_shortCutWidth);
			$('#shortcutli_a').width(_shortCutWidth-_paddingLR*2);
			if(hasAgent == 'true'){
				$('#agentli').width(_agentWidth);
				$('#agentli_a').width(_agentWidth-_paddingLR*2);
			}
			for(var _p = 0;_p<menus.length;_p++){
				var _temp = menus[_p];
				var _width = 90+_paddingLR*2;//90+40
				_wholeWh+=_width;
				//alert(_wholeWh+"==="+_temp.name+"==="+_menuNavWidth)
				if(_wholeWh>_menuNavWidth){
					_pages.push(_p);
					_wholeWh = _width;
					_pageNum+=1;
				}
			...
			for(var _p = 0;_p<menus.length;_p++){
				var _temp = menus[_p];
				var _width = _menuWidth[(_temp.name.length>7?7:_temp.name.length)]+_paddingLR*2;
				_wholeWh+=_width;
				if(_wholeWh>_menuNavWidth){
					_pages.push(_p);
					_wholeWh = _width;
		...
		$('#nextli').unbind().click(function(ev){
			if(_pager == _pageNum){
				return;
			}else{
				_pager++;
			...
			$.initNavigation(menus.slice(_pages[_pager-1],_pages[_pager]));
			ev.stopPropagation();
		});
		$('#prevli').unbind().click(function(ev){
			if(_pager == 1){
				return;
			}else{
				_pager--;
		...
		$('#layout_nav_right').width(_navWidth);//设置右侧菜单区域宽度
		$('#navUi').width(_navWidth);//整个菜单区域包含我的快捷和代理事项
		$('#mainNavLi').width(_menuNavWidth);//菜单ui【去除我的快捷和代理事项】
		$.initNavigation(menus.slice(_pages[_pager-1],_pages[_pager]));
	},
	...
	initNavigation:function(menusArray){
		...
		if(_mainheight == 0){_mainheight = document.documentElement.clientHeight}
		var _submmh = 39;
		...
		for(var i = 0;i<menu.length;i++){
			var _temp = menu[i];
			var _id = _temp.id;
			var _name = escapeStringToHTML(_temp.name,false);
			var _width = _menuWidth[(_name.length>7?7:_name.length)];
			if(_locale == 'en'){
				_width = 90;//90
			}
			_htmlStr+="<li id='"+_id+"' title='"+_name+"' class='nav_item'><a _width='"+_width+"' href='javascript:void(0)' style='width:"+_width+"px'>"+_name+"</a></li>";
		}
		$('#mainNavUi').empty().html(_htmlStr);
		...
		$('.nav_item').each(function(){
			$(this).mouseenter(function(e){
				if(getCtpTop().systemProductId != 7){
					...
					$(this).addClass('current_item_a6s');
				}
				var _id = this.id;
				$.setOffice('hidden');
		        setTimeout(function(){
				...
				if(getCtpTop().systemProductId != 7){
					...
					$(this).removeClass('current_item_a6s');
				}
		        setTimeout(function(){
		          if(hideSecondMenu){
	    ...
	    $('#second_menu').unbind().mouseenter(function(){
	      ...
	      if(currentId!=null){
	       ...
	       if(getCtpTop().systemProductId != 7){
				...
				_mainMenuLi.addClass('current_item_a6s');
			}
	      }
	    }).mouseleave(function(){
	      hideSecondMenu = true;
	      ...
	      if(currentId!=null){
	        ...
	        if(getCtpTop().systemProductId != 7){
				...
				_mainMenuLi.removeClass('current_item_a6s');
			}
	      }
	    });
		//初始化二级菜单
		function initSecondMenu(_id){
			...
			if (_menu && _menu.items && _menu.items.length > 0) {
		        ...
		        _second_menu.append("<div class='upaction'></div>");
		        ...
		        for (var g=0; g<_menu.items.length; g++) {
		          ...
		          var imagesrc = _ctxPath + "/main/skin/frame56/"+skinPathKey+"/menuIcon/nav_second_ico.png";
				  if(_item.icon){
					//组织机构设置特殊处理
				  	if(_item.icon == "unitmanagement.png"){
				  		imagesrc = _ctxPath + "/main/skin/frame56/"+skinPathKey+"/menuIcon/unitmanagement_b.png";
				  	}else{
				  		if(_item.icon.indexOf("fileUpload.do")>0){
							imagesrc = _ctxPath + _item.icon;
						}else{
							imagesrc = _ctxPath + "/main/skin/frame56/"+skinPathKey+"/menuIcon/"+_item.icon;
						}
				  	}
				  }
...
if((_left + _thiswidth + 210)>document.body.clientWidth){
	//超出屏幕宽度
	//alert("超出");
    _third_menu.css({
      'width':210,
    ...
    _third_menu.css({
      'width':210,
      left:$('#second_menu').width()-2,
      top:_top-2
    }).show();
	var _topIframe = _top+_sd.top;
    $('#third_menu_iframe').css({
       'width':210,
      'left':_offset.left+$('#second_menu').width()-4,
      'top':_topIframe,
      'height':_third_menu.height()
    }).show();
}
...
if((_left + _thiswidth + 137 + 15)>document.body.clientWidth){
	//超出屏幕宽度
	//alert("超出");
	_third_menu.css({
		'width':137+15,
	  ...
	  'height':_third_menu.height()
				  ...
				  if(e.target.className=='second_menu_item_name' || e.target.className=='second_menu_item_name_next'){
				  	if(getCtpTop().systemProductId != 7){
				  		...
				  		$('.current_flag_a6s').removeClass('current_flag_a6s');
				  		$('#'+currentId).addClass("current_flag_a6s");
				  	}
				   	$('#second_menu').hide();
				    $('#second_menu_iframe').hide();
				    $.setOffice('visible');
	...
	initMyShortcut:function(myShortcuts){
		...
		//快捷绑定事件
		$('#shortcutli').unbind().mouseenter(function (ev){
			if(getCtpTop().systemProductId != 7){
				$(this).addClass('current_item');
			}else{
				$(this).addClass('current_item_a6s');
			...
			$('.shortcut_list ul li').mouseenter(function (ev){
				//if(skinPathKey=="GOV_red"){}else{}
				...
				if(getCtpTop().systemProductId != 7){
		  		$('.current_flag').removeClass('current_flag');
		  		$('#shortcutli').addClass("current_flag");
		  	}else{
		  		$('.current_flag_a6s').removeClass('current_flag_a6s');
		  		$('#shortcutli').addClass("current_flag_a6s");
		  	}
				_shortcutPop.hide();
		...
		_shortcutPop.mouseenter(function (){
			$(this).show();
			if(getCtpTop().systemProductId != 7){
				$('#shortcutli').addClass('current_item');
			}else{
				$('#shortcutli').addClass('current_item_a6s');
			}
			_shortcutIframe.show();
		}).mouseleave(function (){
			$(this).hide();
			if(getCtpTop().systemProductId != 7){
				$('#shortcutli').removeClass('current_item');
			}else{
				$('#shortcutli').removeClass('current_item_a6s');
			}
			_shortcutIframe.hide();
		});
		//代理事项绑定事件
		if("true" == hasAgent){
			$('#agentli').mouseenter(function (){
				if(getCtpTop().systemProductId != 7){
					$(this).addClass('current_item');
				}else{
					$(this).addClass('current_item_a6s');
				...
				if(getCtpTop().systemProductId != 7){
		  		$('.current_flag').removeClass('current_flag');
		  		$('#agentli').addClass("current_flag");
		  	}else{
		  		$('.current_flag_a6s').removeClass('current_flag_a6s');
		  		$('#agentli').addClass("current_flag_a6s");
		  	}
		...
		for(var i = 0; i < _shortcutNum; i++)
		{
			...
			if(myShortcuts[i].iconKey){
				shortcutIcon = _ctxPath + '/main/skin/frame56/'+skinPathKey+'/menuIcon/'+myShortcuts[i].iconKey;
			}else{
				shortcutIcon = _ctxPath + '/main/skin/frame56/'+skinPathKey+'/images/space_ico.png';
			}
	...
	initPulldownInfor:function(pullDownInfors,_top,_right,_pulldownLiWidth){
		...
		$("#mySet").click(function(){
	      ...
	      var html = "<span class='nowLocation_ico'><img src='/seeyon/main/skin/frame56/harmony/menuIcon/personal.png'/></span>";
  ...
  saveHotSpots : function(callback,data){
	...
	//消息提醒 计算IE6下内容区域---是否加滚动条
	initIsIE6Scroll:function(){
		if ($.browser.msie) {
			if ($.browser.version == '6.0') {
				$(".msg_main_box").css("height","auto");
				var _msgMainHeight = $(".msg_main_box").height();
				if(_msgMainHeight > 340){
					$(".msg_main_box").css("height","340px");
				}
			}
		}		
	},
  ...
  initHotspots : function(){
    ...
    if(getCtpTop().systemProductId != 7){
	    if($.ctx.hotspots.topBgImg){
  		...
  		$(".area").css({
    		"background-image": "url(\'" + _ctxPath + "/main/skin/frame/"+ skinPathKey + "/images/a6s_layout_header2.jpg" + "\')",
    ...
    if($.ctx.hotspots.mainBgImg){
    	if($.ctx.hotspots.mainBgImg.hotspotvalue == ""){
        	...
        	getCtpTop().$(".warp").css("background-image", "url(\'" + _ctxPath + "/" + $.ctx.hotspots.mainBgImg.hotspotvalue + "\')");
    			...
    			getCtpTop().$(".warp").css({
    				"background-image":"url(\'" + _ctxPath + "/main/skin/frame/"+ skinPathKey + "/images/a6s_layout_main2.jpg" + "\')",
    				"background-size":"100% 100%",
    ...
    if($.ctx.hotspots.topBgImg){
      ...
      if(oldHref != _ctxPath + "/main/skin/frame56/" + $.ctx.hotspots.topBgImg.ext10 + "/default.css"+resSuffix){
        getCtpTop().skinPathKey = $.ctx.hotspots.topBgImg.ext10;
        $("#skinFrameCSS").attr("href", _ctxPath + "/main/skin/frame56/" + $.ctx.hotspots.topBgImg.ext10 + "/default.css"+resSuffix);
      }
    }
  }
});
...
function viewTaskInfo4Event(id, actionAfterClose,category) {
  ...
  if (res != null && res != "") {
    ...
    var detailUrl=_ctxPath+"/taskmanage/taskinfo.do?method=openTaskDetailPage&isTimeLine=1&category="+category+"&taskId="+id;
...
function setNavMenuColor(colorValue, colorOpacity, ext10){
	var rgbValue = colorHexToRgb(colorValue);
	if($.browser.msie && ($.browser.version == '8.0' || $.browser.version == '7.0' || $.browser.version == '6.0')){
		if(colorValue == "#02162a" && ext10 == "harmony"){
			rgbValue = "#022c55";
		} else if(colorValue == "#02162a" && ext10 == "peaceful"){
			rgbValue = "#0a3829";
		} else if(colorValue == "#002149" && ext10 == "tint"){
			rgbValue = "#002149";
		} else if(colorValue == "#02162a" && ext10 == "wisdom"){
			rgbValue = "#412845";
		} else if(colorValue == "#02162a" && ext10 == "morning"){
			rgbValue = "#1f2627";
		} else if(colorValue == "#02162a" && ext10 == "material"){
			rgbValue = "#472900";
		}
	...
	if(getCtpTop().systemProductId != 7){
    getCtpTop().$("#layout_nav").css("background-color", rgbValue);
  }else{
  	getCtpTop().$("#layout_nav").css({
  		"background-image":"url(\'" + _ctxPath + "/main/skin/frame/"+ skinPathKey + "/images/a6s_layout_nav.png" + "\')",
...
$(document).ready(function () {
  ...
  $(".return_ioc").click(function(){
    ...
    $("#desktopIframe").show();
    $(this).css("background","");
    imgDisplay = $("#layout_header_bg").detach();
  });
...
function showSpace(i,spaceId,path,type,openType,reloadFlag){
  ...
  //去除我的快捷的选中效果
  if(getCtpTop().systemProductId != 7){
		$("#shortcutli").removeClass('current_flag');
  	$("#agentli").removeClass('current_flag');
	}else{
		$("#shortcutli").removeClass('current_flag_a6s');
  	$("#agentli").removeClass('current_flag_a6s');
	}
  //个人类型空间判断  type== "0" || type == "5" || type == "10" || type == "15" || type == "16"
  if(path.indexOf("ViewPage=plugin/nc/A8")!= -1){
    ...
    var html = '<span class="nowLocation_ico"><img src="'+_ctxPath+'/main/skin/frame56/'+skinPathKey+'/menuIcon/'+currentSpaceType+'.png"></span>';
    html += '<span class="nowLocation_content">';
    html += '<a>NC</a>';
    html += '</span>';
    showLocation(html);
...
function setEBMessageToArea(){
    ...
    if(ebMsgArea.size()>0){
        ...
        _bottom_eb += '<div class="msg_setting_uc eb_open" onclick="openEBCenter();">'+$.i18n("portal.open.dajiawork.js")+'</div>'+//打开大家社区
