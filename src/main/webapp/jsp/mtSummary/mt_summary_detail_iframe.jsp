...
<frame noresize="noresize" src='<c:url value="/common/detail.jsp" />' scrolling="no">
  <frameset  rows="*" cols="*,45" framespacing="0" id="zy">
    <frame frameborder="no" src="${mtSummaryURL}?method=detail&mId=${mId}&recordId=${recordId}&hiddenAuditOpinion=${hiddenAuditOpinion}&listType=${param.listType}" name="detailMainFrame" id="detailMainFrame" scrolling="no" />
    <frame frameborder="no" src="${mtSummaryURL}?method=showMtSummaryDiagram&mId=${mId}&recordId=${recordId }&fisearch=${fisearch}&fagent=${fagent}&fromdoc=${param.fromdoc}&proxy=${proxy}&proxyId=${proxyId}&eventId=${param.eventId}&openType=${openType}" name="detailRightFrame" scrolling="no" id="detailRightFrame" /><%--xiangfan 添加openType，修复GOV-2185 2012-04-24 --%>
  </frameset>
</frameset>
