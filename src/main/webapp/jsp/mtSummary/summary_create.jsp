...
<script type="text/javascript">
	...
	<c:if test="${bean.state != 0}">
		myBar.add(new WebFXMenuButton("send", "<fmt:message key='common.toolbar.save.label' bundle='${v3xCommonI18N}' />", "toSend('save');", [1,4], "", null));
	</c:if>
	
	//保存至草稿箱
	<c:if test="${bean.state == 0}">
		myBar.add(new WebFXMenuButton("save", "<fmt:message key='edoc.receive.register.saveToDraftBox'/>",  "toSend('save');", [1,5], "", null));
	</c:if> 
						...
						<table border="0" height="10" cellpadding="0" cellspacing="0" width="100%" align="center">
							...
							<tr class="bg-summary">
								...
								<%if(com.seeyon.ctp.common.SystemEnvironment.hasPlugin("videoconference")) { %>							
									...
									<% } %>
										<input type="text"  class="input-100per" name="meetingNature" id="meetingNature" disabled="disabled" value="${meetingBean.meetingType=='2'?'视频会议':'普通会议'}"/>
									</td>
								<% } else {%>
									<% if(!com.seeyon.v3x.meeting.contants.MeetingConfig.HAS_MEETING_TYPE) { %>
										<td colspan="4"></td>
	...
	<jsp:include page="../meeting/include/deal_exception.jsp" />
	</body>
</html> 