...
function getOffsetRect(a) {
	var b = a.getBoundingClientRect(), c = document.body, d = document.documentElement, e = window.pageYOffset || d.scrollTop || c.scrollTop, f = window.pageXOffset || d.scrollLeft || c.scrollLeft, g = d.clientTop || c.clientTop || 0, h = d.clientLeft || c.clientLeft || 0, i = b.top + e - g, k = b.left + f - h;
...
if (navigator.userAgent.indexOf("Safari") != -1 || navigator.userAgent.indexOf("Konqueror") != -1) {
	...
	navigator.userAgent.indexOf("Opera") != -1 ? (_isOpera = !0, _OperaRv = parseFloat(navigator.userAgent.substr(navigator.userAgent.indexOf("Opera") + 6, 3))) : navigator.appName.indexOf("Microsoft") != -1 ? (_isIE = !0, navigator.appVersion.indexOf("MSIE 8.0") != -1 && document.compatMode != "BackCompat" && (_isIE = 8)) : (_isFF = !0, w = parseFloat(navigator.userAgent.split("rv:")[1]));
}
dtmlXMLLoaderObject.prototype.doXPath = function (a, b, c, d) {
	...
	if (_isIE) {
		...
		for (var i = h.iterateNext(); i; ) {
			g[g.length] = i, i = h.iterateNext();
		}
		return g;
	}
};
...
function dataProcessor(a) {
	...
	this.messages = [];
	this.styles = {updated:"font-weight:bold;", inserted:"font-weight:bold;", deleted:"text-decoration : line-through;", invalid:"background-color:FFE0E0;", invalid_cell:"border-bottom:2px solid red;", clear:"font-weight:normal;text-decoration:none;"};
...
dataProcessor.prototype = {setTransactionMode:function (a, b) {
	...
	if (a) {
		...
		var c = new dtmlXMLLoaderObject(this.afterUpdate, this, !0), d = this.serverProcessor + (this._user ? getUrlSymbol(this.serverProcessor) + ["dhx_user=" + this._user, "dhx_version=" + this.obj.getUserData(0, "version")].join("&") : "");
	...
	if (this._uActions && this._uActions[c] && !this._uActions[c](d)) {
	...
	if (e.xmlDoc.responseXML) {
		for (var f = e.doXPath("//data/action"), g = 0; g < f.length; g++) {
			var h = f[g], i = h.getAttribute("type"), k = h.getAttribute("sid"), j = h.getAttribute("tid");
			a.afterUpdateCallback(k, j, i, h);
		}
		a.finalizeUpdate();
	}
	...
	this.getUpdates(c, function () {
		...
		if (c.length) {
			...
			for (var f = 0; f < c.length; f++) {
				var g = c[f].getAttribute("status"), h = c[f].getAttribute("id"), i = c[f].getAttribute("parent");
				switch (g) {
				  case "inserted":
					a.callEvent("insertCallback", [c[f], h, i]);
					break;
				  case "updated":
					a.callEvent("updateCallback", [c[f], h, i]);
					break;
				  case "deleted":
					a.callEvent("deleteCallback", [c[f], h, i]);
				}
			}
			a._silent_mode = !1;
		}
	...
	dhtmlXGridObject.prototype._init_point_connector = dhtmlXGridObject.prototype._init_point, dhtmlXGridObject.prototype._init_point = function () {
		...
		if (!this._colls_loaded) {
			for (var d = this.xmlLoader.doXPath("./coll_options", arguments[0]), e = 0; e < d.length; e++) {
				...
				for (var i = this.xmlLoader.doXPath("./item", d[e]), k = 0; k < i.length; k++) {
					var j = i[k].getAttribute("value");
					if (h) {
						var l = i[k].getAttribute("label") || j;
...
window.dhtmlXScheduler = window.scheduler = {version:3};
...
scheduler.xy = {nav_height:22, min_event_height:40, scale_width:50, bar_height:20, scroll_width:18, scale_height:20, month_scale_height:20, menu_width:25, margin_top:0, margin_left:0, editor_width:140};
...
scheduler.select = function (a) {
	if (!this._table_view && this.getEvent(a)._timed && this._select_id != a) {
...
scheduler.getState = function () {
	return {mode:this._mode, date:this._date, min_date:this._min_date, max_date:this._max_date, editor_id:this._edit_id, lightbox_id:this._lightbox_id, new_event:this._new_event};
};
scheduler._click = {dhx_cal_data:function (a) {
	var b = a ? a.target : event.srcElement, c = scheduler._locate_event(b), a = a || event;
	if (!(c && !scheduler.callEvent("onClick", [c, a]) || scheduler.config.readonly)) {
		if (c) {
			scheduler.select(c);
			var d = b.className;
			if (d.indexOf("_icon") != -1) {
				scheduler._click.buttons[d.split(" ")[1].replace("icon_", "")](c);
			}
		} else {
	...
	//lijl添加,选择日期时触发timebutton
	timebutton(scheduler.date.add(scheduler.date[scheduler._mode + "_start"](scheduler._date), b || 1, scheduler._mode));
...
scheduler._on_dbl_click = function (a, b) {
	return ;//xiangfan 添加 2012-04-20 修复GOV-639 双击一条选过的时间报js错误，双击时间在此没有作用，直接屏蔽掉双击事件
	...
	if (!this.config.readonly) {
		...
		switch (c) {
		  ...
		  case "dhx_wa_day_data":
			...
			var d = this._mouse_coords(a), e = this._min_date.valueOf() + (d.y * this.config.time_step + (this._table_view ? 0 : d.x) * 1440) * 60000, e = this._correct_shift(e);
		  ...
		  case "dhx_cal_event_clear":
			...
			if (!this.callEvent("onDblClick", [f, a])) {
				break;
			}
...
scheduler._mouse_coords = function (a) {
	...
	b = a.pageX || a.pageY ? {x:a.pageX, y:a.pageY} : {x:a.clientX + (c.scrollLeft || d.scrollLeft || 0) - c.clientLeft, y:a.clientY + (c.scrollTop || d.scrollTop || 0) - c.clientTop};
	...
	if (this._table_view) {
		for (var f = 0, f = 1; f < this._colsS.heights.length; f++) {
			if (this._colsS.heights[f] > b.y) {
				break;
			}
		}
		...
		b.x = Math.max(0, Math.ceil(b.x / this._cols[0]) - 1), b.y = Math.max(0, Math.ceil(b.y * 60 / (this.config.time_step * this.config.hour_size_px)) - 1) + this.config.first_hour * (60 / this.config.time_step);
	}
	return b;
};
...
scheduler._correct_shift = function (a, b) {
	return a -= ((new Date(scheduler._min_date)).getTimezoneOffset() - (new Date(a)).getTimezoneOffset()) * 60000 * (b ? -1 : 1);
};
	...
	if (this._drag_mode) {
		...
		if (!this._drag_pos || b.custom || this._drag_pos.x != b.x || this._drag_pos.y != b.y) {
			...
			if (this._drag_mode == "create") {
				...
				this._loading = !0;
				var c = this._min_date.valueOf() + (b.y * this.config.time_step + (this._table_view ? 0 : b.x) * 1440) * 60000, c = this._correct_shift(c);
				if (!this._drag_start) {
					this._drag_start = c;
					return;
				}
			...
			if (this._drag_mode == "move") {
				c = this._min_date.valueOf() + (b.y * this.config.time_step + b.x * 1440) * 60000, !b.custom && this._table_view && (c += this.date.time_part(e.start_date) * 1000), c = this._correct_shift(c), d = e.end_date.valueOf() - (e.start_date.valueOf() - c);
			} else {
				c = e.start_date.valueOf();
				if (this._table_view) {
					d = this._min_date.valueOf() + b.y * this.config.time_step * 60000 + (b.custom ? 0 : 86400000), this._mode == "month" && (d = this._correct_shift(d, !1));
				} else {
					if (d = this.date.date_part(new Date(e.end_date)).valueOf() + b.y * this.config.time_step * 60000, this._els.dhx_cal_data[0].style.cursor = "s-resize", this._mode == "week" || this._mode == "day") {
						d = this._correct_shift(d);
					}
				}
				if (this._drag_mode == "new-size") {
					if (d <= this._drag_start) {
						var f = b.shift || (this._table_view && !b.custom ? 86400000 : 0), c = d - (b.shift ? 0 : f), d = this._drag_start + (f || this.config.time_step * 60000);
					} else {
						c = this._drag_start;
					}
				} else {
					d <= c && (d = c + this.config.time_step * 60000);
				}
			}
			var g = new Date(d - 1), h = new Date(c);
			if (this._table_view || g.getDate() == h.getDate() && g.getHours() < this.config.last_hour || scheduler._wa && scheduler._wa._dnd) {
				e.start_date = h, e.end_date = new Date(d), this.config.update_render ? this.update_view() : this.updateEvent(this._drag_id);
			}
		...
		if (scheduler.checkEvent("onMouseMove")) {
			var i = this._locate_event(a.target || a.srcElement);
			this.callEvent("onMouseMove", [i, a]);
		}
	}
};
			...
			if (this.callEvent("onBeforeEventChanged", [b, a, c])) {
				if (c && this.config.edit_on_create) {
					...
					if (this._table_view || this.config.details_on_create) {
						return this._drag_mode = null, this.showLightbox(this._drag_id);
					}
					this._drag_pos = !0;
					this._select_id = this._edit_id = this._drag_id;
				} else {
					this._new_event || this.callEvent(c ? "onEventAdded" : "onEventChanged", [this._drag_id, this.getEvent(this._drag_id)]);
				}
			} else {
...
scheduler.setCurrentView = function (a, b) {
	...
	if (this.callEvent("onBeforeViewChange", [this._mode, this._date, b, a])) {
		var c = "dhx_cal_data", d = this._mode == b && this.config.preserve_scroll ? this._els[c][0].scrollTop : !1;
		...
		for (var f = this._els.dhx_cal_tab, g = 0; g < f.length; g++) {
			f[g].className = "dhx_cal_tab" + (f[g].getAttribute("name") == this._mode + "_tab" ? " active" : "");
		}
		var h = this[this._mode + "_view"];
		h ? h(!0) : this.update_view();
		if (typeof d == "number") {
...
scheduler._reset_scale = function () {
	if (this.templates[this._mode + "_date"]) {
		...
		for (var m = 0; m < l; m++) {
			...
			if (!this._table_view) {
				var n = document.createElement("DIV"), p = "dhx_scale_holder";
		...
		if (this._table_view) {
			this._reset_month_scale(b, h, i);
		} else {
			this._reset_hours_scale(b, h, i);
			if (c.multi_day) {
				var q = "dhx_multi_day";
				this._els[q] && (this._els[q][0].parentNode.removeChild(this._els[q][0]), this._els[q] = null);
				var v = this._els.dhx_cal_navline[0], s = v.offsetHeight + this._els.dhx_cal_header[0].offsetHeight + 1, r = document.createElement("DIV");
				r.className = q;
			...
			if (this.config.mark_now) {
				...
				if (t < this._max_date && t > this._min_date && t.getHours() >= this.config.first_hour && t.getHours() < this.config.last_hour) {
					var A = this.locate_holder_day(t), B = t.getHours() * 60 + t.getMinutes(), y = document.createElement("DIV");
					y.className = "dhx_now_time";
					y.style.top = Math.round((B * 60000 - this.config.first_hour * 3600000) * this.config.hour_size_px / 3600000) % (this.config.hour_size_px * 24) + 1 + "px";
...
scheduler._reset_month_scale = function (a, b, c) {
	var d = scheduler.date.add(b, 1, "month"), e = new Date;
	this.date.date_part(e);
	this.date.date_part(c);
	var f = Math.ceil(Math.round((d.valueOf() - c.valueOf()) / 86400000) / 7), g = [], h = Math.floor(a.clientHeight / f) - 22;
	this._colsS.height = h + 22;
	for (var i = this._colsS.heights = [], k = 0; k <= 7; k++) {
		g[k] = " style='height:" + h + "px; width:" + ((this._cols[k] || 0) - 1) + "px;' ";
	}
	var j = 0;
	this._min_date = c;
	for (var l = "<table cellpadding='0' cellspacing='0'>", k = 0; k < f; k++) {
		if(k == 0) {
			l += "<tr>";
			l += "<td class='dhx_before dhx_month_head_title'>"+meetingLang.meeting_room_view_monday+"</td>";
			l += "<td class='dhx_before dhx_month_head_title'>"+meetingLang.meeting_room_view_tuesday+"</td>";
			l += "<td class='dhx_before dhx_month_head_title'>"+meetingLang.meeting_room_view_wednesday+"</td>";
			l += "<td class='dhx_before dhx_month_head_title'>"+meetingLang.meeting_room_view_thursday+"</td>";
			l += "<td class='dhx_before dhx_month_head_title'>"+meetingLang.meeting_room_view_friday+"</td>";
			l += "<td class='dhx_before dhx_month_head_title'>"+meetingLang.meeting_room_view_saturday+"</td>";
			l += "<td class='dhx_before dhx_month_head_title'>"+meetingLang.meeting_room_view_sunday+"</td>";
			l += "</tr>";
		}
		l += "<tr>";
		for (var o = 0; o < 7; o++) {
...
scheduler.date = {init:function () {
	...
	for (var c = "var temp=date.split(/[^0-9a-zA-Z]+/g);", d = a.match(/%[a-zA-Z]/g), e = 0; e < d.length; e++) {
		switch (d[e]) {
		  case "%j":
		  case "%d":
			c += "set[2]=temp[" + e + "]||1;";
	...
	var d = c.getFullYear(), e = Math.round((c.getTime() - (new Date(d, 0, 1)).getTime()) / 86400000), f = 1 + Math.floor(e / 7);
	return f;
...
scheduler.addEvent = function (a, b, c, d, e) {
	...
	var g = (this.config.event_duration || this.config.time_step) * 60000;
...
scheduler.deleteEvent = function (a, b) {
	...
	if (b || this.callEvent("onBeforeEventDelete", [a, c]) && this.callEvent("onConfirmedBeforeEventDelete", [a, c])) {
		c && (delete this._events[a], this.unselect(a), this.event_updated(c)), this.callEvent("onEventDeleted", [a]);
	}
};
...
scheduler.is_one_day_event = function (a) {
	...
	return b ? (b < 0 && (b = Math.ceil((a.end_date.valueOf() - a.start_date.valueOf()) / 86400000)), b == 1 && !a.end_date.getHours() && !a.end_date.getMinutes() && (a.start_date.getHours() || a.start_date.getMinutes())) : a.start_date.getMonth() == a.end_date.getMonth() && a.start_date.getFullYear() == a.end_date.getFullYear();
};
scheduler.get_visible_events = function () {
	var a = [], b = this["filter_" + this._mode], c;
	for (c in this._events) {
		if (this.is_visible_events(this._events[c]) && (this._table_view || this.config.multi_day || this._events[c]._timed)) {
			(!b || b(c, this._events[c])) && a.push(this._events[c]);
		}
	}
	return a;
};
scheduler.render_view_data = function (a, b) {
	...
	if (this.config.multi_day && !this._table_view) {
		for (var c = [], d = [], e = 0; e < a.length; e++) {
			a[e]._timed ? c.push(a[e]) : d.push(a[e]);
		}
...
scheduler.render_data = function (a, b) {
	for (var a = this._pre_render_events(a, b), c = 0; c < a.length; c++) {
		this._table_view ? this.render_event_bar(a[c]) : this.render_event(a[c]);
	}
};
			...
			if (g.rows) {
				for (var h = 0; h < g.rows.length; h++) {
					...
					if (e[h] * c > this._colsS.height - 22) {
						for (var i = g.rows[h].cells, k = 0; k < i.length; k++) {
							i[k].childNodes[1].style.height = e[h] * c + "px";
						}
						e[h] = (e[h - 1] || 0) + i[0].offsetHeight;
					}
				...
				if (g.parentNode.offsetHeight < g.parentNode.scrollHeight && !g._h_fix) {
					for (h = 0; h < g.rows.length; h++) {
						var j = g.rows[h].cells[6].childNodes[0], l = j.offsetWidth - scheduler.xy.scroll_width + "px";
					//	j.style.width = l;
				...
				if (!a.length && this._els.dhx_multi_day[0].style.visibility == "visible" && (e[0] = -1), a.length || e[0] == -1) {
					var o = g.parentNode.childNodes, m = (e[0] + 1) * c + 1 + "px";
...
scheduler._get_event_sday = function (a) {
	return Math.floor((a.start_date.valueOf() - this._min_date.valueOf()) / 86400000);
};
scheduler._pre_render_events_line = function (a, b) {
	...
	for (var c = [], d = [], e = 0; e < a.length; e++) {
		var f = a[e], g = f.start_date.getHours(), h = f.end_date.getHours();
		f._sday = this._get_event_sday(f);
		c[f._sday] || (c[f._sday] = []);
		if (!b) {
			f._inner = !1;
			for (var i = c[f._sday]; i.length && i[i.length - 1].end_date <= f.start_date; ) {
				i.splice(i.length - 1, 1);
			}
			for (var k = !1, j = 0; j < i.length; j++) {
				if (i[j].end_date.valueOf() < f.start_date.valueOf()) {
					k = !0;
					f._sorder = i[j]._sorder;
					i.splice(j, 1);
					f._inner = !0;
					break;
				}
			...
			i.push(f);
			i.length > (i.max_count || 0) ? (i.max_count = i.length, f._count = i.length) : f._count = f._count ? f._count : 1;
		}
		if (g < this.config.first_hour || h >= this.config.last_hour) {
			if (d.push(f), a[e] = f = this._copy_event(f), g < this.config.first_hour && (f.start_date.setHours(this.config.first_hour), f.start_date.setMinutes(0)), h >= this.config.last_hour && (f.end_date.setMinutes(0), f.end_date.setHours(this.config.last_hour)), f.start_date > f.end_date || g == this.config.last_hour) {
				a.splice(e, 1), e--;
			}
		}
	}
...
scheduler._pre_render_events_table = function (a, b) {
	this._time_order(a);
	for (var c = [], d = [[], [], [], [], [], [], []], e = this._colsS.heights, f, g = this._cols.length, h = 0; h < a.length; h++) {
		var i = a[h], k = f || i.start_date, j = i.end_date;
		if (k < this._min_date) {
			k = this._min_date;
		}
		if (j > this._max_date) {
		...
		i._eday = o % g || g;
		i._length = o - l;
		i._sweek = Math.floor((this._correct_shift(k.valueOf(), 1) - this._min_date.valueOf()) / (86400000 * g));
		var m = d[i._sweek], n;
		for (n = 0; n < m.length; n++) {
			if (m[n]._eday <= i._sday) {
				break;
			}
		}
		if (!i._sorder || !b) {
		...
		if (i._sday + i._length <= g) {
			f = null, c.push(i), m[n] = i, e[i._sweek] = m.length - 1;
		} else {
...
scheduler.render_event = function (a) {
	...
	if (!(a._sday < 0)) {
		...
		if (c) {
			var d = a.start_date.getHours() * 60 + a.start_date.getMinutes(), e = a.end_date.getHours() * 60 + a.end_date.getMinutes() || scheduler.config.last_hour * 60, f = Math.round((d * 60000 - this.config.first_hour * 3600000) * this.config.hour_size_px / 3600000) % (this.config.hour_size_px * 24) + 1, g = Math.max(scheduler.xy.min_event_height, (e - d) * this.config.hour_size_px / 60) + 1, h = Math.floor((c.clientWidth - b) / a._count), i = a._sorder * h + 1;
			a._inner || (h *= a._count - a._sorder);
			if (this.config.cascade_event_display) {
				var k = this.config.cascade_event_count, j = this.config.cascade_event_margin, i = a._sorder % k * j, l = a._inner ? (a._count - a._sorder - 1) % k * j / 2 : 0, h = Math.floor(c.clientWidth - b - i - l);
			}
			var o = this._render_v_bar(a.id, b + i, f, h, g, a._text_style, scheduler.templates.event_header(a.start_date, a.end_date, a), scheduler.templates.event_text(a.start_date, a.end_date, a));
			this._rendered.push(o);
			c.appendChild(o);
			i = i + parseInt(c.style.left, 10) + b;
			if (this._edit_id == a.id) {
				o.style.zIndex = 1;
				h = Math.max(h - 4, scheduler.xy.editor_width);
				o = document.createElement("DIV");
				o.setAttribute("event_id", a.id);
				this.set_xy(o, h, g - 20, i, f + 14);
				o.className = "dhx_cal_editor";
				var m = document.createElement("DIV");
				this.set_xy(m, h - 6, g - 26);
				...
				if (this._quirks7) {
					m.firstChild.style.height = g - 12 + "px";
				}
			...
			if (this._select_id == a.id) {
				...
				for (var n = this.config["icons_" + (this._edit_id == a.id ? "edit" : "select")], p = "", q = a.color ? "background-color:" + a.color + ";" : "", v = a.textColor ? "color:" + a.textColor + ";" : "", s = 0; s < n.length; s++) {
					p += "<div class='dhx_menu_icon " + n[s] + "' style='" + q + "" + v + "' title='" + this.locale.labels[n[s]] + "'></div>";
				}
				var r = this._render_v_bar(a.id, i - b + 1, f, b, n.length * 20 + 26, "", "<div style='" + q + "" + v + "' class='dhx_menu_head'></div>", p, !0);
				r.style.left = i - b + 1;
...
scheduler._render_v_bar = function (a, b, c, d, e, f, g, h, i) {
	var k = document.createElement("DIV"), j = this.getEvent(a), l = "dhx_cal_event", o = scheduler.templates.event_class(j.start_date, j.end_date, j);
	o && (l = l + " " + o);
	var m = j.color ? "background-color:" + j.color + ";" : "", n = j.textColor ? "color:" + j.textColor + ";" : "", p = "<div event_id=\"" + a + "\" class=\"" + l + "\" style=\"position:absolute; top:" + c + "px; left:" + b + "px; width:" + (d - 4) + "px; height:" + e + "px;" + (f || "") + "\">";
	p += "<div class=\"dhx_header\" style=\" width:" + (d - 6) + "px;" + m + "\" >&nbsp;</div>";
	p += "<div class=\"dhx_title\" style=\"" + m + "" + n + "\">" + g + "</div>";
	p += "<div class=\"dhx_body\" style=\" width:" + (d - (this._quirks ? 4 : 14)) + "px; height:" + (e - (this._quirks ? 20 : 30)) + "px;" + m + "" + n + "\">" + h + "</div>";
	p += "<div class=\"dhx_footer\" style=\" width:" + (d - 8) + "px;" + (i ? " margin-top:-1px;" : "") + "" + m + "" + n + "\" ></div></div>";
	k.innerHTML = p;
	return k.firstChild;
};
...
scheduler.locate_holder_day = function (a, b) {
	var c = Math.floor((this._correct_shift(a, 1) - this._min_date) / 86400000);
...
scheduler.render_event_bar = function (a) {
	...
	var e = this.xy.bar_height, f = this._colsS.heights[a._sweek] + (this._colsS.height ? this.xy.month_scale_height + 2 : 2) + a._sorder * e, g = document.createElement("DIV"), h = a._timed ? "dhx_cal_event_clear" : "dhx_cal_event_line", i = scheduler.templates.event_class(a.start_date, a.end_date, a);
	i && (h = h + " " + i);
	var k = a.color ? "background-color:" + a.color + ";" : "", j = a.textColor ? "color:" + a.textColor + ";" : "", l = "<div event_id=\"" + a.id + "\" class=\"" + h + "\" style=\"position:absolute; top:" + f + "px; left:" + c + "px; width:" + (d - c - 15) + "px;" + j + "" + k + "" + (a._text_style || "") + "\">";
...
scheduler._load = function (a, b) {
	...
	if (this._load_mode) {
		...
		for (var e = !0; c < this._max_date; ) {
			c = this.date.add(c, 1, this._load_mode), this._loaded[d(b)] && e ? b = this.date.add(b, 1, this._load_mode) : e = !1;
		}
		var f = c;
		do {
...
scheduler.on_load = function (a) {
	...
	for (var c = 0; c < b.length; c++) {
		this.callEvent("onEventLoading", [b[c]]) && this.addEvent(b[c]);
	}
	this._not_render = !1;
...
scheduler.json.parse = function (a) {
	if (typeof a == "string") {
		eval("scheduler._temp = " + a + ";"), a = scheduler._temp;
	}
	for (var b = [], c = 0; c < a.length; c++) {
		a[c].start_date = scheduler.templates.xml_date(a[c].start_date), a[c].end_date = scheduler.templates.xml_date(a[c].end_date), b.push(a[c]);
	}
	return b;
};
...
scheduler._magic_parser = function (a) {
	...
	for (var d = a.doXPath("//coll_options"), e = 0; e < d.length; e++) {
		var f = d[e].getAttribute("for"), g = this.serverList[f];
		if (g) {
			g.splice(0, g.length);
			for (var h = a.doXPath(".//item", d[e]), i = 0; i < h.length; i++) {
				for (var k = h[i], j = k.attributes, l = {key:h[i].getAttribute("value"), label:h[i].getAttribute("label")}, o = 0; o < j.length; o++) {
					var m = j[o];
					if (!(m.nodeName == "value" || m.nodeName == "label")) {
						l[m.nodeName] = m.nodeValue;
					}
				}
				g.push(l);
			}
	...
	for (e = 0; e < b.length; e++) {
		q[e] = this.xmlNodeToJSON(b[e]), q[e].text = q[e].text || q[e]._tagvalue, q[e].start_date = this.templates.xml_date(q[e].start_date), q[e].end_date = this.templates.xml_date(q[e].end_date);
	}
	return q;
};
...
scheduler.attachEvent("onXLE", function () {
	var a;
	if ((a = this.config.show_loading) && typeof a == "object") {
		this._obj.removeChild(a), this.config.show_loading = !0;
	}
});
scheduler.ical = {parse:function (a) {
	var b = a.match(RegExp(this.c_start + "[^\f]*" + this.c_end, ""));
	if (b.length) {
		b[0] = b[0].replace(/[\r\n]+(?=[a-z \t])/g, " ");
		b[0] = b[0].replace(/\;[^:\r\n]*/g, "");
		for (var c = [], d, e = RegExp("(?:" + this.e_start + ")([^\f]*?)(?:" + this.e_end + ")", "g"); d = e.exec(b); ) {
			for (var f = {}, g, h = /[^\r\n]+[\r\n]+/g; g = h.exec(d[1]); ) {
	...
	var c = a.indexOf(":");
	if (c != -1) {
		var d = a.substr(0, c).toLowerCase(), e = a.substr(c + 1).replace(/\\\,/g, ",").replace(/[\r\n]+$/, "");
...
scheduler.formSection = function (a) {
	...
	var d = b[c], e = document.getElementById(d.id).nextSibling;
	return {getValue:function (a) {
...
scheduler.form_blocks = {template:{render:function (a) {
	...
	if (typeof b == "undefined") {
		b = (a.firstChild.options[0] || {}).value;
	}
	a.firstChild.value = b || "";
}, get_value:function (a) {
	return a.firstChild.value;
	...
	var a = scheduler.config, b = this.date.date_part(new Date), c = 1440, d = 0;
	...
	for (var e = "<select>", f = d, g = b.getDate(); f < c; ) {
		...
		b.setTime(b.valueOf() + this.config.time_step * 60000);
		var i = b.getDate() != g ? 1 : 0, f = i * 1440 + b.getHours() * 60 + b.getMinutes();
	}
	e += "</select> <select>";
	for (f = 1; f < 32; f++) {
	...
	e += "</select> <select>";
	for (f = 0; f < 12; f++) {
		e += "<option value='" + f + "'>" + this.locale.date.month_full[f] + "</option>";
	}
	e += "</select> <select>";
	b = b.getFullYear() - 5;
	for (f = 0; f < 10; f++) {
		e += "<option value='" + (b + f) + "'>" + (b + f) + "</option>";
	}
	e += "</select> ";
	...
	if (scheduler.config.full_day) {
		...
		var g = a.previousSibling.getElementsByTagName("input")[0], h = scheduler.date.time_part(c.start_date) === 0 && scheduler.date.time_part(c.end_date) === 0 && c.end_date.valueOf() - c.start_date.valueOf() < 172800000;
		g.checked = h;
		for (var i in e) {
			e[i].disabled = g.checked;
		}
		g.onclick = function () {
			if (g.checked) {
				var a = new Date(c.start_date), b = new Date(c.end_date);
				scheduler.date.date_part(a);
				b = scheduler.date.add(a, 1, "day");
			}
			for (var f in e) {
				e[f].disabled = g.checked;
			}
	...
	if (scheduler.config.auto_end_date && scheduler.config.event_duration) {
		for (var k = function () {
			...
			c.end_date.setTime(c.start_date.getTime() + scheduler.config.event_duration * 60000);
			d(e, 4, c.end_date);
		}, j = 0; j < 4; j++) {
			e[j].onchange = k;
		}
...
scheduler._fill_lightbox = function (a, b) {
	...
	for (var e = this.config.lightbox.sections, f = 0; f < e.length; f++) {
		var g = document.getElementById(e[f].id).nextSibling, h = this.form_blocks[e[f].type];
		h.set_value.call(this, g, c[e[f].map_to], c, e[f]);
		e[f].focus && h.focus.call(this, g);
	}
...
scheduler._empty_lightbox = function () {
	var a = scheduler._lightbox_id, b = this.getEvent(a), c = this._get_lightbox();
			...
			switch (b.className) {
			  ...
			  default:
				if (b.getAttribute("dhx_button")) {
					...
					if (b.className.indexOf("dhx_custom_button_") != -1) {
						var d = b.parentNode.getAttribute("index"), e = scheduler.form_blocks[scheduler.config.lightbox.sections[d].type], f = b.parentNode.parentNode;
...
scheduler._move_while_dnd = function (a) {
	if (scheduler._dnd_start_lb) {
		...
		var b = scheduler._get_lightbox(), c = a && a.target ? [a.pageX, a.pageY] : [event.clientX, event.clientY];
...
scheduler._get_lightbox = function () {
	if (!this._lightbox) {
		...
		for (var d = 0; d < c.length; d++) {
			b += "<div class='dhx_btn_set'><div dhx_button='1' class='" + c[d] + "'></div><div>" + scheduler.locale.labels[c[d]] + "</div></div>";
		}
		c = this.config.buttons_right;
		for (d = 0; d < c.length; d++) {
			b += "<div class='dhx_btn_set' style='float:right;'><div dhx_button='1' class='" + c[d] + "'></div><div>" + scheduler.locale.labels[c[d]] + "</div></div>";
		}
		b += "</div>";
		a.innerHTML = b;
