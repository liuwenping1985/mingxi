(function () {
	...
	function C(a) {
		...
		var b = "<table style='table-layout:fixed;border-bottom:1px #DDD solid;' cellspacing='0' cellpadding='0'>", c = [];
		...
		for (e = 0; e < this.y_unit.length; e++) {
			...
			h.height = "height:44px;";//lijl设置固定行高
			...
			if (this.fit_events) {
				...
				h.style_height = "height:44px;";//lijl设置固定行高
			}
				...
				for (d = 0; d < scheduler._cols.length; d++) {
					if(d%4==0){//lijl添加,每隔四个小格就已实线显示//在style中加入border-left:1px #DDD solid; 
						if(d<setDisable && isNow)
						  b += "<td class='dhx_matrix_cell " + scheduler.templates[this.name + "_cell_class"](c[e], this._trace_x[d], this.y_unit[e]) + "' style='border-left:1px #868686 solid; width:" + (scheduler._cols[d] - 3) + "px'><div style='width:" + (scheduler._cols[d] - 3) + "px;background: #CFCFCF'></div></td>";
						else
						  b += "<td class='dhx_matrix_cell " + scheduler.templates[this.name + "_cell_class"](c[e], this._trace_x[d], this.y_unit[e]) + "' style='border-left:1px #868686 solid; width:" + (scheduler._cols[d] - 3) + "px'><div style='width:" + (scheduler._cols[d] - 3) + "px'></div></td>";
					}else{
					  if(d<setDisable && isNow)
					    b += "<td class='dhx_matrix_cell " + scheduler.templates[this.name + "_cell_class"](c[e], this._trace_x[d], this.y_unit[e]) + "' style='width:" + (scheduler._cols[d] - 3) + "px'><div style='width:" + (scheduler._cols[d] - 3) + "px;background: #CFCFCF'></div></td>";
					  else 
	...
	function D(a) {
		...
		a.innerHTML = "<div></div>";//lijl去掉左上角的文字(将div设置为空)
