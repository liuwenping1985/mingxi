...
function updateInfoContent(from) {
    popupContentWin(from);
}
...
function popupContentWin(from) {
	...
	if(contentType == "10") {//HTML
		contentDialog = $.dialog({
		     ...
		     id: 'openContent',
		     url: _ctxPath + "/info/info.do?method=openContentDialog&contentViewState="+contentViewState+"&newBusiness="+newBusiness+"&action="+action+"&moduleId="+moduleId+"&contentType="+contentType+"&contentId="+contentId+"&opFrom="+from,
...
function fillContent(content) {
	...
	//$("#content",contentDiv).val(content);
}

function getParentContent() {
	return $.content.getContent();
	/*var contentDiv = getMainBodyDataDiv$();
	return $("#content",contentDiv).val();*/
}
