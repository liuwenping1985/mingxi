...
function sTerm(y,n) {
   unicallog(1,  "y="+y+" n="+n+" sTermInfo[n]="+sTermInfo[n]+" Date.UTC(1900,0,6,2,5)="+Date.UTC(1900,0,6,2,5)+" Date.UTC(1970,0,1,0,0)="+Date.UTC(1970,0,1,0,0) );
// changed by hmisty 2005/07/23
...
function calendar(y,m) {
        unicallog(1,  "i am in calendar() now");
	...
	this.length    = solarDays(y,m);
        ...
        unicallog(1,  "this.length: "+this.length);
        unicallog(1,  "begin loop for(var i=0;i<this.length;i++)");
	for(var i=0;i<this.length;i++) {
		...
		if(lD>lX) {
	       ...
	       //unicallog(1,  "lDObj.dayCyl: "+lDObj.dayCyl);
        ...
        unicallog(1,  "end loop for(var i=0;i<this.length;i++)");

	tmp1=sTerm(y,m*2  )-1;
	tmp2=sTerm(y,m*2+1)-1;
	unicallog(1,  "m: "+m+" tmp1: "+tmp1+" "+solarTerm[m*2]+" tmp2: "+tmp2+" "+solarTerm[m*2+1]);
	this[tmp1].solarTerms = solarTerm[m*2];
     	this[tmp2].solarTerms = solarTerm[m*2+1];
	if(m==3) this[tmp1].color = '#ff5f07';
        
        unicallog(1,  "begin loop for(i in sFtv)");
	for(i in sFtv)
		if(typeof(i)=='number')	
        ...
        unicallog(1,  "begin loop for(i in wFtv)");
	for(i in wFtv)
		if(typeof(i)=='number')	
        ...
        unicallog(1,  "begin loop for(i in lFtv)");
	for(i in lFtv)  
		if(typeof(i)=='number')	
		if(lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
			...
			if(tmp1 >=0 && tmp1<n) {
				...
				if( tmp2 >= 0 && tmp2<this.length) {
                                        unicallog(1,  "tmp2 >= 0 && tmp2("+tmp2+")<this.length("+this.length+")");
                                        unicallog(1,  RegExp.$4);
                                        unicallog(1,  this[tmp2].lunarFestival);
					this[tmp2].lunarFestival += RegExp.$4 + ' ';
                                        unicallog(1,  RegExp.$3);
					if(RegExp.$3=='*') this[tmp2].color = '#ff5f07';
				}
			}
	  	}
     
        unicallog(1,  "begin 黑色星期五");
...
function drawCld(SY,SM) {
        unicallog(1,  "i am in drawCld() now");
        
	var i,sD,s,size;
	unicallog(1,  "begin to create calendar cld");
	cld = new calendar(SY,SM);
        unicallog(1,  "calendar cld creation finished");
	/*
	...
	document.getElementById("gz").innerHTML = '&nbsp;&nbsp;农历' 
        ...
        unicallog(1,  "innerHTML of gz is "+document.getElementById("gz").innerHTML);
...
function initial() {
	...
	document.getElementById("sy").selectedIndex=tY-1900;
	document.getElementById("sm").selectedIndex=tM;
	unicallog(1,  "initial call drawCld() now");
	drawCld(tY,tM);
