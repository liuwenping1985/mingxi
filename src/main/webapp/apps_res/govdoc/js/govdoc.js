...
function setParamsToObject(o,url){
    ...
    o.finishstate = GetQueryString(url,"workflowState");
...
function deleteItems(pageType,grid,tableId,fromMethod){
  ...
  if(rows.length <= 0) {
	  ...
	  $.alert($.i18n('collaboration.govdoc.grid.selectDelete'));
      return true;
   }
    var obj;
...
function editFromWaitSend(grid,app,sub_app){
  ...
  if(count == 1){
    var obj = rows[0];
  //老公文待办  屏蔽进入编辑界面
    if(obj.govdocType == '0' || obj.govdocType == '' || obj.govdocType == null){
...
//	  }
    ...
    if (v.length === 0) {
      $.alert($.i18n('collaboration.edoc.alert.select'));
      return;
    }
    var ids = new Array();
    var check = false;
    ...
    $(v).each(function(index, elem) {
    	//老公文 发送和签收  不允许归档
    	 if(elem.app == 22 || elem.app == 23 || elem.app == 24 || elem.app == 40){
    		$.alert("《"+elem.subject+"》 为旧公文交换或登记数据不支持该功能");
			check = true;
    		return false;
    	}
        //待办的逻辑(还需要翟峰多查询几个条件)
  ...
  //增加判断当前文档是否已经归档了
    ...
    //归档成功
   alert($.i18n('collaboration.grid.alert.archiveSuccess'));
    //删除重复的文档
    cm.transPigeonholeDeleteStepBackDoc(ids, folder[0]);
	if('govdocsent' != pageType){
        // 归档之后没有必要刷新，参考已发公文的归档
		// var url = window.location.search;
		// var o = new Object();
		// setParamsToObject(o,url);
		// $("#"+tableId).ajaxgridLoad(o);
     ...
     if($.ctx.plugins.contains('webmail')){
         ...
         for(var i=0; i<selectBox.length; i++){
             ...
             var bgory= selectedObj.bodyType || selectedObj.app;
 			 if(bgory=="20" && !(app == "19" || app == "20" || app == "21")){
...
function openQueryViews(openForm){
	openQueryViewsForSubApp(openForm, '1,2,4');
}
function openQueryViewsForSubApp(openForm, subapp){
	var _url = _ctxPath + "/govDoc/govDocController.do?method=combinedQuery&openForm="+openForm;
        ...
        buttons: [{
            ...
            handler: function () {
               ...
               if(typeof(o.templeteIds)!='undefined'){
                ...
                if(typeof(o.docMark)!='undefined'){
                    cloneParam.docMark = o.docMark
                }
                if(typeof(o.serialNo)!='undefined'){
                    cloneParam.serialNo = o.serialNo
                }
                if(typeof(o.secretLevel)!='undefined'){
                    cloneParam.secretLevelQuery = o.secretLevel
                }
                if(typeof(o.urgentLevel)!='undefined'){
                    cloneParam.urgentLevel = o.urgentLevel
                }
                if(typeof(finishState)!='undefined'){
                    cloneParam.finishstate = finishState
                }
                if(typeof(listCfgId)!='undefined'){
                	cloneParam.listCfgId = listCfgId
                }
               cloneParam.app = "4";
               if(subapp){
            	   cloneParam.sub_app = subapp;
               } else {
            	   cloneParam.sub_app = '1,2,4';
               }
           	    queryDialog.close();
