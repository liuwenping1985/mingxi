...
function initDeptData(){
	...
	var statId = $("input[name='stat']:checked").val();
     ...
     $("#depts").append("<option value='0'>全部</option>");   
     for(var i=0;i<myArray.length;i++){
    	 $("#depts").append("<option value='"+myArray[i].id+"'>"+myArray[i].name+"</option>"); 
     }
}
//导出Excel
...
function doStat(type){
	...
	if(type ==1){
		statConditionForm.action =edocStatUrl+"?method=workCountByCondition";
		statConditionForm.target = "content";
	}else{
		statConditionForm.action =edocStatUrl+"?method=exportWorkCount";
