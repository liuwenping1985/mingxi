var layout;
var mManager = new edocStatNewManager();
$(document).ready(function () {	
	 ...
	 $("input[name='stat']").click(function() {
		 initDeptData()
		 document.frames["statResultIframe"].document.body.innerText = "";
		 $("#year").click();
	 });
	 ...
	 $("#queryReset").click(function() {
		 ...
		 $("#docMark").find("option").eq(0).attr("selected", "true");
		 $("#docMark_txt").val($("#docMark").find("option").eq(0).html());
		 $("#serialNo").find("option").eq(0).attr("selected", "true");
		 $("#serialNo_txt").val($("#serialNo").find("option").eq(0).html());
		 $("#statRangeId").find("option").eq(0).attr("selected", "true");
		 $("#statRangeType").val($("#statRangeId").find("option").eq(0).attr("statRangeType"));
	 });
...
function initDeptData(){
	...
	myArray = mManager.initWorkStatDeptList(statId); 
    
	$("#statRangeId").append("<option value='0' statRangeType='All'>全部</option>"); 
    if(myArray && myArray!=null && myArray!="1") {
    	for(var i=0;i<myArray.length;i++){
        	$("#statRangeId").append("<option value='"+myArray[i].id+"' statRangeType='Department'>"+myArray[i].name+"</option>"); 
        }
    }
}
//导出Excel
...
function doStat(type){
    ...
    $("#statId").val(statId);
	if(type ==1){
		statConditionForm.action = edocStatUrl+"?method=govdocStatResult";
		statConditionForm.target = "statResultIframe";
	}else{
		statConditionForm.action = edocStatUrl+"?method=exportGovdocStatResult";
...
function initStyle() {
	$("#docMarkSelect").css({"height": "22px"});
	$("#serialNoSelect").css({"height": "22px"});
}

function docMarkChange(obj) {
	if(!obj.attr("name")) {
...
function showInstruction() {
	var url = edocStatUrl + "?method=showInstruction";	
  	var width = 780;
  	var height =381;
...
//签收统计时间显示
function showTime(){
	var test=$('input[name="timeType"]:checked').val();		
	var startyear=$('#yeartype-startyear').val();
	var endyear=$('#yeartype-endyear').val();
			
	if(test==1){		
		var time=startyear+'年'+'--'+endyear+'年';
	};
	if(test==2){
		var startseason=$('#seasontype-startseason').val();
		var endseason=$('#seasontype-endseason').val();
		var time=startyear+'年'+startseason+'季度'+'--'+endyear+'年'+endseason+'季度';				
	};		
	if(test==3){
		var startyear=$('#monthtype-startyear').val();
		var endyear=$('#monthtype-endyear').val()
		var startmonth=$('#monthtype-startmonth').val();
		var endmonth=$('#monthtype-endmonth').val();
		var time=startyear+'年'+startmonth+'月'+'--'+endyear+'年'+endmonth+'月';						
	};
	if(test==4){
		var startday=$('#daytype-startday').val();
		var endday=$('#daytype-endday').val();
		var time=startday+'至'+endday;
	}							
	
	var html='的工作统计结果';
	$('#statTitle').val(time+html);
}