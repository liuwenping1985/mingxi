...
//将所有操作置为可用状态
 function enableOperation(){
  ...
  $("#isHidden").enable();
  try{
	  if(componentDiv.zwIframe.window.getFlowDealOpinion().length>0){
...
//ajax判断事项是否可用,并且是否是重复提交。
 function isAffairValidAndIsLock(affairId){
	 var cm = new colManager();
	 var msg = cm.checkAffairValidAndIsLock(affairId);
	 if($.trim(msg) !=''){
		 $.messageBox({
			 'title' : $.i18n('collaboration.system.prompt.js'), //系统提示
			 'type': 0,
...
//指定回退
 ...
 function stepBackToTargetNodeCallBack(workitemId, processId, caseId, activityId, theStepBackNodeId, submitStyle, falshDialog) {
   ...
   if($.content.getContentDealDomains(domains)) {
	  //添加判断附件是否更改相关属性，若有修改，后台进行保存附件操作
 ...
 var hasSubmitButtonFlag = true;
  function formAddLock(){
    ...
    param.affairState = affairState;
    param.formAppId = formAppId;
    param.formRecordId = formRecordid;
    param.rightId = rightId;
    param.nodePolicy = nodePolicy;
    param.affairReadOnly = deeReadOnly;
    param.affairId = affairId;
    cm.formAddLockPerfect(param, callerResponder);
...
function attributeSettingDialog(affairId){
    var dialog = $.dialog({
        url : _ctxPath+'/collaboration/collaboration.do?method=getAttributeSettingInfo&affairId='+affairId+'&isHistoryFlag='+isHistoryFlag,
        width : 500,
        height : 450,
    	         ...
    	         handler: function () {
            		...
            		fnx.$.content.getContentDomains(function(domains) {
                        if($.content.getContentDealDomains(domains)) {
                        	...
                        	var jsonSubmitCallBack = function(){
        			        	 $("#layout").jsonSubmit({
        		                        action : _ctxPath + '/collaboration/collaboration.do?method=stepBack&affairId='+affairId+'&summaryId='+summaryId+'&trackWorkflowType='+trackWorkflowType,
        		                        domains : domains,
        		                        validate:false,
        		                        callback:function(data){
        		                          closeCollDealPage();
...
function submitFunc(){
    ...
    if(!isAffairValidAndIsLock(affairId)) {
    ...
    //OFFICE正文中直接点修改按钮，不是“正文修改”菜单进去的，这种情况下会这种contentUpdate变量为true;
      if(fnx.contentUpdate || bodyType=='45') fnx.$("#viewState").val("1");
      ...
      if(fnx.$("#viewState").val()=="1" || bodyType=='20'){ 
       ...
       //公告新闻审核提交
	   var __snMsg = "";
	   var bulnewsAudit=function(__domains,snMsg){
      ...
      var sub = function(subBack){
        ...
        //cx 
          if(document.getElementById("fenfa_input_value")){
          ...
          if(document.getElementById("reSign")){
...
function showOrCloseAttachmentList(){
  //获取正文下的附件
    ...
    if(window.screen.height == 768 && window.screen.width == 1024 ){
    	$("#attachmentList").css("width",parseInt($("#attachmentList").css("width"))-100);
    }
...
function summaryHeadHeight(){
   $("#content_workFlow").css("top",$("#summaryHead").height()+4);
}
...
if($( "#attributeSettingFlag" ).length>0){
  ...
  function showOrCloseWorkflowTrace(){
	  var dialog = $.dialog({
			 targetWindow:getCtpTop(),
		     id: 'workflowTrace',
		     url: _ctxPath +"/trace/traceWorkflow.do?method=showWorkflowDetail&affairId="+affairId+"&app=1",
		     width: 800,
		     height: 420,
