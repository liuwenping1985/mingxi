...
function sendCollFun(){
	...
	sendCount++;
//GOV-258 【分保】协同密级未对督办人员、跟踪指定人员进行过滤 start
	//if(checkzdgzryName()){
		//return ;
	//};//检查跟踪人密级程度
	//if(checkSuperisor()){
		//return ;
	//}//检查督办人密级程度
//GOV-258 【分保】协同密级未对督办人员、跟踪指定人员进行过滤 start end gaoyq
	/*var flag = checkWorkFlow();
	if(flag){
		return;
	}*/
...
function getContentTypeChooser(toolbarId, defaultType, callBack){
	...
	for ( var i = 0; i < mtCfg.length; i++) {
		var mtCtg_temp=mtCfg[i];
		var mt =mtCtg_temp.mainbodyType;
		if(mt=="46"){
			continue;
		}
		mtCtg_temp.value = mt;
		mtCtg_temp.id = "_mt_" + mt;
...
function notSpecialCharX(element){
	...
	if($.trim(value) ==""  || $("#subject").val() === $("#subject").attr("defaultValue")){
		...
		return false;
	} else if (!hasColSubject && value.length > 85) {
...
function saveAsTemplete() {
	if($.ctx.plugins.contains('secret') && (typeof(templateSecretLevelEnable)!="undefined" && !templateSecretLevelEnable)){
                ...
                if(over == 5){
                  ...
                  confirm = $.confirm({
                      ...
                      ok_fn: function () {
                          ...
                          if (getCtpTop().isCtpTop) {
	                             var  _win = getCtpTop().$("#main")[0].contentWindow;
	                             if(_win && _win.sectionHandler){
	                        		_win.sectionHandler.reload("templeteSection",true);
                        		 }
                          } else {
								var _win;
...
function doSaveTemplate(over,subject,overId,type,dialog){
    ...
    if($("#id",contentDiv).val() != "-1"
		...
		|| $("#moduleTemplateId",contentDiv).val() != "-1"){
			alert("保存个人模板失败，数据没有被正确设置，请联系开发人员处理。");
		return;
	}
...
function changeSecretLevel(object,processId){
	...
	if(isSysTem || isSysTem=='true'){
		...
		if (pro == null || pro == "" || pro == "undefined") {
			...
			if (pro != null && pro != "" && pro != 'undefined'
				...
				for (var i = 0; i < members.length; i++) {
					...
					if(sls == null || typeof(sls) == "undefined" || sls==""){
						continue;
					}
					sl = sls.split("）")[0];
					switch (sl) {
        ...
        if(flag){
        	$.alert("关联文档中,"+subject+"的密级高于当前公文密级！会导致公文流程中的人员不能查看该文档！");
        }
        //if(min<selectValue){
        	newFlag = true;
        	//}
		...
		var f = false;
	    if(workflowInfoObj.value != "<点击新建流程>" && workflowInfoObj.value != "" && selectValue != ""&&newFlag){
...
function checkzdgzryName(){
	//如果是选择全部人员，不进行过滤
	var rp = document.getElementsByName("radioperson");
	for(var i = 0;i < rp.length;i++){
		if(rp[i].checked && rp[i].id=='radioall'){
			return;
		}
	}

	var pro = document.getElementById("zdgzry").value ;
	if (pro == null || pro == "" || pro == "undefined") {
		return;
	}
	var memberM = new memberManager();
	...
	if (secretlevel != null && secretlevel != "") {
		...
		if (flag) {
			alert("跟踪人员中有低于当前密级的人员：" + notSuitName);
		}
	}
	return flag;
}
function checkSuperisor(){
	var pro = document.getElementById("supervisorIds").value ;
	if (pro == null || pro == "" || pro == "undefined") {
		return;
	}
	var memberM = new memberManager();
	...
	if (secretlevel != null && secretlevel != "") {
		...
		if (flag) {
			alert("督办人员中有低于当前密级的人员：" + notSuitName);
		}
	}
	return flag;
	}