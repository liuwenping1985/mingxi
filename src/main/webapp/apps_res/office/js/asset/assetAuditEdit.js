...
function fnOK() {
	...
	if (!isAffairValidPub($("#affairId").val(),"assetAudit")) {
		return;
	}
		...
		if(rval){
			...
			if (parseInt(auditAttitude) == 0) {
			  ...
			  preSendOrHandleWorkflow(window,pTemp.workFlow.workItemId,"-1",pTemp.workFlow.processId,pTemp.workFlow.activityId,user.id,pTemp.workFlow.caseId,user.loginAccount,pTemp.workFlowJSON,"office",null,window,null,fnWorkFlowCallSubmit);
				...
				var confirm = $.confirm({
			    'msg': $.i18n('office.asset.assetAuditEdit.btygsbsqlcjhdsqrsfjx.js'),
			    ok_fn: function () {
			...
			fnMsgBoxPub($.i18n('office.asset.assetAuditEdit.bgsbsysqyqsrbcl.js'), "alert", function() {
...
function fnWorkFlowCallSubmit(){
	...
	pTemp.ajaxM.assetAudit(auditPO, {
		success : function(rval) {
			...
			if (rval == 0) {
				...
				fnMsgBoxPub($.i18n('office.asset.assetAuditEdit.bgsbsysqyjcx.js'), "alert", function() {
					fnReloadPagePub({page : "assetAudit"});
					fnAutoCloseWindow();
				});
			}else{//rval == 1
				fnMsgBoxPub($.i18n('office.asset.assetAuditEdit.zhbgbgsbsysqyjbjzzxwb.js'), "alert", function() {
					fnReloadPagePub({page : "assetAudit"});
					fnAutoCloseWindow();
				});
			}
...
function fnSetCss() {
	...
	var opt = showOpt;
	if(pTemp.row.workFlow.auditState){
		opt = hideOpt;
		//禁用提交按钮
      ...
      'eastArea' : {
          ...
          spiretBar : {
              ...
              handlerL : function() {
                  ...
                  $("#deal_area").show();
                  $("#deal_area_show").hide();
                  $("#assetApplyIframe")[0].contentWindow.$("#tabs").tab().resetSize();
              },
              handlerR : function() {
                  layout.setEast(30);
                  $("#deal_area").hide();
