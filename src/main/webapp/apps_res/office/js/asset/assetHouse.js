$(function() {
    ...
    pTemp.SBar = officeSBar().init({
        argFunc : "fnAssetHouseSBar",
        searchItem : [ "name" ]
    });
    pTemp.tab = officeTab().addAll([ "id", "name", "managertxt", "scopeName" ]).init("assetHouse", {
        argFunc : "fnAssetHouseTab",
        parentId : $('.layout_center').eq(0).attr('id'),
        slideToggleBtn : true,
        resizable : true,
        "managerName" : "assetHouseManager",
        "managerMethod" : "findAssetHouse"
    });
...
function fnPageReload(areaId) {
	$("#editIframe").attr("src",_path+"/office/assetSet.do?method=assetHouseEdit");
}
...
function fnShowDetail(mode){
    ...
    if (mode === 'modify') {
        if (rows.length == 0) {
            $.alert($.i18n('office.assethouse.edit.js'));
            return;
...
function fnDel() {
    ...
    if (rowIds.length == 0) {
        $.alert($.i18n('office.assethouse.delete.js'));
        return;
    }
    if (rowIds.length > 1) {
    ...
    var hasInfo = pTemp.ajaxM.findAssetInfoOfHouse(rowIds[0]);
	if(typeof(hasInfo.length)!='undefined' && hasInfo.length>0){
		$.alert($.i18n('office.assethouse.cannot.delete.js'));
		return;
	}
	$.confirm({
        'msg' : $.i18n('office.auto.really.delete.js'),
         ok_fn : function() {
        	pTemp.ajaxM.deleteByIds(rowIds[0], {
                success : function() {
...
function fnSBarQuery(condition){
	$("#assetHouse").ajaxgridLoad(condition);
}

function fnAssetHouseTab() {
    return {
        "id" : {
            display : 'id',
            name : 'id',
        ...
        "name" : {
            display : $.i18n('office.assethouse.name.js'),
            name : 'name',
            width : '20%',
            sortable : true,
            align : 'left',
