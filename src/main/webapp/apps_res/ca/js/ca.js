...
function addIPOption(optionValue){
	if(optionValue == ""){
		alert(_("CAAccountLong.ca_IP_is_null"));
		return;
	}
...
function removeIPOption(){
    ...
    for(var i=0; i<selectObj.options.length; i++){
        if(selectObj.options[i].selected){
        	selectedIP = true;
        	if(confirm(_("CAAccountLong.ca_sure_to_remove", selectObj.options[i].value))){
    ...
    if(selectedIP == false){
		alert(_("CAAccountLong.ca_selectIP_to_remove"));
		return;
	}
}
...
function decideLoginWay(checkResult){
	var loginWay = document.getElementById("loginWay");
	var userName = document.getElementById("userName").value;
	if(certName != userName){
		if(checkResult == "IPisIncluding"){
			loginWay.value = "A8";
		}else if(checkResult == "IPisNotIncluding"){
			loginWay.value = "IPisNotIncluding";
		}else if(checkResult == "unCheckIP"){
			loginWay.value = "unCheckIP"; 
		}
	}else{
		loginWay.value = "CA";
	}
