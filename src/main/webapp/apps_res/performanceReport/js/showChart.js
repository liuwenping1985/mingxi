...
function drawComprehensive(){
  	if(!$.isNull(result)){
		  ...
		  var className = 'left';
		  if(Constants_report_id != PROJECTSCHEDULESTATISTICS) { className += ' w100b'; }
		  var styles='';
		  if(chartObjS != null){
			  for(var i=0; i<chartObjS.length; i++) { try{ chartObjS[i].chart.remove(); } catch(e){} }
		  }
		  if(Constants_report_id === COMPREHENSIVEANALYSIS){
				$("#queryResult").append("<li style='background:#fff;font-size:12px;text-align:center' id='chooseChart'><input type='radio' onclick='analysisTypeClick(0);' id='analysisType1' name='analysisType' value='1' checked='checked'>" + $.i18n('performanceReport.workFlowAnalysis.chartUseRadio') + "&nbsp;&nbsp; <input type='radio' onclick='analysisTypeClick(1);' id='analysisType2' name='analysisType' value='2'>" + $.i18n('performanceReport.workFlowAnalysis.charEfficiency') + "&nbsp;&nbsp;<input type='radio'onclick='analysisTypeClick(2);' id='analysisType3' name='analysisType' value='3'>" + $.i18n('performanceReport.workFlowAnalysis.chartOverTimeRatio') + "</td></li>");
			}
			...
			if(!$.isNull(chartWidth)){ anyChartDefualt.width =parseInt(chartWidth); }
			if(!$.isNull(chartHeight)){ anyChartDefualt.heigth =parseInt(chartHeight); }
...
function drawingDefualt(personGroupTab){
	  if(!$.isNull(result)){
          ...
          if(Constants_report_id === WORKDAILYSTATISTICS) {
        	  if(isPortal){
        		  ...
        		  if(personGroupTab === '2' && (appType === '5' || appType === '6' || appType === '7')){
            		  $("#queryResult").html("<div class='h100b' style='line-height:190px;text-align:center;font-size:12px'>" + $.i18n('performanceReport.section.notSupportCharts') + "</div>");
            		  return '';
                  }
        	  }else{
		  ...
		  var styles = '';
		  if(chartObjS){ for(var i=0; i < chartObjS.length; i++){ try{ chartObjS[i].chart.remove(); }catch(e){} } }
		  if(Constants_report_id === PROJECTSCHEDULESTATISTICS){
		  		//项目进度统计
		  		drawProject(0);
		  }else{
...
function drawProject(index){
	...
	for(var i = index; i < index + 6; i++){
		if(i > result.length-1 || (isPortal === "true" && i >= showNumber)){ return; }
...
function renderChildChart(anyChart , id,analysisType) {
  ...
  function onPointClick(e){
	  if(Constants_report_id === COMPREHENSIVEANALYSIS){
		  ...
		  //效率分析
		  openDetailWindow(_ctxPath, e.data.ID,e.data.Name,appTypeName,"efficiency");
	  }else if(Constants_report_id === OVERTIMEANALYSIS){
		  //超时分析
		  openDetailWindow(_ctxPath, e.data.ID,e.data.Name,appTypeName,"timeout");
	  }else if(Constants_report_id === NODEANALYSIS){
		  //节点分析
		  var url = e.data.ID;
		  if(!$.isNull(url)){
		  	//5.6前端优化:慎用eval，使用JSON.parse替换
		 	eval(e.data.ID);
  ...
  function openList(appType, entityType, entityId, state, beginDate, endDate, templateId, appName, statScope){
  		var title = $.i18n('performanceReport.queryMain_js.throughQueryDialog.title');
  		getA8Top().up = window;
  		queryDialog =$.dialog({
	          id : 'url',
	          url: encodeURI(_ctxPath + "/performanceReport/WorkFlowAnalysisController.do?method=statResultListMain&appType=" + appType + "&entityId="
	          ...
	          buttons : [ {
	              text : $.i18n('performanceReport.queryMain_js.button.close'),//关闭
	              handler : function() {
	            	  queryDialog.close();
	              }
	          } ]
  ...
  function openNodeAccessDetailWindow(id, policyName, memberName, overRadio, avgRunWorkTime) {
		...
		var endDate = end_time;
	      ...
	      queryDialog = $.dialog({
	          id : 'url',
	          url: encodeURI(_ctxPath + "/performanceReport/WorkFlowAnalysisController.do?method=nodeAnalysiszNodeAccessFrame&templeteId="+templeteId_+"&nodeId="+id
  ...
  function throughQueryDialog(throughQueryUrl){
  	  var title;
  	  getA8Top().up = window;
  	  if(throughQueryUrl.charAt(0) === "\/"){ throughQueryUrl=throughQueryUrl.substring(1,throughQueryUrl.length); }
  	  if(Constants_report_id === PROJECTSCHEDULESTATISTICS){//项目进度统计
	  	  title = $.i18n('performanceReport.queryMain_js.throughQueryDialog.title');//绩效报表穿透查询列表
		  var action = 'view';
		  var data = new Object();
        ...
        return;
		  ...
		  new projectTaskDetailDialog({"url1":detailUrl,"url2":contentUrl,"url3":treeUrl,"openB":true,"hideBtnC":hideBtnC,"animate":false});
	  	  ...
	  	  if(Constants_report_id === TASKBURNDOWNSTATISTICS){
  	  		//任务燃尽图
		 	title = $.i18n('performanceReport.queryMain.task.content');
		  }else{
  	  	  	title = $.i18n('performanceReport.queryMain_js.throughQueryDialog.title');//绩效报表穿透查询列表
  	  	  }
  	  	  width = $(getCtpTop().document).width()-100;
  ...
  function refreshPage(){
   ...
   function showDetail(id,subject,beginDate,endDate){
	    ...
	    getA8Top().up = window;
	    var url= _ctxPath + "/performanceReport/WorkFlowAnalysisController.do?method=getDetailAccount&templeteId="+id+"&beginDate="+beginDate+"&endDate="+endDate+"&templeteSubject="+templeteSubject;
	    var dialog = $.dialog({
		    id: 'url',
		    url: url,
		    width: 750,
  ...
  function closeAndForwardToCol(subject, content){
      ...
      $("#queryConditionForm").attr("action", _ctxPath + "/performanceReport/performanceQuery.do?method=reportForwardCol");
   ...
   function transmitColSreach(reportTitle,contentHtml){ 
      ...
      $("#queryConditionForm").attr("action", _ctxPath + "/collaboration/collaboration.do?method=reportForwardColl");
      $("#queryConditionForm").submit();
  }
