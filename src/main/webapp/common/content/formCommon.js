...
function selectChoose(fieldId, formId, split, customJson, callBack) {
	...
	var url = _ctxPath+"/form/component.do?method=formInputChoose&needAttr=" + needAttr + "&onlyHANDWRITE="+onlyHANDWRITE+"&choose=" + fieldId + (formId == null ? "" : "&formId=" + formId) + urlParam;
	var obj = new Array();
	obj[0] = window;
	obj[1] = split;
	obj[2] = customJson;
...
function selectFormField(selectType,options) {
	...
	var dialog = $.dialog({
		...
		title:options.title,
		width:600,
		height:$.browser.chrome ? (options.height + 40) : options.height,
		transParams : options,
		targetWindow : getCtpTop(),
		buttons:[{
			text : $.i18n("form.forminputchoose.enter"),
...
function showFormData4Statistical(formType, dataId, rightId, title, frame, formId) {
	...
	if (moduleType == enu.ModuleType.collaboration) {
		...
		if (frame) {
			...
			var dialog = $.dialog({
                url:getUnflowFormViewUrl(true, dataId, moduleType, rightId, 2),
                title : " ",
                targetWindow : getCtpTop(),
