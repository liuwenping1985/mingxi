...
public class FileUploadTag extends BodyTagSupport {
    ...
    /**
     * 文件类型{@link com.seeyon.ctp.common.filemanager.Constants.ATTACHMENT_TYPE_FILE}
     */
    private int                    type;

    /**
     * 所属应用分类 {@link com.seeyon.ctp.common.Constants.ApplicationCategoryEnum}
     */
    ...
    public void init() {
        type = com.seeyon.ctp.common.filemanager.Constants.ATTACHMENT_TYPE.FILE.ordinal(); // 默认为附件模式
        extensions = "";
    ...
    public int doEndTag() throws JspException {
        String url = SystemEnvironment.getContextPath() +
                "/fileUpload.do?type=" + type + "&applicationCategory=" + Strings.escapeNULL(applicationCategory, "")
                        ...
                        + "&maxSize=" + Strings.escapeNULL(maxSize, "") + "&isEncrypt=" + isEncrypt
                        //						+ "&quantity=" + quantity
        ...
        try {
            ...
            out.println("try{");
            out.println("  var ufa = new ActiveXObject('UFIDA_Upload.A8Upload.2');");
            ...
            if (attachments != null && !attachments.isEmpty()) {
                ...
                for (Attachment att : attachments) {
                    ...
                    out.print("'" + escapeEcmaScript(att.getFilename()) + "',");
                    ...
                    out.print( "'" +att.getV()+"'");
                    out.println(");");
                    if (att.getType() == Constants.ATTACHMENT_TYPE.FormFILE.ordinal()
                            || att.getType() == Constants.ATTACHMENT_TYPE.IMAGE.ordinal()) {
                        out.println("fileUploadAttachments.get('" + att.getFileUrl() + "').extSubReference = \""
                                + att.getSubReference() + "\";");
                    } else if (att.getType() == Constants.ATTACHMENT_TYPE.FormDOCUMENT.ordinal()) {
                        out.println("fileUploadAttachments.get('" + genesisIdStr + "').extSubReference = \""
                                + att.getSubReference() + "\";");
                    }
