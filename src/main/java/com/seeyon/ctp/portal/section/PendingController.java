...
public class PendingController extends BaseController {
    ...
    public ModelAndView transPendingMain(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        Map<String, CtpAffair> outSideAffair = new HashMap();
        if(affairs != null) {
            List<CtpAffair> affairList = affairs;
            ...
            for(int k = 0; k < affairList.size(); ++k) {
                ...
                } else if(affair.getObjectId() != null && affair.getObjectId().equals(Long.valueOf(0L))) {
                    outSideAffair.put(String.valueOf(affair.getId()), affair);
                }
            }
        }
        ...
        for(int kkk = 0; kkk < rowList1.size(); ++kkk) {
            PendingRow row = (PendingRow)rowList1.get(kkk);
            Long id = row.getId();
            if(id != null) {
                CtpAffair affair = (CtpAffair)outSideAffair.get(String.valueOf(id));
                if(affair != null) {
                    String userName = "";
                    String pwd = "";
                    ...
                    if(code == null||"".equals(code.trim())){
                        ...
                        userName = code;
                        List<MemberU8Info> mui =  DBAgent.find("from MemberU8Info where userCode='"+code+"'");

                        if(mui!=null&&mui.size()>0){
                            pwd = mui.get(0).getPassword();
                        }else{
                            pwd = code;
                        }
                    }
                    userName = Base64.encodeBase64String(userName.getBytes("UTF-8"));
                    pwd = Base64.encodeBase64String(pwd.getBytes("UTF-8"));
                    row.setLink("/syncU8UserToken.do?method=openPending&userName="+URLEncoder.encode(userName,"utf-8")+"&pwd="+URLEncoder.encode(pwd,"utf-8")+"&affId=" + affair.getId() + "&url=" + URLEncoder.encode(affair.getAddition(), "utf-8"));
        ...
        for(int kkk = 0; kkk < rowList2.size(); ++kkk) {
            PendingRow row = (PendingRow)rowList2.get(kkk);
            Long id = row.getId();
            if(id != null) {
                CtpAffair affair = (CtpAffair)outSideAffair.get(String.valueOf(id));
                if(affair != null) {
    ...
    public ModelAndView showLeftList(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        int pageCount;
        for(pageCount = 0; pageCount < var22; ++pageCount) {
            String c = var23[pageCount];
        ...
        if("pending".equals(dataName)) {
            ...
            appEnums = new ArrayList();
            appEnums.add(Integer.valueOf(ApplicationCategoryEnum.news.getKey()));
            affairList = this.pendingManager.getAffairCountByApp(fp, memberId, preference, appEnums);
        ...
        int i;
        if(affairList != null) {
            for(i = 0; i < ((List)affairList).size(); ++i) {
                CtpAffair affair = (CtpAffair)((List)affairList).get(i);
                if("YES".equals(affair.getExtraAttr("outside_affair"))) {
                    outSideAffair.put(String.valueOf(affair.getId()), affair);
                } else if(affair.getObjectId() != null && affair.getObjectId().equals(Long.valueOf(0L))) {
                    outSideAffair.put(String.valueOf(affair.getId()), affair);
        ...
        for(i = 0; i < rowList.size(); ++i) {
            PendingRow row = (PendingRow)rowList.get(i);
            Long id = row.getId();
            if(id != null) {
                String userName = "";
                String pwd = "";
...
//                    MemberU8Info info = new MemberU8Info();
    ...
    public ModelAndView showStatisticalChart(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(((Boolean)SysFlag.edoc_notShow.getFlag()).booleanValue()) {
