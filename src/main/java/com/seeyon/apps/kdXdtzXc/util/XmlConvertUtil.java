...
public class XmlConvertUtil {
	...
	/**
	 * xml to list xml <nodes><node><key>value1</key><key>value2</key>......</node><node><key
	 * label="key1">value1</key><key
	 * label="key2">value2</key>......</node></nodes>
	 * 
	 * @param xml
	...
	public static Map<String,List> xmltoMapList(String xml) {
		try {
			Map<String,List> datas = new HashMap<String,List>();
			Document document = DocumentHelper.parseText(xml);
			Element nodesElement = document.getRootElement();
			List nodes1 = nodesElement.elements();
			Element nodesElement1=nodesElement.element("Body");
			Element nodesElement2=nodesElement1.element("OutputParameters");
			List nodes = nodesElement2.elements();
//			Document document1=nodesElement.getDocument();
//			document1.elementByID("");
//			List nodes = nodesElement.elements();
			...
			for (Iterator its = nodes.iterator(); its.hasNext();) {
				...
				nodeElement.getText();
				Map map = xmltoMap(nodeElement.asXML());
				if(map.containsKey("ACTIVATE_IND") && String.valueOf(map.get("ACTIVATE_IND")).equals("F"))
					continue;
					
				List<Map> list = datas.get(nodeElement.getName());
				if(list == null)
				{
					list = new ArrayList<Map>();
					datas.put(nodeElement.getName(), list);
				}
				list.add(map);
				map = null;
			}
	...
	public static Map<String,String> newXmltoMap(String xml) {
